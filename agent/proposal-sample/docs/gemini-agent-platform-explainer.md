...
| **ソース入れ替えのリスク** | PoC は差し替えの頻度が最も高い段階です（プロンプト調整、スキーマ変更、モデル差し替え）。Cloud Run はリビジョンが不変で、**イメージと環境変数が 1 つの単位**として残るため、`update-traffic --to-revisions=<旧>=100` で即座に巻き戻せます。GKE だと Deployment のロールアウト履歴と ConfigMap / Secret の版が別管理になりがちで、「イメージは戻したが設定は戻っていない」というズレが起きます。`--no-traffic --tag` でトラフィックを流さずに検証でき、カナリアも 1 コマンドです |
...
> **副次的な利点**: `GEMINI_MODEL` と `GOOGLE_CLOUD_LOCATION` は環境変数なので、Cloud Run ではリビジョン単位で切り替えられます。
> モデルの比較検証（2.5 Flash と 3.7 Flash）を、トラフィック分割で同一条件のまま並べて評価できます
> （差分は[要点版の第7章](./gemini-enterprise-agent-platform.md)を参照）。
