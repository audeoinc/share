...
既存の GKE クラスタを運用しているため、**定常運用の段階では GKE に載せる**のが妥当である。
Cloud Run の主な利点（固定費ゼロ、認証設定の簡便さ、運用対象なし）は、クラスタと運用体制が既にある時点でほぼ相殺されるうえ、
第 5.2 章の VPC Service Controls を導入する場合は、Pod が VPC 内にある GKE のほうが境界設計が素直だからである。

一方、**PoC の段階では Cloud Run を使う。** 定常運用とは判断材料が異なる。

| PoC 固有の論点 | Cloud Run が有利な理由 |
|---|---|
| **同居ワークロードへの影響** | PoC のコード品質は読めない。Gemini の呼び出しは thinking と SSE で 1 リクエストが数十秒プロセスを占有するため、ノードリソースや HPA の挙動への影響が読みにくい。**別基盤に置けば影響が構造的にゼロになる** |
| **既存のセキュリティ設計への変更** | Cloud Run は PoC 用の GSA を紐付けるだけ。クラスタの Workload Identity 設定・RBAC・Ingress を一切変更しない。**審査に出す変更が小さい** |
| **ソース入れ替えのリスク** | PoC は差し替えの頻度が最も高い段階（プロンプト調整、スキーマ変更、モデル差し替え）。Cloud Run はリビジョンが不変で、**イメージと環境変数が 1 つの単位**として残るため、`update-traffic --to-revisions=<旧>=100` で即座に巻き戻せる。`--no-traffic --tag` でトラフィックを流さずに検証でき、カナリアも 1 コマンド。壊れたイメージを push しても影響がサービス内に閉じる（CrashLoopBackOff でノードのリソースを食うことがない） |
| **撤退コスト** | 中止する場合はサービスを削除するだけ。Namespace、RBAC、Ingress ルール、監視設定の後始末が発生しない |
| **費用の可視性** | PoC 単独の費用が分離して見える。GKE のノード費用に混ざると費用対効果の判断ができない |
| **反復速度** | クラスタ変更に承認プロセスがある場合、`gcloud run deploy --source .` との差が大きい |

**移行時に変わるのは 3 点のみ**: 認証の紐付け方（`--service-account` → Workload Identity）、マニフェスト、CI のデプロイ先。
**コンテナイメージとアプリケーションコードは同一**であるため、後戻りのコストは低い。

> **副次的な利点**: `GEMINI_MODEL` と `GOOGLE_CLOUD_LOCATION` は環境変数であるため、Cloud Run ではリビジョン単位で切り替えられる。
> 第 7 章のモデル比較（2.5 Flash と 3.7 Flash）を、トラフィック分割で同一条件のまま並べて評価できる。

> **判断が必要な点**: VPC Service Controls（第 5.2 章）の検証を PoC のスコープに含めるかどうか。
> 含める場合、Cloud Run 側で Direct VPC egress の構成が必要になり、上記の「手軽さ」の利点が一部削れる。
> 境界の検証を本番設計フェーズに回すのであれば、PoC は Cloud Run で問題ない。
...
- **実行基盤**: PoC は Cloud Run、本番は GKE の方針（第 2 章）。GKE 移行時に、Namespace とリソース制限による他ワークロードからの隔離、レプリカ数、`store.py` の永続化方針を決める。VPC-SC の検証を PoC に含めるかどうかは要判断（含める場合は Cloud Run 側で Direct VPC egress の構成が必要）
