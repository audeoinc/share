...
1. **「グローバル」と「非グローバル」で価格が違う（約 10% 増）。**
   第 5.1 章のとおり、データ所在地の要件からリージョンを `asia-northeast1` に固定すると、**非グローバル料金が適用される**。
   2.5 Flash にはこの区別がないため、**モデルを上げると「リージョンを固定するコスト」が価格表の上に初めて現れる。**
   データ所在地の要件と費用がトレードオフとして可視化されるので、稟議の際はこの行を根拠にできる。
...
- **`asia-northeast1` での提供有無**（第 5.1 章）。提供されていない場合、リージョン固定とモデル更新のどちらを優先するかの判断が要る。
...
- **使用モデルとリージョンの整合**: 要件を満たすモデルが `asia-northeast1` で提供されているかの確認（第 7 章）。3.7 Flash に上げる場合は非グローバル料金での試算も併せて行う
