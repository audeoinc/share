...
`07_run_single_view_analysis.sql`の既定値は`audeodb.sample_ds.v_customer_sales_cost_sample`です。別のViewを解析する場合は、ファイル先頭の`target_view_name`を変更してください。単体解析SQLはRepositoryおよび対象Datasetの永続テーブルを更新しません。
