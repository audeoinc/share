...
- **GoogleSQL の落とし穴（学習済み）**：
  ...
  - **オブジェクト命名規則（システム識別子 `lnge_`）**：このシステムが作成・利用する
    ...
    `lnge_vw_t_object_dependency`（実体化テーブルは `vw_` を落とした
...
- **03 パイプラインの構造**：`lnge_render_dynamic_sql`（8 プレースホルダ / 9 パラメータ）で
  ...
  **レポート用ビューの実体化（03 STEP 4b）**：2つのレポートビューは読むたびに全集計が
  ...
  `lnge_vw_t_object_dependency`→`lnge_t_object_dependency`、`materialized_at` を付与、
  起点でクラスタ）。**定義の正本はビュー**なので 01 を直せば次回実行でスキーマも追従。
  MATERIALIZED VIEW は `ARRAY_AGG(... ORDER BY ...)` / `STRING_AGG(DISTINCT ...)` /
  LEFT JOIN / 分析関数 / 相関 ARRAY サブクエリを受け付けないため通常テーブル。
  再構築条件は STEP 4 と同じ（`has_analysis_work OR orphan_direct_dep_deleted > 0`）＋
  「テーブルが未作成なら作る」。`preview_only` では実行しない。失敗しても
  `MATERIALIZE_REPORT_TABLES / FAILED` 行を出すだけでパイプラインは止めない（派生データのため）。
  **注意**：`usage_definition_html` のハイライトは分析関数で集めるので、ビュー経由なら
  レポート側の追加フィルタに追従するが、実体化テーブルでは作成時に確定する
  （パーティションキーに起点カラムと対象オブジェクトが入っているので通常用途では同結果）。
  `usage_definition_text`/`_html` は SQL 全文を行ごとに繰り返すため、
  `materialize_usage_sql_columns = FALSE` で実体化から外せる。
