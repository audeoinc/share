...
- Materialized the two report views into physical tables, rebuilt by a new 03 STEP 4b:
  ...
  filters land on computed columns (`origin_full_name`), so nothing prunes by cluster key;
  the tables are clustered on the origin and are BI-Engine eligible. BigQuery's
  MATERIALIZED VIEW cannot express either definition -- `ARRAY_AGG(... ORDER BY ...)`,
  `STRING_AGG(DISTINCT ...)`, LEFT JOIN, analytic functions and correlated ARRAY
  ...
  a `MATERIALIZE_REPORT_TABLES` / `FAILED` row rather than taking the pipeline down --
  ...
  report's extra filters also narrow the highlights; in the materialized copy the
  ...
  New 03 toggles: `materialize_report_tables` (default TRUE) and
  `materialize_usage_sql_columns` (default TRUE). The latter drops
  `usage_definition_text` / `usage_definition_html` from the materialized copy: both
