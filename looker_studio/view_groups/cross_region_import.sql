...
-- **増やし方は 2 通りある。**
--
--   (1) ここに 1 行足す（**こちらを勧める**）。並べた送り元は 1 回の
--       LOAD DATA でまとめて読む（下の uris_literal が全部の URI を
--       並べる）ので上書きし合わず、その時点で GCS にあるものから毎回
--       テーブルを作り直す。テーブルは 1 本のままで、source_region 列で
--       区別する。**取り込みが 1 本なので、全送り元の鮮度が毎回まとめて
--       確かめられる**（下の (2) の検証）。
--
--   (2) 送り元ごとにこのファイルを別のスケジュールドクエリにする。
--       **ある送り元の取り込みが止まっても気づきにくい。** そのテーブルには
--       古い行が残り続け、build_table.sql の「行があるか」は通ってしまう。
--       鮮度を見ているのは下の (2) だが、そのスケジュールが動いていなければ
--       働かない。送り元が増えて実行時間や権限を分けたくなったときに選ぶ。
...
--
-- **uri は送り元ごとに 1 本ずつ並べる。** uris は配列で、カンマ区切りで
-- 複数の URI を渡せる（1 回のロードジョブで最大 10,000 ファイル。運ぶのは
-- 数 MB なので当たらない）。ただし 1 つの URI に置けるワイルドカードは
-- 1 つだけなので、…/*/views-*.avro のようにはまとめられない。
