...
```
{"ja":"受注日","en":"order date"}          → 受注日 / order date
{"name_ja":"リージョン","name_en":"region","unit":"ISO 3166"}
                                           → リージョン / region / unit: ISO 3166
受注日（JST）                               → そのまま 1 行
{ 壊れた JSON                               → そのまま 1 行
```
...
**拾えなかったキーは捨てずに `key: value` で出す。** どう書いても画面から
消えないようにするため。使いたいキーが従（薄い色）で出ていたら、
