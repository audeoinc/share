...
**タブの位置は「ラジオのあとに続く兄弟のどれかの中」とだけ決めて、間の入れ物は
問わない**（`.vg-orN:checked ~ * .vg-otN`）。帯の固定も `.vg-ohead` と旧世代の
`.vg-outer > .vg-otablist` の両方を対象にしてある。

> **テンプレートの CSS は手で貼るのに対して、カードは日次で作り直す。**
> この 2 つの世代がズレている時間帯は仕組み上どうしても発生するので、
> 外枠の構造を変えたときは**新旧どちらの HTML でも効く CSS** にしておく。
> 片方でしか効かない書き方をすると「タブが反転しない」「帯が固定されない」が
> 同時に起き、しかも画面からは CSS と HTML のどちらが古いのか分からない
> （実際にこれを踏んだ）。`.vg-otN` はこの外枠にしか出てこないので、
> セレクタを広く取っても誤爆しない。
