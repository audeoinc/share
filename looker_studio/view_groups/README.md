...
##### note タブは ラベル ＋ View の description ＋ シートのメモ

`note` タブは**出どころの違う 3 つを、箱を分けて縦に積む**。

| 段 | 出どころ | 変わるとき |
|---|---|---|
| ラベル | `INFORMATION_SCHEMA.TABLE_OPTIONS` の `labels` | View をデプロイしたとき |
| View の description | 同上の `description` | View をデプロイしたとき |
...
1 つも設定されていなくても段は出して「未設定」と書く ―― 消すと、未設定なのか
取り込みに失敗しているのかが画面から読めない。
