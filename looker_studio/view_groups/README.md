...
> **列幅は `<colgroup>` で明示する（30% / 70%）。** ブラウザまかせ
> （`table-layout:auto`）だと、値側に `width:100%` を置いた時点でキー側が
> 「最小幅」まで詰められる。キーは折り返し可にしてあるので、その最小幅が
> **1 文字**になり、キーが縦に伸びる。実際にそれで一度出した。
>
> 30% は妥協点。キーが短い（`ja` / `en`）と左に余白が出るが、はみ出すよりよい。
