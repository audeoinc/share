...
> **配列をレポートから使うときは、マトリクスのビュー（下）を読む。**
> `t_diff` の配列をそのままディメンションにできる場合もある（実機で
> `suffixes` を列に置けている）が、**2 本以上を同時に使ったときに添字の
> 対応が保たれる保証は無い。** 独立に展開されれば suffix × location の
> 総当たりになり、存在しない組み合わせのセルが出る。
> 加えて `t_diff` は 1 行 = base × 基準なので、`ref_index = 0` で絞らないと
> 基準の数だけ重複する（指標に数を置くとそのぶん膨らむ）。
> 3c のビューはその 2 つを SQL 側で済ませてある。
...
**カードのテーブルからは作りにくい。** `t_diff` は 1 行 = base × 基準で、
View ごとの情報は配列（`suffixes` / `locations` / `view_names`）に畳まれている。
配列をそのままディメンションにできる場合もあるが、**2 本を同時に置いたときに
添字の対応が保たれる保証が無い**（総当たりになれば存在しない
location × suffix のセルが出る）うえ、基準の重複も自分で外すことになる。
**対応の保証と重複の除去を SQL 側で済ませたもの**をセクション 3c のビューが出す。
