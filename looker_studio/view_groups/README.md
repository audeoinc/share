...
#### 1 View の DDL は暴れない（前提の絞り込み）

DDL は SQL なので、1 本が極端に大きくなることは考えにくい。**つまり
1 ペアあたりの差分の大きさ `p` はほぼ一定で、メモリは実質 `G²` だけで決まる。**

コードのコメントに残っている実測（基準 1 つぶんの最大 690 KB、G=6 で 4 MB）
から `p ≈ 133 KB` として外挿すると:

| G | 枚数 `min(G,12) × (G−1)` | 概算 |
|---|---|---|
| 6 | 30 | ≈ 4 MB（実測と一致） |
| 12 | 132 | ≈ 17 MB |
| 20 | 228 | ≈ 30 MB |

`MAX_REF_TABS = 12` で頭打ちになったあとも `(G − 1)` が伸び続けるので、
**G が 10 前後で落ちる**。実際に落ちた状況と符合する。

案 C なら 1 行は `(G − 1) × p` なので、G = 20 でも **2.5 MB 程度**。

**この前提が正しければ、案 A（作る前に見積もる）は要らない。** 大きさが
読めない要素が無いので、`G` に上限を置けば足りる。**C ＋ D で組む。**
診断クエリの `max_ddl_kb` が想定外に大きければ、この判断をやり直す。
