...
> **今回（比較タブの色）は `template_style.html` の貼り直しが要る。**
> 貼り直すまでカードの先頭に黄色い案内が出る（`CSS_GEN` の仕組み）ので、
> 出ていれば貼り直せばよく、出ていなければ貼り直し済み。
>
> 世代を上げるのはクラスが増えたときだけとは限らない。**今回は規則の値だけ
> の変更**で、貼らなくてもタブは切り替わる——が、未選択が白のままで選択中と
> 見分けが付かない。つまり**貼り忘れと未修正が画面から区別できない**ので、
> 見た目だけの変更でも世代を上げる。
