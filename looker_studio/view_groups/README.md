...
```bash
node preview.mjs          # dist/preview.html を生成して検証（86 アサーション）
...
の要素（`{{Pn}}` のチップ）が DOM 順で手前に来てタブに重なる。逆に 20 以上に
すると、そのチップの吹き出し（`z-index:20`）がタブの下に隠れる。間を取ると、
地の文はタブが隠し、吹き出しはタブより手前に出る。
