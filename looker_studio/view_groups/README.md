...
> **この文書は作る人・直す人向け。** レポートを見る人向けの読み方は
> `usage.md`（使い方・データの見方）、導入の背景と効果は `introduction.md`。
...
> ### `CSS_GEN` を上げたときは 1〜3 を**全部**やる
>
> 世代の番号は CSS だけでなく**カードの markup にも入る**（`chrome.js` の
> `cssGuard()` が `<div class="vg-cssgenN">` を出す。これは UDF に焼き込ま
> れる）。つまり
>
> | | 番号の出どころ |
> |---|---|
> | `.vg-cssgenN{display:none}` | `template_style.html`（貼る CSS） |
> | `<div class="vg-cssgenN">` | `view_group_html.sql` の UDF → `build_table.sql` が作るカード |
>
> **両方の番号が一致したときだけ案内が消える。** CSS だけ貼り替えても、表の
> カードは古い番号のままなので案内は出っぱなしになる。逆も同じ。
>
> 「見た目だけの変更だから UDF はそのままでよい」は**誤り**。世代を上げた
> 時点で UDF の出力も変わっている。
>
> 世代を上げるのはクラスが増えたときだけとは限らない。規則の値だけの変更で
> あっても、貼り忘れと未修正が画面から区別できないなら上げる（比較タブの色の
> ときがこれ。貼らなくてもタブは切り替わるが、選択中が見分けられない）。
...
**BigQuery を叩かずに確かめられるものは全部ここで確かめる。** 5 本とも
`node <ファイル>` で走り、失敗すると終了コードが 0 でなくなる。
...
検証ではない）。`--check` を付けると生成物を書かずに検証だけ走る
（`preview.mjs` / `build_udf.mjs`）。

> **`usage.md` は画面の文言をそのまま引用している。** タブの並び・打ち切りの
> 案内文・読むオブジェクトの名前は `check_sql.mjs` が突き合わせるので、
> 引用元を直すとそこが落ちる。**落ちたら `usage.md` を直す**（読む人にとっては
> 資料と画面が食い違っているほうが困るため、黙って通さない）。
...
node test.mjs               # アナライザの検証（195 アサーション）
```
...
**タブの色は、そのタブが指すペインの色**。基準タブは基準ペインと同じ薄い赤、
選択中の比較タブは比較ペインと同じ薄い緑にしてある。押した先の色がそのまま
下のペインに出るので、タブと中身が目で繋がる。

| | 元の色（`render.js` の `paneColors`） | 地 | 枠 | 件数の印 |
|---|---|---|---|---|
| 基準（左ペイン）＝ `.vg-tbase` | `base` `#E17B7B` | `#fbeded` | `#efb6b6` | `#f6d7d7` / `#87494a` |
| 比較（右ペイン）＝ 選択中の `.vg-t*` | `after` `#93AE68` | `#f0f4ea` | `#c4d2ac` | `#dfe7d2` / `#58683e` |
| 未選択の `.vg-t*` | —（無彩色） | `#F6F8FA` | `#D0D7DE` | `#EAEEF2` / `#57606A` |

`chromeCss()` は静的なので既定値を焼き込んである。`opts.colors.baseColor` /
`afterColor` を変えたときは、こちらも直す必要がある。

> 選択中はもともと白だった。未選択も地の色が無く白地だったので、**白 → 白**で
> 切り替わったことが読めなかった（切り替え自体は効いていた）。未選択に地の色を
> 入れ、選択中をペインの色にして直してある。
...
```bash
node preview.mjs          # dist/preview.html を生成して検証（178 アサーション）
...
node build_udf.mjs --check  # 生成せず検証だけ（75 アサーション）
...
|---|---|---|
| `table_diff_src` | 素のカード（メモ差し込み前。セクション 2 が作る） | `viewlgc_t_diff_src` |
| `table_diff` | **レポートが読むテーブル**（最新の 1 世代だけ） | `viewlgc_t_diff` |
| `table_base_note` | base ごとのメモ（スプレッドシートの外部テーブル） | `viewlgc_m_base_note` |
| `view_diff` | 素のカードにメモを繋ぐビュー。3b の材料であり、シートを直した内容をその場で見る窓口 | `viewlgc_vw_t_diff` |
| `view_matrix` | **マトリクスのレポートが読むビュー**（1 行 = 1 View） | `viewlgc_vw_t_matrix` |
...
**読み込む側にその縛りは無い。** 拠点（`asia-northeast1`）の `LOAD DATA` から
`asia-southeast1` のバケットをそのまま読める（実環境で確認済み）。
別リージョンなので転送料金はかかるが、運ぶのは数 MB のメタデータだけ。
...
> BigQuery の資料には「読み込むバケットはデータセットと同じロケーションで
> なければならない」と読める記述もあるが、**実際には別リージョンでも読めて
> 転送料金がかかるだけ**（sgp のバケットを tky のテーブルへ `LOAD DATA` する
> 運用が動いている）。

バケットは `sources` の行ごとに持たせてある。事情があって拠点側へコピーして
から読みたいリージョンが出てきたら、**その行の `gcs_prefix` だけ拠点側の
バケットに向ければよい**（スクリプトは変わらない。リージョンごとに混ざって
いてもよい）。

> コピーを挟む場合は、GCS → GCS の転送側で
> **「転送元にないオブジェクトを転送先から削除」を有効にすること。**
> `EXPORT DATA` は書き出す側のバケットしか掃除しない。前回 2 分割で書かれ
...
いちばん困るのは、**欠けた状態で解析が走り、欠けたカードでレポートが
上書きされる**こと。`cross_region_import.sql` は取り込んだあと
3 つ確かめてから通す。

書き出しは別リージョンの別スケジュールなので、**その途中を読むことがある。**
書き出しは 6 本の `EXPORT` を順に流すので、`views` は新しいのに `table_opts` は
まだ前回、という瞬間が実際に存在する。マニフェストをいちばん最後に書いて
あるのはそのためで、下の 3 つはその瞬間を掴んだかどうかを見ている。

| | 見るもの | 拾える壊れ方 |
|---|---|---|
| (1) | `sources` の送り元が全部マニフェストにあるか | 書き出しが丸ごと終わっていない |
| (2) | `collected_at_iso` が `max_staleness_hours` 以内か | 送り元が止まっているのに拠点だけ動いている |
| (3) | 読み込んだ行数がマニフェストの件数と一致するか | 書き出しの途中を掴んだ／古い avro が混ざった |
...
3. `cross_region_import.sql` の `sources` にその送り元とバケットを書き、
   **拠点で** `build_table.sql` より前に流すように登録する
4. `build_table.sql` を、取り込んだ 5 本を `UNION ALL` する形に直す（下記）

リージョンを増やすときは、1 と 2 をもう 1 組作り、`sources` に 1 行足す。
...
|---|---|---|
| **(1) 1 本のテーブルにまとめる（既定）** | `sources` に 2 行、suffix は 1 つ | 同じ suffix を 2 行 |
| (2) 送り元ごとに分ける | スケジュールドクエリを送り元ごとに、suffix も分ける | 違う suffix を 2 行 |

`uris` は配列で、カンマ区切りで複数の URI を渡せる（1 回のロードジョブで
最大 10,000 ファイル。運ぶのは数 MB なので当たらない）。だから (1) は
**並べた送り元を 1 回の `LOAD DATA` でまとめて読む**。上書きし合わないし、
その時点で GCS にあるものから毎回テーブルを作り直す。

**(1) を勧める。鮮度の穴が 1 つ少ない。**
(2) は送り元ごとにスケジュールが分かれるので、**ある送り元の取り込みが
止まったまま気づかない**状態がありうる。そのテーブルには古い行が残り続け、
`build_table.sql` の「行があるか」は通ってしまう。鮮度を見ているのは
取り込み側の `ASSERT` で、そのスケジュールが動いていなければ働かない。
(1) なら取り込みが 1 本なので、**全送り元の鮮度が毎回まとめて確かめられる。**

(2) の利点は、片方の送り元が止まってももう片方の取り込みに波及しないこと。
送り元が増えて実行時間や権限を分けたくなったときに選ぶ。
...
> **対応は添字という形でしか存在しない。** 1 本だけならレポートの
> ディメンションに置ける（実機で `suffixes` を列に置けている）が、
> **2 本を組にして使うことはできない。** Looker Studio から見えるのは
> 独立した繰り返しフィールドが 2 本あるだけで、「この 2 本は同じ添字で
> 組になっている」と伝える手段が無いため、それぞれ独立に展開される。
> つまり**対応はレポートに渡る前に解いておくしかない**。それが 3c のビュー。
> 加えて `t_diff` は 1 行 = base × 基準なので、`ref_index = 0` で絞らないと
> 基準の数だけ重複する（指標に数を置くとそのぶん膨らむ）。3c はそちらも
> 済ませてある。配列が効くのは BigQuery で直に問い合わせるとき。
...
**カードのテーブルからは作れない。** `t_diff` は 1 行 = base × 基準で、
View ごとの情報は配列（`suffixes` / `locations` / `view_names`）に畳まれている。
配列 1 本ならディメンションに置けるが、**列を 2 段にするには 2 本を組にする
必要があり、そこが渡せない**（対応は添字にしかなく、レポート側に「同じ添字で
組になっている」と伝える手段が無い）。**対応を解き、基準の重複も外したもの**を
セクション 3c のビューが出す。
...
構造そのものを積むだけなのでやはり要らない。

**1 レコード = 1 base × 1 基準。** カードに載る基準は 1 つで、どの基準を見るかは
レポートのコントロール（`ref_label`）で選ぶ。カードの先頭には、いま何を基準に
しているかが文字で出る。

```
基準グループ  abjp, abuk, abus 3   （レポートの「基準」で切り替え）
─────────────────────────────────────────────────────────
[ 基準 abjp, abuk, abus 3 ][ cdjp, cduk, cdus 3 ][ efjp, efuk, efus 3 ]
```

#### なぜ行に分けるのか（グループ数の二乗を線形にする）

以前は**全基準を 1 枚のカードに載せて、カードの中のタブで選んで**いた。
そのとき比較ペインは `G×(G−1)` 枚 ―― **グループ数の二乗**になる。

リージョンをまたいで View を集めると base あたりの View 数が増え、`G` が伸びる。
そこで **`Resources exceeded during execution: UDF out of memory`** が出て、
日次の生成ごと落ちた。
...
は基準 1 つにつき `G−1` 枚で線形なので、**基準を行に分ければ 1 行は線形に戻る**
（G=20 でも 2.5 MB 程度）。BigQuery に置く総量は変わらないが、**1 行が小さく
なることと、UDF が 1 回に組み立てる量が小さくなること**が効く。

> **タブで選ぶ道は残していない。** 残すと、この UDF は 32 KB 制限に対して
> 余裕が無い（いま 89%）うえに、落ちた原因そのものをいつでも呼び戻せて
> しまう。`REF_BUDGET` と `MAX_REF_TABS` ごと削除した。

`ref_index`（0 起点）と `ref_label`（基準グループの見出し）は、この用途の
ために残してあった列。**レポートのコントロールは `ref_label` を使う。**

#### 解析側は「代表 ＋ 差分」で持つ（View 数をメモリから外す）

上の対処は**描画側**の話。`t_diff_src` は 1 文の中で解析（`viewlgc_analyze`）と
描画（`viewlgc_render`）を両方走らせるので、`UDF out of memory` が出ても
**どちらが原因かはエラーからは分からない。** 描画側を線形にしたあと、今度は
解析側が上限になった。

解析はまず全 View をトークン列にする。`raw` は `{ kind, text }` の配列で、
3 KB の SQL が 560 トークン ≒ **37 KB**（オブジェクトの分だけ 12 倍）。
これを View の本数だけ抱えると、**リージョンを足すたびに増える。**

ところが同じグループのメンバは α 等価、つまり**実体名と値しか違わない**。
だから代表の `raw` と「違う添字だけ」を持てば元に戻せる。
...
リージョンを足して増えるのはメンバ数であってグループ数ではないので、
**リージョンの本数がメモリから外れる**のが効く。通る最小ヒープを二分探索して
測った実測値:

| グループ | リージョン | View | 以前 | いま |
|---|---|---|---|---|
| 20 | 24 | 480 | 30 MB | 8 MB |
| 40 | 24 | 960 | 58 MB | 8 MB |
| 40 | 48 | 1920 | **190 MB** | **16 MB** |

出力は 1 バイトも変えていない（旧実装と `sql` / `params` / `missBy` が完全一致
することを、7 通りの設定と複雑なサンプルで確認済み）。

> 添字が揃うことは `normalizeSpace` が 1 対 1（空白の文字だけ差し替える）で
> あることと、α 等価が長さと種類の一致を要求することから言える。**メンバに
> トークン列を持たせ直すと元に戻る**ので、`node test.mjs` が「メンバはトークン列を
> 持たない」を見ている。

#### 行数の上限

`max_ref_rows`（[A]、既定 24）で 1 base あたりの基準の数を止める。`G` が極端に
大きい base では、基準を全部用意しても読む人が選びきれず、行数と生成時間だけが
伸びる。**打ち切ったことはカードに書く**（`maxRefRows` を描画側に渡している）
―― 選べないだけなのに「グループが無い」と読めるのがいちばん困る。

> グループが 0 件の base でも行が消えないようにしてある。空の配列を
> `CROSS JOIN` で展開すると base ごと落ちて、**解析できなかった base ほど
> カードが黙って消える**。
...
（note タブの中にさらに description とラベルの段がある。`.vg-d*` / `.vg-lb*`）
...
#### 注記は「下敷きを全部 → 文字を全部」の順に描く

注記には白い下敷き（`opacity:.92`）を敷いて、線の上でも読めるようにしている。
**これを注記ごとに「下敷き → 文字」と積んではいけない。** 結合キーが増えると
注記が縦に伸びて辺どうしで重なり、隣の下敷きが前の注記の**下端を削る**。
削られるのは行の最下部＝**下線（`_`）**なので、

```
customer_account_id = account_id_customer   ← 本当はこう
customer account id = account id customer   ← こう見える
```

となり、「文字列が途中で切れる」と読める。列名は下線で区切るのが普通なので、
実データではほぼ必ず出る。下敷きを全部描いてから文字を全部描けば、重なっても
文字は生き残る。

#### 注記は元の箱と同じ高さに置き、行の高さを注記に合わせる

**結合キーは省略しない。** 全部を図に出す（丸めも tooltip 頼みもしない）。
そのために置き方と行送りを 2 つ変えてある。

**1. 注記は元の箱と同じ高さに置く。** 以前は辺の中点だった。中点だと、同じ
相手（最終 SELECT など）へ向かう辺が多いときに注記が中央へ寄って**固まり**、
どの JOIN の条件なのかも読めない。元にそろえれば行がそのまま手掛かりになる。

```
中点に置く                        元にそろえる
  dim_customer ─┐                   dim_customer ──  customer_id = cust_id
  dim_product  ─┤  ← 注記が          dim_product  ──  product_id = prod_id
  dim_store    ─┤     ここに固まる    dim_store    ──  store_id = shop_id
  dim_region   ─┘                   dim_region   ──  region_code = reg_cd
```

**2. 行の高さは注記で決める。** `rowH[r] = max(BOX_H, その行から出る注記の高さ)`
とし、箱は帯の中央に置く。注記も箱の中心にそろえるので、帯の中で釣り合って
**重ならない**。重ならなければ削られないので、省略する理由が無くなる。

1 つの元から同じ溝へ複数の辺が出ることがある（同じ段の 2 つの CTE を作る、など）。
そのときは縦に積むので、要る高さは**合計**で取る。

#### 図の中で文字を詰めない（縦にも横にも伸ばす）

**この図では省略しない。**結合キーも実体名も、詰めずに全部出す。

| | 以前 | いま |
|---|---|---|
| 結合キー | 4 行 ＋「ほか N 件」 | **全部の行を出す**。行の高さを注記に合わせて広げる |
| 実体名 | 380px で頭打ち、超えたら `…` | **上限なし**。箱をいちばん長い名前に合わせて広げる |

どちらも「何を読んで、何と突き合わせているか」そのもので、そこを削ると図の
用が足りない。`fact_order_transaction_line_item_daily_snapshot_v2_final` の
ような名前は実際にある。

図は縦にも横にも伸びるが、`.vg-erdbox` がスクロールするので読める。
**削るより伸ばす**ほうを採っている。

箱の幅は全部の箱で揃える。1 つ長い名前があると全部が広がるが、段ごとに幅が
違うと段の境目が読み取りにくくなるので、揃えるほうを採る。

`<title>`（tooltip）は残してあるが、いまは保険。全文は図に出ている。
...
#### 参照関係のグループはロジック差分とは別に括り直す

**ロジックが割れていても参照先は同じことが多い。** SELECT する列や WHERE だけ
違う版などがそれで、そのまま積むと**同じ図が何枚も並ぶ**。そこで
`erdGroups()` が括り直す。カラム定義が列の構成で括り直すのと同じ考え方
（3 つのタブで括り方が違うのは意図的）。

| タブ | 何で括るか |
|---|---|
| ロジック差分 | α 等価（パラメータ化した SQL が一致するか） |
| **参照関係** | **参照名の base 部分（suffix を除いた実体名）と辺** |
| カラム定義 | 列の構成（名前・型・NULL 可否・description） |

**base 部分で比べるのが要点。** 3 通り試して測った結果:

| 比べ方 | サンプル 9 View / ロジック 3 グループ | 問題 |
|---|---|---|
| 実際の名前 | **9 枚** | View ごとに割れる（各 View が自分の suffix の表を読む） |
| 形だけ | **2 枚** | `orders_*` と `customers_*` まで同じ図になる |
| **base 部分** | **2 枚** | — |

```
orders_abjp  + suffix abjp → orders
orders_efjp  + suffix efjp → orders     ← 同じ → 1 枚にまとまる
customers_cdjp + suffix cdjp → customers ← 別 → 別の図のまま
```
...
| 節 | base の出し方 |
|---|---|
| パラメータ由来 | **値どうしの共通部分**を取り、直前の `_` まで戻す |
| そうでない（全 View で同じ名前） | View の suffix の**末尾に一致する部分**を落とす |

どちらか片方だけでは足りない。

- View の suffix を落とすだけだと、**中間語を使ったとき**に落ちない
  （`sales_v2_txjp` が `orders_txjp` を読む形。`_v2_txjp` で終わらない）
- 値の共通部分だけだと、**メンバが 1 本のグループ**で出せない
  （差が出ないので実体名がパラメータ化されず、`orders_jp` のまま base になる）

2 つ目は「ロジックは違うが参照は同じなのに別グループになる」として実際に
表に出た。1 本しかない版（ある地域だけ直した View など）は珍しくない。

末尾の落としは**丸ごと一致だけではない**。参照先が View より短い suffix を
持つことがある。
...
後者はデータセット名だけが 4 文字で表は地域だけ、という並び
（`suffix_tail_lengths` がある理由と同じ事情）。共通の表
（`calendar_master` など）は View の suffix と関係が無いので落ちない。

**箱の名前は、まとめた全グループの値をならしたもの**（共通部分 ＋ `*`）。
1 本目の View の値をそのまま出すと、まとめた相手には当てはまらない名前が出る
（`orders_ab*` と `orders_ef*` をまとめた図に `orders_abjp` と出てしまう）。

```
orders_*        ← abjp…abus と efjp…efus をまとめた図
orders_cd*      ← cd だけの図
```

実際の対応は箱の `<title>`（カーソルを乗せると出る）に**まとめた全グループぶん**
並べる。ならした名前だけだと、どの View が何を読んでいるか辿れなくなる。

> **見出しの「N グループ」はロジックのグループ数のまま。** 帯はタブ共通なので
> ここだけ別の数にはできない。図の枚数と食い違って見えるぶんは、図の上の注記が
> 「ロジックは 3 グループに割れていますが、参照関係は 2 通りです」と書く。
...
1 列 = 1 カラム定義**の表にする。セルは**型とモード**（`NULLABLE` /
`REQUIRED` / `REPEATED`）。型を大きく、モードはその下に小さく添える。
表記は BigQuery のコンソールに合わせてある。

> **列はロジック グループではない**（`columnGroups` が `columnSig` で束ね直す）。
> SQL が α 等価でも参照先の型や description が違えば別の列になるので、
> **ロジック差分のタブと列の本数は合わない。** 必ず誤読されるところなので、
> 表の上の案内文でそう書いてある。
...
束ごとのタブにしていないのは、列名も型も短くて横に並べても収まるから。
全部を一度に見せれば、どこが揃っていないかをタブを押さずに見つけられる。
SQL は横に長いので差分側は 2 ペインだが、こちらは事情が違う。

**カラム定義はロジック グループではなく View ごとの属性。** 同じロジック
グループなら SQL は α 等価だが、参照先テーブルの型が違えば出力列の型も違う
（`amount` が jp は `NUMERIC`、us は `FLOAT64` など）。**これはロジック差分には
出てこない。SQL は同一だから。** この表のいちばんの値打ちがそこなので、
ロジック グループの代表 1 本を黙って出すのではなく、**カラム定義そのもので
束ね直して**、違えば必ず別の列にする。

| 見た目 | 意味 |
|---|---|
| 枠（黄）＋ `⚠` | **束の中で並び順（`ordinal`）が食い違う**、または**この列を持たない View がいる**。ホバーで `abus = #4 FLOAT64 NULLABLE` のように suffix ごとの内訳が出る（並び順が見えるのはここだけ） |
| 地色（緑） | **その行でいちばん多い内容**（型 / NULL 制約 / description）と違う |
| `—`（灰） | その束にこの列が無い |

**`⚠` が並び順と欠落だけなのは、束ね方の裏返し。** 列は `columnSig`
（列名・型・モード・description）で束ねてあるので、型や NULL 制約が束の中で
食い違うことはない。**鍵に入れていない軸は並び順だけ**で、そこが唯一の
`⚠` の出どころになる（`cellInfo` の `mixed`）。

**緑に「基準」は無い。** 全グループを横に並べている以上、どれか 1 つを基準に
立てなくても違いは読める。色を付けるのは**多数派と違うセル**
（`majority(infos.map(c => c.sig))` と比べる）。ロジック差分の `ref_label` は
この表には効かない。

**並び順の違いは列どうしの差として扱わない。** 列を 1 本足すと以降の番号が
まとめてずれるので、差にすると本当に見たい型の差がその中に埋もれる。

行の並びは**先頭の束**（いちばん多いカラム定義）の `ordinal_position` 順。
そこに無い列は後ろに足す（`columnOrder`）。

**列の説明（`COLUMN_FIELD_PATHS.description`）は、列名の欄ではなく束ごとの
セルに出す。** 説明も View に付いた属性なので、束によって違うことがある
（片方だけ書いてある・文面が更新されている）。1 か所にまとめて出すとその差が
消えるが、セルに置けば型や NULL 制約と同じように横に並んで見える。

> **説明は `columnSig` の鍵に入っている**ので、文面が違えばその時点で別の束に
> なる。つまり束の**中**で説明が割れることはない。`descHtml` 側に複数の説明を
> 並べる枝（どの suffix のものかを添えて全部出す）は残してあるが、いまの
> 束ね方では通らない。鍵から説明を外したときに効く。
...
罫線は**行の間だけ**。囲うと表が主役になってしまうが、note の主役はメモのほう。
段の見出しに使っている色（`#EAEEF2`）と同じ濃さにそろえて、行が目で追えるぶん
だけ引く。

#### description が 1 つも無ければ段ごと出さない

以前は「description が設定されていません」と 1 行書いていたが、設定していない
base では毎回それが出るだけで読む手掛かりにならない。ラベルの段を空のときに
引っ込めるのと同じ扱いにした。

**取れなかった場合とは書き分ける。** 組がひとつも作れない
（`descs_json` が空）のは「未設定」ではなく `INFORMATION_SCHEMA.TABLE_OPTIONS`
が読めていないということなので、そちらは案内を出す。黙って消すと気づけない。
一部の View にだけ付いている場合も出す（何が付いていて何が付いていないかは
読みたい情報）。
...
SQL」ではない。どのグループに属しているかはパネルの見出しに出す。

**base の View を 1 本残らず出す。** ロジックが同じ View も別グループの View も
全部並ぶ。だから上限（`chrome.js` の `MAX_SQL_TABS`、**128 枚**）は「割れ方」では
なく **base の View 数**で決まり、リージョンを足すたびに伸びる。ここだけグループ
ではなく View 単位なので、同じ base でも枚数が桁ひとつ多くなりうる。

**並びはリージョン → suffix の文字数 → アルファベット順。**

リージョンを先に見るのは、タブを**リージョンごとの行に分ける**ため。128 枚が
1 本に流れると、どれがどこの View なのか読み取れない（note の見出しでも同じ
ことが起きた）。並びがそのまま行の切れ目になるので、描画側は切れ目を入れる
だけで済む。

```
asia-northeast1  [abjp][abuk][abus][cdjp][cduk][cdus]
─────────────────────────────────────────────────────
asia-southeast1  [efjp][efuk][efus]
```

文字数を suffix より先に見るのは、単純な辞書順だと中間語つきの suffix
（`v2_txjp`）が素のもの（`txjp`）に混ざって並ぶため。文字数を先に見ると素の
ものが先に固まり、枝番や版はその後ろにまとまる。

> **タブは `.vg-sreg` の直接の子にしておくこと。** 選択中を塗る規則が
> `.vg-srN:checked ~ .vg-stablist > .vg-sreg > .vg-stN` の形（兄弟 ~ 子 > 子）
> で、**子孫結合子に変えるとこの viz では効かない**。段を 1 つ増やすこと自体は
> 外側タブ（`.vg-ohead > .vg-otablist > .vg-otN`）で確かめが取れている。
>
> リージョンが取れていない環境では見出しを `View` にして 1 行にまとめる。
> **markup の形は変えない**（変えると規則を 2 通り用意することになる）。
>
> 行の切れ目は**両列にまたがる 1 本**（`.vg-srule`）で引く。左右のセルそれぞれに
> `border-top` を付ける形も試したが、`align-items:baseline` だとセルの上端が
> そろわず線が段違いになった。

```
suffix が jp / us / txjp / txus / v2_jp のとき

  辞書順        jp, txjp, txus, us, v2_jp    ← us が txjp と txus に割り込む
  文字数 → 辞書  jp, us, txjp, txus, v2_jp    ← 長さごとに固まる
```

規則はループで作るので**UDF のコード量は増えない**。描画メモリにもほぼ効かない
（144 View で 0.46 MB。比較ペインの 13 MB に対して小さい）。保存されるバイト数には
効く（SQL タブは基準の行ごとに毎回描かれるので base 全体では ×行数）。

##### 上限は配る CSS の大きさで決まる（32 KB）

**配る CSS は `viewlgc_group_css` の定義本文として BigQuery に載る。**
関数の定義本文は 32 KB までで、これは **JavaScript でも SQL でも同じ**。
かつて JS の枠に収まらなくなって SQL 側へ焼き込んだ経緯があり、そのとき
「SQL なら上限と無関係」と書いて見張りを外していた。24 → 128 に上げたときに

```
Definition body too long 43771; max allowed 32768 bytes
```

で `viewlgc_group_css` の `CREATE` が落ちた。**生成は通り、BigQuery で落ちる**形。

対処は 2 つ入れてある。

1. **同じ飾りの規則をセレクタで束ねる**（`chrome.js` の `groupRule()`）。
   添字ごとに飾りを書くと飾りが添字の数だけ複製される。束ねれば 1 回で済む。
   ...
   1 枚あたり 153 → **89 バイト**。128 枚で 42.2 KB → **30.5 KB**。
   セレクタの形（`兄弟 ~ 親 > 子`）は動かしていない。`,` で束ねても
   一つひとつのセレクタは同じで、実機（Chromium）で全タブ階層の切り替えを
   確かめてある。

2. **CSS を `CSS_PARTS` 本の関数に分けて載せる。** 上限は**関数ごと**なので、
   分ければ天井が上がる。外から呼ぶ名前は従来どおり 1 つで、まとめ役が部品を
   連結して返す。

   ```
   viewlgc_group_css_p1()  …  部品（引数なし。直に呼ぶ用途は無い）
   viewlgc_group_css_p2()
   viewlgc_group_css_p3()
   viewlgc_group_css_p4()
   viewlgc_group_css(options_json)
     = p1() || p2() || p3() || p4()     ← 本体は数百バイト
   ```

   分けるのは**行の切れ目**で、連結すると 1 バイトも違わず元に戻る
   （`splitCss` の `join('') === css`）。生成のたびに突き合わせている。
   部品の数は固定にしてある。増減させると使われない関数がデータセットに残るため。

3. **`node build_udf.mjs` が部品の大きさを見張る。** 超えると生成が失敗し、
   `view_group_html.sql` を書き出さない（＝ BigQuery まで持って行けない）。

| | 定義本文 | 残り |
|---|---|---|
| 1 本に載せていた頃（128 枚） | 31,381 B | 1,387 B |
| **4 分割のいちばん大きい部品（いま）** | **12,841 B** | **19,927 B** |

1 枚あたり約 89 バイトなので、**1000 枚を超えるあたりまでは載る**。足りなく
なったら `CSS_PARTS` を増やす（`build_udf.mjs`）。
...
##### note タブは ラベル ＋ リージョン ＋ View の description ＋ シートのメモ

`note` タブは**出どころの違う 4 つを、箱を分けて縦に積む**（上から順に）。
...
上の 3 段は**中身が無ければ段ごと出さない**（`renderLabels` / `renderRegions` /
`renderDesc` がそれぞれ `''` を返す）。メモの段だけは常に出る（未登録なら
「未登録」の枠）。

1 本のテキストに繋いでいた頃は水平線 1 本しか手掛かりが無く、どこまでが公式の
説明でどこからが運用メモなのかが読み取れなかった。更新のされ方も違うので、
箱を分けて名前を付けてある。

**`description` は base の中で割れていたら全部出す。** 同じロジックの View なのに
説明が違えばそれ自体が情報なので、黙って 1 つだけ出さない（割れた数だけタブに
する）。1 種類でも見出しは出すが、そのときはラジオを持たない `<span>` にする。
**1 つも設定されていなければ段ごと出さない**（「description が設定されて
いません」と毎回書いても読む手掛かりにならないため。下の節を参照）。

`option_value` は JSON 文字列の体裁（`"説明文"`）なので剥がして中身を取り出し、
剥がせない形なら生のまま使う。

ビューが増やす列は `has_note` / `note_md` / `note_html` / `note_updated_at` /
`note_updated_by`。**View の description は列にしない。** あちらはカードを作る
とき（セクション 2）に焼き込んであり、ビューが繋ぐのはシートのメモだけ。

##### ラベルは「量に合わせて重さを変える」

View に付けた `labels`（key:value）を note タブの先頭に出す。ラベルは View ごとの
属性だが、実際にはほぼ base の中で揃う。**揃っているのが普通なので、そこに段を
1 つ使わない。**

| base の中の状態 | 出し方 |
|---|---|
| 1 つも付いていない | **何も出さない** |
| 全 View で同じ（大半） | 見出しも罫線も無い**チップ 1 列**だけ |
| 割れている | **段に昇格**。見出し ＋ 注意書き ＋ タブ（description と同じ形）。見出しの帯にも `ラベル不一致` バッジが出る |

割れているのは滅多に起きず、起きたときは**それ自体が拾いたい差**（同じロジックの
View なのに 8 本が `sales` で 1 本だけ `finance`、など）なので、そのときだけ場所を
取る。「8 本に付いていて 1 本だけ無い」も割れとして扱う（`LEFT JOIN` なので
付いていない View も 1 組として残る）。

1 つも付いていないときに `description` のような「未設定」の行を出さないのは、
labels は付いていない View のほうが普通だから。書くと、ラベルを使っていない
base 全部にその 1 行が並ぶ。

**キーはアルファベット順**に並べ直す（SQL 側と描画側の両方で）。並びが実行の
たびに変われば、値が同じでも別の組に見える。

> **`labels` の `option_value` は JSON ではない。** 同じ `TABLE_OPTIONS` でも
> `description` と違い、
> `[STRUCT("domain", "sales"), STRUCT("tier", "gold")]` という SQL リテラル風の
> 文字列で入っている。`PARSE_JSON` では読めない（黙って NULL になり、ラベルが
> 1 つも付いていないのと同じ見え方になる）ので、引用符で囲まれた文字列を順に
> 拾って 2 つずつ組にしている。ラベルに使える文字に引用符は含まれないので、
> この拾い方で欠けたり混ざったりしない。`node check_sql.mjs` が見張っている。
...
5. **Templated Record の並べ替えを `ref_index` 昇順**にする
   （**既定の基準を「View がいちばん多いグループ」にするため**。下記）

> **既定の基準は `ref_index` 昇順で決まる。** グループは解析の時点で
> **メンバの多い順**に並べてあるので（`groupByLogic` が
> `b.members.length - a.members.length` で並べ替える）、**`ref_index = 0` が
> いちばん View の多いグループ**になる。
>
> `ref_label` をまだ選んでいない状態では、その base の基準ぶんのレコードが
> 全部該当する。Templated Record は 1 レコードしか描かないので、**どれが出るかは
> グラフの並べ替えで決まる。** `ref_index` 昇順にしておけば先頭 = 最大グループが
> 既定で出て、コントロールで選べばそちらが勝つ。
>
> 各行の基準グループの View 数は `ref_view_count` 列で読める。`ref_index` の
> 意味に頼らず `ref_view_count` 降順で並べても同じ結果になる（同数のときは
> suffix 順で決まる）。

> **`ref_label` のコントロールが要る。** 1 レコード = 1 base × 1 基準なので、
> base だけ選ぶと基準の数だけレコードが該当してしまう。2 つ目を置かないと
> どの基準が出るか決まらない。
>
> 同じページに置けばコントロールどうしが効き合うので、`base` を選んだあとの
> `ref_label` にはその base のグループだけが並ぶ。
>
> 基準を行に分けた理由は「基準は行が持つ」の節を参照
> （全基準を 1 枚に載せると比較ペインがグループ数の二乗になり、
> リージョンをまたいだときに UDF のメモリを使い切った）。
...
> **集計するチャートを作るときは、行が base ごとに複数あることに注意。**
> 1 レコード = 1 base × 1 基準なので、`view_count` や `group_count` をそのまま
> 合計すると基準の数ぶん膨らむ。`base` でグループ化して `MAX()` を取るか、
> `ref_index = 0` で絞る。Templated Record 側（1 レコードを描くだけ）は
> 影響を受けない。
...
コメントアウトしてある** — あれだけがスプレッドシートの外部テーブル（Drive）を
読むので時間がかかることがあり、メモを直したときにしか見ないため。
見たいときは 4 文（`SET` から `EXECUTE IMMEDIATE` まで）のコメントを外す。
...
JSON として読んだもの（`{"ja": "受注日", "en": "order date"}`）。**読むだけで、
このツールから書く手段は無い。**

シートのメモ（`note_sheet_url`）と同じ感覚で編集させたくなるが、**置き場所が
まったく違う**ので同じ作りにはできない。

| | メモ | カラムの description |
|---|---|---|
| 置き場所 | スプレッドシート（外部テーブル） | BigQuery のカラムのメタデータ |
| 書き換え | シートを直すだけ。ビューがその場で拾う | View の定義そのものを書き換える必要がある |
| 反映 | 3b を流し直せばレポートに出る | デプロイの範囲。次の日次実行で拾う |

着手するなら、**先に決めるのはここ**。

- **どこを「正」にするか。** View の DDL が正なら、編集画面は DDL を書き換える
  提案を出すだけ（実際の反映はデプロイ）になる。別の台帳を正にするなら、
  台帳 → DDL の反映をどう担保するかが要る。デプロイの仕組みと衝突しやすいので、
  ここを曖昧にしたまま画面から作らないこと。
- **書き戻す先の権限。** 読み取りしか要らない今の構成と違い、対象データセットへの
  書き込み権限が要る。スケジュールドクエリの権限とは別に考える。
- **キーの綴り。** 描画側（`columns.js` の `DESC_JA_KEYS` / `DESC_EN_KEYS`）は
...
同じ変換器（`markdown.js`）を入れた下書きページが既にあるので、土台はある。
