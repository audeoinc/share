...
#### 選択中のタブを塗る規則は「兄弟 > 子」から動かさない

```css
.vg-orN:checked ~ .vg-otablist > .vg-otN { … }
```

**この viz で radio + `:checked` が動くことを確かめたときの形がこれ**
（`templated_record/samples/07_radio_tabs_test.html`）。見出しを別の入れ物で包んで
子孫（`~ .vg-ohead .vg-otN`）に変えたところ、**手元のブラウザでは動くのに実機では
反転しなくなった**。埋め込み先で何かが起きているが、原因を特定できていないので、
確認が取れている形から動かさない方針にしている。

そのため**見出しは `.vg-otablist` の中に直接置く**。包む入れ物を挟むと上の形が
崩れる。並びは `.vg-otablist` を `flex-wrap` にして、見出しだけ 1 行占有させる。

ラジオは `.vg-outer` の直下。ここに置いてあれば `.vg-otablist` も `.vg-opanels`
も後ろに続く兄弟になり、どちらの規則も「兄弟 > 子」で書ける。

> **テンプレートの CSS は手で貼るのに対して、カードは日次で作り直す。**
> この 2 つの世代がズレている時間帯は仕組み上どうしても発生する。上の形なら
> 旧世代のカード（見出しが帯の外にあるもの）でも `.vg-otablist > .vg-otN` は
> 同じなので、そのまま効く。外枠を変えるときは**新旧どちらの HTML でも効くか**を
> 必ず確かめること。片方でしか効かないと「タブが反転しない」「帯が固定されない」が
> 同時に起き、しかも画面からは CSS と HTML のどちらが古いのか分からない。
...
ため（低いチャートでは `100vh`、高いチャートでは `640px` が効く）。`max-height`
なので、中身が収まればスクロールバーも出ない。

> **640px はレポートのチャートの高さに合わせて変える値。** テンプレートに貼る
...
並べて一望できたほうが読みやすく、切り替えの操作も要らない。並びは差分と同じで
基準が先頭。同じ base を見ているのに差分と ERD で並びが違うと、対応を取り直す
手間が要る。
