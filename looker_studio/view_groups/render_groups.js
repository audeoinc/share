...
function chromeCss() {
  const rules = [
    ...
    // ★ カードの高さ。ここだけは環境に合わせて変える値。
    //   レポートのチャートの高さに合わせて 2000px の数字を書き換える。
    //
    // カードを自前のスクロール箱にしている。position:sticky は「いちばん近い
    // スクロールする祖先」を基準にするので、埋め込み先まかせにすると
    //   ・overflow:hidden の祖先が 1 つあるだけで効かない
    //   ・iframe が中身の高さぶん伸びる形だと、中にスクロールする要素が
    //     1 つも無く、100vh も中身の高さになるので上限として働かない
    // という具合に落ちる。実際 Looker では両方とも効かなかった。
    // 固定の px を上限にすれば、埋め込み先がどの形でもこの箱が必ず
    // スクロールする側になり、帯はその中で貼り付く。
    //   min() で 100vh と併記しているのは、チャートがこれより低いときに
    //   はみ出さないため。max-height なので、中身が収まればスクロールバーも
    //   出ない（低いチャートでは 100vh、高いチャートでは 2000px が効く）。
    // --vg-bar は上の帯（見出し＋タブ）の高さ。カラム定義の列名行をその下に
    //   貼り付けるのに使う。帯の中身を変えたら合わせて直す（preview.mjs が
    //   実際に描いて位置を突き合わせる）。
