...
// 1 レコードに載せる差分の上限。基準を 1 つ増やすたびに比較ペインがグループ数
// ぶん増える（全部で G×(G−1) 枚）ので、際限なく積むと 1 レコードが重くなる。
//
// **ここは「まず起きない値」にしておく。** 打ち切りは異常な入力を止めるための
// 安全弁であって、通常の運用で当たると「基準を選べない」という分かりにくい形で
// 表に出る（実際 600 KB にしていたときに、3 グループ × 500 行の SQL で基準タブが
// 1 枚しか出なかった）。
//
// ペイン 1 枚の大きさは SQL の長さにほぼ比例する。実測で 100 行なら 30 KB 前後、
// 500 行なら 500 KB 前後。基準ごとに (G−1) 枚できるので、3 グループ × 500 行で
// 1 レコード 3 MB ほどになる。重いが、基準を選べないよりはよいという判断。
// 短くする手立て（変更行の前後だけ描くなど）はこちらの描画側には無い。
const REF_BUDGET = 8 * 1024 * 1024;
