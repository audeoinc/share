...
//
// 効くのは配る CSS の大きさ。これは group_css の**定義本文**として BigQuery に
// 載り、本文は 32 KB までしか無い（JavaScript でも SQL でも同じ）。24 → 128 に
// 上げたときに 43,771 バイトになり、CREATE が
//   Definition body too long 43771; max allowed 32768 bytes
// で落ちた。手当てが 2 つ入っている。
//   ・groupRule() が同じ飾りの規則を ',' で束ねる（1 枚あたり 153 → 89 バイト）
//   ・CSS を CSS_PARTS 本の関数に分けて載せ、group_css が連結して返す
//     （**上限は関数ごと**なので、分ければ天井が上がる）
// いまは 4 分割で、いちばん大きい部品が 12.8 KB。1 枚 89 バイトなので
// **1000 枚を超えるあたりまでは載る**。足りなくなったら CSS_PARTS を増やす。
//
// **上げるときは node build_udf.mjs が部品の大きさを見張る。** 超えると生成が
