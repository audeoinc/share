...
`)}</pre></div>`,lines:n.length}}function sqlPanel(s,n){if(n==null||String(n)==="")return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(s.viewName)}</span></div>`+notice("\u3053\u306E View \u306E SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002");const e=sqlBody(n);return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(s.viewName)}</span><span class="vg-sqlmeta">\u30B0\u30EB\u30FC\u30D7: ${esc(s.group)}</span><span class="vg-sqlmeta">${e.lines} \u884C</span></div>`+e.html}function renderSql(s,n){const e=sqlViews(s);if(!e.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const l=n||{};if(!e.some(a=>l[a.viewName]))return notice("SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.VIEWS \u304B\u3089 view_definition \u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const o=e.slice(0,24),i="vgs"+hashId(s.base+"|"+e.map(a=>a.viewName).join("|")),t=o.map((a,f)=>`<input class="vg-sr vg-sr${f+1}" type="radio" name="${i}" id="${i}-${f+1}"${f===0?" checked":""}>`).join(""),r=o.map((a,f)=>`<label class="vg-stab vg-st${f+1}" for="${i}-${f+1}">${esc(a.suffix)}</label>`).join(""),c=o.map((a,f)=>`<div class="vg-spanel vg-sp${f+1}">${sqlPanel(a,l[a.viewName])}</div>`).join("");return(e.length>o.length?notice(`View \u304C\u591A\u3044\u305F\u3081\u5148\u982D 24 \u4EF6\u306E\u307F\u30BF\u30D6\u8868\u793A\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${e.length} \u4EF6\uFF09\u3002`):"")+`<div class="vg-stabs">${t}<div class="vg-stablist"><span class="vg-slabel">View</span>${r}</div><div class="vg-spanels">${c}</div></div>`}function renderSqlBase(s,n){return`<div class="vg-root">${renderSql(s,n)}</div>`}function suffixByView(s){const n={},e=s&&s.groups||[];for(let l=0;l<e.length;l++){const o=e[l],i=o.members||[];for(let t=0;t<i.length;t++){const r=i[t]&&i[t].viewName;r&&(n[r]=o.suffixes&&o.suffixes[t]||r)}}return n}function descGroups(s,n){const e=Array.isArray(n)?n:[],l=suffixByView(s),o=[];for(let i=0;i<e.length;i++){const t=e[i]||{},r=Array.isArray(t.v)?t.v:t.v==null||t.v===""?[]:[t.v],c=r.map(a=>l[a]||a).sort((a,f)=>a<f?-1:a>f?1:0),u=String(t.h==null?"":t.h);o.push({label:c.join(", ")||"(View \u306A\u3057)",count:r.length,html:u,empty:u.trim()===""})}return o.sort((i,t)=>(i.empty===t.empty?0:i.empty?1:-1)||t.count-i.count||(i.label<t.label?-1:i.label>t.label?1:0)),o}function descPanel(s){return s.empty?notice("\u3053\u306E View \u306B\u306F description \u304C\u8A2D\u5B9A\u3055\u308C\u3066\u3044\u307E\u305B\u3093\u3002"):s.html}function renderDesc(s,n){const e=descGroups(s,n);if(!e.some(a=>!a.empty))return"";const l='<div class="vg-nhead">View \u306E description</div>';if(e.length===1)return`<div class="vg-nsec">${l}<div class="vg-dbox">${descPanel(e[0])}</div></div>`;const o=e.slice(0,16),i="vgd"+hashId((s&&s.base||"")+"|"+e.map(a=>a.label).join("|")),t=o.map((a,f)=>`<input class="vg-dr vg-dr${f+1}" type="radio" name="${i}" id="${i}-${f+1}"${f===0?" checked":""}>`).join(""),r=o.map((a,f)=>`<label class="vg-dtab vg-dt${f+1}" for="${i}-${f+1}">${esc(a.label)}<span class="vg-tabn">${a.count}</span></label>`).join(""),c=o.map((a,f)=>`<div class="vg-dpanel vg-dp${f+1}">${descPanel(a)}</div>`).join(""),u=e.length>o.length?notice(`description \u304C\u591A\u3044\u305F\u3081\u5148\u982D 16 \u7A2E\u985E\u306E\u307F\u51FA\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${e.length} \u7A2E\u985E\uFF09\u3002`):"";return`<div class="vg-nsec">${l}${u}<div class="vg-dtabs">${t}<div class="vg-dtablist">${r}</div><div class="vg-dpanels">${c}</div></div></div>`}function renderNote(s,n,e){const l=renderDesc(s,n),o=String(e==null?"":e);return l?l+`<div class="vg-nsec"><div class="vg-nhead">\u30E1\u30E2</div>${o}</div>`:o}function __opts(s){if(!s)return{};try{return JSON.parse(s)||{}}catch(n){return{}}}function __notice(s){return'<div class="vg-notice">'+String(s).replace(/[<>&]/g,"")+"</div>"}function __run(s,n,e,l,o,i,t){var r=__opts(t),c;try{c=JSON.parse(s)}catch(x){c=null}if(!c)return String(n||__notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002"));var u={};try{for(var a=JSON.parse(l)||[],f=0;f<a.length;f++)u[a[f].v]=a[f].cols||[]}catch(x){u={}}var p={};try{for(var m=JSON.parse(o)||[],d=0;d<m.length;d++)p[m[d].v]=m[d].s}catch(x){p={}}var g=[];try{g=JSON.parse(i)||[]}catch(x){g=[]}for(var h="",S="",$=c.bases||[],b=0;b<$.length;b++)h+=renderColumnsBase($[b],u),S+=renderSqlBase($[b],p);h||(h=__notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),S||(S=__notice("SQL \u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002"));var j=c.bases&&c.bases.length?c.bases[0]:null;return wrapPage(String(n||""),String(e||__notice("\u53C2\u7167\u95A2\u4FC2\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002")),h,S,renderNote(j,g,NOTE_MARK),j)}return __run(analysis_json,diff_html,erd_html,columns_json,sql_json,descs_json,options_json);
...
.vg-dr1:checked ~ .vg-dtablist > .vg-dt1{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr2:checked ~ .vg-dpanels > .vg-dp2{display:block}
.vg-dr2:checked ~ .vg-dtablist > .vg-dt2{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr3:checked ~ .vg-dpanels > .vg-dp3{display:block}
.vg-dr3:checked ~ .vg-dtablist > .vg-dt3{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr4:checked ~ .vg-dpanels > .vg-dp4{display:block}
.vg-dr4:checked ~ .vg-dtablist > .vg-dt4{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr5:checked ~ .vg-dpanels > .vg-dp5{display:block}
.vg-dr5:checked ~ .vg-dtablist > .vg-dt5{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr6:checked ~ .vg-dpanels > .vg-dp6{display:block}
.vg-dr6:checked ~ .vg-dtablist > .vg-dt6{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr7:checked ~ .vg-dpanels > .vg-dp7{display:block}
.vg-dr7:checked ~ .vg-dtablist > .vg-dt7{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr8:checked ~ .vg-dpanels > .vg-dp8{display:block}
.vg-dr8:checked ~ .vg-dtablist > .vg-dt8{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr9:checked ~ .vg-dpanels > .vg-dp9{display:block}
.vg-dr9:checked ~ .vg-dtablist > .vg-dt9{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr10:checked ~ .vg-dpanels > .vg-dp10{display:block}
.vg-dr10:checked ~ .vg-dtablist > .vg-dt10{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr11:checked ~ .vg-dpanels > .vg-dp11{display:block}
.vg-dr11:checked ~ .vg-dtablist > .vg-dt11{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr12:checked ~ .vg-dpanels > .vg-dp12{display:block}
.vg-dr12:checked ~ .vg-dtablist > .vg-dt12{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr13:checked ~ .vg-dpanels > .vg-dp13{display:block}
.vg-dr13:checked ~ .vg-dtablist > .vg-dt13{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr14:checked ~ .vg-dpanels > .vg-dp14{display:block}
.vg-dr14:checked ~ .vg-dtablist > .vg-dt14{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr15:checked ~ .vg-dpanels > .vg-dp15{display:block}
.vg-dr15:checked ~ .vg-dtablist > .vg-dt15{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
.vg-dr16:checked ~ .vg-dpanels > .vg-dp16{display:block}
.vg-dr16:checked ~ .vg-dtablist > .vg-dt16{background:#DAFBE1;border-color:#4AC26B;color:#1A7F37}
