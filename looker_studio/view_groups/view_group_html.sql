...
var O=Object.defineProperty,R=Object.defineProperties;var C=Object.getOwnPropertyDescriptors;var y=Object.getOwnPropertySymbols;var M=Object.prototype.hasOwnProperty,L=Object.prototype.propertyIsEnumerable;var A=(e,t,s)=>t in e?O(e,t,{enumerable:!0,configurable:!0,writable:!0,value:s}):e[t]=s,T=(e,t)=>{for(var s in t||(t={}))M.call(t,s)&&A(e,s,t[s]);if(y)for(var s of y(t))L.call(t,s)&&A(e,s,t[s]);return e},$=(e,t)=>R(e,C(t));const MAX_TABS=12,OUTER_TABS=["note","\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206","\u53C2\u7167\u95A2\u4FC2","\u30AB\u30E9\u30E0\u5B9A\u7FA9"],NOTE_MARK="<!--VG_NOTE-->";function esc(e){return String(e==null?"":e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function hashId(e){let t=2166136261;for(let s=0;s<String(e).length;s++)t^=String(e).charCodeAt(s),t=Math.imul(t,16777619)>>>0;return t.toString(36)}const label=e=>e.suffixes.map((t,s)=>t||e.members[s]&&e.members[s].viewName||"(suffix \u306A\u3057)").join(", ");function badge(e,t,s){return`<span class="vg-badge" style="color:${t};background:${s}">${esc(e)}</span>`}function header(e,t,s,n){const o=s>1;return`<div class="vg-header"><span class="vg-title">${esc(e)}</span>`+badge(`${t} View`,"#57606A","#EAEEF2")+badge(`${s} \u30B0\u30EB\u30FC\u30D7`,o?"#9A6700":"#1A7F37",o?"#FFF8C5":"#DAFBE1")+(n?badge("suffix \u672A\u8A8D\u8B58","#9A6700","#FFF8C5"):"")+"</div>"}function notice(e){return`<div class="vg-notice">${esc(e)}</div>`}const KIND_TEXT={entity:"\u5B9F\u4F53\u540D",string:"\u5024\uFF08\u6587\u5B57\u5217\uFF09",number:"\u5024\uFF08\u6570\u5024\uFF09",ident:"\u540D\u524D",quoted:"\u540D\u524D",keyword:"\u4E88\u7D04\u8A9E",punct:"\u8A18\u53F7",comment:"\u30B3\u30E1\u30F3\u30C8"},kindText=e=>KIND_TEXT[e]||e;function referenceIndex(e,t){const s=(e.groups||[]).length,n=Math.trunc(Number((t||{}).referenceIndex));return Number.isFinite(n)&&n>=0&&n<s?n:0}function wrapPage(e,t,s,n){const o=n||{},i="vgo"+hashId(o.base||""),c=[`<div class="vg-root">${NOTE_MARK}</div>`,e,t,s],l=OUTER_TABS.map((h,p)=>`<input class="vg-or vg-or${p+1}" type="radio" name="${i}" id="${i}-${p+1}"${p===0?" checked":""}>`).join(""),a=OUTER_TABS.map((h,p)=>`<label class="vg-otab vg-ot${p+1}" for="${i}-${p+1}">${esc(h)}</label>`).join(""),d=OUTER_TABS.map((h,p)=>`<div class="vg-opanel vg-op${p+1}">${c[p]||""}</div>`).join(""),f=o.base?header(o.base,o.viewCount,(o.groups||[]).length,o.unmatched):"";return`<div class="vg-outer">${l}<div class="vg-otablist">${f}${a}</div><div class="vg-opanels">${d}</div></div>`}const DQ="\\u0022",DQ3=DQ+DQ+DQ,TOKEN_RE=new RegExp(["(`[^`]*`)","([rbRB]{1,2}(?:'''[\\s\\S]*?'''|"+DQ3+"[\\s\\S]*?"+DQ3+"|'[^']*'|"+DQ+"[^"+DQ+"]*"+DQ+"))","('''[\\s\\S]*?''')","("+DQ3+"[\\s\\S]*?"+DQ3+")","('(?:\\\\.|''|[^'\\\\])*')","("+DQ+"(?:\\\\.|"+DQ+DQ+"|[^"+DQ+"\\\\])*"+DQ+")","(--[^\\n]*|#[^\\n]*)","(/\\*[\\s\\S]*?\\*/)","(0[xX][0-9a-fA-F]+|(?:\\d+(?:\\.\\d+)?|\\.\\d+)(?:[eE][+-]?\\d+)?)","([A-Za-z_][A-Za-z0-9_]*)","(\\s+)","([^\\s])"].join("|"),"g"),KEYWORDS=new Set("SELECT FROM WHERE JOIN INNER LEFT RIGHT FULL OUTER CROSS ON USING AND OR NOT IN IS NULL LIKE BETWEEN EXISTS ANY ALL SOME GROUP BY HAVING ORDER ASC DESC LIMIT OFFSET QUALIFY WINDOW UNION INTERSECT EXCEPT DISTINCT AS CASE WHEN THEN ELSE END CREATE OR REPLACE VIEW TABLE FUNCTION IF EXISTS OPTIONS WITH RECURSIVE OVER PARTITION UNNEST STRUCT ARRAY CAST SAFE_CAST EXTRACT INTERVAL DATE DATETIME TIME TIMESTAMP INT64 FLOAT64 NUMERIC BIGNUMERIC STRING BYTES BOOL TRUE FALSE COUNT SUM AVG MIN MAX COALESCE IFNULL NULLIF ROWS RANGE PRECEDING FOLLOWING CURRENT ROW".split(/\s+/));function tokenizeSql(e){const t=[],s=String(e==null?"":e);let n;for(TOKEN_RE.lastIndex=0;(n=TOKEN_RE.exec(s))!==null;){let o;n[1]?o="quoted":n[2]||n[3]||n[4]||n[5]||n[6]?o="string":n[7]||n[8]?o="comment":n[9]?o="number":n[10]?o=KEYWORDS.has(n[10].toUpperCase())?"keyword":"ident":n[11]?o="space":o="punct",t.push({kind:o,text:n[0]})}return t}function stripOptionsClause(e){const t=[];let s=0;for(;s<e.length;){const n=e[s];if(n.kind==="keyword"&&n.text.toUpperCase()==="OPTIONS"){let o=s+1;for(;o<e.length&&e[o].kind==="space";)o++;if(o<e.length&&e[o].text==="("){let i=0,c=o;for(;c<e.length;c++)if(e[c].text==="(")i++;else if(e[c].text===")"&&(i--,i===0)){c++;break}for(;t.length>0&&t[t.length-1].kind==="space";)t.pop();s=c;continue}}t.push(n),s++}return t}function markEntities(e){const t=e.slice(),s=i=>{let c=i+1;for(;c<t.length&&t[c].kind==="space";)c++;return c},n=[];let o=null;for(let i=0;i<t.length;i++){const c=t[i];if(c.kind==="space"||c.kind==="comment")continue;if(c.text==="("){n.push(o),o=null;continue}if(c.text===")"){n.pop(),o=null;continue}const l=c.text.toUpperCase();if(o=c.kind==="keyword"||c.kind==="ident"?l:null,c.kind!=="keyword"||l!=="FROM"&&l!=="JOIN"||n.length>0&&n[n.length-1]==="EXTRACT")continue;let a=s(i);for(;!(a>=t.length);){const d=t[a];if(d.kind==="quoted")t[a]={kind:"entity",text:d.text},a=s(a);else if(d.kind==="ident"){const f=[];let h=a;for(;;){f.push(h);const g=s(h);if(g<t.length&&t[g].text==="."){const u=s(g);if(u<t.length&&t[u].kind==="ident"){h=u;continue}}break}const p=s(h);if(p<t.length&&t[p].text==="(")break;for(const g of f)t[g]={kind:"entity",text:t[g].text};a=p}else break;if(a<t.length&&t[a].kind==="keyword"&&t[a].text.toUpperCase()==="AS"){const f=s(a);f<t.length&&(t[f].kind==="ident"||t[f].kind==="quoted")&&(a=s(f))}else a<t.length&&t[a].kind==="ident"&&(a=s(a));if(a<t.length&&t[a].text===","){a=s(a);continue}break}}return t}const DEFAULT_SUFFIX_RE=/^(.*?)_([A-Za-z0-9]{1,6})$/,DEFAULT_SUBSTITUTABLE=["entity","number","string"],SUFFIX_MARK="\0",LITERAL_MARK="\u0001",BOX_W_MIN=130,BOX_W_MAX=380,NAME_CHAR_W=6.65,BOX_H=40,GAP_MIN=78,CHAR_W=6.5,LINE_H=14,GAP_Y=14,PAD=10,kw=e=>e&&e.kind==="keyword"?e.text.toUpperCase():null;function prepare(e){const t=[];let s=0;for(const n of markEntities(tokenizeSql(e)))n.kind==="space"||n.kind==="comment"||(n.text===")"&&s--,t.push({kind:n.kind,text:n.text,depth:s}),n.text==="("&&s++);return t}function cteRanges(e){const t=[];if(!(e[0]&&kw(e[0])==="WITH"))return{ctes:t,mainFrom:0};let s=1;for(;;){const n=e[s],o=e[s+1],i=e[s+2];if(!i||n.kind!=="ident"||kw(o)!=="AS"||i.text!=="(")break;const c=i.depth;let l=s+3;for(;l<e.length&&!(e[l].text===")"&&e[l].depth===c);)l++;if(t.push({name:n.text,from:s+3,to:l,depth:c+1}),s=l+1,e[s]&&e[s].text===","){s++;continue}break}return{ctes:t,mainFrom:s}}const STOP=new Set(["WHERE","GROUP","ORDER","QUALIFY","WINDOW","HAVING","UNION","INTERSECT","EXCEPT","LIMIT","SELECT","JOIN","FROM"]);function skipParen(e,t,s){const n=e[t].depth;let o=t+1;for(;o<s&&!(e[o].text===")"&&e[o].depth===n);)o++;return o+1}function readSource(e,t,s){if(t>=s)return{node:null,next:t};const n=e[t];if(n.text==="(")return{node:{name:"(\u30B5\u30D6\u30AF\u30A8\u30EA)",kind:"subquery"},next:skipParen(e,t,s)};if(kw(n)==="UNNEST"){let o=t+1,i="";if(e[o]&&e[o].text==="("){const c=e[o].depth;let l=o+1;for(;l<s&&!(e[l].text===")"&&e[l].depth===c);)i+=e[l].text,l++;o=l+1}return{node:{name:"UNNEST("+i+")",kind:"unnest"},next:o}}if(n.kind==="entity"||n.kind==="quoted"||n.kind==="ident"){const o=[];let i=t;for(;i<s&&(e[i].kind==="entity"||e[i].kind==="quoted"||e[i].kind==="ident");){if(o.push(e[i].text),i++,i<s&&e[i].text==="."){i++;continue}break}return i<s&&e[i].text==="("?{node:null,next:skipParen(e,i,s)}:{node:{name:o.join("."),kind:"ref"},next:i}}return{node:null,next:t+1}}function skipAlias(e,t,s){if(t<s&&kw(e[t])==="AS"){const n=e[t+1];return n&&(n.kind==="ident"||n.kind==="quoted")?t+2:t+1}return t<s&&e[t].kind==="ident"&&!STOP.has(e[t].text.toUpperCase())?t+1:t}function readOnKeys(e,t,s){const n=[];let o=t;for(;o<s;){const i=kw(e[o]);if(i&&STOP.has(i)||i==="LEFT"||i==="RIGHT"||i==="FULL"||i==="INNER"||i==="CROSS")break;if(e[o].text==="="){const c=(d,f,h)=>d&&f&&h&&f.text==="."?h.text:null,l=c(e[o-3],e[o-2],e[o-1]),a=c(e[o+1],e[o+2],e[o+3]);l&&a&&n.push(l===a?l:l+" = "+a)}o++}return{keys:n,next:o}}function readUsingKeys(e,t,s){const n=[];let o=t;if(e[o]&&e[o].text==="("){const i=e[o].depth;for(o++;o<s&&!(e[o].text===")"&&e[o].depth===i);)e[o].kind==="ident"&&n.push(e[o].text),o++;o++}return{keys:n,next:o}}function joinKind(e,t,s){for(let n=t-1;n>=s&&t-n<=3;n--){const o=kw(e[n]);if(o==="LEFT"||o==="RIGHT"||o==="FULL"||o==="CROSS")return o;if(o==="INNER")return"INNER"}return"INNER"}function scanScope(e,t,s,n){const o=[];for(let i=t;i<s;i++){const c=kw(e[i]);if(c!=="FROM"&&c!=="JOIN")continue;const l=e[i].depth>n,a=c==="JOIN"?joinKind(e,i,t):null;let d=i+1;for(;;){const f=readSource(e,d,s);if(d=f.next,f.node){d=skipAlias(e,d,s);let h=[];if(!l&&d<s&&kw(e[d])==="ON"){const p=readOnKeys(e,d+1,s);h=p.keys,d=p.next}else if(!l&&d<s&&kw(e[d])==="USING"){const p=readUsingKeys(e,d+1,s);h=p.keys,d=p.next}f.node.kind!=="unnest"&&o.push({name:f.node.name,kind:f.node.kind,joinType:a,keys:h,nested:l})}if(d<s&&e[d].text===","&&e[d].depth===e[i].depth){d++;continue}break}i=Math.max(i,d-1)}return o}function buildGraph(e,t){const s=new Map((t||[]).map(u=>[u.name,u])),n=u=>{const m=Object.keys(u.values);return m.length?u.values[m[0]]:null},o=new Map,i=String(e).replace(/\{\{(P\d+)\}\}/g,(u,m)=>{const k=s.get(m);if(!k)return u;const E=n(k);return E==null?u:(o.set(E,k),E)}),c=prepare(i),{ctes:l,mainFrom:a}=cteRanges(c),d=new Set(l.map(u=>u.name)),f=new Map,h=[],p=(u,m)=>{const k=m+":"+u;if(!f.has(k)){const E=[],r=o.get(String(u));if(r)E.push(r);else for(const x of String(u).split(".")){const v=o.get(x);v&&E.indexOf(v)<0&&E.push(v)}f.set(k,{id:k,name:u,label:u,kind:m,params:E})}return k},g=l.map(u=>({id:p(u.name,"cte"),from:u.from,to:u.to,depth:u.depth}));g.push({id:p("(\u6700\u7D42 SELECT)","output"),from:a,to:c.length,depth:0});for(const u of g)for(const m of scanScope(c,u.from,u.to,u.depth)){const k=m.kind==="ref"?d.has(m.name)?"cte":"table":m.kind,E=p(m.name,k);if(E===u.id)continue;const r=h.find(x=>x.from===E&&x.to===u.id);r?(r.keys.length||(r.keys=m.keys),r.joinType||(r.joinType=m.joinType),r.nested=r.nested&&m.nested):h.push({from:E,to:u.id,joinType:m.joinType,keys:m.keys,nested:m.nested})}return{nodes:[...f.values()],edges:h}}function layout(e){const{nodes:t,edges:s}=e,n=new Map(t.map(r=>[r.id,[]])),o=new Map(t.map(r=>[r.id,[]]));for(const r of s)n.has(r.to)&&n.get(r.to).push(r.from),o.has(r.from)&&o.get(r.from).push(r.to);const i=new Map(t.map(r=>[r.id,0]));for(let r=0;r<t.length+1;r++){let x=!1;for(const v of t){const w=n.get(v.id);if(!w.length)continue;const b=Math.max(...w.map(F=>i.get(F)||0))+1;b>i.get(v.id)&&(i.set(v.id,b),x=!0)}if(!x)break}const c=Math.max(0,...t.map(r=>i.get(r.id))),l=new Map(t.map(r=>[r.id,o.get(r.id).length?1/0:c]));for(let r=0;r<t.length+1;r++){let x=!1;for(const v of t){const w=o.get(v.id);if(!w.length)continue;const b=Math.min(...w.map(F=>l.get(F)));b-1<l.get(v.id)&&(l.set(v.id,b-1),x=!0)}if(!x)break}for(const r of t)Number.isFinite(l.get(r.id))||l.set(r.id,c);const a=[];for(const r of t){const x=Math.max(0,l.get(r.id));(a[x]||(a[x]=[])).push(r)}const d=new Map;a.forEach((r,x)=>{r&&(x>0&&r.sort((v,w)=>{const b=F=>{const N=n.get(F.id).filter(S=>d.has(S));return N.length?N.reduce((S,I)=>S+d.get(I),0)/N.length:Number.MAX_SAFE_INTEGER};return b(v)-b(w)}),r.forEach((v,w)=>d.set(v.id,w)))});const f=boxWidth(t),h=new Map;a.forEach((r,x)=>(r||[]).forEach(v=>h.set(v.id,x)));const p=new Array(Math.max(0,a.length-1)).fill(GAP_MIN);for(const r of s){const x=(h.get(r.to)||0)-1;x<0||x>=p.length||(p[x]=Math.max(p[x],linesWidth(edgeLines(r))+22))}const g=[];let u=PAD;for(let r=0;r<a.length;r++)g[r]=u,u+=f+(p[r]||0);const m=[];a.forEach((r,x)=>{(r||[]).forEach((v,w)=>{m.push($(T({},v),{x:g[x],y:PAD+w*(BOX_H+GAP_Y),w:f,h:BOX_H}))})});const k=new Map(m.map(r=>[r.id,r])),E=Math.max(1,...a.map(r=>(r||[]).length));return{nodes:m,edges:s.map(r=>$(T({},r),{a:k.get(r.from),b:k.get(r.to)})).filter(r=>r.a&&r.b),gaps:p,colOf:h,width:(g[a.length-1]||PAD)+f+PAD,height:PAD*2+E*BOX_H+Math.max(0,E-1)*GAP_Y+6}}const KIND={table:{text:"\u5B9F\u30C6\u30FC\u30D6\u30EB",fill:"#FFFFFF",stroke:"#8C6D3F",bar:"#C9A227"},cte:{text:"CTE",fill:"#FFFFFF",stroke:"#3F6D8C",bar:"#4E8FBF"},output:{text:"\u6700\u7D42 SELECT",fill:"#FFFFFF",stroke:"#6D3F8C",bar:"#8250DF"},subquery:{text:"\u30B5\u30D6\u30AF\u30A8\u30EA",fill:"#FFFFFF",stroke:"#6E7781",bar:"#9AA4AE"}},kindOf=e=>KIND[e]||KIND.subquery;function shortName(e){const t=String(e).replace(/`/g,""),s=t.lastIndexOf(".");return s>=0?t.slice(s+1):t}function boxWidth(e){let t=BOX_W_MIN;for(const s of e)t=Math.max(t,shortName(s.label).length*NAME_CHAR_W+24);return Math.min(t,BOX_W_MAX)}function fit(e,t){const s=String(e),n=Math.floor((t-24)/NAME_CHAR_W);return s.length>n?s.slice(0,n-1)+"\u2026":s}function edgeLines(e){const t=[];e.joinType&&t.push(e.joinType==="INNER"?"JOIN":e.joinType+" JOIN");for(const s of e.keys||[])t.push(s);return!t.length&&e.nested&&t.push("\u30B5\u30D6\u30AF\u30A8\u30EA"),t}function linesWidth(e){let t=0;for(const s of e)t=Math.max(t,s.length*CHAR_W);return t}function edgeLabel(e){const t=edgeLines(e);return t.length?t.length>1?t[0]+" / "+t.slice(1).join(", "):t[0]:""}function toSvg(e){const t=[];t.push(`<svg viewBox="0 0 ${e.width} ${e.height}" width="${e.width}" height="${e.height}" role="img" aria-label="\u53C2\u7167\u95A2\u4FC2\u56F3" xmlns="http://www.w3.org/2000/svg">`),t.push('<defs><marker id="vgarrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="7" markerHeight="7" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#8C96A0"/></marker></defs>');const s=[];for(const n of e.edges){const o=n.a.x+n.a.w,i=n.a.y+n.a.h/2,c=n.b.x,l=n.b.y+n.b.h/2,a=e.gaps&&e.gaps[(e.colOf.get(n.to)||0)-1]||GAP_MIN,d=c>o?c-a/2:o+a/2,f=`M${o},${i} H${d} V${l} H${c}`,h=edgeLines(n);t.push(`<path d="${f}" fill="none" stroke="#8C96A0" stroke-width="1.2" ${n.nested?'stroke-dasharray="4 3" ':""}marker-end="url(#vgarrow)">`+(h.length?`<title>${esc(edgeLabel(n))}</title>`:"")+"</path>"),h.length&&s.push({x:d,y:(i+l)/2,lines:h})}for(const n of s){const o=linesWidth(n.lines)+12,i=n.lines.length*LINE_H,c=n.y-i/2;t.push(`<rect x="${(n.x-o/2).toFixed(1)}" y="${c.toFixed(1)}" width="${o.toFixed(1)}" height="${i}" rx="2" fill="#FFFFFF" opacity="0.92"/>`);const l=n.lines.map((a,d)=>`<tspan x="${n.x}" y="${(c+LINE_H*d+11).toFixed(1)}">${esc(a)}</tspan>`).join("");t.push(`<text text-anchor="middle" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" fill="#57606A">${l}</text>`)}for(const n of e.nodes){const o=kindOf(n.kind),i=n.label+(n.params.length?`
`+n.params.map(c=>c.name+": "+Object.keys(c.values).map(l=>l+" = "+c.values[l]).join(" / ")).join(`
`):"");t.push(`<g><title>${esc(i)}</title>`),t.push(`<rect x="${n.x}" y="${n.y}" width="${n.w}" height="${n.h}" rx="5" fill="${o.fill}" stroke="${o.stroke}" stroke-width="1"/>`),t.push(`<rect x="${n.x}" y="${n.y}" width="4" height="${n.h}" rx="2" fill="${o.bar}"/>`),t.push(`<text x="${n.x+11}" y="${n.y+17}" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" font-weight="600" fill="#24292F">${esc(fit(shortName(n.label),n.w))}</text>`),t.push(`<text x="${n.x+11}" y="${n.y+31}" font-family="Roboto,system-ui,sans-serif" font-size="9" fill="#8C96A0">${esc(o.text)}${n.params.length?" \u30FB\u30D1\u30E9\u30E1\u30FC\u30BF":""}</text>`),t.push("</g>")}return t.push("</svg>"),t.join("")}function groupSvg(e){return toSvg(layout(buildGraph(e.sql,e.params)))}function erdStack(e,t){return[e[t],...e.filter((n,o)=>o!==t)].map((n,o)=>'<div class="vg-erdblock"><div class="vg-erdhead">'+(o===0?'<span class="vg-tbadge">\u57FA\u6E96</span>':"")+`<span class="vg-erdname">${esc(label(n))}</span><span class="vg-tabn">${n.members.length}</span></div><div class="vg-erdbox">${groupSvg(n)}</div></div>`).join("")}function erdLegend(){const e=(t,s)=>`<span class="vg-lg"><span class="vg-lgm ${t}"></span>${esc(s)}</span>`;return'<div class="vg-legend">'+e("vg-lgt","\u5B9F\u30C6\u30FC\u30D6\u30EB")+e("vg-lgc","CTE")+e("vg-lgo","\u6700\u7D42 SELECT")+'<span class="vg-lg"><span class="vg-lgd"></span>\u30B5\u30D6\u30AF\u30A8\u30EA\u7D4C\u7531\u306E\u53C2\u7167</span></div>'}function renderErd(e,t){return e.groups.length?notice("FROM / JOIN \u304B\u3089\u8D77\u3053\u3057\u305F\u53C2\u7167\u95A2\u4FC2\u3067\u3059\u3002\u77E2\u5370\u306F\u300C\u8AAD\u3093\u3067\u4F5C\u308B\u300D\u5411\u304D\u3001\u6CE8\u8A18\u306F JOIN \u306E\u7A2E\u5225\u3068\u7D50\u5408\u30AD\u30FC\u3002\u30AB\u30FC\u30C7\u30A3\u30CA\u30EA\u30C6\u30A3\u3068\u4E3B\u30AD\u30FC\u306F SQL \u304B\u3089\u306F\u5206\u304B\u3089\u306A\u3044\u306E\u3067\u63CF\u3044\u3066\u3044\u307E\u305B\u3093\u3002\u30D1\u30E9\u30E1\u30FC\u30BF\u5316\u3057\u305F\u540D\u524D\u306F\u3001\u57FA\u6E96\u306E View \u306E\u5024\u3067\u8868\u793A\u3057\u3066\u3044\u307E\u3059\u3002")+erdLegend()+erdStack(e.groups,t):notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002")}function renderErdBase(e,t){return'<div class="vg-root">'+header(e.base,e.viewCount,e.groups.length,e.unmatched)+renderErd(e,referenceIndex(e,t))+"</div>"}const WARN="\u26A0";function groupColumns(e,t){const s=new Map,n=e.members||[];for(let o=0;o<n.length;o++){const i=n[o],c=e.suffixes&&e.suffixes[o]||i.viewName,l=t[i.viewName]||[];for(let a=0;a<l.length;a++){const d=l[a];let f=s.get(d.n);f||(f={name:d.n,desc:"",order:a,types:[]},s.set(d.n,f)),!f.desc&&d.d&&(f.desc=d.d),f.types.push({suffix:c,type:d.t})}}return s}function columnOrder(e,t,s){const n=new Set,o=[],i=c=>{const l=[...c.values()].sort((a,d)=>a.order-d.order);for(const a of l)n.has(a.name)||(n.add(a.name),o.push(a.name))};i(t[s]);for(let c=0;c<e.length;c++)c!==s&&i(t[c]);return o}function cellTypes(e,t){if(!e)return{text:null,kinds:[],mixed:!1};const s=[];for(const o of e.types)s.indexOf(o.type)<0&&s.push(o.type);const n=s.length>1||e.types.length!==t;return{text:s.join(" / "),kinds:s,mixed:n}}function mixedTip(e,t){const s=e.types.map(o=>`${o.suffix} = ${o.type}`),n=e.types.map(o=>o.suffix);for(let o=0;o<(t.members||[]).length;o++){const i=t.suffixes&&t.suffixes[o]||t.members[o].viewName;n.indexOf(i)<0&&s.push(`${i} = (\u3053\u306E\u5217\u3092\u6301\u305F\u306A\u3044)`)}return s.join(`
`)}function renderColumns(e,t,s){const n=e.groups||[];if(!n.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const o=referenceIndex(e,s||{}),i=n.map(g=>groupColumns(g,t||{})),c=columnOrder(n,i,o);if(!c.length)return notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.COLUMNS \u304B\u3089\u5217\u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const l=[o].concat(n.map((g,u)=>u).filter(g=>g!==o));let a=0,d=0;const f=c.map(g=>{const u=cellTypes(i[o].get(g),n[o].members.length),m=i[o].get(g)||l.map(r=>i[r].get(g)).filter(r=>r)[0],k=l.map((r,x)=>{const v=n[r],w=i[r].get(g),b=cellTypes(w,v.members.length),F=["vg-ccell"];b.text?x>0&&b.text!==u.text&&(F.push("vg-cdiff"),a++):F.push("vg-cnone"),b.mixed&&(F.push("vg-cmix"),d++);const N=b.text?esc(b.text)+(b.mixed?`<span class="vg-cwarn" data-tip="${esc(mixedTip(w,v))}">${WARN}</span>`:""):"\u2014";return`<td class="${F.join(" ")}">${N}</td>`}).join(""),E=m&&m.desc?`<div class="vg-cdesc">${esc(m.desc)}</div>`:"";return`<tr><th class="vg-cname">${esc(g)}${E}</th>${k}</tr>`}).join(""),h=l.map((g,u)=>`<th class="vg-chead${u===0?" vg-cref":""}">`+(u===0?'<span class="vg-tbadge">\u57FA\u6E96</span>':"")+`${esc(label(n[g]))}<span class="vg-tabn">${n[g].members.length}</span></th>`).join(""),p=[];return d&&p.push(notice(`\u540C\u3058\u30B0\u30EB\u30FC\u30D7\u306E\u4E2D\u3067\u5217\u306E\u578B\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u7B87\u6240\u304C ${d} \u4EF6\u3042\u308A\u307E\u3059\uFF08${WARN} \u306E\u5370\uFF09\u3002SQL \u304C\u540C\u4E00\u3067\u3082\u53C2\u7167\u5148\u30C6\u30FC\u30D6\u30EB\u306E\u578B\u304C\u9055\u3048\u3070\u3053\u3046\u306A\u308B\u306E\u3067\u3001\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206\u306B\u306F\u51FA\u3066\u304D\u307E\u305B\u3093\u3002`)),a&&p.push(notice(`\u57FA\u6E96\u30B0\u30EB\u30FC\u30D7\u3068\u578B\u304C\u9055\u3046\u7B87\u6240\u304C ${a} \u4EF6\u3042\u308A\u307E\u3059\uFF08\u8272\u4ED8\u304D\u306E\u30BB\u30EB\uFF09\u3002`)),p.length||p.push(notice("\u5168\u30B0\u30EB\u30FC\u30D7\u3067\u5217\u540D\u3082\u578B\u3082\u4E00\u81F4\u3057\u3066\u3044\u307E\u3059\u3002")),p.join("")+`<div class="vg-ctablewrap"><table class="vg-ctable"><thead><tr><th class="vg-chead vg-cnamehead">\u5217\u540D</th>${h}</tr></thead><tbody>${f}</tbody></table></div>`}function renderColumnsBase(e,t,s){return`<div class="vg-root">${renderColumns(e,t,s)}</div>`}function columnsCss(){return[".vg-ctablewrap{overflow-x:auto}",".vg-ctable{border-collapse:collapse;font-size:12px}",".vg-chead{position:sticky;top:0;z-index:1;padding:6px 12px;border:1px solid #D0D7DE;background:#F6F8FA;color:#24292F;font-weight:600;text-align:left;white-space:nowrap}",".vg-cref{background:#fbeded;border-color:#efb6b6}",".vg-cnamehead{min-width:180px}",".vg-cname{padding:5px 12px;border:1px solid #D0D7DE;text-align:left;vertical-align:top;font-weight:600;color:#24292F;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}",".vg-cdesc{margin:2px 0 0;font:11px/1.5 'Roboto','Segoe UI',system-ui,sans-serif;font-weight:400;color:#57606A;white-space:normal;max-width:320px}",".vg-ccell{padding:5px 12px;border:1px solid #D0D7DE;vertical-align:top;white-space:nowrap;color:#24292F;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}",".vg-cdiff{background:#dfe7d2}",".vg-cnone{color:#8C959F;background:#FAFAFA}",".vg-cmix{box-shadow:inset 0 0 0 2px #D4A72C}",".vg-cwarn{position:relative;margin-left:6px;color:#9A6700;cursor:help}",".vg-cwarn::after{content:attr(data-tip);display:none;position:absolute;z-index:20;left:0;top:calc(100% + 5px);width:max-content;max-width:340px;padding:6px 10px;border-radius:6px;background:#24292F;color:#fff;font:11px/1.7 ui-monospace,SFMono-Regular,Consolas,monospace;white-space:pre-wrap;text-align:left;box-shadow:0 2px 10px rgba(0,0,0,.30);pointer-events:none}",".vg-cwarn:hover::after{display:block}"].join(`
`)}function __opts(e){if(!e)return{};try{return JSON.parse(e)||{}}catch(t){return{}}}function __notice(e){return'<div class="vg-notice">'+String(e).replace(/[<>&]/g,"")+"</div>"}function __run(e,t,s,n){var o=__opts(n),i;try{i=JSON.parse(e)}catch(g){i=null}if(!i)return String(t||__notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002"));var c={};try{for(var l=JSON.parse(s)||[],a=0;a<l.length;a++)c[l[a].v]=l[a].cols||[]}catch(g){c={}}for(var d="",f="",h=i.bases||[],p=0;p<h.length;p++)d+=renderErdBase(h[p],o),f+=renderColumnsBase(h[p],c,o);return d||(d=__notice("\u56F3\u306B\u3067\u304D\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),f||(f=__notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),wrapPage(String(t||""),d,f,i.bases&&i.bases.length?i.bases[0]:null)}return __run(analysis_json,diff_html,columns_json,options_json);
