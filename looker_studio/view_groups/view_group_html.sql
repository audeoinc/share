...
var B=Object.defineProperty,D=Object.defineProperties;var W=Object.getOwnPropertyDescriptors;var $=Object.getOwnPropertySymbols;var j=Object.prototype.hasOwnProperty,G=Object.prototype.propertyIsEnumerable;var _=(e,t,o)=>t in e?B(e,t,{enumerable:!0,configurable:!0,writable:!0,value:o}):e[t]=o,C=(e,t)=>{for(var o in t||(t={}))j.call(t,o)&&_(e,o,t[o]);if($)for(var o of $(t))G.call(t,o)&&_(e,o,t[o]);return e},L=(e,t)=>D(e,W(t));const MAX_TABS=12,MAX_SQL_TABS=128,MAX_DESC_TABS=16,MAX_LABEL_TABS=8,OUTER_TABS=["note","\u30AB\u30E9\u30E0\u5B9A\u7FA9","\u53C2\u7167\u95A2\u4FC2","\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206","SQL"],MAX_OUTER_TABS=8,NOTE_MARK="<!--VG_NOTE-->",CSS_GEN=11;function groupRule(e,t,o){const s=[];for(let n=1;n<=e;n++){const i=t(n);if(Array.isArray(i))for(const c of i)s.push(c);else s.push(i)}return s.join(",")+"{"+o+"}"}function regionByView(e){const t={},o=Array.isArray(e)?e:[];for(let s=0;s<o.length;s++){const n=o[s]||{},i=Array.isArray(n.v)?n.v:[];for(let c=0;c<i.length;c++)t[i[c]]=String(n.r==null?"":n.r)}return t}function esc(e){return String(e==null?"":e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function hashId(e){let t=2166136261;for(let o=0;o<String(e).length;o++)t^=String(e).charCodeAt(o),t=Math.imul(t,16777619)>>>0;return t.toString(36)}const label=e=>e.suffixes.map((t,o)=>t||e.members[o]&&e.members[o].viewName||"(suffix \u306A\u3057)").join(", ");function badge(e,t,o){return`<span class="vg-badge" style="color:${t};background:${o}">${esc(e)}</span>`}function header(e,t,o,s,n){const i=o>1;return`<div class="vg-header"><span class="vg-title">${esc(e)}</span>`+badge(`${t} View`,"#57606A","#EAEEF2")+badge(`${o} \u30B0\u30EB\u30FC\u30D7`,i?"#9A6700":"#1A7F37",i?"#FFF8C5":"#DAFBE1")+(s?badge("suffix \u672A\u8A8D\u8B58","#9A6700","#FFF8C5"):"")+(n?badge("\u30E9\u30D9\u30EB\u4E0D\u4E00\u81F4","#9A6700","#FFF8C5"):"")+"</div>"}function notice(e){return`<div class="vg-notice">${esc(e)}</div>`}const KIND_TEXT={entity:"\u5B9F\u4F53\u540D",string:"\u5024\uFF08\u6587\u5B57\u5217\uFF09",number:"\u5024\uFF08\u6570\u5024\uFF09",ident:"\u540D\u524D",quoted:"\u540D\u524D",keyword:"\u4E88\u7D04\u8A9E",punct:"\u8A18\u53F7",comment:"\u30B3\u30E1\u30F3\u30C8"},kindText=e=>KIND_TEXT[e]||e,DQ="\\u0022",DQ3=DQ+DQ+DQ,TOKEN_RE=new RegExp(["(`[^`]*`)","([rbRB]{1,2}(?:'''[\\s\\S]*?'''|"+DQ3+"[\\s\\S]*?"+DQ3+"|'[^']*'|"+DQ+"[^"+DQ+"]*"+DQ+"))","('''[\\s\\S]*?''')","("+DQ3+"[\\s\\S]*?"+DQ3+")","('(?:\\\\.|''|[^'\\\\])*')","("+DQ+"(?:\\\\.|"+DQ+DQ+"|[^"+DQ+"\\\\])*"+DQ+")","(--[^\\n]*|#[^\\n]*)","(/\\*[\\s\\S]*?\\*/)","(0[xX][0-9a-fA-F]+|(?:\\d+(?:\\.\\d+)?|\\.\\d+)(?:[eE][+-]?\\d+)?)","([A-Za-z_][A-Za-z0-9_]*)","(\\s+)","([^\\s])"].join("|"),"g"),KEYWORDS=new Set("SELECT FROM WHERE JOIN INNER LEFT RIGHT FULL OUTER CROSS ON USING AND OR NOT IN IS NULL LIKE BETWEEN EXISTS ANY ALL SOME GROUP BY HAVING ORDER ASC DESC LIMIT OFFSET QUALIFY WINDOW UNION INTERSECT EXCEPT DISTINCT AS CASE WHEN THEN ELSE END CREATE OR REPLACE VIEW TABLE FUNCTION IF EXISTS OPTIONS WITH RECURSIVE OVER PARTITION UNNEST STRUCT ARRAY CAST SAFE_CAST EXTRACT INTERVAL DATE DATETIME TIME TIMESTAMP INT64 FLOAT64 NUMERIC BIGNUMERIC STRING BYTES BOOL TRUE FALSE COUNT SUM AVG MIN MAX COALESCE IFNULL NULLIF ROWS RANGE PRECEDING FOLLOWING CURRENT ROW".split(/\s+/));function tokenizeSql(e){const t=[],o=String(e==null?"":e);let s;for(TOKEN_RE.lastIndex=0;(s=TOKEN_RE.exec(o))!==null;){let n;s[1]?n="quoted":s[2]||s[3]||s[4]||s[5]||s[6]?n="string":s[7]||s[8]?n="comment":s[9]?n="number":s[10]?n=KEYWORDS.has(s[10].toUpperCase())?"keyword":"ident":s[11]?n="space":n="punct",t.push({kind:n,text:s[0]})}return t}function markEntities(e){const t=e.slice(),o=i=>{let c=i+1;for(;c<t.length&&t[c].kind==="space";)c++;return c},s=[];let n=null;for(let i=0;i<t.length;i++){const c=t[i];if(c.kind==="space"||c.kind==="comment")continue;if(c.text==="("){s.push(n),n=null;continue}if(c.text===")"){s.pop(),n=null;continue}const f=c.text.toUpperCase();if(n=c.kind==="keyword"||c.kind==="ident"?f:null,c.kind!=="keyword"||f!=="FROM"&&f!=="JOIN"||s.length>0&&s[s.length-1]==="EXTRACT")continue;let l=o(i);for(;!(l>=t.length);){const u=t[l];if(u.kind==="quoted")t[l]={kind:"entity",text:u.text},l=o(l);else if(u.kind==="ident"){const p=[];let m=l;for(;;){p.push(m);const k=o(m);if(k<t.length&&t[k].text==="."){const a=o(k);if(a<t.length&&t[a].kind==="ident"){m=a;continue}}break}const x=o(m);if(x<t.length&&t[x].text==="(")break;for(const k of p)t[k]={kind:"entity",text:t[k].text};l=x}else break;if(l<t.length&&t[l].kind==="keyword"&&t[l].text.toUpperCase()==="AS"){const p=o(l);p<t.length&&(t[p].kind==="ident"||t[p].kind==="quoted")&&(l=o(p))}else l<t.length&&t[l].kind==="ident"&&(l=o(l));if(l<t.length&&t[l].text===","){l=o(l);continue}break}}return t}const DEFAULT_SUFFIX_RE=/^(.*?)_([A-Za-z0-9]{1,6})$/,DEFAULT_SUBSTITUTABLE=["entity","number","string"],SUFFIX_MARK="\0",LITERAL_MARK="\u0001",BOX_W_MIN=130,NAME_CHAR_W=6.65,BOX_H=40,GAP_MIN=78,CHAR_W=6.5,LINE_H=14,GAP_Y=14,PAD=10,kw=e=>e&&e.kind==="keyword"?e.text.toUpperCase():null;function prepare(e){const t=[];let o=0;for(const s of markEntities(tokenizeSql(e)))s.kind==="space"||s.kind==="comment"||(s.text===")"&&o--,t.push({kind:s.kind,text:s.text,depth:o}),s.text==="("&&o++);return t}function cteRanges(e){const t=[];if(!(e[0]&&kw(e[0])==="WITH"))return{ctes:t,mainFrom:0};let o=1;for(;;){const s=e[o],n=e[o+1],i=e[o+2];if(!i||s.kind!=="ident"||kw(n)!=="AS"||i.text!=="(")break;const c=i.depth;let f=o+3;for(;f<e.length&&!(e[f].text===")"&&e[f].depth===c);)f++;if(t.push({name:s.text,from:o+3,to:f,depth:c+1}),o=f+1,e[o]&&e[o].text===","){o++;continue}break}return{ctes:t,mainFrom:o}}const STOP=new Set(["WHERE","GROUP","ORDER","QUALIFY","WINDOW","HAVING","UNION","INTERSECT","EXCEPT","LIMIT","SELECT","JOIN","FROM"]);function skipParen(e,t,o){const s=e[t].depth;let n=t+1;for(;n<o&&!(e[n].text===")"&&e[n].depth===s);)n++;return n+1}function readSource(e,t,o){if(t>=o)return{node:null,next:t};const s=e[t];if(s.text==="(")return{node:{name:"(\u30B5\u30D6\u30AF\u30A8\u30EA)",kind:"subquery"},next:skipParen(e,t,o)};if(kw(s)==="UNNEST"){let n=t+1,i="";if(e[n]&&e[n].text==="("){const c=e[n].depth;let f=n+1;for(;f<o&&!(e[f].text===")"&&e[f].depth===c);)i+=e[f].text,f++;n=f+1}return{node:{name:"UNNEST("+i+")",kind:"unnest"},next:n}}if(s.kind==="entity"||s.kind==="quoted"||s.kind==="ident"){const n=[];let i=t;for(;i<o&&(e[i].kind==="entity"||e[i].kind==="quoted"||e[i].kind==="ident");){if(n.push(e[i].text),i++,i<o&&e[i].text==="."){i++;continue}break}return i<o&&e[i].text==="("?{node:null,next:skipParen(e,i,o)}:{node:{name:n.join("."),kind:"ref"},next:i}}return{node:null,next:t+1}}function skipAlias(e,t,o){if(t<o&&kw(e[t])==="AS"){const s=e[t+1];return s&&(s.kind==="ident"||s.kind==="quoted")?t+2:t+1}return t<o&&e[t].kind==="ident"&&!STOP.has(e[t].text.toUpperCase())?t+1:t}function readOnKeys(e,t,o){const s=[];let n=t;for(;n<o;){const i=kw(e[n]);if(i&&STOP.has(i)||i==="LEFT"||i==="RIGHT"||i==="FULL"||i==="INNER"||i==="CROSS")break;if(e[n].text==="="){const c=(u,p,m)=>u&&p&&m&&p.text==="."?m.text:null,f=c(e[n-3],e[n-2],e[n-1]),l=c(e[n+1],e[n+2],e[n+3]);f&&l&&s.push(f===l?f:f+" = "+l)}n++}return{keys:s,next:n}}function readUsingKeys(e,t,o){const s=[];let n=t;if(e[n]&&e[n].text==="("){const i=e[n].depth;for(n++;n<o&&!(e[n].text===")"&&e[n].depth===i);)e[n].kind==="ident"&&s.push(e[n].text),n++;n++}return{keys:s,next:n}}function joinKind(e,t,o){for(let s=t-1;s>=o&&t-s<=3;s--){const n=kw(e[s]);if(n==="LEFT"||n==="RIGHT"||n==="FULL"||n==="CROSS")return n;if(n==="INNER")return"INNER"}return"INNER"}function scanScope(e,t,o,s){const n=[];for(let i=t;i<o;i++){const c=kw(e[i]);if(c!=="FROM"&&c!=="JOIN")continue;const f=e[i].depth>s,l=c==="JOIN"?joinKind(e,i,t):null;let u=i+1;for(;;){const p=readSource(e,u,o);if(u=p.next,p.node){u=skipAlias(e,u,o);let m=[];if(!f&&u<o&&kw(e[u])==="ON"){const x=readOnKeys(e,u+1,o);m=x.keys,u=x.next}else if(!f&&u<o&&kw(e[u])==="USING"){const x=readUsingKeys(e,u+1,o);m=x.keys,u=x.next}p.node.kind!=="unnest"&&n.push({name:p.node.name,kind:p.node.kind,joinType:l,keys:m,nested:f})}if(u<o&&e[u].text===","&&e[u].depth===e[i].depth){u++;continue}break}i=Math.max(i,u-1)}return n}function buildGraph(e,t){const o=new Map((t||[]).map(a=>[a.name,a])),s=a=>{const d=Object.keys(a.values);return d.length?a.values[d[0]]:null},n=new Map,i=String(e).replace(/\{\{(P\d+)\}\}/g,(a,d)=>{const E=o.get(d);if(!E)return a;const S=s(E);return S==null?a:(n.set(S,E),S)}),c=prepare(i),{ctes:f,mainFrom:l}=cteRanges(c),u=new Set(f.map(a=>a.name)),p=new Map,m=[],x=(a,d)=>{const E=d+":"+a;if(!p.has(E)){const S=[],T=n.get(String(a));if(T)S.push(T);else for(const N of String(a).split(".")){const I=n.get(N);I&&S.indexOf(I)<0&&S.push(I)}p.set(E,{id:E,name:a,label:a,kind:d,params:S})}return E},k=f.map(a=>({id:x(a.name,"cte"),from:a.from,to:a.to,depth:a.depth}));k.push({id:x("(\u6700\u7D42 SELECT)","output"),from:l,to:c.length,depth:0});for(const a of k)for(const d of scanScope(c,a.from,a.to,a.depth)){const E=d.kind==="ref"?u.has(d.name)?"cte":"table":d.kind,S=x(d.name,E);if(S===a.id)continue;const T=m.find(N=>N.from===S&&N.to===a.id);T?(T.keys.length||(T.keys=d.keys),T.joinType||(T.joinType=d.joinType),T.nested=T.nested&&d.nested):m.push({from:S,to:a.id,joinType:d.joinType,keys:d.keys,nested:d.nested})}return{nodes:[...p.values()],edges:m}}function layout(e){const{nodes:t,edges:o}=e,s=new Map(t.map(r=>[r.id,[]])),n=new Map(t.map(r=>[r.id,[]]));for(const r of o)s.has(r.to)&&s.get(r.to).push(r.from),n.has(r.from)&&n.get(r.from).push(r.to);const i=new Map(t.map(r=>[r.id,0]));for(let r=0;r<t.length+1;r++){let h=!1;for(const g of t){const y=s.get(g.id);if(!y.length)continue;const F=Math.max(...y.map(A=>i.get(A)||0))+1;F>i.get(g.id)&&(i.set(g.id,F),h=!0)}if(!h)break}const c=Math.max(0,...t.map(r=>i.get(r.id))),f=new Map(t.map(r=>[r.id,n.get(r.id).length?1/0:c]));for(let r=0;r<t.length+1;r++){let h=!1;for(const g of t){const y=n.get(g.id);if(!y.length)continue;const F=Math.min(...y.map(A=>f.get(A)));F-1<f.get(g.id)&&(f.set(g.id,F-1),h=!0)}if(!h)break}for(const r of t)Number.isFinite(f.get(r.id))||f.set(r.id,c);const l=[];for(const r of t){const h=Math.max(0,f.get(r.id));(l[h]||(l[h]=[])).push(r)}const u=new Map;l.forEach((r,h)=>{r&&(h>0&&r.sort((g,y)=>{const F=A=>{const b=s.get(A.id).filter(M=>u.has(M));return b.length?b.reduce((M,U)=>M+u.get(U),0)/b.length:Number.MAX_SAFE_INTEGER};return F(g)-F(y)}),r.forEach((g,y)=>u.set(g.id,y)))});const p=boxWidth(t),m=new Map;l.forEach((r,h)=>(r||[]).forEach(g=>m.set(g.id,h)));const x=new Array(Math.max(0,l.length-1)).fill(GAP_MIN);for(const r of o){const h=(m.get(r.to)||0)-1;h<0||h>=x.length||(x[h]=Math.max(x[h],linesWidth(edgeLines(r))+22))}const k=[];let a=PAD;for(let r=0;r<l.length;r++)k[r]=a,a+=p+(x[r]||0);const d=new Map;for(const r of o){const h=edgeLines(r);if(!h.length)continue;const g=r.from+"\0"+((m.get(r.to)||0)-1);d.set(g,(d.get(g)||0)+h.length*LINE_H)}const E=new Map;for(const[r,h]of d){const g=r.slice(0,r.indexOf("\0"));E.set(g,Math.max(E.get(g)||0,h))}const S=Math.max(1,...l.map(r=>(r||[]).length)),T=new Array(S).fill(BOX_H);l.forEach(r=>(r||[]).forEach((h,g)=>{T[g]=Math.max(T[g],E.get(h.id)||0)}));const N=[];let I=PAD;for(let r=0;r<S;r++)N[r]=I,I+=T[r]+GAP_Y;const v=[];l.forEach((r,h)=>{(r||[]).forEach((g,y)=>{v.push(L(C({},g),{x:k[h],y:N[y]+(T[y]-BOX_H)/2,w:p,h:BOX_H}))})});const w=new Map(v.map(r=>[r.id,r]));let O=0,R=I-GAP_Y+PAD+6;for(const r of o){const h=w.get(r.from),g=w.get(r.to);if(!h||!g)continue;const y=edgeLines(r);if(!y.length)continue;const F=h.y+h.h/2,A=y.length*LINE_H;O=Math.min(O,F-A/2-PAD),R=Math.max(R,F+A/2+PAD)}return{nodes:v,edges:o.map(r=>L(C({},r),{a:w.get(r.from),b:w.get(r.to)})).filter(r=>r.a&&r.b),gaps:x,colOf:m,y0:O,width:(k[l.length-1]||PAD)+p+PAD,height:R-O}}const KIND={table:{text:"\u5B9F\u30C6\u30FC\u30D6\u30EB",fill:"#FFFFFF",stroke:"#8C6D3F",bar:"#C9A227"},cte:{text:"CTE",fill:"#FFFFFF",stroke:"#3F6D8C",bar:"#4E8FBF"},output:{text:"\u6700\u7D42 SELECT",fill:"#FFFFFF",stroke:"#6D3F8C",bar:"#8250DF"},subquery:{text:"\u30B5\u30D6\u30AF\u30A8\u30EA",fill:"#FFFFFF",stroke:"#6E7781",bar:"#9AA4AE"}},kindOf=e=>KIND[e]||KIND.subquery;function shortName(e){const t=String(e).replace(/`/g,""),o=t.lastIndexOf(".");return o>=0?t.slice(o+1):t}function boxWidth(e){let t=BOX_W_MIN;for(const o of e)t=Math.max(t,shortName(o.label).length*NAME_CHAR_W+24);return t}function fit(e,t){const o=String(e),s=Math.floor((t-24)/NAME_CHAR_W);return o.length>s?o.slice(0,s-1)+"\u2026":o}function edgeLines(e){const t=[];e.joinType&&t.push(e.joinType==="INNER"?"JOIN":e.joinType+" JOIN");for(const o of e.keys||[])t.push(o);return!t.length&&e.nested&&t.push("\u30B5\u30D6\u30AF\u30A8\u30EA"),t}function linesWidth(e){let t=0;for(const o of e)t=Math.max(t,o.length*CHAR_W);return t}function edgeLabel(e){const t=edgeLines(e);return t.length?t.length>1?t[0]+" / "+t.slice(1).join(", "):t[0]:""}function toSvg(e){const t=[];t.push(`<svg viewBox="0 ${(e.y0||0).toFixed(1)} ${e.width} ${e.height}" width="${e.width}" height="${e.height}" role="img" aria-label="\u53C2\u7167\u95A2\u4FC2\u56F3" xmlns="http://www.w3.org/2000/svg">`),t.push('<defs><marker id="vgarrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="7" markerHeight="7" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#8C96A0"/></marker></defs>');const o=[],s=new Map;for(const n of e.edges){const i=n.a.x+n.a.w,c=n.a.y+n.a.h/2,f=n.b.x,l=n.b.y+n.b.h/2,u=(e.colOf.get(n.to)||0)-1,p=e.gaps&&e.gaps[u]||GAP_MIN,m=f>i?f-p/2:i+p/2,x=`M${i},${c} H${m} V${l} H${f}`,k=edgeLines(n);if(t.push(`<path d="${x}" fill="none" stroke="#8C96A0" stroke-width="1.2" ${n.nested?'stroke-dasharray="4 3" ':""}marker-end="url(#vgarrow)">`+(k.length?`<title>${esc(edgeLabel(n))}</title>`:"")+"</path>"),!k.length)continue;const a=n.from+"\0"+u,d=s.get(a)||{total:0,used:0};d.total+=k.length*LINE_H,s.set(a,d),o.push({x:m,y:c,lines:k,full:edgeLabel(n),key:a})}for(const n of o){const i=s.get(n.key),c=n.lines.length*LINE_H;n.y=n.y-i.total/2+i.used+c/2,i.used+=c}for(const n of o){const i=linesWidth(n.lines)+12,c=n.lines.length*LINE_H;t.push(`<rect x="${(n.x-i/2).toFixed(1)}" y="${(n.y-c/2).toFixed(1)}" width="${i.toFixed(1)}" height="${c}" rx="2" fill="#FFFFFF" opacity="0.92"/>`)}for(const n of o){const i=n.y-n.lines.length*LINE_H/2,c=n.lines.map((f,l)=>`<tspan x="${n.x}" y="${(i+LINE_H*l+11).toFixed(1)}">${esc(f)}</tspan>`).join("");t.push(`<text text-anchor="middle" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" fill="#57606A"><title>${esc(n.full)}</title>${c}</text>`)}for(const n of e.nodes){const i=kindOf(n.kind),c=n.tipLines||n.params.map(l=>l.name+": "+Object.keys(l.values).map(u=>u+" = "+l.values[u]).join(" / ")),f=n.label+(c.length?`
`+c.join(`
`):"");t.push(`<g><title>${esc(f)}</title>`),t.push(`<rect x="${n.x}" y="${n.y}" width="${n.w}" height="${n.h}" rx="5" fill="${i.fill}" stroke="${i.stroke}" stroke-width="1"/>`),t.push(`<rect x="${n.x}" y="${n.y}" width="4" height="${n.h}" rx="2" fill="${i.bar}"/>`),t.push(`<text x="${n.x+11}" y="${n.y+17}" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" font-weight="600" fill="#24292F">${esc(fit(shortName(n.label),n.w))}</text>`),t.push(`<text x="${n.x+11}" y="${n.y+31}" font-family="Roboto,system-ui,sans-serif" font-size="9" fill="#8C96A0">${esc(i.text)}${n.params.length?" \u30FB\u30D1\u30E9\u30E1\u30FC\u30BF":""}</text>`),t.push("</g>")}return t.push("</svg>"),t.join("")}function refBase(e,t){const o=shortName(e);if(!t)return o;const s=String(t);for(let n=0;n<o.length;n++){if(o[n]!=="_")continue;const i=o.slice(n+1);if(s===i||s.endsWith("_"+i)||s.endsWith(i))return o.slice(0,n)}return o}function paramBase(e,t){const o=[];for(const s of Object.keys(e.values)){const n=shortName(e.values[s]);o.indexOf(n)<0&&o.push(n)}if(o.length>=2){let s=0;for(;s<o[0].length&&o.every(i=>i[s]===o[0][s]);)s++;const n=o[0].slice(0,s).lastIndexOf("_");if(n>0)return o[0].slice(0,n)}return refBase(o[0],t)}function nodeBase(e,t){const o=shortName(e.name);for(const s of t.params||[])for(const n of Object.keys(s.values))if(shortName(s.values[n])===o)return paramBase(s,(t.suffixes||[])[0]||null);return refBase(o,(t.suffixes||[])[0]||null)}function erdSignature(e){const t=buildGraph(e.sql,e.params),o=new Map(t.nodes.map(s=>[s.id,nodeBase(s,e)+"/"+s.kind]));return JSON.stringify({n:t.nodes.map(s=>o.get(s.id)).sort(),e:t.edges.map(s=>o.get(s.from)+">"+o.get(s.to)+":"+(s.joinType||"")+":"+(s.keys||[]).join("&")).sort()})}function erdGroups(e){const t=new Map,o=[];for(const s of e.groups||[]){const n=erdSignature(s);let i=t.get(n);i||(i={groups:[],suffixes:[],members:[]},t.set(n,i),o.push(i)),i.groups.push(s);for(let c=0;c<(s.suffixes||[]).length;c++)i.suffixes.push(s.suffixes[c]),i.members.push((s.members||[])[c])}return o}function commonStem(e){const t=e.filter(s=>s!=null).map(String);if(!t.length)return"";if(t.every(s=>s===t[0]))return t[0];let o=0;for(;o<t[0].length&&t.every(s=>s[o]===t[0][o]);)o++;return t[0].slice(0,o)+"*"}function groupSvg(e){const t=e.groups||[e],o=buildGraph(t[0].sql,t[0].params),s=t.map((n,i)=>i===0?o:buildGraph(n.sql,n.params));for(const n of o.nodes){const i=nodeBase(n,t[0]),c=[],f=[],l=[];if(t.forEach((u,p)=>{const m=(u.suffixes||[])[0]||null;let x=0;for(const a of u.params||[]){const d=Object.keys(a.values);if(paramBase(a,m)===i){x++,l.push(a);for(const E of d)c.push(shortName(a.values[E]));f.push((t.length>1?label(u)+" / ":"")+a.name+": "+d.map(E=>E+" = "+a.values[E]).join(" / "))}}if(x)return;const k=s[p].nodes.filter(a=>nodeBase(a,u)===i);for(const a of k){const d=shortName(a.name);c.indexOf(d)<0&&c.push(d),t.length>1&&f.push(label(u)+" / "+d)}}),!l.length){n.params=[],n.tipLines=[];continue}n.label=commonStem(c),n.params=l,n.tipLines=f}return toSvg(layout(o))}function erdStack(e){return e.map(t=>`<div class="vg-erdblock"><div class="vg-erdhead"><span class="vg-erdname">${esc(label(t))}</span><span class="vg-tabn">${t.members.length}</span></div><div class="vg-erdbox">${groupSvg(t)}</div></div>`).join("")}function erdLegend(){const e=(t,o)=>`<span class="vg-lg"><span class="vg-lgm ${t}"></span>${esc(o)}</span>`;return'<div class="vg-legend">'+e("vg-lgt","\u5B9F\u30C6\u30FC\u30D6\u30EB")+e("vg-lgc","CTE")+e("vg-lgo","\u6700\u7D42 SELECT")+'<span class="vg-lg"><span class="vg-lgd"></span>\u30B5\u30D6\u30AF\u30A8\u30EA\u7D4C\u7531\u306E\u53C2\u7167</span></div>'}function renderErd(e){if(!e.groups.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const t=erdGroups(e),o=t.length<e.groups.length;return notice("FROM / JOIN \u304B\u3089\u8D77\u3053\u3057\u305F\u53C2\u7167\u95A2\u4FC2\u3067\u3059\u3002\u77E2\u5370\u306F\u300C\u8AAD\u3093\u3067\u4F5C\u308B\u300D\u5411\u304D\u3001\u6CE8\u8A18\u306F JOIN \u306E\u7A2E\u5225\u3068\u7D50\u5408\u30AD\u30FC\u3002\u30AB\u30FC\u30C7\u30A3\u30CA\u30EA\u30C6\u30A3\u3068\u4E3B\u30AD\u30FC\u306F SQL \u304B\u3089\u306F\u5206\u304B\u3089\u306A\u3044\u306E\u3067\u63CF\u3044\u3066\u3044\u307E\u305B\u3093\u3002\u3053\u3053\u3067\u306E\u62EC\u308A\u306F\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206\u3068\u306F\u5225\u3067\u3059\u3002\u53C2\u7167\u540D\u306E base \u90E8\u5206\uFF08suffix \u3092\u9664\u3044\u305F\u5B9F\u4F53\u540D\uFF09\u304C\u540C\u3058\u3082\u306E\u306F 1 \u679A\u306B\u307E\u3068\u3081\u3066\u3044\u307E\u3059\u3002"+(o?`\u30ED\u30B8\u30C3\u30AF\u306F ${e.groups.length} \u30B0\u30EB\u30FC\u30D7\u306B\u5272\u308C\u3066\u3044\u307E\u3059\u304C\u3001\u53C2\u7167\u95A2\u4FC2\u306F ${t.length} \u901A\u308A\u3067\u3059\u3002`:"")+"\u540D\u524D\u304C View \u3054\u3068\u306B\u9055\u3046\u7B87\u6240\u306F\u5171\u901A\u90E8\u5206\u3092\u51FA\u3057\u3066\u6B8B\u308A\u3092 * \u306B\u3057\u3066\u3044\u307E\u3059\uFF08\u5B9F\u969B\u306E\u5BFE\u5FDC\u306F\u7BB1\u306B\u30AB\u30FC\u30BD\u30EB\u3092\u4E57\u305B\u308B\u3068\u51FA\u307E\u3059\uFF09\u3002")+erdLegend()+erdStack(t)}function renderErdBase(e){return'<div class="vg-root">'+header(e.base,e.viewCount,e.groups.length,e.unmatched)+renderErd(e)+"</div>"}function __opts(e){if(!e)return{};try{return JSON.parse(e)||{}}catch(t){return{}}}function __notice(e){return'<div class="vg-notice">'+String(e).replace(/[<>&]/g,"")+"</div>"}function __run(e,t){var o;try{o=JSON.parse(e)}catch(c){o=null}if(!o)return __notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002");for(var s="",n=o.bases||[],i=0;i<n.length;i++)s+=renderErdBase(n[i]);return s||__notice("\u56F3\u306B\u3067\u304D\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")}return __run(analysis_json,options_json);

""";

DECLARE js_page STRING DEFAULT r"""
const MAX_TABS=12,MAX_SQL_TABS=128,MAX_DESC_TABS=16,MAX_LABEL_TABS=8,OUTER_TABS=["note","\u30AB\u30E9\u30E0\u5B9A\u7FA9","\u53C2\u7167\u95A2\u4FC2","\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206","SQL"],MAX_OUTER_TABS=8,NOTE_MARK="<!--VG_NOTE-->",CSS_GEN=11;function cssGuard(){return`<div class="vg-cssgen11" style="margin:0 0 10px;padding:8px 12px;border:1px solid #D4A72C;border-radius:6px;background:#FFF8C5;color:#7D4E00;font:12px/1.6 'Roboto','Segoe UI',system-ui,sans-serif">\u3053\u306E\u30AB\u30FC\u30C9\u306B\u306F\u65B0\u3057\u3044 CSS\uFF08\u4E16\u4EE3 11\uFF09\u304C\u8981\u308A\u307E\u3059\u3002template_style.html \u3092\u8CBC\u308A\u76F4\u3057\u3066\u304F\u3060\u3055\u3044\u3002\u8CBC\u308A\u66FF\u3048\u308B\u307E\u3067\u3001\u30BF\u30D6\u304C\u6B63\u3057\u304F\u51FA\u307E\u305B\u3093\uFF08\u5207\u308A\u66FF\u308F\u3089\u306A\u3044\u30FB\u4E2D\u8EAB\u304C\u5168\u90E8\u540C\u6642\u306B\u51FA\u308B\u30FB\u9078\u629E\u4E2D\u304C\u5206\u304B\u3089\u306A\u3044\uFF09\u3002</div>`}function groupRule(s,n,e){const t=[];for(let i=1;i<=s;i++){const o=n(i);if(Array.isArray(o))for(const l of o)t.push(l);else t.push(o)}return t.join(",")+"{"+e+"}"}function regionByView(s){const n={},e=Array.isArray(s)?s:[];for(let t=0;t<e.length;t++){const i=e[t]||{},o=Array.isArray(i.v)?i.v:[];for(let l=0;l<o.length;l++)n[o[l]]=String(i.r==null?"":i.r)}return n}function esc(s){return String(s==null?"":s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function hashId(s){let n=2166136261;for(let e=0;e<String(s).length;e++)n^=String(s).charCodeAt(e),n=Math.imul(n,16777619)>>>0;return n.toString(36)}const label=s=>s.suffixes.map((n,e)=>n||s.members[e]&&s.members[e].viewName||"(suffix \u306A\u3057)").join(", ");function badge(s,n,e){return`<span class="vg-badge" style="color:${n};background:${e}">${esc(s)}</span>`}function header(s,n,e,t,i){const o=e>1;return`<div class="vg-header"><span class="vg-title">${esc(s)}</span>`+badge(`${n} View`,"#57606A","#EAEEF2")+badge(`${e} \u30B0\u30EB\u30FC\u30D7`,o?"#9A6700":"#1A7F37",o?"#FFF8C5":"#DAFBE1")+(t?badge("suffix \u672A\u8A8D\u8B58","#9A6700","#FFF8C5"):"")+(i?badge("\u30E9\u30D9\u30EB\u4E0D\u4E00\u81F4","#9A6700","#FFF8C5"):"")+"</div>"}function notice(s){return`<div class="vg-notice">${esc(s)}</div>`}const KIND_TEXT={entity:"\u5B9F\u4F53\u540D",string:"\u5024\uFF08\u6587\u5B57\u5217\uFF09",number:"\u5024\uFF08\u6570\u5024\uFF09",ident:"\u540D\u524D",quoted:"\u540D\u524D",keyword:"\u4E88\u7D04\u8A9E",punct:"\u8A18\u53F7",comment:"\u30B3\u30E1\u30F3\u30C8"},kindText=s=>KIND_TEXT[s]||s;function wrapPage(s,n,e,t,i,o,l){const r=o||{},a="vgo"+hashId(r.base||""),c=[`<div class="vg-root">${i==null||i===""?NOTE_MARK:String(i)}</div>`,e,n,s,t],g=OUTER_TABS.map((h,m)=>`<input class="vg-or vg-or${m+1}" type="radio" name="${a}" id="${a}-${m+1}"${m===0?" checked":""}>`).join(""),f=OUTER_TABS.map((h,m)=>`<label class="vg-otab vg-ot${m+1}" for="${a}-${m+1}">${esc(h)}</label>`).join(""),v=OUTER_TABS.map((h,m)=>`<div class="vg-opanel vg-op${m+1}">${c[m]||""}</div>`).join(""),d=r.base?header(r.base,r.viewCount,(r.groups||[]).length,r.unmatched,l):"";return`<div class="vg-outer">${cssGuard()}${g}<div class="vg-otablist">${d}${f}</div><div class="vg-opanels">${v}</div></div>`}const WARN="\u26A0",MAX_TABLE_W=1e3,MIN_COL_W=150,NAME_COL_W=180;function breakType(s){return String(s).replace(/(&lt;|,)/g,"$1<wbr>")}const DESC_JA_KEYS=["ja","jp","ja_name","name_ja","japanese","logical_name_ja","logicalNameJa","ja_logical_name","\u548C\u540D","\u65E5\u672C\u8A9E","\u65E5\u672C\u8A9E\u8AD6\u7406\u540D","\u8AD6\u7406\u540D"],DESC_EN_KEYS=["en","en_name","name_en","english","logical_name_en","logicalNameEn","en_logical_name","\u82F1\u8A9E","\u82F1\u8A9E\u8AD6\u7406\u540D"];function pickKey(s,n){for(let e=0;e<n.length;e++){const t=n[e];if(!Object.prototype.hasOwnProperty.call(s,t))continue;const i=s[t];if(!(i==null||String(i)===""))return{key:t,value:String(i)}}return null}function parseDesc(s){const n=String(s==null?"":s).trim();if(!n)return null;if(n.charAt(0)!=="{")return{raw:n};let e=null;try{e=JSON.parse(n)}catch(u){e=null}if(!e||typeof e!="object"||Array.isArray(e))return{raw:n};const t=pickKey(e,DESC_JA_KEYS),i=pickKey(e,DESC_EN_KEYS),o={};t&&(o[t.key]=!0),i&&(o[i.key]=!0);const l=[],r=Object.keys(e);for(let u=0;u<r.length;u++){const c=r[u];if(o[c])continue;const g=e[c];g==null||g===""||l.push({key:c,value:typeof g=="object"?JSON.stringify(g):String(g)})}const a=[];t&&a.push(t),i&&a.push(i);for(let u=0;u<l.length;u++)a.push(l[u]);return{ja:t?t.value:"",en:i?i.value:"",rest:l,pairs:a}}function descHtml(s){const n=parseDesc(s);return n?n.raw?`<div class="vg-cdesc">${esc(n.raw)}</div>`:n.pairs.length?`<table class="vg-cdtable">${n.pairs.map(t=>`<tr><th class="vg-cdk">${esc(t.key)}</th><td class="vg-cdv">${esc(t.value)}</td></tr>`).join("")}</table>`:"":""}function structFields(s){const n=String(s==null?"":s),e=n.indexOf("STRUCT<");if(e<0)return[];const t=[];let i="",o=0;for(let r=e+7;r<n.length;r++){const a=n.charAt(r);if(a===">"&&o===0){t.push(i);break}if(a==="<")o++;else if(a===">")o--;else if(a===","&&o===0){t.push(i),i="";continue}i+=a}const l=[];for(let r=0;r<t.length;r++){const a=t[r].trim().split(/[\s<]/)[0];a&&l.push(a)}return l}function assignOrder(s){const n=e=>("000"+e).slice(-4);for(const e of s.values()){const t=e.name.split("."),i=s.get(t[0]),o=i&&i.vals[0]&&i.vals[0].ord!=null?i.vals[0].ord:9999;let l=n(o),r=t[0];for(let a=1;a<t.length;a++){const u=s.get(r),g=(u&&u.vals[0]?structFields(u.vals[0].type):[]).indexOf(t[a]);l+="."+n(g<0?9999:g),r+="."+t[a]}e.sortKey=l}}function typeShape(s){const n=String(s==null?"":s).trim(),e=n.slice(0,6).toUpperCase()==="ARRAY<";let t=n;e&&(t=n.slice(n.indexOf("<")+1,n.lastIndexOf(">")).trim());const i=t.slice(0,7).toUpperCase()==="STRUCT<";return{repeated:e,record:i,name:i?"RECORD":t}}const isNested=s=>String(s).indexOf(".")>=0;function columnSig(s){const n=(s||[]).map(e=>[String(e.n==null?"":e.n),String(e.t==null?"":e.t),e.u==null||e.u===""?"":String(e.u).toUpperCase(),String(e.d==null?"":e.d)].join("\u0001"));return n.sort(),n.join("\u0002")}function columnGroups(s,n){const e=new Map,t=[],i=s&&s.groups||[];for(let l=0;l<i.length;l++){const r=i[l],a=r.members||[];for(let u=0;u<a.length;u++){const c=a[u],g=columnSig(n[c.viewName]);let f=e.get(g);f||(f={suffixes:[],members:[]},e.set(g,f),t.push(f)),f.suffixes.push((r.suffixes&&r.suffixes[u])!=null?r.suffixes[u]:null),f.members.push(c)}}const o=l=>String(l.suffixes[0]==null?l.members[0].viewName:l.suffixes[0]);for(const l of t){const r=l.members.map((a,u)=>u).sort((a,u)=>{const c=String(l.suffixes[a]==null?l.members[a].viewName:l.suffixes[a]),g=String(l.suffixes[u]==null?l.members[u].viewName:l.suffixes[u]);return c<g?-1:c>g?1:0});l.suffixes=r.map(a=>l.suffixes[a]),l.members=r.map(a=>l.members[a])}return t.sort((l,r)=>r.members.length-l.members.length||(o(l)<o(r)?-1:o(l)>o(r)?1:0)),t}function descCell(s){return!s||!s.descs.length?"":s.descs.length===1?descHtml(s.descs[0].text):s.descs.map(n=>`<div class="vg-cdescwho">${esc(n.suffixes.join(", "))}</div>`+descHtml(n.text)).join("")}function groupColumns(s,n){const e=new Map,t=s.members||[];for(let i=0;i<t.length;i++){const o=t[i],l=s.suffixes&&s.suffixes[i]||o.viewName,r=n[o.viewName]||[];for(let a=0;a<r.length;a++){const u=r[a];let c=e.get(u.n);if(c||(c={name:u.n,descs:[],order:a,vals:[]},e.set(u.n,c)),u.d){const g=String(u.d);let f=null;for(let v=0;v<c.descs.length;v++)c.descs[v].text===g&&(f=c.descs[v]);f?f.suffixes.push(l):c.descs.push({text:g,suffixes:[l]})}c.vals.push({suffix:l,type:u.t,ord:u.o==null?a+1:u.o,nullable:u.u==null||u.u===""?null:String(u.u).toUpperCase()!=="NO"})}}return assignOrder(e),e}function columnOrder(s){const n=new Set,e=[];for(const t of s){const i=[...t.values()].sort((o,l)=>o.sortKey<l.sortKey?-1:o.sortKey>l.sortKey?1:0);for(const o of i)n.has(o.name)||(n.add(o.name),e.push(o.name))}return e}function majority(s){const n=new Map;for(const i of s)n.set(i,(n.get(i)||0)+1);let e=null,t=-1;for(const i of s){const o=n.get(i);o>t&&(e=i,t=o)}return e}const nullText=s=>s===null?"UNKNOWN":s?"NULLABLE":"REQUIRED";function uniq(s){const n=[];for(const e of s)n.indexOf(e)<0&&n.push(e);return n}function cellInfo(s,n){if(!s)return{text:null,meta:"",sig:null,mixed:!1};const e=uniq(s.vals.map(r=>r.type)),t=uniq(s.vals.map(r=>nullText(r.nullable))),i=uniq(s.vals.map(r=>String(r.ord))).length>1||s.vals.length!==n,o=s.vals.map(r=>typeShape(r.type)),l=s.descs.map(r=>r.text).join("\u0001");return{text:e.join(" / "),shape:uniq(o.map(r=>r.name)).join(" / "),repeated:o.length>0&&o.every(r=>r.repeated),record:o.some(r=>r.record),nulls:t.join(" / "),sig:e.join(" / ")+"|"+t.join(" / ")+"|"+l,mixed:i}}function mixedTip(s,n){const e=isNested(s.name),t=s.vals.map(o=>e?`${o.suffix} = ${o.type}`:`${o.suffix} = #${o.ord} ${o.type} ${nullText(o.nullable)}`),i=s.vals.map(o=>o.suffix);for(let o=0;o<(n.members||[]).length;o++){const l=n.suffixes&&n.suffixes[o]||n.members[o].viewName;i.indexOf(l)<0&&t.push(`${l} = (\u3053\u306E\u5217\u3092\u6301\u305F\u306A\u3044)`)}return t.join(`
`)}function renderColumns(s,n){if(!(s.groups||[]).length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const e=columnGroups(s,n||{}),t=e.map(d=>groupColumns(d,n||{})),i=columnOrder(t);if(!i.length)return notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.COLUMNS \u304B\u3089\u5217\u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const o=new Set;for(const d of i){const h=d.lastIndexOf(".");h>0&&o.add(d.slice(0,h))}let l=0,r=0;const a=i.map(d=>{const h=d.split("."),m=h.length-1,_=m>0,b=e.map((p,$)=>cellInfo(t[$].get(d),p.members.length)),S=majority(b.map(p=>p.sig));b.some(p=>p.sig!==S)&&l++;const w=b.map((p,$)=>{const O=e[$],N=t[$].get(d),j=["vg-ccell"];p.text?p.sig!==S&&j.push("vg-cdiff"):j.push("vg-cnone"),p.mixed&&(j.push("vg-cmix"),r++);const C=!p.record||o.has(d)?p.shape:p.text,E=_?p.repeated?"REPEATED":"":p.repeated?"REPEATED":p.nulls,T=p.text?breakType(esc(C))+(p.mixed?`<span class="vg-cwarn" data-tip="${esc(mixedTip(N,O))}">${WARN}</span>`:"")+(E?`<span class="vg-cmeta">${esc(E)}</span>`:"")+descCell(N):"\u2014";return`<td class="${j.join(" ")}">${T}</td>`}).join(""),y="vg-cname"+(m?` vg-cd${Math.min(m,3)}`:""),x=m?`<span class="vg-cnestmark">\u2514</span>${esc(h[h.length-1])}`:esc(d);return`<tr><th class="${y}">${x}</th>${w}</tr>`}).join(""),u=e.map(d=>`<th class="vg-chead">${esc(label(d))}<span class="vg-tabn">${d.members.length}</span></th>`).join(""),c=[];e.length===1?c.push(notice(`${s.viewCount||e[0].members.length} \u672C\u306E View \u3059\u3079\u3066\u3067\u30AB\u30E9\u30E0\u5B9A\u7FA9\uFF08\u5217\u540D\u30FB\u578B\u30FB\u30E2\u30FC\u30C9\u30FB\u8AAC\u660E\uFF09\u304C\u4E00\u81F4\u3057\u3066\u3044\u307E\u3059\u3002`)):(c.push(notice(`\u30AB\u30E9\u30E0\u5B9A\u7FA9\u304C ${e.length} \u7A2E\u985E\u3042\u308A\u307E\u3059\uFF08\u5217\u304C\u305D\u306E\u672C\u6570\uFF09\u3002\u5217\u306F\u30ED\u30B8\u30C3\u30AF \u30B0\u30EB\u30FC\u30D7\u3067\u306F\u306A\u304F\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3067\u675F\u306D\u3066\u3044\u308B\u306E\u3067\u3001SQL \u304C\u540C\u4E00\u3067\u3082\u53C2\u7167\u5148\u30C6\u30FC\u30D6\u30EB\u306E\u578B\u3084 description \u304C\u9055\u3048\u3070\u5225\u306E\u5217\u306B\u306A\u308A\u307E\u3059\uFF08\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206\u306E\u30BF\u30D6\u306B\u306F\u51FA\u3066\u3053\u306A\u3044\u5DEE\u3067\u3059\uFF09\u3002`)),l&&c.push(notice(`\u578B\u30FB\u30E2\u30FC\u30C9\u30FB\u8AAC\u660E\u306E\u3044\u305A\u308C\u304B\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u5217\u304C ${l} \u4EF6\u3042\u308A\u307E\u3059\uFF08\u8272\u4ED8\u304D\u306E\u30BB\u30EB\u3002\u305D\u306E\u5217\u3067\u3044\u3061\u3070\u3093\u591A\u3044\u5185\u5BB9\u3068\u9055\u3046\u3082\u306E\uFF09\u3002`))),r&&c.push(notice(`\u540C\u3058\u5217\u306E\u4E2D\u3067\u4E26\u3073\u9806\uFF08ordinal\uFF09\u3060\u3051\u304C\u98DF\u3044\u9055\u3063\u3066\u3044\u308B\u7B87\u6240\u304C${r} \u4EF6\u3042\u308A\u307E\u3059\uFF08${WARN} \u306E\u5370\u3002\u30DB\u30D0\u30FC\u3059\u308B\u3068 suffix \u3054\u3068\u306E\u5185\u8A33\u304C\u51FA\u307E\u3059\uFF09\u3002\u4E26\u3073\u9806\u306F\u5217\u306E\u675F\u306D\u65B9\u306B\u3082\u8868\u306E\u4E2D\u8EAB\u306B\u3082\u4F7F\u3063\u3066\u3044\u306A\u3044\u306E\u3067\u3001\u3053\u3053\u306B\u3060\u3051\u51FA\u307E\u3059\u3002`));const g=`<colgroup><col style="width:${NAME_COL_W}px">`+e.map(()=>"<col>").join("")+"</colgroup>",f=NAME_COL_W+MIN_COL_W*e.length,v=f>MAX_TABLE_W?` style="min-width:${f}px"`:"";return c.join("")+`<div class="vg-ctablewrap"><table class="vg-ctable"${v}>${g}<thead><tr><th class="vg-chead vg-cnamehead">\u5217\u540D</th>${u}</tr></thead><tbody>${a}</tbody></table></div>`}function renderColumnsBase(s,n){return`<div class="vg-root">${renderColumns(s,n)}</div>`}function sqlViews(s,n){const e=regionByView(n),t=[],i=s.groups||[];for(let o=0;o<i.length;o++){const l=i[o],r=l.members||[];for(let a=0;a<r.length;a++)t.push({suffix:l.suffixes&&l.suffixes[a]||r[a].viewName||"(suffix \u306A\u3057)",viewName:r[a].viewName,region:e[r[a].viewName]||"",group:label(l),groupSize:r.length})}return t.sort((o,l)=>{const r=String(o.suffix),a=String(l.suffix);return String(o.region).localeCompare(String(l.region))||r.length-a.length||r.localeCompare(a)}),t}function sqlBody(s){const n=String(s).replace(/\r\n?/g,`
`).split(`
`);for(;n.length>1&&n[n.length-1].trim()==="";)n.pop();const e=String(n.length).length;return{html:`<div class="vg-sqlbox"><pre class="vg-sqlpre">${n.map((i,o)=>{const l=String(o+1);return`<span class="vg-sqln">${new Array(e-l.length+1).join(" ")}${l}</span> `+esc(i)}).join(`
`)}</pre></div>`,lines:n.length}}function sqlPanel(s,n){if(n==null||String(n)==="")return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(s.viewName)}</span></div>`+notice("\u3053\u306E View \u306E SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002");const e=sqlBody(n);return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(s.viewName)}</span><span class="vg-sqlmeta">\u30B0\u30EB\u30FC\u30D7: ${esc(s.group)}</span><span class="vg-sqlmeta">${e.lines} \u884C</span></div>`+e.html}function renderSql(s,n,e){const t=sqlViews(s,e);if(!t.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const i=n||{};if(!t.some(v=>i[v.viewName]))return notice("SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.VIEWS \u304B\u3089 view_definition \u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const o=t.slice(0,128),l="vgs"+hashId(s.base+"|"+t.map(v=>v.viewName).join("|")),r=o.map((v,d)=>`<input class="vg-sr vg-sr${d+1}" type="radio" name="${l}" id="${l}-${d+1}"${d===0?" checked":""}>`).join(""),a=[];let u=null;o.forEach((v,d)=>{const h=v.region||"";(u===null||u.key!==h)&&(u={key:h,tabs:[]},a.push(u)),u.tabs.push(`<label class="vg-stab vg-st${d+1}" for="${l}-${d+1}">${esc(v.suffix)}</label>`)});const c=a.map((v,d)=>(d>0?'<div class="vg-srule"></div>':"")+`<div class="vg-sregname">${esc(v.key||"View")}</div><div class="vg-sreg">${v.tabs.join("")}</div>`).join(""),g=o.map((v,d)=>`<div class="vg-spanel vg-sp${d+1}">${sqlPanel(v,i[v.viewName])}</div>`).join("");return(t.length>o.length?notice(`View \u304C\u591A\u3044\u305F\u3081\u5148\u982D 128 \u4EF6\u306E\u307F\u30BF\u30D6\u8868\u793A\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${t.length} \u4EF6\uFF09\u3002`):"")+`<div class="vg-stabs">${r}<div class="vg-stablist">${c}</div><div class="vg-spanels">${g}</div></div>`}function renderSqlBase(s,n,e){return`<div class="vg-root">${renderSql(s,n,e)}</div>`}const DESC_TAB_ON="background:#EAEEF2;border-color:#8C959F;color:#24292F";function suffixByView(s){const n={},e=s&&s.groups||[];for(let t=0;t<e.length;t++){const i=e[t],o=i.members||[];for(let l=0;l<o.length;l++){const r=o[l]&&o[l].viewName;r&&(n[r]=i.suffixes&&i.suffixes[l]||r)}}return n}const cmp=(s,n)=>s<n?-1:s>n?1:0;function tabGroups(s,n,e){const t=suffixByView(s),i=Array.isArray(n)?n:[],o=[];for(let l=0;l<i.length;l++){const r=i[l]||{},a=Array.isArray(r.v)?r.v:r.v==null||r.v===""?[]:[r.v],u={label:a.map(c=>t[c]||c).sort(cmp).join(", ")||"(View \u306A\u3057)",count:a.length};e(u,r),o.push(u)}return o.sort((l,r)=>(l.empty===r.empty?0:l.empty?1:-1)||r.count-l.count||cmp(l.label,r.label)),o}function descGroups(s,n){return tabGroups(s,n,(e,t)=>{e.html=String(t.h==null?"":t.h),e.empty=e.html.trim()===""})}function labelGroups(s,n){return tabGroups(s,n,(e,t)=>{const i=Array.isArray(t.l)?t.l:[],o=[];for(let l=0;l<i.length;l++){const r=i[l]||{},a=String(r.k==null?"":r.k);a!==""&&o.push({k:a,v:String(r.v==null?"":r.v)})}o.sort((l,r)=>cmp(l.k,r.k)),e.pairs=o,e.empty=o.length===0})}function descPanel(s){return s.empty?notice("description \u304C\u8A2D\u5B9A\u3055\u308C\u3066\u3044\u307E\u305B\u3093\u3002"):s.html}const tabText=s=>`${esc(s.label)}<span class="vg-tabn">${s.count}</span>`;function renderDesc(s,n){const e=descGroups(s,n),t='<div class="vg-nhead">View \u306E description</div>';if(!e.length)return`<div class="vg-nsec">${t}`+notice("View \u306E description \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.TABLE_OPTIONS \u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002")+"</div>";if(e.every(c=>c.empty))return"";if(e.length===1)return`<div class="vg-nsec">${t}<div class="vg-dtablist"><span class="vg-dtab vg-dstatic">${tabText(e[0])}</span></div><div class="vg-dbox">${descPanel(e[0])}</div></div>`;const i=e.slice(0,16),o="vgd"+hashId((s&&s.base||"")+"|"+e.map(c=>c.label).join("|")),l=i.map((c,g)=>`<input class="vg-dr vg-dr${g+1}" type="radio" name="${o}" id="${o}-${g+1}"${g===0?" checked":""}>`).join(""),r=i.map((c,g)=>`<label class="vg-dtab vg-dt${g+1}" for="${o}-${g+1}">${tabText(c)}</label>`).join(""),a=i.map((c,g)=>`<div class="vg-dpanel vg-dp${g+1}">${descPanel(c)}</div>`).join(""),u=e.length>i.length?notice(`description \u304C\u591A\u3044\u305F\u3081\u5148\u982D 16 \u7A2E\u985E\u306E\u307F\u51FA\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${e.length} \u7A2E\u985E\uFF09\u3002`):"";return`<div class="vg-nsec">${t}${u}<div class="vg-dtabs">${l}<div class="vg-dtablist">${r}</div><div class="vg-dpanels">${a}</div></div></div>`}function labelChips(s){return s.empty?notice("\u30E9\u30D9\u30EB\u304C\u8A2D\u5B9A\u3055\u308C\u3066\u3044\u307E\u305B\u3093\u3002"):'<div class="vg-lbchips">'+s.pairs.map(n=>`<span class="vg-lbchip"><span class="vg-lbk">${esc(n.k)}</span><span class="vg-lbv">${esc(n.v||"(\u7A7A)")}</span></span>`).join("")+"</div>"}function labelsSplit(s,n){return labelGroups(s,n).length>1}function renderLabels(s,n){const e=labelGroups(s,n);if(!e.length||e[0].empty)return"";if(e.length===1)return labelChips(e[0]);const t=e.slice(0,8),i="vgl"+hashId((s&&s.base||"")+"|"+e.map(u=>u.label+"="+u.pairs.map(c=>c.k+":"+c.v).join(",")).join("|")),o=t.map((u,c)=>`<input class="vg-lbr vg-lbr${c+1}" type="radio" name="${i}" id="${i}-${c+1}"${c===0?" checked":""}>`).join(""),l=t.map((u,c)=>`<label class="vg-lbtab vg-lbt${c+1}" for="${i}-${c+1}">${tabText(u)}</label>`).join(""),r=t.map((u,c)=>`<div class="vg-lbpanel vg-lbp${c+1}">${labelChips(u)}</div>`).join(""),a=e.length>t.length?notice(`\u30E9\u30D9\u30EB\u306E\u7A2E\u985E\u304C\u591A\u3044\u305F\u3081\u5148\u982D 8 \u7A2E\u985E\u306E\u307F\u51FA\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${e.length} \u7A2E\u985E\uFF09\u3002`):"";return'<div class="vg-nsec"><div class="vg-nhead">\u30E9\u30D9\u30EB</div>'+notice("\u540C\u3058 base \u306E\u4E2D\u3067\u30E9\u30D9\u30EB\u304C\u5272\u308C\u3066\u3044\u307E\u3059\u3002")+a+`<div class="vg-lbtabs">${o}<div class="vg-lbtablist">${l}</div><div class="vg-lbpanels">${r}</div></div></div>`}function renderRegions(s,n){const e=regionByView(n),t=suffixByView(s),i=new Map,o=s&&s.groups||[];for(let r=0;r<o.length;r++){const a=o[r].members||[];for(let u=0;u<a.length;u++){const c=a[u].viewName,g=e[c];g==null||g===""||(i.has(g)||i.set(g,[]),i.get(g).push(t[c]||c))}}return i.size?`<div class="vg-nsec"><div class="vg-nhead">View \u306E\u30EA\u30FC\u30B8\u30E7\u30F3</div><table class="vg-loctable">${[...i.keys()].sort(cmp).map(r=>`<tr><th class="vg-lock">${esc(r)}</th><td class="vg-locn">${i.get(r).length}</td><td class="vg-locv">${esc(i.get(r).slice().sort(cmp).join(", "))}</td></tr>`).join("")}</table></div>`:""}function renderNote(s,n,e,t,i){const o=String(e==null?"":e);return renderLabels(s,t)+renderRegions(s,i)+renderDesc(s,n)+`<div class="vg-nsec"><div class="vg-nhead">\u30E1\u30E2</div>${o}</div>`}function __opts(s){if(!s)return{};try{return JSON.parse(s)||{}}catch(n){return{}}}function __notice(s){return'<div class="vg-notice">'+String(s).replace(/[<>&]/g,"")+"</div>"}function __run(s,n,e,t,i,o,l,r,a){var u=__opts(a),c;try{c=JSON.parse(s)}catch($){c=null}if(!c)return String(n||__notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002"));var g={};try{for(var f=JSON.parse(t)||[],v=0;v<f.length;v++)g[f[v].v]=f[v].cols||[]}catch($){g={}}var d={};try{for(var h=JSON.parse(i)||[],m=0;m<h.length;m++)d[h[m].v]=h[m].s}catch($){d={}}var _=[];try{_=JSON.parse(o)||[]}catch($){_=[]}var b=[];try{b=JSON.parse(l)||[]}catch($){b=[]}var S=[];try{S=JSON.parse(r)||[]}catch($){S=[]}for(var A="",w="",y=c.bases||[],x=0;x<y.length;x++)A+=renderColumnsBase(y[x],g),w+=renderSqlBase(y[x],d,S);A||(A=__notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),w||(w=__notice("SQL \u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002"));var p=c.bases&&c.bases.length?c.bases[0]:null;return wrapPage(String(n||""),String(e||__notice("\u53C2\u7167\u95A2\u4FC2\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002")),A,w,renderNote(p,_,NOTE_MARK,b,S),p,labelsSplit(p,b))}return __run(analysis_json,diff_html,erd_html,columns_json,sql_json,descs_json,labels_json,regions_json,options_json);
...
DECLARE css_part_1 STRING DEFAULT r""".vg-cssgen11{display:none}
...
DECLARE css_part_2 STRING DEFAULT r""".vg-md>:last-child,.vg-mdq>:last-child{margin-bottom:0}
...
.vg-stablist{display:grid;grid-template-columns:max-content minmax(0,1fr);align-items:baseline;column-gap:12px;margin:0 0 10px}
.vg-srule{grid-column:1/-1;border-top:1px solid #EAEEF2;margin:5px 0}
.vg-sregname{color:#57606A;font-size:11px;font-weight:600;white-space:nowrap;line-height:1.9;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}
...
DECLARE css_part_4 STRING DEFAULT r""".vg-sr1:checked ~ .vg-stablist > .vg-sreg > .vg-st1,.vg-sr2:checked ~ .vg-stablist > .vg-sreg > .vg-st2,.vg-sr3:checked ~ .vg-stablist > .vg-sreg > .vg-st3,.vg-sr4:checked ~ .vg-stablist > .vg-sreg > .vg-st4,.vg-sr5:checked ~ .vg-stablist > .vg-sreg > .vg-st5,.vg-sr6:checked ~ .vg-stablist > .vg-sreg > .vg-st6,.vg-sr7:checked ~ .vg-stablist > .vg-sreg > .vg-st7,.vg-sr8:checked ~ .vg-stablist > .vg-sreg > .vg-st8,.vg-sr9:checked ~ .vg-stablist > .vg-sreg > .vg-st9,.vg-sr10:checked ~ .vg-stablist > .vg-sreg > .vg-st10,.vg-sr11:checked ~ .vg-stablist > .vg-sreg > .vg-st11,.vg-sr12:checked ~ .vg-stablist > .vg-sreg > .vg-st12,.vg-sr13:checked ~ .vg-stablist > .vg-sreg > .vg-st13,.vg-sr14:checked ~ .vg-stablist > .vg-sreg > .vg-st14,.vg-sr15:checked ~ .vg-stablist > .vg-sreg > .vg-st15,.vg-sr16:checked ~ .vg-stablist > .vg-sreg > .vg-st16,.vg-sr17:checked ~ .vg-stablist > .vg-sreg > .vg-st17,.vg-sr18:checked ~ .vg-stablist > .vg-sreg > .vg-st18,.vg-sr19:checked ~ .vg-stablist > .vg-sreg > .vg-st19,.vg-sr20:checked ~ .vg-stablist > .vg-sreg > .vg-st20,.vg-sr21:checked ~ .vg-stablist > .vg-sreg > .vg-st21,.vg-sr22:checked ~ .vg-stablist > .vg-sreg > .vg-st22,.vg-sr23:checked ~ .vg-stablist > .vg-sreg > .vg-st23,.vg-sr24:checked ~ .vg-stablist > .vg-sreg > .vg-st24,.vg-sr25:checked ~ .vg-stablist > .vg-sreg > .vg-st25,.vg-sr26:checked ~ .vg-stablist > .vg-sreg > .vg-st26,.vg-sr27:checked ~ .vg-stablist > .vg-sreg > .vg-st27,.vg-sr28:checked ~ .vg-stablist > .vg-sreg > .vg-st28,.vg-sr29:checked ~ .vg-stablist > .vg-sreg > .vg-st29,.vg-sr30:checked ~ .vg-stablist > .vg-sreg > .vg-st30,.vg-sr31:checked ~ .vg-stablist > .vg-sreg > .vg-st31,.vg-sr32:checked ~ .vg-stablist > .vg-sreg > .vg-st32,.vg-sr33:checked ~ .vg-stablist > .vg-sreg > .vg-st33,.vg-sr34:checked ~ .vg-stablist > .vg-sreg > .vg-st34,.vg-sr35:checked ~ .vg-stablist > .vg-sreg > .vg-st35,.vg-sr36:checked ~ .vg-stablist > .vg-sreg > .vg-st36,.vg-sr37:checked ~ .vg-stablist > .vg-sreg > .vg-st37,.vg-sr38:checked ~ .vg-stablist > .vg-sreg > .vg-st38,.vg-sr39:checked ~ .vg-stablist > .vg-sreg > .vg-st39,.vg-sr40:checked ~ .vg-stablist > .vg-sreg > .vg-st40,.vg-sr41:checked ~ .vg-stablist > .vg-sreg > .vg-st41,.vg-sr42:checked ~ .vg-stablist > .vg-sreg > .vg-st42,.vg-sr43:checked ~ .vg-stablist > .vg-sreg > .vg-st43,.vg-sr44:checked ~ .vg-stablist > .vg-sreg > .vg-st44,.vg-sr45:checked ~ .vg-stablist > .vg-sreg > .vg-st45,.vg-sr46:checked ~ .vg-stablist > .vg-sreg > .vg-st46,.vg-sr47:checked ~ .vg-stablist > .vg-sreg > .vg-st47,.vg-sr48:checked ~ .vg-stablist > .vg-sreg > .vg-st48,.vg-sr49:checked ~ .vg-stablist > .vg-sreg > .vg-st49,.vg-sr50:checked ~ .vg-stablist > .vg-sreg > .vg-st50,.vg-sr51:checked ~ .vg-stablist > .vg-sreg > .vg-st51,.vg-sr52:checked ~ .vg-stablist > .vg-sreg > .vg-st52,.vg-sr53:checked ~ .vg-stablist > .vg-sreg > .vg-st53,.vg-sr54:checked ~ .vg-stablist > .vg-sreg > .vg-st54,.vg-sr55:checked ~ .vg-stablist > .vg-sreg > .vg-st55,.vg-sr56:checked ~ .vg-stablist > .vg-sreg > .vg-st56,.vg-sr57:checked ~ .vg-stablist > .vg-sreg > .vg-st57,.vg-sr58:checked ~ .vg-stablist > .vg-sreg > .vg-st58,.vg-sr59:checked ~ .vg-stablist > .vg-sreg > .vg-st59,.vg-sr60:checked ~ .vg-stablist > .vg-sreg > .vg-st60,.vg-sr61:checked ~ .vg-stablist > .vg-sreg > .vg-st61,.vg-sr62:checked ~ .vg-stablist > .vg-sreg > .vg-st62,.vg-sr63:checked ~ .vg-stablist > .vg-sreg > .vg-st63,.vg-sr64:checked ~ .vg-stablist > .vg-sreg > .vg-st64,.vg-sr65:checked ~ .vg-stablist > .vg-sreg > .vg-st65,.vg-sr66:checked ~ .vg-stablist > .vg-sreg > .vg-st66,.vg-sr67:checked ~ .vg-stablist > .vg-sreg > .vg-st67,.vg-sr68:checked ~ .vg-stablist > .vg-sreg > .vg-st68,.vg-sr69:checked ~ .vg-stablist > .vg-sreg > .vg-st69,.vg-sr70:checked ~ .vg-stablist > .vg-sreg > .vg-st70,.vg-sr71:checked ~ .vg-stablist > .vg-sreg > .vg-st71,.vg-sr72:checked ~ .vg-stablist > .vg-sreg > .vg-st72,.vg-sr73:checked ~ .vg-stablist > .vg-sreg > .vg-st73,.vg-sr74:checked ~ .vg-stablist > .vg-sreg > .vg-st74,.vg-sr75:checked ~ .vg-stablist > .vg-sreg > .vg-st75,.vg-sr76:checked ~ .vg-stablist > .vg-sreg > .vg-st76,.vg-sr77:checked ~ .vg-stablist > .vg-sreg > .vg-st77,.vg-sr78:checked ~ .vg-stablist > .vg-sreg > .vg-st78,.vg-sr79:checked ~ .vg-stablist > .vg-sreg > .vg-st79,.vg-sr80:checked ~ .vg-stablist > .vg-sreg > .vg-st80,.vg-sr81:checked ~ .vg-stablist > .vg-sreg > .vg-st81,.vg-sr82:checked ~ .vg-stablist > .vg-sreg > .vg-st82,.vg-sr83:checked ~ .vg-stablist > .vg-sreg > .vg-st83,.vg-sr84:checked ~ .vg-stablist > .vg-sreg > .vg-st84,.vg-sr85:checked ~ .vg-stablist > .vg-sreg > .vg-st85,.vg-sr86:checked ~ .vg-stablist > .vg-sreg > .vg-st86,.vg-sr87:checked ~ .vg-stablist > .vg-sreg > .vg-st87,.vg-sr88:checked ~ .vg-stablist > .vg-sreg > .vg-st88,.vg-sr89:checked ~ .vg-stablist > .vg-sreg > .vg-st89,.vg-sr90:checked ~ .vg-stablist > .vg-sreg > .vg-st90,.vg-sr91:checked ~ .vg-stablist > .vg-sreg > .vg-st91,.vg-sr92:checked ~ .vg-stablist > .vg-sreg > .vg-st92,.vg-sr93:checked ~ .vg-stablist > .vg-sreg > .vg-st93,.vg-sr94:checked ~ .vg-stablist > .vg-sreg > .vg-st94,.vg-sr95:checked ~ .vg-stablist > .vg-sreg > .vg-st95,.vg-sr96:checked ~ .vg-stablist > .vg-sreg > .vg-st96,.vg-sr97:checked ~ .vg-stablist > .vg-sreg > .vg-st97,.vg-sr98:checked ~ .vg-stablist > .vg-sreg > .vg-st98,.vg-sr99:checked ~ .vg-stablist > .vg-sreg > .vg-st99,.vg-sr100:checked ~ .vg-stablist > .vg-sreg > .vg-st100,.vg-sr101:checked ~ .vg-stablist > .vg-sreg > .vg-st101,.vg-sr102:checked ~ .vg-stablist > .vg-sreg > .vg-st102,.vg-sr103:checked ~ .vg-stablist > .vg-sreg > .vg-st103,.vg-sr104:checked ~ .vg-stablist > .vg-sreg > .vg-st104,.vg-sr105:checked ~ .vg-stablist > .vg-sreg > .vg-st105,.vg-sr106:checked ~ .vg-stablist > .vg-sreg > .vg-st106,.vg-sr107:checked ~ .vg-stablist > .vg-sreg > .vg-st107,.vg-sr108:checked ~ .vg-stablist > .vg-sreg > .vg-st108,.vg-sr109:checked ~ .vg-stablist > .vg-sreg > .vg-st109,.vg-sr110:checked ~ .vg-stablist > .vg-sreg > .vg-st110,.vg-sr111:checked ~ .vg-stablist > .vg-sreg > .vg-st111,.vg-sr112:checked ~ .vg-stablist > .vg-sreg > .vg-st112,.vg-sr113:checked ~ .vg-stablist > .vg-sreg > .vg-st113,.vg-sr114:checked ~ .vg-stablist > .vg-sreg > .vg-st114,.vg-sr115:checked ~ .vg-stablist > .vg-sreg > .vg-st115,.vg-sr116:checked ~ .vg-stablist > .vg-sreg > .vg-st116,.vg-sr117:checked ~ .vg-stablist > .vg-sreg > .vg-st117,.vg-sr118:checked ~ .vg-stablist > .vg-sreg > .vg-st118,.vg-sr119:checked ~ .vg-stablist > .vg-sreg > .vg-st119,.vg-sr120:checked ~ .vg-stablist > .vg-sreg > .vg-st120,.vg-sr121:checked ~ .vg-stablist > .vg-sreg > .vg-st121,.vg-sr122:checked ~ .vg-stablist > .vg-sreg > .vg-st122,.vg-sr123:checked ~ .vg-stablist > .vg-sreg > .vg-st123,.vg-sr124:checked ~ .vg-stablist > .vg-sreg > .vg-st124,.vg-sr125:checked ~ .vg-stablist > .vg-sreg > .vg-st125,.vg-sr126:checked ~ .vg-stablist > .vg-sreg > .vg-st126,.vg-sr127:checked ~ .vg-stablist > .vg-sreg > .vg-st127,.vg-sr128:checked ~ .vg-stablist > .vg-sreg > .vg-st128{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
