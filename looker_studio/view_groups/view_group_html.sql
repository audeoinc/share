...
var M=Object.defineProperty,j=Object.defineProperties;var U=Object.getOwnPropertyDescriptors;var R=Object.getOwnPropertySymbols;var D=Object.prototype.hasOwnProperty,q=Object.prototype.propertyIsEnumerable;var C=(t,e,n)=>e in t?M(t,e,{enumerable:!0,configurable:!0,writable:!0,value:n}):t[e]=n,F=(t,e)=>{for(var n in e||(e={}))D.call(e,n)&&C(t,n,e[n]);if(R)for(var n of R(e))q.call(e,n)&&C(t,n,e[n]);return t},O=(t,e)=>j(t,U(e));const MAX_TABS=12,MAX_REF_TABS=12,MAX_SQL_TABS=24,REF_BUDGET=41943040,OUTER_TABS=["note","\u30AB\u30E9\u30E0\u5B9A\u7FA9","\u53C2\u7167\u95A2\u4FC2","\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206","SQL"],MAX_OUTER_TABS=8,NOTE_MARK="<!--VG_NOTE-->";function esc(t){return String(t==null?"":t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function hashId(t){let e=2166136261;for(let n=0;n<String(t).length;n++)e^=String(t).charCodeAt(n),e=Math.imul(e,16777619)>>>0;return e.toString(36)}const label=t=>t.suffixes.map((e,n)=>e||t.members[n]&&t.members[n].viewName||"(suffix \u306A\u3057)").join(", ");function badge(t,e,n){return`<span class="vg-badge" style="color:${e};background:${n}">${esc(t)}</span>`}function header(t,e,n,s){const o=n>1;return`<div class="vg-header"><span class="vg-title">${esc(t)}</span>`+badge(`${e} View`,"#57606A","#EAEEF2")+badge(`${n} \u30B0\u30EB\u30FC\u30D7`,o?"#9A6700":"#1A7F37",o?"#FFF8C5":"#DAFBE1")+(s?badge("suffix \u672A\u8A8D\u8B58","#9A6700","#FFF8C5"):"")+"</div>"}function notice(t){return`<div class="vg-notice">${esc(t)}</div>`}const KIND_TEXT={entity:"\u5B9F\u4F53\u540D",string:"\u5024\uFF08\u6587\u5B57\u5217\uFF09",number:"\u5024\uFF08\u6570\u5024\uFF09",ident:"\u540D\u524D",quoted:"\u540D\u524D",keyword:"\u4E88\u7D04\u8A9E",punct:"\u8A18\u53F7",comment:"\u30B3\u30E1\u30F3\u30C8"},kindText=t=>KIND_TEXT[t]||t;function wrapPage(t,e,n,s,o){const i=o||{},r="vgo"+hashId(i.base||""),c=[`<div class="vg-root">${NOTE_MARK}</div>`,n,e,t,s],a=OUTER_TABS.map((h,g)=>`<input class="vg-or vg-or${g+1}" type="radio" name="${r}" id="${r}-${g+1}"${g===0?" checked":""}>`).join(""),u=OUTER_TABS.map((h,g)=>`<label class="vg-otab vg-ot${g+1}" for="${r}-${g+1}">${esc(h)}</label>`).join(""),d=OUTER_TABS.map((h,g)=>`<div class="vg-opanel vg-op${g+1}">${c[g]||""}</div>`).join(""),p=i.base?header(i.base,i.viewCount,(i.groups||[]).length,i.unmatched):"";return`<div class="vg-outer">${a}<div class="vg-otablist">${p}${u}</div><div class="vg-opanels">${d}</div></div>`}const DQ="\\u0022",DQ3=DQ+DQ+DQ,TOKEN_RE=new RegExp(["(`[^`]*`)","([rbRB]{1,2}(?:'''[\\s\\S]*?'''|"+DQ3+"[\\s\\S]*?"+DQ3+"|'[^']*'|"+DQ+"[^"+DQ+"]*"+DQ+"))","('''[\\s\\S]*?''')","("+DQ3+"[\\s\\S]*?"+DQ3+")","('(?:\\\\.|''|[^'\\\\])*')","("+DQ+"(?:\\\\.|"+DQ+DQ+"|[^"+DQ+"\\\\])*"+DQ+")","(--[^\\n]*|#[^\\n]*)","(/\\*[\\s\\S]*?\\*/)","(0[xX][0-9a-fA-F]+|(?:\\d+(?:\\.\\d+)?|\\.\\d+)(?:[eE][+-]?\\d+)?)","([A-Za-z_][A-Za-z0-9_]*)","(\\s+)","([^\\s])"].join("|"),"g"),KEYWORDS=new Set("SELECT FROM WHERE JOIN INNER LEFT RIGHT FULL OUTER CROSS ON USING AND OR NOT IN IS NULL LIKE BETWEEN EXISTS ANY ALL SOME GROUP BY HAVING ORDER ASC DESC LIMIT OFFSET QUALIFY WINDOW UNION INTERSECT EXCEPT DISTINCT AS CASE WHEN THEN ELSE END CREATE OR REPLACE VIEW TABLE FUNCTION IF EXISTS OPTIONS WITH RECURSIVE OVER PARTITION UNNEST STRUCT ARRAY CAST SAFE_CAST EXTRACT INTERVAL DATE DATETIME TIME TIMESTAMP INT64 FLOAT64 NUMERIC BIGNUMERIC STRING BYTES BOOL TRUE FALSE COUNT SUM AVG MIN MAX COALESCE IFNULL NULLIF ROWS RANGE PRECEDING FOLLOWING CURRENT ROW".split(/\s+/));function tokenizeSql(t){const e=[],n=String(t==null?"":t);let s;for(TOKEN_RE.lastIndex=0;(s=TOKEN_RE.exec(n))!==null;){let o;s[1]?o="quoted":s[2]||s[3]||s[4]||s[5]||s[6]?o="string":s[7]||s[8]?o="comment":s[9]?o="number":s[10]?o=KEYWORDS.has(s[10].toUpperCase())?"keyword":"ident":s[11]?o="space":o="punct",e.push({kind:o,text:s[0]})}return e}function markEntities(t){const e=t.slice(),n=i=>{let r=i+1;for(;r<e.length&&e[r].kind==="space";)r++;return r},s=[];let o=null;for(let i=0;i<e.length;i++){const r=e[i];if(r.kind==="space"||r.kind==="comment")continue;if(r.text==="("){s.push(o),o=null;continue}if(r.text===")"){s.pop(),o=null;continue}const c=r.text.toUpperCase();if(o=r.kind==="keyword"||r.kind==="ident"?c:null,r.kind!=="keyword"||c!=="FROM"&&c!=="JOIN"||s.length>0&&s[s.length-1]==="EXTRACT")continue;let a=n(i);for(;!(a>=e.length);){const u=e[a];if(u.kind==="quoted")e[a]={kind:"entity",text:u.text},a=n(a);else if(u.kind==="ident"){const d=[];let p=a;for(;;){d.push(p);const g=n(p);if(g<e.length&&e[g].text==="."){const f=n(g);if(f<e.length&&e[f].kind==="ident"){p=f;continue}}break}const h=n(p);if(h<e.length&&e[h].text==="(")break;for(const g of d)e[g]={kind:"entity",text:e[g].text};a=h}else break;if(a<e.length&&e[a].kind==="keyword"&&e[a].text.toUpperCase()==="AS"){const d=n(a);d<e.length&&(e[d].kind==="ident"||e[d].kind==="quoted")&&(a=n(d))}else a<e.length&&e[a].kind==="ident"&&(a=n(a));if(a<e.length&&e[a].text===","){a=n(a);continue}break}}return e}const DEFAULT_SUFFIX_RE=/^(.*?)_([A-Za-z0-9]{1,6})$/,DEFAULT_SUBSTITUTABLE=["entity","number","string"],SUFFIX_MARK="\0",LITERAL_MARK="\u0001",BOX_W_MIN=130,BOX_W_MAX=380,NAME_CHAR_W=6.65,BOX_H=40,GAP_MIN=78,CHAR_W=6.5,LINE_H=14,GAP_Y=14,PAD=10,kw=t=>t&&t.kind==="keyword"?t.text.toUpperCase():null;function prepare(t){const e=[];let n=0;for(const s of markEntities(tokenizeSql(t)))s.kind==="space"||s.kind==="comment"||(s.text===")"&&n--,e.push({kind:s.kind,text:s.text,depth:n}),s.text==="("&&n++);return e}function cteRanges(t){const e=[];if(!(t[0]&&kw(t[0])==="WITH"))return{ctes:e,mainFrom:0};let n=1;for(;;){const s=t[n],o=t[n+1],i=t[n+2];if(!i||s.kind!=="ident"||kw(o)!=="AS"||i.text!=="(")break;const r=i.depth;let c=n+3;for(;c<t.length&&!(t[c].text===")"&&t[c].depth===r);)c++;if(e.push({name:s.text,from:n+3,to:c,depth:r+1}),n=c+1,t[n]&&t[n].text===","){n++;continue}break}return{ctes:e,mainFrom:n}}const STOP=new Set(["WHERE","GROUP","ORDER","QUALIFY","WINDOW","HAVING","UNION","INTERSECT","EXCEPT","LIMIT","SELECT","JOIN","FROM"]);function skipParen(t,e,n){const s=t[e].depth;let o=e+1;for(;o<n&&!(t[o].text===")"&&t[o].depth===s);)o++;return o+1}function readSource(t,e,n){if(e>=n)return{node:null,next:e};const s=t[e];if(s.text==="(")return{node:{name:"(\u30B5\u30D6\u30AF\u30A8\u30EA)",kind:"subquery"},next:skipParen(t,e,n)};if(kw(s)==="UNNEST"){let o=e+1,i="";if(t[o]&&t[o].text==="("){const r=t[o].depth;let c=o+1;for(;c<n&&!(t[c].text===")"&&t[c].depth===r);)i+=t[c].text,c++;o=c+1}return{node:{name:"UNNEST("+i+")",kind:"unnest"},next:o}}if(s.kind==="entity"||s.kind==="quoted"||s.kind==="ident"){const o=[];let i=e;for(;i<n&&(t[i].kind==="entity"||t[i].kind==="quoted"||t[i].kind==="ident");){if(o.push(t[i].text),i++,i<n&&t[i].text==="."){i++;continue}break}return i<n&&t[i].text==="("?{node:null,next:skipParen(t,i,n)}:{node:{name:o.join("."),kind:"ref"},next:i}}return{node:null,next:e+1}}function skipAlias(t,e,n){if(e<n&&kw(t[e])==="AS"){const s=t[e+1];return s&&(s.kind==="ident"||s.kind==="quoted")?e+2:e+1}return e<n&&t[e].kind==="ident"&&!STOP.has(t[e].text.toUpperCase())?e+1:e}function readOnKeys(t,e,n){const s=[];let o=e;for(;o<n;){const i=kw(t[o]);if(i&&STOP.has(i)||i==="LEFT"||i==="RIGHT"||i==="FULL"||i==="INNER"||i==="CROSS")break;if(t[o].text==="="){const r=(u,d,p)=>u&&d&&p&&d.text==="."?p.text:null,c=r(t[o-3],t[o-2],t[o-1]),a=r(t[o+1],t[o+2],t[o+3]);c&&a&&s.push(c===a?c:c+" = "+a)}o++}return{keys:s,next:o}}function readUsingKeys(t,e,n){const s=[];let o=e;if(t[o]&&t[o].text==="("){const i=t[o].depth;for(o++;o<n&&!(t[o].text===")"&&t[o].depth===i);)t[o].kind==="ident"&&s.push(t[o].text),o++;o++}return{keys:s,next:o}}function joinKind(t,e,n){for(let s=e-1;s>=n&&e-s<=3;s--){const o=kw(t[s]);if(o==="LEFT"||o==="RIGHT"||o==="FULL"||o==="CROSS")return o;if(o==="INNER")return"INNER"}return"INNER"}function scanScope(t,e,n,s){const o=[];for(let i=e;i<n;i++){const r=kw(t[i]);if(r!=="FROM"&&r!=="JOIN")continue;const c=t[i].depth>s,a=r==="JOIN"?joinKind(t,i,e):null;let u=i+1;for(;;){const d=readSource(t,u,n);if(u=d.next,d.node){u=skipAlias(t,u,n);let p=[];if(!c&&u<n&&kw(t[u])==="ON"){const h=readOnKeys(t,u+1,n);p=h.keys,u=h.next}else if(!c&&u<n&&kw(t[u])==="USING"){const h=readUsingKeys(t,u+1,n);p=h.keys,u=h.next}d.node.kind!=="unnest"&&o.push({name:d.node.name,kind:d.node.kind,joinType:a,keys:p,nested:c})}if(u<n&&t[u].text===","&&t[u].depth===t[i].depth){u++;continue}break}i=Math.max(i,u-1)}return o}function buildGraph(t,e){const n=new Map((e||[]).map(f=>[f.name,f])),s=f=>{const m=Object.keys(f.values);return m.length?f.values[m[0]]:null},o=new Map,i=String(t).replace(/\{\{(P\d+)\}\}/g,(f,m)=>{const x=n.get(m);if(!x)return f;const S=s(x);return S==null?f:(o.set(S,x),S)}),r=prepare(i),{ctes:c,mainFrom:a}=cteRanges(r),u=new Set(c.map(f=>f.name)),d=new Map,p=[],h=(f,m)=>{const x=m+":"+f;if(!d.has(x)){const S=[],$=o.get(String(f));if($)S.push($);else for(const b of String(f).split(".")){const l=o.get(b);l&&S.indexOf(l)<0&&S.push(l)}d.set(x,{id:x,name:f,label:f,kind:m,params:S})}return x},g=c.map(f=>({id:h(f.name,"cte"),from:f.from,to:f.to,depth:f.depth}));g.push({id:h("(\u6700\u7D42 SELECT)","output"),from:a,to:r.length,depth:0});for(const f of g)for(const m of scanScope(r,f.from,f.to,f.depth)){const x=m.kind==="ref"?u.has(m.name)?"cte":"table":m.kind,S=h(m.name,x);if(S===f.id)continue;const $=p.find(b=>b.from===S&&b.to===f.id);$?($.keys.length||($.keys=m.keys),$.joinType||($.joinType=m.joinType),$.nested=$.nested&&m.nested):p.push({from:S,to:f.id,joinType:m.joinType,keys:m.keys,nested:m.nested})}return{nodes:[...d.values()],edges:p}}function layout(t){const{nodes:e,edges:n}=t,s=new Map(e.map(l=>[l.id,[]])),o=new Map(e.map(l=>[l.id,[]]));for(const l of n)s.has(l.to)&&s.get(l.to).push(l.from),o.has(l.from)&&o.get(l.from).push(l.to);const i=new Map(e.map(l=>[l.id,0]));for(let l=0;l<e.length+1;l++){let E=!1;for(const k of e){const N=s.get(k.id);if(!N.length)continue;const v=Math.max(...N.map(w=>i.get(w)||0))+1;v>i.get(k.id)&&(i.set(k.id,v),E=!0)}if(!E)break}const r=Math.max(0,...e.map(l=>i.get(l.id))),c=new Map(e.map(l=>[l.id,o.get(l.id).length?1/0:r]));for(let l=0;l<e.length+1;l++){let E=!1;for(const k of e){const N=o.get(k.id);if(!N.length)continue;const v=Math.min(...N.map(w=>c.get(w)));v-1<c.get(k.id)&&(c.set(k.id,v-1),E=!0)}if(!E)break}for(const l of e)Number.isFinite(c.get(l.id))||c.set(l.id,r);const a=[];for(const l of e){const E=Math.max(0,c.get(l.id));(a[E]||(a[E]=[])).push(l)}const u=new Map;a.forEach((l,E)=>{l&&(E>0&&l.sort((k,N)=>{const v=w=>{const A=s.get(w.id).filter(T=>u.has(T));return A.length?A.reduce((T,y)=>T+u.get(y),0)/A.length:Number.MAX_SAFE_INTEGER};return v(k)-v(N)}),l.forEach((k,N)=>u.set(k.id,N)))});const d=boxWidth(e),p=new Map;a.forEach((l,E)=>(l||[]).forEach(k=>p.set(k.id,E)));const h=new Array(Math.max(0,a.length-1)).fill(GAP_MIN);for(const l of n){const E=(p.get(l.to)||0)-1;E<0||E>=h.length||(h[E]=Math.max(h[E],linesWidth(edgeLines(l))+22))}const g=[];let f=PAD;for(let l=0;l<a.length;l++)g[l]=f,f+=d+(h[l]||0);const m=[];a.forEach((l,E)=>{(l||[]).forEach((k,N)=>{m.push(O(F({},k),{x:g[E],y:PAD+N*(BOX_H+GAP_Y),w:d,h:BOX_H}))})});const x=new Map(m.map(l=>[l.id,l])),S=Math.max(1,...a.map(l=>(l||[]).length));let $=0,b=PAD*2+S*BOX_H+Math.max(0,S-1)*GAP_Y+6;for(const l of n){const E=x.get(l.from),k=x.get(l.to);if(!E||!k)continue;const N=edgeLines(l);if(!N.length)continue;const v=(E.y+E.h/2+k.y+k.h/2)/2,w=N.length*LINE_H;$=Math.min($,v-w/2-PAD),b=Math.max(b,v+w/2+PAD)}return{nodes:m,edges:n.map(l=>O(F({},l),{a:x.get(l.from),b:x.get(l.to)})).filter(l=>l.a&&l.b),gaps:h,colOf:p,y0:$,width:(g[a.length-1]||PAD)+d+PAD,height:b-$}}const KIND={table:{text:"\u5B9F\u30C6\u30FC\u30D6\u30EB",fill:"#FFFFFF",stroke:"#8C6D3F",bar:"#C9A227"},cte:{text:"CTE",fill:"#FFFFFF",stroke:"#3F6D8C",bar:"#4E8FBF"},output:{text:"\u6700\u7D42 SELECT",fill:"#FFFFFF",stroke:"#6D3F8C",bar:"#8250DF"},subquery:{text:"\u30B5\u30D6\u30AF\u30A8\u30EA",fill:"#FFFFFF",stroke:"#6E7781",bar:"#9AA4AE"}},kindOf=t=>KIND[t]||KIND.subquery;function shortName(t){const e=String(t).replace(/`/g,""),n=e.lastIndexOf(".");return n>=0?e.slice(n+1):e}function boxWidth(t){let e=BOX_W_MIN;for(const n of t)e=Math.max(e,shortName(n.label).length*NAME_CHAR_W+24);return Math.min(e,BOX_W_MAX)}function fit(t,e){const n=String(t),s=Math.floor((e-24)/NAME_CHAR_W);return n.length>s?n.slice(0,s-1)+"\u2026":n}function edgeLines(t){const e=[];t.joinType&&e.push(t.joinType==="INNER"?"JOIN":t.joinType+" JOIN");for(const n of t.keys||[])e.push(n);return!e.length&&t.nested&&e.push("\u30B5\u30D6\u30AF\u30A8\u30EA"),e}function linesWidth(t){let e=0;for(const n of t)e=Math.max(e,n.length*CHAR_W);return e}function edgeLabel(t){const e=edgeLines(t);return e.length?e.length>1?e[0]+" / "+e.slice(1).join(", "):e[0]:""}function toSvg(t){const e=[];e.push(`<svg viewBox="0 ${(t.y0||0).toFixed(1)} ${t.width} ${t.height}" width="${t.width}" height="${t.height}" role="img" aria-label="\u53C2\u7167\u95A2\u4FC2\u56F3" xmlns="http://www.w3.org/2000/svg">`),e.push('<defs><marker id="vgarrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="7" markerHeight="7" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#8C96A0"/></marker></defs>');const n=[];for(const s of t.edges){const o=s.a.x+s.a.w,i=s.a.y+s.a.h/2,r=s.b.x,c=s.b.y+s.b.h/2,a=t.gaps&&t.gaps[(t.colOf.get(s.to)||0)-1]||GAP_MIN,u=r>o?r-a/2:o+a/2,d=`M${o},${i} H${u} V${c} H${r}`,p=edgeLines(s);e.push(`<path d="${d}" fill="none" stroke="#8C96A0" stroke-width="1.2" ${s.nested?'stroke-dasharray="4 3" ':""}marker-end="url(#vgarrow)">`+(p.length?`<title>${esc(edgeLabel(s))}</title>`:"")+"</path>"),p.length&&n.push({x:u,y:(i+c)/2,lines:p})}for(const s of n){const o=linesWidth(s.lines)+12,i=s.lines.length*LINE_H,r=s.y-i/2;e.push(`<rect x="${(s.x-o/2).toFixed(1)}" y="${r.toFixed(1)}" width="${o.toFixed(1)}" height="${i}" rx="2" fill="#FFFFFF" opacity="0.92"/>`);const c=s.lines.map((a,u)=>`<tspan x="${s.x}" y="${(r+LINE_H*u+11).toFixed(1)}">${esc(a)}</tspan>`).join("");e.push(`<text text-anchor="middle" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" fill="#57606A">${c}</text>`)}for(const s of t.nodes){const o=kindOf(s.kind),i=s.label+(s.params.length?`
`+s.params.map(r=>r.name+": "+Object.keys(r.values).map(c=>c+" = "+r.values[c]).join(" / ")).join(`
`):"");e.push(`<g><title>${esc(i)}</title>`),e.push(`<rect x="${s.x}" y="${s.y}" width="${s.w}" height="${s.h}" rx="5" fill="${o.fill}" stroke="${o.stroke}" stroke-width="1"/>`),e.push(`<rect x="${s.x}" y="${s.y}" width="4" height="${s.h}" rx="2" fill="${o.bar}"/>`),e.push(`<text x="${s.x+11}" y="${s.y+17}" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" font-weight="600" fill="#24292F">${esc(fit(shortName(s.label),s.w))}</text>`),e.push(`<text x="${s.x+11}" y="${s.y+31}" font-family="Roboto,system-ui,sans-serif" font-size="9" fill="#8C96A0">${esc(o.text)}${s.params.length?" \u30FB\u30D1\u30E9\u30E1\u30FC\u30BF":""}</text>`),e.push("</g>")}return e.push("</svg>"),e.join("")}function groupSvg(t){return toSvg(layout(buildGraph(t.sql,t.params)))}function erdStack(t){return t.map(e=>`<div class="vg-erdblock"><div class="vg-erdhead"><span class="vg-erdname">${esc(label(e))}</span><span class="vg-tabn">${e.members.length}</span></div><div class="vg-erdbox">${groupSvg(e)}</div></div>`).join("")}function erdLegend(){const t=(e,n)=>`<span class="vg-lg"><span class="vg-lgm ${e}"></span>${esc(n)}</span>`;return'<div class="vg-legend">'+t("vg-lgt","\u5B9F\u30C6\u30FC\u30D6\u30EB")+t("vg-lgc","CTE")+t("vg-lgo","\u6700\u7D42 SELECT")+'<span class="vg-lg"><span class="vg-lgd"></span>\u30B5\u30D6\u30AF\u30A8\u30EA\u7D4C\u7531\u306E\u53C2\u7167</span></div>'}function renderErd(t){return t.groups.length?notice("FROM / JOIN \u304B\u3089\u8D77\u3053\u3057\u305F\u53C2\u7167\u95A2\u4FC2\u3067\u3059\u3002\u77E2\u5370\u306F\u300C\u8AAD\u3093\u3067\u4F5C\u308B\u300D\u5411\u304D\u3001\u6CE8\u8A18\u306F JOIN \u306E\u7A2E\u5225\u3068\u7D50\u5408\u30AD\u30FC\u3002\u30AB\u30FC\u30C7\u30A3\u30CA\u30EA\u30C6\u30A3\u3068\u4E3B\u30AD\u30FC\u306F SQL \u304B\u3089\u306F\u5206\u304B\u3089\u306A\u3044\u306E\u3067\u63CF\u3044\u3066\u3044\u307E\u305B\u3093\u3002\u30D1\u30E9\u30E1\u30FC\u30BF\u5316\u3057\u305F\u540D\u524D\u306F\u3001\u305D\u306E\u30B0\u30EB\u30FC\u30D7\u306E\u5148\u982D\u306E View \u306E\u5024\u3067\u8868\u793A\u3057\u3066\u3044\u307E\u3059\u3002")+erdLegend()+erdStack(t.groups):notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002")}function renderErdBase(t){return'<div class="vg-root">'+header(t.base,t.viewCount,t.groups.length,t.unmatched)+renderErd(t)+"</div>"}const WARN="\u26A0";function breakType(t){return String(t).replace(/(&lt;|,)/g,"$1<wbr>")}const DESC_JA_KEYS=["ja","jp","ja_name","name_ja","japanese","logical_name_ja","logicalNameJa","ja_logical_name","\u548C\u540D","\u65E5\u672C\u8A9E","\u65E5\u672C\u8A9E\u8AD6\u7406\u540D","\u8AD6\u7406\u540D"],DESC_EN_KEYS=["en","en_name","name_en","english","logical_name_en","logicalNameEn","en_logical_name","\u82F1\u8A9E","\u82F1\u8A9E\u8AD6\u7406\u540D"];function pickKey(t,e){for(let n=0;n<e.length;n++){const s=e[n];if(!Object.prototype.hasOwnProperty.call(t,s))continue;const o=t[s];if(!(o==null||String(o)===""))return{key:s,value:String(o)}}return null}function parseDesc(t){const e=String(t==null?"":t).trim();if(!e)return null;if(e.charAt(0)!=="{")return{raw:e};let n=null;try{n=JSON.parse(e)}catch(a){n=null}if(!n||typeof n!="object"||Array.isArray(n))return{raw:e};const s=pickKey(n,DESC_JA_KEYS),o=pickKey(n,DESC_EN_KEYS),i={};s&&(i[s.key]=!0),o&&(i[o.key]=!0);const r=[],c=Object.keys(n);for(let a=0;a<c.length;a++){const u=c[a];if(i[u])continue;const d=n[u];d==null||d===""||r.push({key:u,value:typeof d=="object"?JSON.stringify(d):String(d)})}return{ja:s?s.value:"",en:o?o.value:"",rest:r}}function descHtml(t){const e=parseDesc(t);if(!e)return"";if(e.raw)return`<div class="vg-cdesc">${esc(e.raw)}</div>`;let n="";e.ja&&(n+=`<div class="vg-cdesc">${esc(e.ja)}</div>`),e.en&&(n+=`<div class="vg-cdesc vg-cdescsub">${esc(e.en)}</div>`);for(let s=0;s<e.rest.length;s++)n+=`<div class="vg-cdesc vg-cdescsub">${esc(e.rest[s].key)}: ${esc(e.rest[s].value)}</div>`;return n}function structFields(t){const e=String(t==null?"":t),n=e.indexOf("STRUCT<");if(n<0)return[];const s=[];let o="",i=0;for(let c=n+7;c<e.length;c++){const a=e.charAt(c);if(a===">"&&i===0){s.push(o);break}if(a==="<")i++;else if(a===">")i--;else if(a===","&&i===0){s.push(o),o="";continue}o+=a}const r=[];for(let c=0;c<s.length;c++){const a=s[c].trim().split(/[\s<]/)[0];a&&r.push(a)}return r}function assignOrder(t){const e=n=>("000"+n).slice(-4);for(const n of t.values()){const s=n.name.split("."),o=t.get(s[0]),i=o&&o.vals[0]&&o.vals[0].ord!=null?o.vals[0].ord:9999;let r=e(i),c=s[0];for(let a=1;a<s.length;a++){const u=t.get(c),p=(u&&u.vals[0]?structFields(u.vals[0].type):[]).indexOf(s[a]);r+="."+e(p<0?9999:p),c+="."+s[a]}n.sortKey=r}}function typeShape(t){const e=String(t==null?"":t).trim(),n=e.slice(0,6).toUpperCase()==="ARRAY<";let s=e;n&&(s=e.slice(e.indexOf("<")+1,e.lastIndexOf(">")).trim());const o=s.slice(0,7).toUpperCase()==="STRUCT<";return{repeated:n,record:o,name:o?"RECORD":s}}const isNested=t=>String(t).indexOf(".")>=0;function descCell(t){return!t||!t.descs.length?"":t.descs.length===1?descHtml(t.descs[0].text):t.descs.map(e=>`<div class="vg-cdescwho">${esc(e.suffixes.join(", "))}</div>`+descHtml(e.text)).join("")}function groupColumns(t,e){const n=new Map,s=t.members||[];for(let o=0;o<s.length;o++){const i=s[o],r=t.suffixes&&t.suffixes[o]||i.viewName,c=e[i.viewName]||[];for(let a=0;a<c.length;a++){const u=c[a];let d=n.get(u.n);if(d||(d={name:u.n,descs:[],order:a,vals:[]},n.set(u.n,d)),u.d){const p=String(u.d);let h=null;for(let g=0;g<d.descs.length;g++)d.descs[g].text===p&&(h=d.descs[g]);h?h.suffixes.push(r):d.descs.push({text:p,suffixes:[r]})}d.vals.push({suffix:r,type:u.t,ord:u.o==null?a+1:u.o,nullable:u.u==null||u.u===""?null:String(u.u).toUpperCase()!=="NO"})}}return assignOrder(n),n}function columnOrder(t){const e=new Set,n=[];for(const s of t){const o=[...s.values()].sort((i,r)=>i.sortKey<r.sortKey?-1:i.sortKey>r.sortKey?1:0);for(const i of o)e.has(i.name)||(e.add(i.name),n.push(i.name))}return n}function majority(t){const e=new Map;for(const o of t)e.set(o,(e.get(o)||0)+1);let n=null,s=-1;for(const o of t){const i=e.get(o);i>s&&(n=o,s=i)}return n}const nullText=t=>t===null?"NULL \u4E0D\u660E":t?"NULL \u53EF":"NOT NULL";function uniq(t){const e=[];for(const n of t)e.indexOf(n)<0&&e.push(n);return e}function cellInfo(t,e){if(!t)return{text:null,meta:"",sig:null,mixed:!1};const n=uniq(t.vals.map(c=>c.type)),s=uniq(t.vals.map(c=>c.ord)),o=uniq(t.vals.map(c=>nullText(c.nullable))),i=uniq(t.vals.map(c=>`${c.ord}|${c.type}|${c.nullable}`)).length>1||t.vals.length!==e,r=t.vals.map(c=>typeShape(c.type));return{text:n.join(" / "),shape:uniq(r.map(c=>c.name)).join(" / "),repeated:r.length>0&&r.every(c=>c.repeated),record:r.some(c=>c.record),ords:s.join(" / "),nulls:o.join(" / "),meta:`#${s.join(" / ")} \xB7 ${o.join(" / ")}`,sig:n.join(" / ")+"|"+o.join(" / "),mixed:i}}function mixedTip(t,e){const n=isNested(t.name),s=t.vals.map(i=>n?`${i.suffix} = ${i.type}`:`${i.suffix} = #${i.ord} ${i.type} ${nullText(i.nullable)}`),o=t.vals.map(i=>i.suffix);for(let i=0;i<(e.members||[]).length;i++){const r=e.suffixes&&e.suffixes[i]||e.members[i].viewName;o.indexOf(r)<0&&s.push(`${r} = (\u3053\u306E\u5217\u3092\u6301\u305F\u306A\u3044)`)}return s.join(`
`)}function renderColumns(t,e){const n=t.groups||[];if(!n.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const s=n.map(f=>groupColumns(f,e||{})),o=columnOrder(s);if(!o.length)return notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.COLUMNS \u304B\u3089\u5217\u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const i=new Set;for(const f of o){const m=f.lastIndexOf(".");m>0&&i.add(f.slice(0,m))}let r=0,c=0;const a=o.map(f=>{const m=f.split("."),x=m.length-1,S=x>0,$=n.map((v,w)=>cellInfo(s[w].get(f),v.members.length)),b=majority($.map(v=>v.sig));$.some(v=>v.sig!==b)&&r++;const E=$.map((v,w)=>{const A=n[w],T=s[w].get(f),y=["vg-ccell"];v.text?v.sig!==b&&y.push("vg-cdiff"):y.push("vg-cnone"),v.mixed&&(y.push("vg-cmix"),c++);const L=!v.record||i.has(f)?v.shape:v.text,I=S?v.repeated?"REPEATED":"":`#${v.ords} \xB7 ${v.repeated?"REPEATED":v.nulls}`,_=v.text?breakType(esc(L))+(v.mixed?`<span class="vg-cwarn" data-tip="${esc(mixedTip(T,A))}">${WARN}</span>`:"")+(I?`<div class="vg-cmeta">${esc(I)}</div>`:"")+descCell(T):"\u2014";return`<td class="${y.join(" ")}">${_}</td>`}).join(""),k="vg-cname"+(x?` vg-cd${Math.min(x,3)}`:""),N=x?`<span class="vg-cnestmark">\u2514</span>${esc(m[m.length-1])}`:esc(f);return`<tr><th class="${k}">${N}</th>${E}</tr>`}).join(""),u=n.map(f=>`<th class="vg-chead">${esc(label(f))}<span class="vg-tabn">${f.members.length}</span></th>`).join(""),d=[];c&&d.push(notice(`\u540C\u3058\u30B0\u30EB\u30FC\u30D7\u306E\u4E2D\u3067\u578B\u30FBNULL \u5236\u7D04\u30FB\u4E26\u3073\u9806\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u7B87\u6240\u304C ${c} \u4EF6\u3042\u308A\u307E\u3059\uFF08${WARN} \u306E\u5370\uFF09\u3002SQL \u304C\u540C\u4E00\u3067\u3082\u53C2\u7167\u5148\u30C6\u30FC\u30D6\u30EB\u306E\u578B\u304C\u9055\u3048\u3070\u3053\u3046\u306A\u308B\u306E\u3067\u3001\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206\u306B\u306F\u51FA\u3066\u304D\u307E\u305B\u3093\u3002`)),r&&d.push(notice(`\u30B0\u30EB\u30FC\u30D7\u9593\u3067\u578B\u307E\u305F\u306F NULL \u5236\u7D04\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u5217\u304C ${r} \u4EF6\u3042\u308A\u307E\u3059\uFF08\u8272\u4ED8\u304D\u306E\u30BB\u30EB\u3002\u305D\u306E\u5217\u3067\u3044\u3061\u3070\u3093\u591A\u3044\u5024\u3068\u9055\u3046\u3082\u306E\uFF09\u3002\u4E26\u3073\u9806\uFF08#\uFF09\u306E\u9055\u3044\u306B\u306F\u8272\u3092\u4ED8\u3051\u3066\u3044\u307E\u305B\u3093\uFF08\u5217\u3092 1 \u672C\u8DB3\u3059\u3068\u4EE5\u964D\u304C\u307E\u3068\u3081\u3066\u305A\u308C\u3001\u578B\u306E\u5DEE\u304C\u57CB\u3082\u308C\u308B\u305F\u3081\uFF09\u3002`)),d.length||d.push(notice("\u5168\u30B0\u30EB\u30FC\u30D7\u3067\u5217\u540D\u30FB\u578B\u30FBNULL \u5236\u7D04\u304C\u4E00\u81F4\u3057\u3066\u3044\u307E\u3059\u3002"));const p=18,h=((100-p)/n.length).toFixed(3),g=`<colgroup><col style="width:${p}%">`+n.map(()=>`<col style="width:${h}%">`).join("")+"</colgroup>";return d.join("")+`<div class="vg-ctablewrap"><table class="vg-ctable">${g}<thead><tr><th class="vg-chead vg-cnamehead">\u5217\u540D</th>${u}</tr></thead><tbody>${a}</tbody></table></div>`}function renderColumnsBase(t,e){return`<div class="vg-root">${renderColumns(t,e)}</div>`}function sqlViews(t){const e=[],n=t.groups||[];for(let s=0;s<n.length;s++){const o=n[s],i=o.members||[];for(let r=0;r<i.length;r++)e.push({suffix:o.suffixes&&o.suffixes[r]||i[r].viewName||"(suffix \u306A\u3057)",viewName:i[r].viewName,group:label(o),groupSize:i.length})}return e.sort((s,o)=>String(s.suffix).localeCompare(String(o.suffix))),e}function sqlBody(t){const e=String(t).replace(/\r\n?/g,`
`).split(`
`);for(;e.length>1&&e[e.length-1].trim()==="";)e.pop();const n=String(e.length).length;return{html:`<div class="vg-sqlbox"><pre class="vg-sqlpre">${e.map((o,i)=>{const r=String(i+1);return`<span class="vg-sqln">${new Array(n-r.length+1).join(" ")}${r}</span> `+esc(o)}).join(`
`)}</pre></div>`,lines:e.length}}function sqlPanel(t,e){if(e==null||String(e)==="")return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(t.viewName)}</span></div>`+notice("\u3053\u306E View \u306E SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002");const n=sqlBody(e);return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(t.viewName)}</span><span class="vg-sqlmeta">\u30B0\u30EB\u30FC\u30D7: ${esc(t.group)}</span><span class="vg-sqlmeta">${n.lines} \u884C</span></div>`+n.html}function renderSql(t,e){const n=sqlViews(t);if(!n.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const s=e||{};if(!n.some(d=>s[d.viewName]))return notice("SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.VIEWS \u304B\u3089 view_definition \u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const o=n.slice(0,24),i="vgs"+hashId(t.base+"|"+n.map(d=>d.viewName).join("|")),r=o.map((d,p)=>`<input class="vg-sr vg-sr${p+1}" type="radio" name="${i}" id="${i}-${p+1}"${p===0?" checked":""}>`).join(""),c=o.map((d,p)=>`<label class="vg-stab vg-st${p+1}" for="${i}-${p+1}">${esc(d.suffix)}</label>`).join(""),a=o.map((d,p)=>`<div class="vg-spanel vg-sp${p+1}">${sqlPanel(d,s[d.viewName])}</div>`).join("");return(n.length>o.length?notice(`View \u304C\u591A\u3044\u305F\u3081\u5148\u982D 24 \u4EF6\u306E\u307F\u30BF\u30D6\u8868\u793A\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${n.length} \u4EF6\uFF09\u3002`):"")+`<div class="vg-stabs">${r}<div class="vg-stablist"><span class="vg-slabel">View</span>${c}</div><div class="vg-spanels">${a}</div></div>`}function renderSqlBase(t,e){return`<div class="vg-root">${renderSql(t,e)}</div>`}function __opts(t){if(!t)return{};try{return JSON.parse(t)||{}}catch(e){return{}}}function __notice(t){return'<div class="vg-notice">'+String(t).replace(/[<>&]/g,"")+"</div>"}function __run(t,e,n,s,o){var i=__opts(o),r;try{r=JSON.parse(t)}catch($){r=null}if(!r)return String(e||__notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002"));var c={};try{for(var a=JSON.parse(n)||[],u=0;u<a.length;u++)c[a[u].v]=a[u].cols||[]}catch($){c={}}var d={};try{for(var p=JSON.parse(s)||[],h=0;h<p.length;h++)d[p[h].v]=p[h].s}catch($){d={}}for(var g="",f="",m="",x=r.bases||[],S=0;S<x.length;S++)g+=renderErdBase(x[S]),f+=renderColumnsBase(x[S],c),m+=renderSqlBase(x[S],d);return g||(g=__notice("\u56F3\u306B\u3067\u304D\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),f||(f=__notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),m||(m=__notice("SQL \u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),wrapPage(String(e||""),g,f,m,r.bases&&r.bases.length?r.bases[0]:null)}return __run(analysis_json,diff_html,columns_json,sql_json,options_json);
...
.vg-cdescwho{margin:4px 0 0;font:11px/1.5 'Roboto','Segoe UI',system-ui,sans-serif;font-weight:600;color:#8C959F}
