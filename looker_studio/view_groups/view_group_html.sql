...
const MAX_TABS=12,MAX_REF_TABS=12,MAX_SQL_TABS=24,REF_BUDGET=41943040,OUTER_TABS=["note","\u30AB\u30E9\u30E0\u5B9A\u7FA9","\u53C2\u7167\u95A2\u4FC2","\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206","SQL"],MAX_OUTER_TABS=8,NOTE_MARK="<!--VG_NOTE-->";function esc(e){return String(e==null?"":e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function hashId(e){let s=2166136261;for(let n=0;n<String(e).length;n++)s^=String(e).charCodeAt(n),s=Math.imul(s,16777619)>>>0;return s.toString(36)}const label=e=>e.suffixes.map((s,n)=>s||e.members[n]&&e.members[n].viewName||"(suffix \u306A\u3057)").join(", ");function badge(e,s,n){return`<span class="vg-badge" style="color:${s};background:${n}">${esc(e)}</span>`}function header(e,s,n,l){const t=n>1;return`<div class="vg-header"><span class="vg-title">${esc(e)}</span>`+badge(`${s} View`,"#57606A","#EAEEF2")+badge(`${n} \u30B0\u30EB\u30FC\u30D7`,t?"#9A6700":"#1A7F37",t?"#FFF8C5":"#DAFBE1")+(l?badge("suffix \u672A\u8A8D\u8B58","#9A6700","#FFF8C5"):"")+"</div>"}function notice(e){return`<div class="vg-notice">${esc(e)}</div>`}const KIND_TEXT={entity:"\u5B9F\u4F53\u540D",string:"\u5024\uFF08\u6587\u5B57\u5217\uFF09",number:"\u5024\uFF08\u6570\u5024\uFF09",ident:"\u540D\u524D",quoted:"\u540D\u524D",keyword:"\u4E88\u7D04\u8A9E",punct:"\u8A18\u53F7",comment:"\u30B3\u30E1\u30F3\u30C8"},kindText=e=>KIND_TEXT[e]||e;function wrapPage(e,s,n,l,t){const o=t||{},i="vgo"+hashId(o.base||""),a=[`<div class="vg-root">${NOTE_MARK}</div>`,n,s,e,l],c=OUTER_TABS.map((v,p)=>`<input class="vg-or vg-or${p+1}" type="radio" name="${i}" id="${i}-${p+1}"${p===0?" checked":""}>`).join(""),u=OUTER_TABS.map((v,p)=>`<label class="vg-otab vg-ot${p+1}" for="${i}-${p+1}">${esc(v)}</label>`).join(""),r=OUTER_TABS.map((v,p)=>`<div class="vg-opanel vg-op${p+1}">${a[p]||""}</div>`).join(""),f=o.base?header(o.base,o.viewCount,(o.groups||[]).length,o.unmatched):"";return`<div class="vg-outer">${c}<div class="vg-otablist">${f}${u}</div><div class="vg-opanels">${r}</div></div>`}const WARN="\u26A0";function breakType(e){return String(e).replace(/(&lt;|,)/g,"$1<wbr>")}const DESC_JA_KEYS=["ja","jp","ja_name","name_ja","japanese","logical_name_ja","logicalNameJa","ja_logical_name","\u548C\u540D","\u65E5\u672C\u8A9E","\u65E5\u672C\u8A9E\u8AD6\u7406\u540D","\u8AD6\u7406\u540D"],DESC_EN_KEYS=["en","en_name","name_en","english","logical_name_en","logicalNameEn","en_logical_name","\u82F1\u8A9E","\u82F1\u8A9E\u8AD6\u7406\u540D"];function pickKey(e,s){for(let n=0;n<s.length;n++){const l=s[n];if(!Object.prototype.hasOwnProperty.call(e,l))continue;const t=e[l];if(!(t==null||String(t)===""))return{key:l,value:String(t)}}return null}function parseDesc(e){const s=String(e==null?"":e).trim();if(!s)return null;if(s.charAt(0)!=="{")return{raw:s};let n=null;try{n=JSON.parse(s)}catch(u){n=null}if(!n||typeof n!="object"||Array.isArray(n))return{raw:s};const l=pickKey(n,DESC_JA_KEYS),t=pickKey(n,DESC_EN_KEYS),o={};l&&(o[l.key]=!0),t&&(o[t.key]=!0);const i=[],a=Object.keys(n);for(let u=0;u<a.length;u++){const r=a[u];if(o[r])continue;const f=n[r];f==null||f===""||i.push({key:r,value:typeof f=="object"?JSON.stringify(f):String(f)})}const c=[];l&&c.push(l),t&&c.push(t);for(let u=0;u<i.length;u++)c.push(i[u]);return{ja:l?l.value:"",en:t?t.value:"",rest:i,pairs:c}}function descHtml(e){const s=parseDesc(e);return s?s.raw?`<div class="vg-cdesc">${esc(s.raw)}</div>`:s.pairs.length?`<table class="vg-cdtable">${s.pairs.map(l=>`<tr><th class="vg-cdk">${esc(l.key)}</th><td class="vg-cdv">${esc(l.value)}</td></tr>`).join("")}</table>`:"":""}function structFields(e){const s=String(e==null?"":e),n=s.indexOf("STRUCT<");if(n<0)return[];const l=[];let t="",o=0;for(let a=n+7;a<s.length;a++){const c=s.charAt(a);if(c===">"&&o===0){l.push(t);break}if(c==="<")o++;else if(c===">")o--;else if(c===","&&o===0){l.push(t),t="";continue}t+=c}const i=[];for(let a=0;a<l.length;a++){const c=l[a].trim().split(/[\s<]/)[0];c&&i.push(c)}return i}function assignOrder(e){const s=n=>("000"+n).slice(-4);for(const n of e.values()){const l=n.name.split("."),t=e.get(l[0]),o=t&&t.vals[0]&&t.vals[0].ord!=null?t.vals[0].ord:9999;let i=s(o),a=l[0];for(let c=1;c<l.length;c++){const u=e.get(a),f=(u&&u.vals[0]?structFields(u.vals[0].type):[]).indexOf(l[c]);i+="."+s(f<0?9999:f),a+="."+l[c]}n.sortKey=i}}function typeShape(e){const s=String(e==null?"":e).trim(),n=s.slice(0,6).toUpperCase()==="ARRAY<";let l=s;n&&(l=s.slice(s.indexOf("<")+1,s.lastIndexOf(">")).trim());const t=l.slice(0,7).toUpperCase()==="STRUCT<";return{repeated:n,record:t,name:t?"RECORD":l}}const isNested=e=>String(e).indexOf(".")>=0;function descCell(e){return!e||!e.descs.length?"":e.descs.length===1?descHtml(e.descs[0].text):e.descs.map(s=>`<div class="vg-cdescwho">${esc(s.suffixes.join(", "))}</div>`+descHtml(s.text)).join("")}function groupColumns(e,s){const n=new Map,l=e.members||[];for(let t=0;t<l.length;t++){const o=l[t],i=e.suffixes&&e.suffixes[t]||o.viewName,a=s[o.viewName]||[];for(let c=0;c<a.length;c++){const u=a[c];let r=n.get(u.n);if(r||(r={name:u.n,descs:[],order:c,vals:[]},n.set(u.n,r)),u.d){const f=String(u.d);let v=null;for(let p=0;p<r.descs.length;p++)r.descs[p].text===f&&(v=r.descs[p]);v?v.suffixes.push(i):r.descs.push({text:f,suffixes:[i]})}r.vals.push({suffix:i,type:u.t,ord:u.o==null?c+1:u.o,nullable:u.u==null||u.u===""?null:String(u.u).toUpperCase()!=="NO"})}}return assignOrder(n),n}function columnOrder(e){const s=new Set,n=[];for(const l of e){const t=[...l.values()].sort((o,i)=>o.sortKey<i.sortKey?-1:o.sortKey>i.sortKey?1:0);for(const o of t)s.has(o.name)||(s.add(o.name),n.push(o.name))}return n}function majority(e){const s=new Map;for(const t of e)s.set(t,(s.get(t)||0)+1);let n=null,l=-1;for(const t of e){const o=s.get(t);o>l&&(n=t,l=o)}return n}const nullText=e=>e===null?"UNKNOWN":e?"NULLABLE":"REQUIRED";function uniq(e){const s=[];for(const n of e)s.indexOf(n)<0&&s.push(n);return s}function cellInfo(e,s){if(!e)return{text:null,meta:"",sig:null,mixed:!1};const n=uniq(e.vals.map(a=>a.type)),l=uniq(e.vals.map(a=>a.ord)),t=uniq(e.vals.map(a=>nullText(a.nullable))),o=uniq(e.vals.map(a=>`${a.ord}|${a.type}|${a.nullable}`)).length>1||e.vals.length!==s,i=e.vals.map(a=>typeShape(a.type));return{text:n.join(" / "),shape:uniq(i.map(a=>a.name)).join(" / "),repeated:i.length>0&&i.every(a=>a.repeated),record:i.some(a=>a.record),nulls:t.join(" / "),sig:n.join(" / ")+"|"+t.join(" / "),mixed:o}}function mixedTip(e,s){const n=isNested(e.name),l=e.vals.map(o=>n?`${o.suffix} = ${o.type}`:`${o.suffix} = #${o.ord} ${o.type} ${nullText(o.nullable)}`),t=e.vals.map(o=>o.suffix);for(let o=0;o<(s.members||[]).length;o++){const i=s.suffixes&&s.suffixes[o]||s.members[o].viewName;t.indexOf(i)<0&&l.push(`${i} = (\u3053\u306E\u5217\u3092\u6301\u305F\u306A\u3044)`)}return l.join(`
`)}function renderColumns(e,s){const n=e.groups||[];if(!n.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const l=n.map(d=>groupColumns(d,s||{})),t=columnOrder(l);if(!t.length)return notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.COLUMNS \u304B\u3089\u5217\u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const o=new Set;for(const d of t){const h=d.lastIndexOf(".");h>0&&o.add(d.slice(0,h))}let i=0,a=0;const c=t.map(d=>{const h=d.split("."),m=h.length-1,$=m>0,b=n.map((g,S)=>cellInfo(l[S].get(d),g.members.length)),x=majority(b.map(g=>g.sig));b.some(g=>g.sig!==x)&&i++;const N=b.map((g,S)=>{const E=n[S],_=l[S].get(d),w=["vg-ccell"];g.text?g.sig!==x&&w.push("vg-cdiff"):w.push("vg-cnone"),g.mixed&&(w.push("vg-cmix"),a++);const O=!g.record||o.has(d)?g.shape:g.text,j=$?g.repeated?"REPEATED":"":g.repeated?"REPEATED":g.nulls,T=g.text?breakType(esc(O))+(g.mixed?`<span class="vg-cwarn" data-tip="${esc(mixedTip(_,E))}">${WARN}</span>`:"")+(j?`<span class="vg-cmeta">${esc(j)}</span>`:"")+descCell(_):"\u2014";return`<td class="${w.join(" ")}">${T}</td>`}).join(""),y="vg-cname"+(m?` vg-cd${Math.min(m,3)}`:""),A=m?`<span class="vg-cnestmark">\u2514</span>${esc(h[h.length-1])}`:esc(d);return`<tr><th class="${y}">${A}</th>${N}</tr>`}).join(""),u=n.map(d=>`<th class="vg-chead">${esc(label(d))}<span class="vg-tabn">${d.members.length}</span></th>`).join(""),r=[];a&&r.push(notice(`\u540C\u3058\u30B0\u30EB\u30FC\u30D7\u306E\u4E2D\u3067\u578B\u30FBNULL \u5236\u7D04\u30FB\u4E26\u3073\u9806\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u7B87\u6240\u304C ${a} \u4EF6\u3042\u308A\u307E\u3059\uFF08${WARN} \u306E\u5370\u3002\u30DB\u30D0\u30FC\u3059\u308B\u3068 suffix \u3054\u3068\u306E\u5185\u8A33\u304C\u51FA\u307E\u3059\uFF09\u3002SQL \u304C\u540C\u4E00\u3067\u3082\u53C2\u7167\u5148\u30C6\u30FC\u30D6\u30EB\u306E\u578B\u304C\u9055\u3048\u3070\u3053\u3046\u306A\u308B\u306E\u3067\u3001\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206\u306B\u306F\u51FA\u3066\u304D\u307E\u305B\u3093\u3002`)),i&&r.push(notice(`\u30B0\u30EB\u30FC\u30D7\u9593\u3067\u578B\u307E\u305F\u306F NULL \u5236\u7D04\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u5217\u304C ${i} \u4EF6\u3042\u308A\u307E\u3059\uFF08\u8272\u4ED8\u304D\u306E\u30BB\u30EB\u3002\u305D\u306E\u5217\u3067\u3044\u3061\u3070\u3093\u591A\u3044\u5024\u3068\u9055\u3046\u3082\u306E\uFF09\u3002\u4E26\u3073\u9806\u306E\u9055\u3044\u306F\u5DEE\u3068\u3057\u3066\u6271\u3063\u3066\u3044\u307E\u305B\u3093\uFF08\u5217\u3092 1 \u672C\u8DB3\u3059\u3068\u4EE5\u964D\u304C\u307E\u3068\u3081\u3066\u305A\u308C\u3001\u578B\u306E\u5DEE\u304C\u57CB\u3082\u308C\u308B\u305F\u3081\uFF09\u3002`)),r.length||r.push(notice("\u5168\u30B0\u30EB\u30FC\u30D7\u3067\u5217\u540D\u30FB\u578B\u30FBNULL \u5236\u7D04\u304C\u4E00\u81F4\u3057\u3066\u3044\u307E\u3059\u3002"));const f=18,v=((100-f)/n.length).toFixed(3),p=`<colgroup><col style="width:${f}%">`+n.map(()=>`<col style="width:${v}%">`).join("")+"</colgroup>";return r.join("")+`<div class="vg-ctablewrap"><table class="vg-ctable">${p}<thead><tr><th class="vg-chead vg-cnamehead">\u5217\u540D</th>${u}</tr></thead><tbody>${c}</tbody></table></div>`}function renderColumnsBase(e,s){return`<div class="vg-root">${renderColumns(e,s)}</div>`}function sqlViews(e){const s=[],n=e.groups||[];for(let l=0;l<n.length;l++){const t=n[l],o=t.members||[];for(let i=0;i<o.length;i++)s.push({suffix:t.suffixes&&t.suffixes[i]||o[i].viewName||"(suffix \u306A\u3057)",viewName:o[i].viewName,group:label(t),groupSize:o.length})}return s.sort((l,t)=>String(l.suffix).localeCompare(String(t.suffix))),s}function sqlBody(e){const s=String(e).replace(/\r\n?/g,`
`).split(`
`);for(;s.length>1&&s[s.length-1].trim()==="";)s.pop();const n=String(s.length).length;return{html:`<div class="vg-sqlbox"><pre class="vg-sqlpre">${s.map((t,o)=>{const i=String(o+1);return`<span class="vg-sqln">${new Array(n-i.length+1).join(" ")}${i}</span> `+esc(t)}).join(`
`)}</pre></div>`,lines:s.length}}function sqlPanel(e,s){if(s==null||String(s)==="")return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(e.viewName)}</span></div>`+notice("\u3053\u306E View \u306E SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002");const n=sqlBody(s);return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(e.viewName)}</span><span class="vg-sqlmeta">\u30B0\u30EB\u30FC\u30D7: ${esc(e.group)}</span><span class="vg-sqlmeta">${n.lines} \u884C</span></div>`+n.html}function renderSql(e,s){const n=sqlViews(e);if(!n.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const l=s||{};if(!n.some(r=>l[r.viewName]))return notice("SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.VIEWS \u304B\u3089 view_definition \u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const t=n.slice(0,24),o="vgs"+hashId(e.base+"|"+n.map(r=>r.viewName).join("|")),i=t.map((r,f)=>`<input class="vg-sr vg-sr${f+1}" type="radio" name="${o}" id="${o}-${f+1}"${f===0?" checked":""}>`).join(""),a=t.map((r,f)=>`<label class="vg-stab vg-st${f+1}" for="${o}-${f+1}">${esc(r.suffix)}</label>`).join(""),c=t.map((r,f)=>`<div class="vg-spanel vg-sp${f+1}">${sqlPanel(r,l[r.viewName])}</div>`).join("");return(n.length>t.length?notice(`View \u304C\u591A\u3044\u305F\u3081\u5148\u982D 24 \u4EF6\u306E\u307F\u30BF\u30D6\u8868\u793A\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${n.length} \u4EF6\uFF09\u3002`):"")+`<div class="vg-stabs">${i}<div class="vg-stablist"><span class="vg-slabel">View</span>${a}</div><div class="vg-spanels">${c}</div></div>`}function renderSqlBase(e,s){return`<div class="vg-root">${renderSql(e,s)}</div>`}function __opts(e){if(!e)return{};try{return JSON.parse(e)||{}}catch(s){return{}}}function __notice(e){return'<div class="vg-notice">'+String(e).replace(/[<>&]/g,"")+"</div>"}function __run(e,s,n,l,t,o){var i=__opts(o),a;try{a=JSON.parse(e)}catch(b){a=null}if(!a)return String(s||__notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002"));var c={};try{for(var u=JSON.parse(l)||[],r=0;r<u.length;r++)c[u[r].v]=u[r].cols||[]}catch(b){c={}}var f={};try{for(var v=JSON.parse(t)||[],p=0;p<v.length;p++)f[v[p].v]=v[p].s}catch(b){f={}}for(var d="",h="",m=a.bases||[],$=0;$<m.length;$++)d+=renderColumnsBase(m[$],c),h+=renderSqlBase(m[$],f);return d||(d=__notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),h||(h=__notice("SQL \u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),wrapPage(String(s||""),String(n||__notice("\u53C2\u7167\u95A2\u4FC2\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002")),d,h,a.bases&&a.bases.length?a.bases[0]:null)}return __run(analysis_json,diff_html,erd_html,columns_json,sql_json,options_json);
