...
var L=Object.defineProperty,_=Object.defineProperties;var M=Object.getOwnPropertyDescriptors;var I=Object.getOwnPropertySymbols;var C=Object.prototype.hasOwnProperty,j=Object.prototype.propertyIsEnumerable;var R=(e,t,n)=>t in e?L(e,t,{enumerable:!0,configurable:!0,writable:!0,value:n}):e[t]=n,F=(e,t)=>{for(var n in t||(t={}))C.call(t,n)&&R(e,n,t[n]);if(I)for(var n of I(t))j.call(t,n)&&R(e,n,t[n]);return e},O=(e,t)=>_(e,M(t));const MAX_TABS=12,MAX_REF_TABS=12,MAX_SQL_TABS=24,REF_BUDGET=41943040,OUTER_TABS=["note","\u30AB\u30E9\u30E0\u5B9A\u7FA9","\u53C2\u7167\u95A2\u4FC2","\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206","SQL"],MAX_OUTER_TABS=8,NOTE_MARK="<!--VG_NOTE-->";function esc(e){return String(e==null?"":e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function hashId(e){let t=2166136261;for(let n=0;n<String(e).length;n++)t^=String(e).charCodeAt(n),t=Math.imul(t,16777619)>>>0;return t.toString(36)}const label=e=>e.suffixes.map((t,n)=>t||e.members[n]&&e.members[n].viewName||"(suffix \u306A\u3057)").join(", ");function badge(e,t,n){return`<span class="vg-badge" style="color:${t};background:${n}">${esc(e)}</span>`}function header(e,t,n,s){const o=n>1;return`<div class="vg-header"><span class="vg-title">${esc(e)}</span>`+badge(`${t} View`,"#57606A","#EAEEF2")+badge(`${n} \u30B0\u30EB\u30FC\u30D7`,o?"#9A6700":"#1A7F37",o?"#FFF8C5":"#DAFBE1")+(s?badge("suffix \u672A\u8A8D\u8B58","#9A6700","#FFF8C5"):"")+"</div>"}function notice(e){return`<div class="vg-notice">${esc(e)}</div>`}const KIND_TEXT={entity:"\u5B9F\u4F53\u540D",string:"\u5024\uFF08\u6587\u5B57\u5217\uFF09",number:"\u5024\uFF08\u6570\u5024\uFF09",ident:"\u540D\u524D",quoted:"\u540D\u524D",keyword:"\u4E88\u7D04\u8A9E",punct:"\u8A18\u53F7",comment:"\u30B3\u30E1\u30F3\u30C8"},kindText=e=>KIND_TEXT[e]||e;function wrapPage(e,t,n,s,o){const i=o||{},r="vgo"+hashId(i.base||""),l=[`<div class="vg-root">${NOTE_MARK}</div>`,n,t,e,s],c=OUTER_TABS.map((h,g)=>`<input class="vg-or vg-or${g+1}" type="radio" name="${r}" id="${r}-${g+1}"${g===0?" checked":""}>`).join(""),a=OUTER_TABS.map((h,g)=>`<label class="vg-otab vg-ot${g+1}" for="${r}-${g+1}">${esc(h)}</label>`).join(""),f=OUTER_TABS.map((h,g)=>`<div class="vg-opanel vg-op${g+1}">${l[g]||""}</div>`).join(""),d=i.base?header(i.base,i.viewCount,(i.groups||[]).length,i.unmatched):"";return`<div class="vg-outer">${c}<div class="vg-otablist">${d}${a}</div><div class="vg-opanels">${f}</div></div>`}const DQ="\\u0022",DQ3=DQ+DQ+DQ,TOKEN_RE=new RegExp(["(`[^`]*`)","([rbRB]{1,2}(?:'''[\\s\\S]*?'''|"+DQ3+"[\\s\\S]*?"+DQ3+"|'[^']*'|"+DQ+"[^"+DQ+"]*"+DQ+"))","('''[\\s\\S]*?''')","("+DQ3+"[\\s\\S]*?"+DQ3+")","('(?:\\\\.|''|[^'\\\\])*')","("+DQ+"(?:\\\\.|"+DQ+DQ+"|[^"+DQ+"\\\\])*"+DQ+")","(--[^\\n]*|#[^\\n]*)","(/\\*[\\s\\S]*?\\*/)","(0[xX][0-9a-fA-F]+|(?:\\d+(?:\\.\\d+)?|\\.\\d+)(?:[eE][+-]?\\d+)?)","([A-Za-z_][A-Za-z0-9_]*)","(\\s+)","([^\\s])"].join("|"),"g"),KEYWORDS=new Set("SELECT FROM WHERE JOIN INNER LEFT RIGHT FULL OUTER CROSS ON USING AND OR NOT IN IS NULL LIKE BETWEEN EXISTS ANY ALL SOME GROUP BY HAVING ORDER ASC DESC LIMIT OFFSET QUALIFY WINDOW UNION INTERSECT EXCEPT DISTINCT AS CASE WHEN THEN ELSE END CREATE OR REPLACE VIEW TABLE FUNCTION IF EXISTS OPTIONS WITH RECURSIVE OVER PARTITION UNNEST STRUCT ARRAY CAST SAFE_CAST EXTRACT INTERVAL DATE DATETIME TIME TIMESTAMP INT64 FLOAT64 NUMERIC BIGNUMERIC STRING BYTES BOOL TRUE FALSE COUNT SUM AVG MIN MAX COALESCE IFNULL NULLIF ROWS RANGE PRECEDING FOLLOWING CURRENT ROW".split(/\s+/));function tokenizeSql(e){const t=[],n=String(e==null?"":e);let s;for(TOKEN_RE.lastIndex=0;(s=TOKEN_RE.exec(n))!==null;){let o;s[1]?o="quoted":s[2]||s[3]||s[4]||s[5]||s[6]?o="string":s[7]||s[8]?o="comment":s[9]?o="number":s[10]?o=KEYWORDS.has(s[10].toUpperCase())?"keyword":"ident":s[11]?o="space":o="punct",t.push({kind:o,text:s[0]})}return t}function markEntities(e){const t=e.slice(),n=i=>{let r=i+1;for(;r<t.length&&t[r].kind==="space";)r++;return r},s=[];let o=null;for(let i=0;i<t.length;i++){const r=t[i];if(r.kind==="space"||r.kind==="comment")continue;if(r.text==="("){s.push(o),o=null;continue}if(r.text===")"){s.pop(),o=null;continue}const l=r.text.toUpperCase();if(o=r.kind==="keyword"||r.kind==="ident"?l:null,r.kind!=="keyword"||l!=="FROM"&&l!=="JOIN"||s.length>0&&s[s.length-1]==="EXTRACT")continue;let c=n(i);for(;!(c>=t.length);){const a=t[c];if(a.kind==="quoted")t[c]={kind:"entity",text:a.text},c=n(c);else if(a.kind==="ident"){const f=[];let d=c;for(;;){f.push(d);const g=n(d);if(g<t.length&&t[g].text==="."){const p=n(g);if(p<t.length&&t[p].kind==="ident"){d=p;continue}}break}const h=n(d);if(h<t.length&&t[h].text==="(")break;for(const g of f)t[g]={kind:"entity",text:t[g].text};c=h}else break;if(c<t.length&&t[c].kind==="keyword"&&t[c].text.toUpperCase()==="AS"){const f=n(c);f<t.length&&(t[f].kind==="ident"||t[f].kind==="quoted")&&(c=n(f))}else c<t.length&&t[c].kind==="ident"&&(c=n(c));if(c<t.length&&t[c].text===","){c=n(c);continue}break}}return t}const DEFAULT_SUFFIX_RE=/^(.*?)_([A-Za-z0-9]{1,6})$/,DEFAULT_SUBSTITUTABLE=["entity","number","string"],SUFFIX_MARK="\0",LITERAL_MARK="\u0001",BOX_W_MIN=130,BOX_W_MAX=380,NAME_CHAR_W=6.65,BOX_H=40,GAP_MIN=78,CHAR_W=6.5,LINE_H=14,GAP_Y=14,PAD=10,kw=e=>e&&e.kind==="keyword"?e.text.toUpperCase():null;function prepare(e){const t=[];let n=0;for(const s of markEntities(tokenizeSql(e)))s.kind==="space"||s.kind==="comment"||(s.text===")"&&n--,t.push({kind:s.kind,text:s.text,depth:n}),s.text==="("&&n++);return t}function cteRanges(e){const t=[];if(!(e[0]&&kw(e[0])==="WITH"))return{ctes:t,mainFrom:0};let n=1;for(;;){const s=e[n],o=e[n+1],i=e[n+2];if(!i||s.kind!=="ident"||kw(o)!=="AS"||i.text!=="(")break;const r=i.depth;let l=n+3;for(;l<e.length&&!(e[l].text===")"&&e[l].depth===r);)l++;if(t.push({name:s.text,from:n+3,to:l,depth:r+1}),n=l+1,e[n]&&e[n].text===","){n++;continue}break}return{ctes:t,mainFrom:n}}const STOP=new Set(["WHERE","GROUP","ORDER","QUALIFY","WINDOW","HAVING","UNION","INTERSECT","EXCEPT","LIMIT","SELECT","JOIN","FROM"]);function skipParen(e,t,n){const s=e[t].depth;let o=t+1;for(;o<n&&!(e[o].text===")"&&e[o].depth===s);)o++;return o+1}function readSource(e,t,n){if(t>=n)return{node:null,next:t};const s=e[t];if(s.text==="(")return{node:{name:"(\u30B5\u30D6\u30AF\u30A8\u30EA)",kind:"subquery"},next:skipParen(e,t,n)};if(kw(s)==="UNNEST"){let o=t+1,i="";if(e[o]&&e[o].text==="("){const r=e[o].depth;let l=o+1;for(;l<n&&!(e[l].text===")"&&e[l].depth===r);)i+=e[l].text,l++;o=l+1}return{node:{name:"UNNEST("+i+")",kind:"unnest"},next:o}}if(s.kind==="entity"||s.kind==="quoted"||s.kind==="ident"){const o=[];let i=t;for(;i<n&&(e[i].kind==="entity"||e[i].kind==="quoted"||e[i].kind==="ident");){if(o.push(e[i].text),i++,i<n&&e[i].text==="."){i++;continue}break}return i<n&&e[i].text==="("?{node:null,next:skipParen(e,i,n)}:{node:{name:o.join("."),kind:"ref"},next:i}}return{node:null,next:t+1}}function skipAlias(e,t,n){if(t<n&&kw(e[t])==="AS"){const s=e[t+1];return s&&(s.kind==="ident"||s.kind==="quoted")?t+2:t+1}return t<n&&e[t].kind==="ident"&&!STOP.has(e[t].text.toUpperCase())?t+1:t}function readOnKeys(e,t,n){const s=[];let o=t;for(;o<n;){const i=kw(e[o]);if(i&&STOP.has(i)||i==="LEFT"||i==="RIGHT"||i==="FULL"||i==="INNER"||i==="CROSS")break;if(e[o].text==="="){const r=(a,f,d)=>a&&f&&d&&f.text==="."?d.text:null,l=r(e[o-3],e[o-2],e[o-1]),c=r(e[o+1],e[o+2],e[o+3]);l&&c&&s.push(l===c?l:l+" = "+c)}o++}return{keys:s,next:o}}function readUsingKeys(e,t,n){const s=[];let o=t;if(e[o]&&e[o].text==="("){const i=e[o].depth;for(o++;o<n&&!(e[o].text===")"&&e[o].depth===i);)e[o].kind==="ident"&&s.push(e[o].text),o++;o++}return{keys:s,next:o}}function joinKind(e,t,n){for(let s=t-1;s>=n&&t-s<=3;s--){const o=kw(e[s]);if(o==="LEFT"||o==="RIGHT"||o==="FULL"||o==="CROSS")return o;if(o==="INNER")return"INNER"}return"INNER"}function scanScope(e,t,n,s){const o=[];for(let i=t;i<n;i++){const r=kw(e[i]);if(r!=="FROM"&&r!=="JOIN")continue;const l=e[i].depth>s,c=r==="JOIN"?joinKind(e,i,t):null;let a=i+1;for(;;){const f=readSource(e,a,n);if(a=f.next,f.node){a=skipAlias(e,a,n);let d=[];if(!l&&a<n&&kw(e[a])==="ON"){const h=readOnKeys(e,a+1,n);d=h.keys,a=h.next}else if(!l&&a<n&&kw(e[a])==="USING"){const h=readUsingKeys(e,a+1,n);d=h.keys,a=h.next}f.node.kind!=="unnest"&&o.push({name:f.node.name,kind:f.node.kind,joinType:c,keys:d,nested:l})}if(a<n&&e[a].text===","&&e[a].depth===e[i].depth){a++;continue}break}i=Math.max(i,a-1)}return o}function buildGraph(e,t){const n=new Map((t||[]).map(p=>[p.name,p])),s=p=>{const m=Object.keys(p.values);return m.length?p.values[m[0]]:null},o=new Map,i=String(e).replace(/\{\{(P\d+)\}\}/g,(p,m)=>{const $=n.get(m);if(!$)return p;const x=s($);return x==null?p:(o.set(x,$),x)}),r=prepare(i),{ctes:l,mainFrom:c}=cteRanges(r),a=new Set(l.map(p=>p.name)),f=new Map,d=[],h=(p,m)=>{const $=m+":"+p;if(!f.has($)){const x=[],E=o.get(String(p));if(E)x.push(E);else for(const b of String(p).split(".")){const u=o.get(b);u&&x.indexOf(u)<0&&x.push(u)}f.set($,{id:$,name:p,label:p,kind:m,params:x})}return $},g=l.map(p=>({id:h(p.name,"cte"),from:p.from,to:p.to,depth:p.depth}));g.push({id:h("(\u6700\u7D42 SELECT)","output"),from:c,to:r.length,depth:0});for(const p of g)for(const m of scanScope(r,p.from,p.to,p.depth)){const $=m.kind==="ref"?a.has(m.name)?"cte":"table":m.kind,x=h(m.name,$);if(x===p.id)continue;const E=d.find(b=>b.from===x&&b.to===p.id);E?(E.keys.length||(E.keys=m.keys),E.joinType||(E.joinType=m.joinType),E.nested=E.nested&&m.nested):d.push({from:x,to:p.id,joinType:m.joinType,keys:m.keys,nested:m.nested})}return{nodes:[...f.values()],edges:d}}function layout(e){const{nodes:t,edges:n}=e,s=new Map(t.map(u=>[u.id,[]])),o=new Map(t.map(u=>[u.id,[]]));for(const u of n)s.has(u.to)&&s.get(u.to).push(u.from),o.has(u.from)&&o.get(u.from).push(u.to);const i=new Map(t.map(u=>[u.id,0]));for(let u=0;u<t.length+1;u++){let S=!1;for(const k of t){const v=s.get(k.id);if(!v.length)continue;const N=Math.max(...v.map(w=>i.get(w)||0))+1;N>i.get(k.id)&&(i.set(k.id,N),S=!0)}if(!S)break}const r=Math.max(0,...t.map(u=>i.get(u.id))),l=new Map(t.map(u=>[u.id,o.get(u.id).length?1/0:r]));for(let u=0;u<t.length+1;u++){let S=!1;for(const k of t){const v=o.get(k.id);if(!v.length)continue;const N=Math.min(...v.map(w=>l.get(w)));N-1<l.get(k.id)&&(l.set(k.id,N-1),S=!0)}if(!S)break}for(const u of t)Number.isFinite(l.get(u.id))||l.set(u.id,r);const c=[];for(const u of t){const S=Math.max(0,l.get(u.id));(c[S]||(c[S]=[])).push(u)}const a=new Map;c.forEach((u,S)=>{u&&(S>0&&u.sort((k,v)=>{const N=w=>{const y=s.get(w.id).filter(T=>a.has(T));return y.length?y.reduce((T,A)=>T+a.get(A),0)/y.length:Number.MAX_SAFE_INTEGER};return N(k)-N(v)}),u.forEach((k,v)=>a.set(k.id,v)))});const f=boxWidth(t),d=new Map;c.forEach((u,S)=>(u||[]).forEach(k=>d.set(k.id,S)));const h=new Array(Math.max(0,c.length-1)).fill(GAP_MIN);for(const u of n){const S=(d.get(u.to)||0)-1;S<0||S>=h.length||(h[S]=Math.max(h[S],linesWidth(edgeLines(u))+22))}const g=[];let p=PAD;for(let u=0;u<c.length;u++)g[u]=p,p+=f+(h[u]||0);const m=[];c.forEach((u,S)=>{(u||[]).forEach((k,v)=>{m.push(O(F({},k),{x:g[S],y:PAD+v*(BOX_H+GAP_Y),w:f,h:BOX_H}))})});const $=new Map(m.map(u=>[u.id,u])),x=Math.max(1,...c.map(u=>(u||[]).length));let E=0,b=PAD*2+x*BOX_H+Math.max(0,x-1)*GAP_Y+6;for(const u of n){const S=$.get(u.from),k=$.get(u.to);if(!S||!k)continue;const v=edgeLines(u);if(!v.length)continue;const N=(S.y+S.h/2+k.y+k.h/2)/2,w=v.length*LINE_H;E=Math.min(E,N-w/2-PAD),b=Math.max(b,N+w/2+PAD)}return{nodes:m,edges:n.map(u=>O(F({},u),{a:$.get(u.from),b:$.get(u.to)})).filter(u=>u.a&&u.b),gaps:h,colOf:d,y0:E,width:(g[c.length-1]||PAD)+f+PAD,height:b-E}}const KIND={table:{text:"\u5B9F\u30C6\u30FC\u30D6\u30EB",fill:"#FFFFFF",stroke:"#8C6D3F",bar:"#C9A227"},cte:{text:"CTE",fill:"#FFFFFF",stroke:"#3F6D8C",bar:"#4E8FBF"},output:{text:"\u6700\u7D42 SELECT",fill:"#FFFFFF",stroke:"#6D3F8C",bar:"#8250DF"},subquery:{text:"\u30B5\u30D6\u30AF\u30A8\u30EA",fill:"#FFFFFF",stroke:"#6E7781",bar:"#9AA4AE"}},kindOf=e=>KIND[e]||KIND.subquery;function shortName(e){const t=String(e).replace(/`/g,""),n=t.lastIndexOf(".");return n>=0?t.slice(n+1):t}function boxWidth(e){let t=BOX_W_MIN;for(const n of e)t=Math.max(t,shortName(n.label).length*NAME_CHAR_W+24);return Math.min(t,BOX_W_MAX)}function fit(e,t){const n=String(e),s=Math.floor((t-24)/NAME_CHAR_W);return n.length>s?n.slice(0,s-1)+"\u2026":n}function edgeLines(e){const t=[];e.joinType&&t.push(e.joinType==="INNER"?"JOIN":e.joinType+" JOIN");for(const n of e.keys||[])t.push(n);return!t.length&&e.nested&&t.push("\u30B5\u30D6\u30AF\u30A8\u30EA"),t}function linesWidth(e){let t=0;for(const n of e)t=Math.max(t,n.length*CHAR_W);return t}function edgeLabel(e){const t=edgeLines(e);return t.length?t.length>1?t[0]+" / "+t.slice(1).join(", "):t[0]:""}function toSvg(e){const t=[];t.push(`<svg viewBox="0 ${(e.y0||0).toFixed(1)} ${e.width} ${e.height}" width="${e.width}" height="${e.height}" role="img" aria-label="\u53C2\u7167\u95A2\u4FC2\u56F3" xmlns="http://www.w3.org/2000/svg">`),t.push('<defs><marker id="vgarrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="7" markerHeight="7" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#8C96A0"/></marker></defs>');const n=[];for(const s of e.edges){const o=s.a.x+s.a.w,i=s.a.y+s.a.h/2,r=s.b.x,l=s.b.y+s.b.h/2,c=e.gaps&&e.gaps[(e.colOf.get(s.to)||0)-1]||GAP_MIN,a=r>o?r-c/2:o+c/2,f=`M${o},${i} H${a} V${l} H${r}`,d=edgeLines(s);t.push(`<path d="${f}" fill="none" stroke="#8C96A0" stroke-width="1.2" ${s.nested?'stroke-dasharray="4 3" ':""}marker-end="url(#vgarrow)">`+(d.length?`<title>${esc(edgeLabel(s))}</title>`:"")+"</path>"),d.length&&n.push({x:a,y:(i+l)/2,lines:d})}for(const s of n){const o=linesWidth(s.lines)+12,i=s.lines.length*LINE_H,r=s.y-i/2;t.push(`<rect x="${(s.x-o/2).toFixed(1)}" y="${r.toFixed(1)}" width="${o.toFixed(1)}" height="${i}" rx="2" fill="#FFFFFF" opacity="0.92"/>`);const l=s.lines.map((c,a)=>`<tspan x="${s.x}" y="${(r+LINE_H*a+11).toFixed(1)}">${esc(c)}</tspan>`).join("");t.push(`<text text-anchor="middle" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" fill="#57606A">${l}</text>`)}for(const s of e.nodes){const o=kindOf(s.kind),i=s.label+(s.params.length?`
`+s.params.map(r=>r.name+": "+Object.keys(r.values).map(l=>l+" = "+r.values[l]).join(" / ")).join(`
`):"");t.push(`<g><title>${esc(i)}</title>`),t.push(`<rect x="${s.x}" y="${s.y}" width="${s.w}" height="${s.h}" rx="5" fill="${o.fill}" stroke="${o.stroke}" stroke-width="1"/>`),t.push(`<rect x="${s.x}" y="${s.y}" width="4" height="${s.h}" rx="2" fill="${o.bar}"/>`),t.push(`<text x="${s.x+11}" y="${s.y+17}" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" font-weight="600" fill="#24292F">${esc(fit(shortName(s.label),s.w))}</text>`),t.push(`<text x="${s.x+11}" y="${s.y+31}" font-family="Roboto,system-ui,sans-serif" font-size="9" fill="#8C96A0">${esc(o.text)}${s.params.length?" \u30FB\u30D1\u30E9\u30E1\u30FC\u30BF":""}</text>`),t.push("</g>")}return t.push("</svg>"),t.join("")}function groupSvg(e){return toSvg(layout(buildGraph(e.sql,e.params)))}function erdStack(e){return e.map(t=>`<div class="vg-erdblock"><div class="vg-erdhead"><span class="vg-erdname">${esc(label(t))}</span><span class="vg-tabn">${t.members.length}</span></div><div class="vg-erdbox">${groupSvg(t)}</div></div>`).join("")}function erdLegend(){const e=(t,n)=>`<span class="vg-lg"><span class="vg-lgm ${t}"></span>${esc(n)}</span>`;return'<div class="vg-legend">'+e("vg-lgt","\u5B9F\u30C6\u30FC\u30D6\u30EB")+e("vg-lgc","CTE")+e("vg-lgo","\u6700\u7D42 SELECT")+'<span class="vg-lg"><span class="vg-lgd"></span>\u30B5\u30D6\u30AF\u30A8\u30EA\u7D4C\u7531\u306E\u53C2\u7167</span></div>'}function renderErd(e){return e.groups.length?notice("FROM / JOIN \u304B\u3089\u8D77\u3053\u3057\u305F\u53C2\u7167\u95A2\u4FC2\u3067\u3059\u3002\u77E2\u5370\u306F\u300C\u8AAD\u3093\u3067\u4F5C\u308B\u300D\u5411\u304D\u3001\u6CE8\u8A18\u306F JOIN \u306E\u7A2E\u5225\u3068\u7D50\u5408\u30AD\u30FC\u3002\u30AB\u30FC\u30C7\u30A3\u30CA\u30EA\u30C6\u30A3\u3068\u4E3B\u30AD\u30FC\u306F SQL \u304B\u3089\u306F\u5206\u304B\u3089\u306A\u3044\u306E\u3067\u63CF\u3044\u3066\u3044\u307E\u305B\u3093\u3002\u30D1\u30E9\u30E1\u30FC\u30BF\u5316\u3057\u305F\u540D\u524D\u306F\u3001\u305D\u306E\u30B0\u30EB\u30FC\u30D7\u306E\u5148\u982D\u306E View \u306E\u5024\u3067\u8868\u793A\u3057\u3066\u3044\u307E\u3059\u3002")+erdLegend()+erdStack(e.groups):notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002")}function renderErdBase(e){return'<div class="vg-root">'+header(e.base,e.viewCount,e.groups.length,e.unmatched)+renderErd(e)+"</div>"}const WARN="\u26A0";function breakType(e){return String(e).replace(/(&lt;|,)/g,"$1<wbr>")}const DESC_JA_KEYS=["ja","jp","ja_name","name_ja","japanese","logical_name_ja","logicalNameJa","ja_logical_name","\u548C\u540D","\u65E5\u672C\u8A9E","\u65E5\u672C\u8A9E\u8AD6\u7406\u540D","\u8AD6\u7406\u540D"],DESC_EN_KEYS=["en","en_name","name_en","english","logical_name_en","logicalNameEn","en_logical_name","\u82F1\u8A9E","\u82F1\u8A9E\u8AD6\u7406\u540D"];function pickKey(e,t){for(let n=0;n<t.length;n++){const s=t[n];if(!Object.prototype.hasOwnProperty.call(e,s))continue;const o=e[s];if(!(o==null||String(o)===""))return{key:s,value:String(o)}}return null}function parseDesc(e){const t=String(e==null?"":e).trim();if(!t)return null;if(t.charAt(0)!=="{")return{raw:t};let n=null;try{n=JSON.parse(t)}catch(c){n=null}if(!n||typeof n!="object"||Array.isArray(n))return{raw:t};const s=pickKey(n,DESC_JA_KEYS),o=pickKey(n,DESC_EN_KEYS),i={};s&&(i[s.key]=!0),o&&(i[o.key]=!0);const r=[],l=Object.keys(n);for(let c=0;c<l.length;c++){const a=l[c];if(i[a])continue;const f=n[a];f==null||f===""||r.push({key:a,value:typeof f=="object"?JSON.stringify(f):String(f)})}return{ja:s?s.value:"",en:o?o.value:"",rest:r}}function descHtml(e){const t=parseDesc(e);if(!t)return"";if(t.raw)return`<div class="vg-cdesc">${esc(t.raw)}</div>`;let n="";t.ja&&(n+=`<div class="vg-cdesc">${esc(t.ja)}</div>`),t.en&&(n+=`<div class="vg-cdesc vg-cdescsub">${esc(t.en)}</div>`);for(let s=0;s<t.rest.length;s++)n+=`<div class="vg-cdesc vg-cdescsub">${esc(t.rest[s].key)}: ${esc(t.rest[s].value)}</div>`;return n}function structFields(e){const t=String(e==null?"":e),n=t.indexOf("STRUCT<");if(n<0)return[];const s=[];let o="",i=0;for(let l=n+7;l<t.length;l++){const c=t.charAt(l);if(c===">"&&i===0){s.push(o);break}if(c==="<")i++;else if(c===">")i--;else if(c===","&&i===0){s.push(o),o="";continue}o+=c}const r=[];for(let l=0;l<s.length;l++){const c=s[l].trim().split(/[\s<]/)[0];c&&r.push(c)}return r}function assignOrder(e){const t=n=>("000"+n).slice(-4);for(const n of e.values()){const s=n.name.split("."),o=e.get(s[0]),i=o&&o.vals[0]&&o.vals[0].ord!=null?o.vals[0].ord:9999;let r=t(i),l=s[0];for(let c=1;c<s.length;c++){const a=e.get(l),d=(a&&a.vals[0]?structFields(a.vals[0].type):[]).indexOf(s[c]);r+="."+t(d<0?9999:d),l+="."+s[c]}n.sortKey=r}}const isNested=e=>String(e).indexOf(".")>=0;function descCell(e){return!e||!e.descs.length?"":e.descs.length===1?descHtml(e.descs[0].text):e.descs.map(t=>`<div class="vg-cdescwho">${esc(t.suffixes.join(", "))}</div>`+descHtml(t.text)).join("")}function groupColumns(e,t){const n=new Map,s=e.members||[];for(let o=0;o<s.length;o++){const i=s[o],r=e.suffixes&&e.suffixes[o]||i.viewName,l=t[i.viewName]||[];for(let c=0;c<l.length;c++){const a=l[c];let f=n.get(a.n);if(f||(f={name:a.n,descs:[],order:c,vals:[]},n.set(a.n,f)),a.d){const d=String(a.d);let h=null;for(let g=0;g<f.descs.length;g++)f.descs[g].text===d&&(h=f.descs[g]);h?h.suffixes.push(r):f.descs.push({text:d,suffixes:[r]})}f.vals.push({suffix:r,type:a.t,ord:a.o==null?c+1:a.o,nullable:a.u==null||a.u===""?null:String(a.u).toUpperCase()!=="NO"})}}return assignOrder(n),n}function columnOrder(e){const t=new Set,n=[];for(const s of e){const o=[...s.values()].sort((i,r)=>i.sortKey<r.sortKey?-1:i.sortKey>r.sortKey?1:0);for(const i of o)t.has(i.name)||(t.add(i.name),n.push(i.name))}return n}function majority(e){const t=new Map;for(const o of e)t.set(o,(t.get(o)||0)+1);let n=null,s=-1;for(const o of e){const i=t.get(o);i>s&&(n=o,s=i)}return n}const nullText=e=>e===null?"NULL \u4E0D\u660E":e?"NULL \u53EF":"NOT NULL";function uniq(e){const t=[];for(const n of e)t.indexOf(n)<0&&t.push(n);return t}function cellInfo(e,t){if(!e)return{text:null,meta:"",sig:null,mixed:!1};const n=uniq(e.vals.map(r=>r.type)),s=uniq(e.vals.map(r=>r.ord)),o=uniq(e.vals.map(r=>nullText(r.nullable))),i=uniq(e.vals.map(r=>`${r.ord}|${r.type}|${r.nullable}`)).length>1||e.vals.length!==t;return{text:n.join(" / "),meta:`#${s.join(" / ")} \xB7 ${o.join(" / ")}`,sig:n.join(" / ")+"|"+o.join(" / "),mixed:i}}function mixedTip(e,t){const n=isNested(e.name),s=e.vals.map(i=>n?`${i.suffix} = ${i.type}`:`${i.suffix} = #${i.ord} ${i.type} ${nullText(i.nullable)}`),o=e.vals.map(i=>i.suffix);for(let i=0;i<(t.members||[]).length;i++){const r=t.suffixes&&t.suffixes[i]||t.members[i].viewName;o.indexOf(r)<0&&s.push(`${r} = (\u3053\u306E\u5217\u3092\u6301\u305F\u306A\u3044)`)}return s.join(`
`)}function renderColumns(e,t){const n=e.groups||[];if(!n.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const s=n.map(g=>groupColumns(g,t||{})),o=columnOrder(s);if(!o.length)return notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.COLUMNS \u304B\u3089\u5217\u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");let i=0,r=0;const l=o.map(g=>{const p=g.split("."),m=p.length-1,$=m>0,x=n.map((v,N)=>cellInfo(s[N].get(g),v.members.length)),E=majority(x.map(v=>v.sig));x.some(v=>v.sig!==E)&&i++;const u=x.map((v,N)=>{const w=n[N],y=s[N].get(g),T=["vg-ccell"];v.text?v.sig!==E&&T.push("vg-cdiff"):T.push("vg-cnone"),v.mixed&&(T.push("vg-cmix"),r++);const A=v.text?breakType(esc(v.text))+(v.mixed?`<span class="vg-cwarn" data-tip="${esc(mixedTip(y,w))}">${WARN}</span>`:"")+($?"":`<div class="vg-cmeta">${esc(v.meta)}</div>`)+descCell(y):"\u2014";return`<td class="${T.join(" ")}">${A}</td>`}).join(""),S="vg-cname"+(m?` vg-cd${Math.min(m,3)}`:""),k=m?`<span class="vg-cnestmark">\u2514</span>${esc(p[p.length-1])}`:esc(g);return`<tr><th class="${S}">${k}</th>${u}</tr>`}).join(""),c=n.map(g=>`<th class="vg-chead">${esc(label(g))}<span class="vg-tabn">${g.members.length}</span></th>`).join(""),a=[];r&&a.push(notice(`\u540C\u3058\u30B0\u30EB\u30FC\u30D7\u306E\u4E2D\u3067\u578B\u30FBNULL \u5236\u7D04\u30FB\u4E26\u3073\u9806\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u7B87\u6240\u304C ${r} \u4EF6\u3042\u308A\u307E\u3059\uFF08${WARN} \u306E\u5370\uFF09\u3002SQL \u304C\u540C\u4E00\u3067\u3082\u53C2\u7167\u5148\u30C6\u30FC\u30D6\u30EB\u306E\u578B\u304C\u9055\u3048\u3070\u3053\u3046\u306A\u308B\u306E\u3067\u3001\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206\u306B\u306F\u51FA\u3066\u304D\u307E\u305B\u3093\u3002`)),i&&a.push(notice(`\u30B0\u30EB\u30FC\u30D7\u9593\u3067\u578B\u307E\u305F\u306F NULL \u5236\u7D04\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u5217\u304C ${i} \u4EF6\u3042\u308A\u307E\u3059\uFF08\u8272\u4ED8\u304D\u306E\u30BB\u30EB\u3002\u305D\u306E\u5217\u3067\u3044\u3061\u3070\u3093\u591A\u3044\u5024\u3068\u9055\u3046\u3082\u306E\uFF09\u3002\u4E26\u3073\u9806\uFF08#\uFF09\u306E\u9055\u3044\u306B\u306F\u8272\u3092\u4ED8\u3051\u3066\u3044\u307E\u305B\u3093\uFF08\u5217\u3092 1 \u672C\u8DB3\u3059\u3068\u4EE5\u964D\u304C\u307E\u3068\u3081\u3066\u305A\u308C\u3001\u578B\u306E\u5DEE\u304C\u57CB\u3082\u308C\u308B\u305F\u3081\uFF09\u3002`)),a.length||a.push(notice("\u5168\u30B0\u30EB\u30FC\u30D7\u3067\u5217\u540D\u30FB\u578B\u30FBNULL \u5236\u7D04\u304C\u4E00\u81F4\u3057\u3066\u3044\u307E\u3059\u3002"));const f=18,d=((100-f)/n.length).toFixed(3),h=`<colgroup><col style="width:${f}%">`+n.map(()=>`<col style="width:${d}%">`).join("")+"</colgroup>";return a.join("")+`<div class="vg-ctablewrap"><table class="vg-ctable">${h}<thead><tr><th class="vg-chead vg-cnamehead">\u5217\u540D</th>${c}</tr></thead><tbody>${l}</tbody></table></div>`}function renderColumnsBase(e,t){return`<div class="vg-root">${renderColumns(e,t)}</div>`}function sqlViews(e){const t=[],n=e.groups||[];for(let s=0;s<n.length;s++){const o=n[s],i=o.members||[];for(let r=0;r<i.length;r++)t.push({suffix:o.suffixes&&o.suffixes[r]||i[r].viewName||"(suffix \u306A\u3057)",viewName:i[r].viewName,group:label(o),groupSize:i.length})}return t.sort((s,o)=>String(s.suffix).localeCompare(String(o.suffix))),t}function sqlBody(e){const t=String(e).replace(/\r\n?/g,`
`).split(`
...
`)}</pre></div>`,lines:t.length}}function sqlPanel(e,t){if(t==null||String(t)==="")return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(e.viewName)}</span></div>`+notice("\u3053\u306E View \u306E SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002");const n=sqlBody(t);return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(e.viewName)}</span><span class="vg-sqlmeta">\u30B0\u30EB\u30FC\u30D7: ${esc(e.group)}</span><span class="vg-sqlmeta">${n.lines} \u884C</span></div>`+n.html}function renderSql(e,t){const n=sqlViews(e);if(!n.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const s=t||{};if(!n.some(f=>s[f.viewName]))return notice("SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.VIEWS \u304B\u3089 view_definition \u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const o=n.slice(0,24),i="vgs"+hashId(e.base+"|"+n.map(f=>f.viewName).join("|")),r=o.map((f,d)=>`<input class="vg-sr vg-sr${d+1}" type="radio" name="${i}" id="${i}-${d+1}"${d===0?" checked":""}>`).join(""),l=o.map((f,d)=>`<label class="vg-stab vg-st${d+1}" for="${i}-${d+1}">${esc(f.suffix)}</label>`).join(""),c=o.map((f,d)=>`<div class="vg-spanel vg-sp${d+1}">${sqlPanel(f,s[f.viewName])}</div>`).join("");return(n.length>o.length?notice(`View \u304C\u591A\u3044\u305F\u3081\u5148\u982D 24 \u4EF6\u306E\u307F\u30BF\u30D6\u8868\u793A\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${n.length} \u4EF6\uFF09\u3002`):"")+`<div class="vg-stabs">${r}<div class="vg-stablist"><span class="vg-slabel">View</span>${l}</div><div class="vg-spanels">${c}</div></div>`}function renderSqlBase(e,t){return`<div class="vg-root">${renderSql(e,t)}</div>`}function __opts(e){if(!e)return{};try{return JSON.parse(e)||{}}catch(t){return{}}}function __notice(e){return'<div class="vg-notice">'+String(e).replace(/[<>&]/g,"")+"</div>"}function __run(e,t,n,s,o){var i=__opts(o),r;try{r=JSON.parse(e)}catch(E){r=null}if(!r)return String(t||__notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002"));var l={};try{for(var c=JSON.parse(n)||[],a=0;a<c.length;a++)l[c[a].v]=c[a].cols||[]}catch(E){l={}}var f={};try{for(var d=JSON.parse(s)||[],h=0;h<d.length;h++)f[d[h].v]=d[h].s}catch(E){f={}}for(var g="",p="",m="",$=r.bases||[],x=0;x<$.length;x++)g+=renderErdBase($[x]),p+=renderColumnsBase($[x],l),m+=renderSqlBase($[x],f);return g||(g=__notice("\u56F3\u306B\u3067\u304D\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),p||(p=__notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),m||(m=__notice("SQL \u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),wrapPage(String(t||""),g,p,m,r.bases&&r.bases.length?r.bases[0]:null)}return __run(analysis_json,diff_html,columns_json,sql_json,options_json);
