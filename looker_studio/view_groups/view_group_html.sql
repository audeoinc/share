...
var C=Object.defineProperty,L=Object.defineProperties;var $=Object.getOwnPropertyDescriptors;var v=Object.getOwnPropertySymbols;var _=Object.prototype.hasOwnProperty,U=Object.prototype.propertyIsEnumerable;var b=(e,t,n)=>t in e?C(e,t,{enumerable:!0,configurable:!0,writable:!0,value:n}):e[t]=n,O=(e,t)=>{for(var n in t||(t={}))_.call(t,n)&&b(e,n,t[n]);if(v)for(var n of v(t))U.call(t,n)&&b(e,n,t[n]);return e},w=(e,t)=>L(e,$(t));const MAX_TABS=12,MAX_SQL_TABS=128,MAX_DESC_TABS=16,MAX_LABEL_TABS=8,OUTER_TABS=["note","\u30AB\u30E9\u30E0\u5B9A\u7FA9","\u53C2\u7167\u95A2\u4FC2","\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206","SQL"],MAX_OUTER_TABS=8,NOTE_MARK="<!--VG_NOTE-->",CSS_GEN=8;function esc(e){return String(e==null?"":e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function hashId(e){let t=2166136261;for(let n=0;n<String(e).length;n++)t^=String(e).charCodeAt(n),t=Math.imul(t,16777619)>>>0;return t.toString(36)}const label=e=>e.suffixes.map((t,n)=>t||e.members[n]&&e.members[n].viewName||"(suffix \u306A\u3057)").join(", ");function badge(e,t,n){return`<span class="vg-badge" style="color:${t};background:${n}">${esc(e)}</span>`}function header(e,t,n,o,s){const i=n>1;return`<div class="vg-header"><span class="vg-title">${esc(e)}</span>`+badge(`${t} View`,"#57606A","#EAEEF2")+badge(`${n} \u30B0\u30EB\u30FC\u30D7`,i?"#9A6700":"#1A7F37",i?"#FFF8C5":"#DAFBE1")+(o?badge("suffix \u672A\u8A8D\u8B58","#9A6700","#FFF8C5"):"")+(s?badge("\u30E9\u30D9\u30EB\u4E0D\u4E00\u81F4","#9A6700","#FFF8C5"):"")+"</div>"}function notice(e){return`<div class="vg-notice">${esc(e)}</div>`}const KIND_TEXT={entity:"\u5B9F\u4F53\u540D",string:"\u5024\uFF08\u6587\u5B57\u5217\uFF09",number:"\u5024\uFF08\u6570\u5024\uFF09",ident:"\u540D\u524D",quoted:"\u540D\u524D",keyword:"\u4E88\u7D04\u8A9E",punct:"\u8A18\u53F7",comment:"\u30B3\u30E1\u30F3\u30C8"},kindText=e=>KIND_TEXT[e]||e,DQ="\\u0022",DQ3=DQ+DQ+DQ,TOKEN_RE=new RegExp(["(`[^`]*`)","([rbRB]{1,2}(?:'''[\\s\\S]*?'''|"+DQ3+"[\\s\\S]*?"+DQ3+"|'[^']*'|"+DQ+"[^"+DQ+"]*"+DQ+"))","('''[\\s\\S]*?''')","("+DQ3+"[\\s\\S]*?"+DQ3+")","('(?:\\\\.|''|[^'\\\\])*')","("+DQ+"(?:\\\\.|"+DQ+DQ+"|[^"+DQ+"\\\\])*"+DQ+")","(--[^\\n]*|#[^\\n]*)","(/\\*[\\s\\S]*?\\*/)","(0[xX][0-9a-fA-F]+|(?:\\d+(?:\\.\\d+)?|\\.\\d+)(?:[eE][+-]?\\d+)?)","([A-Za-z_][A-Za-z0-9_]*)","(\\s+)","([^\\s])"].join("|"),"g"),KEYWORDS=new Set("SELECT FROM WHERE JOIN INNER LEFT RIGHT FULL OUTER CROSS ON USING AND OR NOT IN IS NULL LIKE BETWEEN EXISTS ANY ALL SOME GROUP BY HAVING ORDER ASC DESC LIMIT OFFSET QUALIFY WINDOW UNION INTERSECT EXCEPT DISTINCT AS CASE WHEN THEN ELSE END CREATE OR REPLACE VIEW TABLE FUNCTION IF EXISTS OPTIONS WITH RECURSIVE OVER PARTITION UNNEST STRUCT ARRAY CAST SAFE_CAST EXTRACT INTERVAL DATE DATETIME TIME TIMESTAMP INT64 FLOAT64 NUMERIC BIGNUMERIC STRING BYTES BOOL TRUE FALSE COUNT SUM AVG MIN MAX COALESCE IFNULL NULLIF ROWS RANGE PRECEDING FOLLOWING CURRENT ROW".split(/\s+/));function tokenizeSql(e){const t=[],n=String(e==null?"":e);let o;for(TOKEN_RE.lastIndex=0;(o=TOKEN_RE.exec(n))!==null;){let s;o[1]?s="quoted":o[2]||o[3]||o[4]||o[5]||o[6]?s="string":o[7]||o[8]?s="comment":o[9]?s="number":o[10]?s=KEYWORDS.has(o[10].toUpperCase())?"keyword":"ident":o[11]?s="space":s="punct",t.push({kind:s,text:o[0]})}return t}function markEntities(e){const t=e.slice(),n=i=>{let c=i+1;for(;c<t.length&&t[c].kind==="space";)c++;return c},o=[];let s=null;for(let i=0;i<t.length;i++){const c=t[i];if(c.kind==="space"||c.kind==="comment")continue;if(c.text==="("){o.push(s),s=null;continue}if(c.text===")"){o.pop(),s=null;continue}const l=c.text.toUpperCase();if(s=c.kind==="keyword"||c.kind==="ident"?l:null,c.kind!=="keyword"||l!=="FROM"&&l!=="JOIN"||o.length>0&&o[o.length-1]==="EXTRACT")continue;let a=n(i);for(;!(a>=t.length);){const u=t[a];if(u.kind==="quoted")t[a]={kind:"entity",text:u.text},a=n(a);else if(u.kind==="ident"){const d=[];let p=a;for(;;){d.push(p);const F=n(p);if(F<t.length&&t[F].text==="."){const f=n(F);if(f<t.length&&t[f].kind==="ident"){p=f;continue}}break}const h=n(p);if(h<t.length&&t[h].text==="(")break;for(const F of d)t[F]={kind:"entity",text:t[F].text};a=h}else break;if(a<t.length&&t[a].kind==="keyword"&&t[a].text.toUpperCase()==="AS"){const d=n(a);d<t.length&&(t[d].kind==="ident"||t[d].kind==="quoted")&&(a=n(d))}else a<t.length&&t[a].kind==="ident"&&(a=n(a));if(a<t.length&&t[a].text===","){a=n(a);continue}break}}return t}const DEFAULT_SUFFIX_RE=/^(.*?)_([A-Za-z0-9]{1,6})$/,DEFAULT_SUBSTITUTABLE=["entity","number","string"],SUFFIX_MARK="\0",LITERAL_MARK="\u0001",BOX_W_MIN=130,BOX_W_MAX=380,NAME_CHAR_W=6.65,BOX_H=40,GAP_MIN=78,CHAR_W=6.5,LINE_H=14,GAP_Y=14,PAD=10,kw=e=>e&&e.kind==="keyword"?e.text.toUpperCase():null;function prepare(e){const t=[];let n=0;for(const o of markEntities(tokenizeSql(e)))o.kind==="space"||o.kind==="comment"||(o.text===")"&&n--,t.push({kind:o.kind,text:o.text,depth:n}),o.text==="("&&n++);return t}function cteRanges(e){const t=[];if(!(e[0]&&kw(e[0])==="WITH"))return{ctes:t,mainFrom:0};let n=1;for(;;){const o=e[n],s=e[n+1],i=e[n+2];if(!i||o.kind!=="ident"||kw(s)!=="AS"||i.text!=="(")break;const c=i.depth;let l=n+3;for(;l<e.length&&!(e[l].text===")"&&e[l].depth===c);)l++;if(t.push({name:o.text,from:n+3,to:l,depth:c+1}),n=l+1,e[n]&&e[n].text===","){n++;continue}break}return{ctes:t,mainFrom:n}}const STOP=new Set(["WHERE","GROUP","ORDER","QUALIFY","WINDOW","HAVING","UNION","INTERSECT","EXCEPT","LIMIT","SELECT","JOIN","FROM"]);function skipParen(e,t,n){const o=e[t].depth;let s=t+1;for(;s<n&&!(e[s].text===")"&&e[s].depth===o);)s++;return s+1}function readSource(e,t,n){if(t>=n)return{node:null,next:t};const o=e[t];if(o.text==="(")return{node:{name:"(\u30B5\u30D6\u30AF\u30A8\u30EA)",kind:"subquery"},next:skipParen(e,t,n)};if(kw(o)==="UNNEST"){let s=t+1,i="";if(e[s]&&e[s].text==="("){const c=e[s].depth;let l=s+1;for(;l<n&&!(e[l].text===")"&&e[l].depth===c);)i+=e[l].text,l++;s=l+1}return{node:{name:"UNNEST("+i+")",kind:"unnest"},next:s}}if(o.kind==="entity"||o.kind==="quoted"||o.kind==="ident"){const s=[];let i=t;for(;i<n&&(e[i].kind==="entity"||e[i].kind==="quoted"||e[i].kind==="ident");){if(s.push(e[i].text),i++,i<n&&e[i].text==="."){i++;continue}break}return i<n&&e[i].text==="("?{node:null,next:skipParen(e,i,n)}:{node:{name:s.join("."),kind:"ref"},next:i}}return{node:null,next:t+1}}function skipAlias(e,t,n){if(t<n&&kw(e[t])==="AS"){const o=e[t+1];return o&&(o.kind==="ident"||o.kind==="quoted")?t+2:t+1}return t<n&&e[t].kind==="ident"&&!STOP.has(e[t].text.toUpperCase())?t+1:t}function readOnKeys(e,t,n){const o=[];let s=t;for(;s<n;){const i=kw(e[s]);if(i&&STOP.has(i)||i==="LEFT"||i==="RIGHT"||i==="FULL"||i==="INNER"||i==="CROSS")break;if(e[s].text==="="){const c=(u,d,p)=>u&&d&&p&&d.text==="."?p.text:null,l=c(e[s-3],e[s-2],e[s-1]),a=c(e[s+1],e[s+2],e[s+3]);l&&a&&o.push(l===a?l:l+" = "+a)}s++}return{keys:o,next:s}}function readUsingKeys(e,t,n){const o=[];let s=t;if(e[s]&&e[s].text==="("){const i=e[s].depth;for(s++;s<n&&!(e[s].text===")"&&e[s].depth===i);)e[s].kind==="ident"&&o.push(e[s].text),s++;s++}return{keys:o,next:s}}function joinKind(e,t,n){for(let o=t-1;o>=n&&t-o<=3;o--){const s=kw(e[o]);if(s==="LEFT"||s==="RIGHT"||s==="FULL"||s==="CROSS")return s;if(s==="INNER")return"INNER"}return"INNER"}function scanScope(e,t,n,o){const s=[];for(let i=t;i<n;i++){const c=kw(e[i]);if(c!=="FROM"&&c!=="JOIN")continue;const l=e[i].depth>o,a=c==="JOIN"?joinKind(e,i,t):null;let u=i+1;for(;;){const d=readSource(e,u,n);if(u=d.next,d.node){u=skipAlias(e,u,n);let p=[];if(!l&&u<n&&kw(e[u])==="ON"){const h=readOnKeys(e,u+1,n);p=h.keys,u=h.next}else if(!l&&u<n&&kw(e[u])==="USING"){const h=readUsingKeys(e,u+1,n);p=h.keys,u=h.next}d.node.kind!=="unnest"&&s.push({name:d.node.name,kind:d.node.kind,joinType:a,keys:p,nested:l})}if(u<n&&e[u].text===","&&e[u].depth===e[i].depth){u++;continue}break}i=Math.max(i,u-1)}return s}function buildGraph(e,t){const n=new Map((t||[]).map(f=>[f.name,f])),o=f=>{const m=Object.keys(f.values);return m.length?f.values[m[0]]:null},s=new Map,i=String(e).replace(/\{\{(P\d+)\}\}/g,(f,m)=>{const E=n.get(m);if(!E)return f;const k=o(E);return k==null?f:(s.set(k,E),k)}),c=prepare(i),{ctes:l,mainFrom:a}=cteRanges(c),u=new Set(l.map(f=>f.name)),d=new Map,p=[],h=(f,m)=>{const E=m+":"+f;if(!d.has(E)){const k=[],S=s.get(String(f));if(S)k.push(S);else for(const A of String(f).split(".")){const r=s.get(A);r&&k.indexOf(r)<0&&k.push(r)}d.set(E,{id:E,name:f,label:f,kind:m,params:k})}return E},F=l.map(f=>({id:h(f.name,"cte"),from:f.from,to:f.to,depth:f.depth}));F.push({id:h("(\u6700\u7D42 SELECT)","output"),from:a,to:c.length,depth:0});for(const f of F)for(const m of scanScope(c,f.from,f.to,f.depth)){const E=m.kind==="ref"?u.has(m.name)?"cte":"table":m.kind,k=h(m.name,E);if(k===f.id)continue;const S=p.find(A=>A.from===k&&A.to===f.id);S?(S.keys.length||(S.keys=m.keys),S.joinType||(S.joinType=m.joinType),S.nested=S.nested&&m.nested):p.push({from:k,to:f.id,joinType:m.joinType,keys:m.keys,nested:m.nested})}return{nodes:[...d.values()],edges:p}}function layout(e){const{nodes:t,edges:n}=e,o=new Map(t.map(r=>[r.id,[]])),s=new Map(t.map(r=>[r.id,[]]));for(const r of n)o.has(r.to)&&o.get(r.to).push(r.from),s.has(r.from)&&s.get(r.from).push(r.to);const i=new Map(t.map(r=>[r.id,0]));for(let r=0;r<t.length+1;r++){let g=!1;for(const x of t){const T=o.get(x.id);if(!T.length)continue;const N=Math.max(...T.map(I=>i.get(I)||0))+1;N>i.get(x.id)&&(i.set(x.id,N),g=!0)}if(!g)break}const c=Math.max(0,...t.map(r=>i.get(r.id))),l=new Map(t.map(r=>[r.id,s.get(r.id).length?1/0:c]));for(let r=0;r<t.length+1;r++){let g=!1;for(const x of t){const T=s.get(x.id);if(!T.length)continue;const N=Math.min(...T.map(I=>l.get(I)));N-1<l.get(x.id)&&(l.set(x.id,N-1),g=!0)}if(!g)break}for(const r of t)Number.isFinite(l.get(r.id))||l.set(r.id,c);const a=[];for(const r of t){const g=Math.max(0,l.get(r.id));(a[g]||(a[g]=[])).push(r)}const u=new Map;a.forEach((r,g)=>{r&&(g>0&&r.sort((x,T)=>{const N=I=>{const y=o.get(I.id).filter(R=>u.has(R));return y.length?y.reduce((R,M)=>R+u.get(M),0)/y.length:Number.MAX_SAFE_INTEGER};return N(x)-N(T)}),r.forEach((x,T)=>u.set(x.id,T)))});const d=boxWidth(t),p=new Map;a.forEach((r,g)=>(r||[]).forEach(x=>p.set(x.id,g)));const h=new Array(Math.max(0,a.length-1)).fill(GAP_MIN);for(const r of n){const g=(p.get(r.to)||0)-1;g<0||g>=h.length||(h[g]=Math.max(h[g],linesWidth(edgeLines(r))+22))}const F=[];let f=PAD;for(let r=0;r<a.length;r++)F[r]=f,f+=d+(h[r]||0);const m=[];a.forEach((r,g)=>{(r||[]).forEach((x,T)=>{m.push(w(O({},x),{x:F[g],y:PAD+T*(BOX_H+GAP_Y),w:d,h:BOX_H}))})});const E=new Map(m.map(r=>[r.id,r])),k=Math.max(1,...a.map(r=>(r||[]).length));let S=0,A=PAD*2+k*BOX_H+Math.max(0,k-1)*GAP_Y+6;for(const r of n){const g=E.get(r.from),x=E.get(r.to);if(!g||!x)continue;const T=edgeLines(r);if(!T.length)continue;const N=(g.y+g.h/2+x.y+x.h/2)/2,I=T.length*LINE_H;S=Math.min(S,N-I/2-PAD),A=Math.max(A,N+I/2+PAD)}return{nodes:m,edges:n.map(r=>w(O({},r),{a:E.get(r.from),b:E.get(r.to)})).filter(r=>r.a&&r.b),gaps:h,colOf:p,y0:S,width:(F[a.length-1]||PAD)+d+PAD,height:A-S}}const KIND={table:{text:"\u5B9F\u30C6\u30FC\u30D6\u30EB",fill:"#FFFFFF",stroke:"#8C6D3F",bar:"#C9A227"},cte:{text:"CTE",fill:"#FFFFFF",stroke:"#3F6D8C",bar:"#4E8FBF"},output:{text:"\u6700\u7D42 SELECT",fill:"#FFFFFF",stroke:"#6D3F8C",bar:"#8250DF"},subquery:{text:"\u30B5\u30D6\u30AF\u30A8\u30EA",fill:"#FFFFFF",stroke:"#6E7781",bar:"#9AA4AE"}},kindOf=e=>KIND[e]||KIND.subquery;function shortName(e){const t=String(e).replace(/`/g,""),n=t.lastIndexOf(".");return n>=0?t.slice(n+1):t}function boxWidth(e){let t=BOX_W_MIN;for(const n of e)t=Math.max(t,shortName(n.label).length*NAME_CHAR_W+24);return Math.min(t,BOX_W_MAX)}function fit(e,t){const n=String(e),o=Math.floor((t-24)/NAME_CHAR_W);return n.length>o?n.slice(0,o-1)+"\u2026":n}function edgeLines(e){const t=[];e.joinType&&t.push(e.joinType==="INNER"?"JOIN":e.joinType+" JOIN");for(const n of e.keys||[])t.push(n);return!t.length&&e.nested&&t.push("\u30B5\u30D6\u30AF\u30A8\u30EA"),t}function linesWidth(e){let t=0;for(const n of e)t=Math.max(t,n.length*CHAR_W);return t}function edgeLabel(e){const t=edgeLines(e);return t.length?t.length>1?t[0]+" / "+t.slice(1).join(", "):t[0]:""}function toSvg(e){const t=[];t.push(`<svg viewBox="0 ${(e.y0||0).toFixed(1)} ${e.width} ${e.height}" width="${e.width}" height="${e.height}" role="img" aria-label="\u53C2\u7167\u95A2\u4FC2\u56F3" xmlns="http://www.w3.org/2000/svg">`),t.push('<defs><marker id="vgarrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="7" markerHeight="7" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#8C96A0"/></marker></defs>');const n=[];for(const o of e.edges){const s=o.a.x+o.a.w,i=o.a.y+o.a.h/2,c=o.b.x,l=o.b.y+o.b.h/2,a=e.gaps&&e.gaps[(e.colOf.get(o.to)||0)-1]||GAP_MIN,u=c>s?c-a/2:s+a/2,d=`M${s},${i} H${u} V${l} H${c}`,p=edgeLines(o);t.push(`<path d="${d}" fill="none" stroke="#8C96A0" stroke-width="1.2" ${o.nested?'stroke-dasharray="4 3" ':""}marker-end="url(#vgarrow)">`+(p.length?`<title>${esc(edgeLabel(o))}</title>`:"")+"</path>"),p.length&&n.push({x:u,y:(i+l)/2,lines:p})}for(const o of n){const s=linesWidth(o.lines)+12,i=o.lines.length*LINE_H,c=o.y-i/2;t.push(`<rect x="${(o.x-s/2).toFixed(1)}" y="${c.toFixed(1)}" width="${s.toFixed(1)}" height="${i}" rx="2" fill="#FFFFFF" opacity="0.92"/>`);const l=o.lines.map((a,u)=>`<tspan x="${o.x}" y="${(c+LINE_H*u+11).toFixed(1)}">${esc(a)}</tspan>`).join("");t.push(`<text text-anchor="middle" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" fill="#57606A">${l}</text>`)}for(const o of e.nodes){const s=kindOf(o.kind),i=o.tipLines||o.params.map(l=>l.name+": "+Object.keys(l.values).map(a=>a+" = "+l.values[a]).join(" / ")),c=o.label+(i.length?`
`+i.join(`
`):"");t.push(`<g><title>${esc(c)}</title>`),t.push(`<rect x="${o.x}" y="${o.y}" width="${o.w}" height="${o.h}" rx="5" fill="${s.fill}" stroke="${s.stroke}" stroke-width="1"/>`),t.push(`<rect x="${o.x}" y="${o.y}" width="4" height="${o.h}" rx="2" fill="${s.bar}"/>`),t.push(`<text x="${o.x+11}" y="${o.y+17}" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" font-weight="600" fill="#24292F">${esc(fit(shortName(o.label),o.w))}</text>`),t.push(`<text x="${o.x+11}" y="${o.y+31}" font-family="Roboto,system-ui,sans-serif" font-size="9" fill="#8C96A0">${esc(s.text)}${o.params.length?" \u30FB\u30D1\u30E9\u30E1\u30FC\u30BF":""}</text>`),t.push("</g>")}return t.push("</svg>"),t.join("")}function refBase(e,t){const n=shortName(e);if(!t)return n;const o="_"+t;return n.length>o.length&&n.slice(-o.length)===o?n.slice(0,-o.length):n}function erdSignature(e){const t=(e.suffixes||[])[0]||null,n=buildGraph(e.sql,e.params),o=new Map(n.nodes.map(s=>[s.id,refBase(s.name,t)+"/"+s.kind]));return JSON.stringify({n:n.nodes.map(s=>o.get(s.id)).sort(),e:n.edges.map(s=>o.get(s.from)+">"+o.get(s.to)+":"+(s.joinType||"")+":"+(s.keys||[]).join("&")).sort()})}function erdGroups(e){const t=new Map,n=[];for(const o of e.groups||[]){const s=erdSignature(o);let i=t.get(s);i||(i={groups:[],suffixes:[],members:[]},t.set(s,i),n.push(i)),i.groups.push(o);for(let c=0;c<(o.suffixes||[]).length;c++)i.suffixes.push(o.suffixes[c]),i.members.push((o.members||[])[c])}return n}function commonStem(e){const t=e.filter(o=>o!=null).map(String);if(!t.length)return"";if(t.every(o=>o===t[0]))return t[0];let n=0;for(;n<t[0].length&&t.every(o=>o[n]===t[0][n]);)n++;return t[0].slice(0,n)+"*"}function groupSvg(e){const t=e.groups||[e],n=buildGraph(t[0].sql,t[0].params),o=(t[0].suffixes||[])[0]||null;for(const s of n.nodes){const i=refBase(s.name,o),c=[],l=[],a=[];for(const u of t)for(const d of u.params||[]){const p=Object.keys(d.values);if(p.some(h=>refBase(d.values[h],h)===i)){a.push(d);for(const h of p)c.push(shortName(d.values[h]));l.push((t.length>1?label(u)+" / ":"")+d.name+": "+p.map(h=>h+" = "+d.values[h]).join(" / "))}}if(!a.length){s.params=[],s.tipLines=[];continue}s.label=commonStem(c),s.params=a,s.tipLines=l}return toSvg(layout(n))}function erdStack(e){return e.map(t=>`<div class="vg-erdblock"><div class="vg-erdhead"><span class="vg-erdname">${esc(label(t))}</span><span class="vg-tabn">${t.members.length}</span></div><div class="vg-erdbox">${groupSvg(t)}</div></div>`).join("")}function erdLegend(){const e=(t,n)=>`<span class="vg-lg"><span class="vg-lgm ${t}"></span>${esc(n)}</span>`;return'<div class="vg-legend">'+e("vg-lgt","\u5B9F\u30C6\u30FC\u30D6\u30EB")+e("vg-lgc","CTE")+e("vg-lgo","\u6700\u7D42 SELECT")+'<span class="vg-lg"><span class="vg-lgd"></span>\u30B5\u30D6\u30AF\u30A8\u30EA\u7D4C\u7531\u306E\u53C2\u7167</span></div>'}function renderErd(e){if(!e.groups.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const t=erdGroups(e),n=t.length<e.groups.length;return notice("FROM / JOIN \u304B\u3089\u8D77\u3053\u3057\u305F\u53C2\u7167\u95A2\u4FC2\u3067\u3059\u3002\u77E2\u5370\u306F\u300C\u8AAD\u3093\u3067\u4F5C\u308B\u300D\u5411\u304D\u3001\u6CE8\u8A18\u306F JOIN \u306E\u7A2E\u5225\u3068\u7D50\u5408\u30AD\u30FC\u3002\u30AB\u30FC\u30C7\u30A3\u30CA\u30EA\u30C6\u30A3\u3068\u4E3B\u30AD\u30FC\u306F SQL \u304B\u3089\u306F\u5206\u304B\u3089\u306A\u3044\u306E\u3067\u63CF\u3044\u3066\u3044\u307E\u305B\u3093\u3002\u3053\u3053\u3067\u306E\u62EC\u308A\u306F\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206\u3068\u306F\u5225\u3067\u3059\u3002\u53C2\u7167\u540D\u306E base \u90E8\u5206\uFF08suffix \u3092\u9664\u3044\u305F\u5B9F\u4F53\u540D\uFF09\u304C\u540C\u3058\u3082\u306E\u306F 1 \u679A\u306B\u307E\u3068\u3081\u3066\u3044\u307E\u3059\u3002"+(n?`\u30ED\u30B8\u30C3\u30AF\u306F ${e.groups.length} \u30B0\u30EB\u30FC\u30D7\u306B\u5272\u308C\u3066\u3044\u307E\u3059\u304C\u3001\u53C2\u7167\u95A2\u4FC2\u306F ${t.length} \u901A\u308A\u3067\u3059\u3002`:"")+"\u540D\u524D\u304C View \u3054\u3068\u306B\u9055\u3046\u7B87\u6240\u306F\u5171\u901A\u90E8\u5206\u3092\u51FA\u3057\u3066\u6B8B\u308A\u3092 * \u306B\u3057\u3066\u3044\u307E\u3059\uFF08\u5B9F\u969B\u306E\u5BFE\u5FDC\u306F\u7BB1\u306B\u30AB\u30FC\u30BD\u30EB\u3092\u4E57\u305B\u308B\u3068\u51FA\u307E\u3059\uFF09\u3002")+erdLegend()+erdStack(t)}function renderErdBase(e){return'<div class="vg-root">'+header(e.base,e.viewCount,e.groups.length,e.unmatched)+renderErd(e)+"</div>"}function __opts(e){if(!e)return{};try{return JSON.parse(e)||{}}catch(t){return{}}}function __notice(e){return'<div class="vg-notice">'+String(e).replace(/[<>&]/g,"")+"</div>"}function __run(e,t){var n;try{n=JSON.parse(e)}catch(c){n=null}if(!n)return __notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002");for(var o="",s=n.bases||[],i=0;i<s.length;i++)o+=renderErdBase(s[i]);return o||__notice("\u56F3\u306B\u3067\u304D\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")}return __run(analysis_json,options_json);

""";

DECLARE js_page STRING DEFAULT r"""
const MAX_TABS=12,MAX_SQL_TABS=128,MAX_DESC_TABS=16,MAX_LABEL_TABS=8,OUTER_TABS=["note","\u30AB\u30E9\u30E0\u5B9A\u7FA9","\u53C2\u7167\u95A2\u4FC2","\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206","SQL"],MAX_OUTER_TABS=8,NOTE_MARK="<!--VG_NOTE-->",CSS_GEN=8;function cssGuard(){return`<div class="vg-cssgen8" style="margin:0 0 10px;padding:8px 12px;border:1px solid #D4A72C;border-radius:6px;background:#FFF8C5;color:#7D4E00;font:12px/1.6 'Roboto','Segoe UI',system-ui,sans-serif">\u3053\u306E\u30AB\u30FC\u30C9\u306B\u306F\u65B0\u3057\u3044 CSS\uFF08\u4E16\u4EE3 8\uFF09\u304C\u8981\u308A\u307E\u3059\u3002template_style.html \u3092\u8CBC\u308A\u76F4\u3057\u3066\u304F\u3060\u3055\u3044\u3002\u8CBC\u308A\u66FF\u3048\u308B\u307E\u3067\u3001\u30BF\u30D6\u304C\u6B63\u3057\u304F\u51FA\u307E\u305B\u3093\uFF08\u5207\u308A\u66FF\u308F\u3089\u306A\u3044\u30FB\u4E2D\u8EAB\u304C\u5168\u90E8\u540C\u6642\u306B\u51FA\u308B\u30FB\u9078\u629E\u4E2D\u304C\u5206\u304B\u3089\u306A\u3044\uFF09\u3002</div>`}function esc(s){return String(s==null?"":s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function hashId(s){let n=2166136261;for(let e=0;e<String(s).length;e++)n^=String(s).charCodeAt(e),n=Math.imul(n,16777619)>>>0;return n.toString(36)}const label=s=>s.suffixes.map((n,e)=>n||s.members[e]&&s.members[e].viewName||"(suffix \u306A\u3057)").join(", ");function badge(s,n,e){return`<span class="vg-badge" style="color:${n};background:${e}">${esc(s)}</span>`}function header(s,n,e,o,l){const i=e>1;return`<div class="vg-header"><span class="vg-title">${esc(s)}</span>`+badge(`${n} View`,"#57606A","#EAEEF2")+badge(`${e} \u30B0\u30EB\u30FC\u30D7`,i?"#9A6700":"#1A7F37",i?"#FFF8C5":"#DAFBE1")+(o?badge("suffix \u672A\u8A8D\u8B58","#9A6700","#FFF8C5"):"")+(l?badge("\u30E9\u30D9\u30EB\u4E0D\u4E00\u81F4","#9A6700","#FFF8C5"):"")+"</div>"}function notice(s){return`<div class="vg-notice">${esc(s)}</div>`}const KIND_TEXT={entity:"\u5B9F\u4F53\u540D",string:"\u5024\uFF08\u6587\u5B57\u5217\uFF09",number:"\u5024\uFF08\u6570\u5024\uFF09",ident:"\u540D\u524D",quoted:"\u540D\u524D",keyword:"\u4E88\u7D04\u8A9E",punct:"\u8A18\u53F7",comment:"\u30B3\u30E1\u30F3\u30C8"},kindText=s=>KIND_TEXT[s]||s;function wrapPage(s,n,e,o,l,i,t){const r=i||{},u="vgo"+hashId(r.base||""),a=[`<div class="vg-root">${l==null||l===""?NOTE_MARK:String(l)}</div>`,e,n,s,o],d=OUTER_TABS.map((m,v)=>`<input class="vg-or vg-or${v+1}" type="radio" name="${u}" id="${u}-${v+1}"${v===0?" checked":""}>`).join(""),p=OUTER_TABS.map((m,v)=>`<label class="vg-otab vg-ot${v+1}" for="${u}-${v+1}">${esc(m)}</label>`).join(""),h=OUTER_TABS.map((m,v)=>`<div class="vg-opanel vg-op${v+1}">${a[v]||""}</div>`).join(""),g=r.base?header(r.base,r.viewCount,(r.groups||[]).length,r.unmatched,t):"";return`<div class="vg-outer">${cssGuard()}${d}<div class="vg-otablist">${g}${p}</div><div class="vg-opanels">${h}</div></div>`}const WARN="\u26A0",MAX_TABLE_W=1e3,MIN_COL_W=150,NAME_COL_W=180;function breakType(s){return String(s).replace(/(&lt;|,)/g,"$1<wbr>")}const DESC_JA_KEYS=["ja","jp","ja_name","name_ja","japanese","logical_name_ja","logicalNameJa","ja_logical_name","\u548C\u540D","\u65E5\u672C\u8A9E","\u65E5\u672C\u8A9E\u8AD6\u7406\u540D","\u8AD6\u7406\u540D"],DESC_EN_KEYS=["en","en_name","name_en","english","logical_name_en","logicalNameEn","en_logical_name","\u82F1\u8A9E","\u82F1\u8A9E\u8AD6\u7406\u540D"];function pickKey(s,n){for(let e=0;e<n.length;e++){const o=n[e];if(!Object.prototype.hasOwnProperty.call(s,o))continue;const l=s[o];if(!(l==null||String(l)===""))return{key:o,value:String(l)}}return null}function parseDesc(s){const n=String(s==null?"":s).trim();if(!n)return null;if(n.charAt(0)!=="{")return{raw:n};let e=null;try{e=JSON.parse(n)}catch(c){e=null}if(!e||typeof e!="object"||Array.isArray(e))return{raw:n};const o=pickKey(e,DESC_JA_KEYS),l=pickKey(e,DESC_EN_KEYS),i={};o&&(i[o.key]=!0),l&&(i[l.key]=!0);const t=[],r=Object.keys(e);for(let c=0;c<r.length;c++){const a=r[c];if(i[a])continue;const d=e[a];d==null||d===""||t.push({key:a,value:typeof d=="object"?JSON.stringify(d):String(d)})}const u=[];o&&u.push(o),l&&u.push(l);for(let c=0;c<t.length;c++)u.push(t[c]);return{ja:o?o.value:"",en:l?l.value:"",rest:t,pairs:u}}function descHtml(s){const n=parseDesc(s);return n?n.raw?`<div class="vg-cdesc">${esc(n.raw)}</div>`:n.pairs.length?`<table class="vg-cdtable">${n.pairs.map(o=>`<tr><th class="vg-cdk">${esc(o.key)}</th><td class="vg-cdv">${esc(o.value)}</td></tr>`).join("")}</table>`:"":""}function structFields(s){const n=String(s==null?"":s),e=n.indexOf("STRUCT<");if(e<0)return[];const o=[];let l="",i=0;for(let r=e+7;r<n.length;r++){const u=n.charAt(r);if(u===">"&&i===0){o.push(l);break}if(u==="<")i++;else if(u===">")i--;else if(u===","&&i===0){o.push(l),l="";continue}l+=u}const t=[];for(let r=0;r<o.length;r++){const u=o[r].trim().split(/[\s<]/)[0];u&&t.push(u)}return t}function assignOrder(s){const n=e=>("000"+e).slice(-4);for(const e of s.values()){const o=e.name.split("."),l=s.get(o[0]),i=l&&l.vals[0]&&l.vals[0].ord!=null?l.vals[0].ord:9999;let t=n(i),r=o[0];for(let u=1;u<o.length;u++){const c=s.get(r),d=(c&&c.vals[0]?structFields(c.vals[0].type):[]).indexOf(o[u]);t+="."+n(d<0?9999:d),r+="."+o[u]}e.sortKey=t}}function typeShape(s){const n=String(s==null?"":s).trim(),e=n.slice(0,6).toUpperCase()==="ARRAY<";let o=n;e&&(o=n.slice(n.indexOf("<")+1,n.lastIndexOf(">")).trim());const l=o.slice(0,7).toUpperCase()==="STRUCT<";return{repeated:e,record:l,name:l?"RECORD":o}}const isNested=s=>String(s).indexOf(".")>=0;function columnSig(s){const n=(s||[]).map(e=>[String(e.n==null?"":e.n),String(e.t==null?"":e.t),e.u==null||e.u===""?"":String(e.u).toUpperCase(),String(e.d==null?"":e.d)].join("\u0001"));return n.sort(),n.join("\u0002")}function columnGroups(s,n){const e=new Map,o=[],l=s&&s.groups||[];for(let t=0;t<l.length;t++){const r=l[t],u=r.members||[];for(let c=0;c<u.length;c++){const a=u[c],d=columnSig(n[a.viewName]);let p=e.get(d);p||(p={suffixes:[],members:[]},e.set(d,p),o.push(p)),p.suffixes.push((r.suffixes&&r.suffixes[c])!=null?r.suffixes[c]:null),p.members.push(a)}}const i=t=>String(t.suffixes[0]==null?t.members[0].viewName:t.suffixes[0]);for(const t of o){const r=t.members.map((u,c)=>c).sort((u,c)=>{const a=String(t.suffixes[u]==null?t.members[u].viewName:t.suffixes[u]),d=String(t.suffixes[c]==null?t.members[c].viewName:t.suffixes[c]);return a<d?-1:a>d?1:0});t.suffixes=r.map(u=>t.suffixes[u]),t.members=r.map(u=>t.members[u])}return o.sort((t,r)=>r.members.length-t.members.length||(i(t)<i(r)?-1:i(t)>i(r)?1:0)),o}function descCell(s){return!s||!s.descs.length?"":s.descs.length===1?descHtml(s.descs[0].text):s.descs.map(n=>`<div class="vg-cdescwho">${esc(n.suffixes.join(", "))}</div>`+descHtml(n.text)).join("")}function groupColumns(s,n){const e=new Map,o=s.members||[];for(let l=0;l<o.length;l++){const i=o[l],t=s.suffixes&&s.suffixes[l]||i.viewName,r=n[i.viewName]||[];for(let u=0;u<r.length;u++){const c=r[u];let a=e.get(c.n);if(a||(a={name:c.n,descs:[],order:u,vals:[]},e.set(c.n,a)),c.d){const d=String(c.d);let p=null;for(let h=0;h<a.descs.length;h++)a.descs[h].text===d&&(p=a.descs[h]);p?p.suffixes.push(t):a.descs.push({text:d,suffixes:[t]})}a.vals.push({suffix:t,type:c.t,ord:c.o==null?u+1:c.o,nullable:c.u==null||c.u===""?null:String(c.u).toUpperCase()!=="NO"})}}return assignOrder(e),e}function columnOrder(s){const n=new Set,e=[];for(const o of s){const l=[...o.values()].sort((i,t)=>i.sortKey<t.sortKey?-1:i.sortKey>t.sortKey?1:0);for(const i of l)n.has(i.name)||(n.add(i.name),e.push(i.name))}return e}function majority(s){const n=new Map;for(const l of s)n.set(l,(n.get(l)||0)+1);let e=null,o=-1;for(const l of s){const i=n.get(l);i>o&&(e=l,o=i)}return e}const nullText=s=>s===null?"UNKNOWN":s?"NULLABLE":"REQUIRED";function uniq(s){const n=[];for(const e of s)n.indexOf(e)<0&&n.push(e);return n}function cellInfo(s,n){if(!s)return{text:null,meta:"",sig:null,mixed:!1};const e=uniq(s.vals.map(r=>r.type)),o=uniq(s.vals.map(r=>nullText(r.nullable))),l=uniq(s.vals.map(r=>String(r.ord))).length>1||s.vals.length!==n,i=s.vals.map(r=>typeShape(r.type)),t=s.descs.map(r=>r.text).join("\u0001");return{text:e.join(" / "),shape:uniq(i.map(r=>r.name)).join(" / "),repeated:i.length>0&&i.every(r=>r.repeated),record:i.some(r=>r.record),nulls:o.join(" / "),sig:e.join(" / ")+"|"+o.join(" / ")+"|"+t,mixed:l}}function mixedTip(s,n){const e=isNested(s.name),o=s.vals.map(i=>e?`${i.suffix} = ${i.type}`:`${i.suffix} = #${i.ord} ${i.type} ${nullText(i.nullable)}`),l=s.vals.map(i=>i.suffix);for(let i=0;i<(n.members||[]).length;i++){const t=n.suffixes&&n.suffixes[i]||n.members[i].viewName;l.indexOf(t)<0&&o.push(`${t} = (\u3053\u306E\u5217\u3092\u6301\u305F\u306A\u3044)`)}return o.join(`
`)}function renderColumns(s,n){if(!(s.groups||[]).length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const e=columnGroups(s,n||{}),o=e.map(g=>groupColumns(g,n||{})),l=columnOrder(o);if(!l.length)return notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.COLUMNS \u304B\u3089\u5217\u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const i=new Set;for(const g of l){const m=g.lastIndexOf(".");m>0&&i.add(g.slice(0,m))}let t=0,r=0;const u=l.map(g=>{const m=g.split("."),v=m.length-1,S=v>0,$=e.map((f,j)=>cellInfo(o[j].get(g),f.members.length)),b=majority($.map(f=>f.sig));$.some(f=>f.sig!==b)&&t++;const x=$.map((f,j)=>{const O=e[j],N=o[j].get(g),y=["vg-ccell"];f.text?f.sig!==b&&y.push("vg-cdiff"):y.push("vg-cnone"),f.mixed&&(y.push("vg-cmix"),r++);const C=!f.record||i.has(g)?f.shape:f.text,E=S?f.repeated?"REPEATED":"":f.repeated?"REPEATED":f.nulls,T=f.text?breakType(esc(C))+(f.mixed?`<span class="vg-cwarn" data-tip="${esc(mixedTip(N,O))}">${WARN}</span>`:"")+(E?`<span class="vg-cmeta">${esc(E)}</span>`:"")+descCell(N):"\u2014";return`<td class="${y.join(" ")}">${T}</td>`}).join(""),w="vg-cname"+(v?` vg-cd${Math.min(v,3)}`:""),_=v?`<span class="vg-cnestmark">\u2514</span>${esc(m[m.length-1])}`:esc(g);return`<tr><th class="${w}">${_}</th>${x}</tr>`}).join(""),c=e.map(g=>`<th class="vg-chead">${esc(label(g))}<span class="vg-tabn">${g.members.length}</span></th>`).join(""),a=[];e.length===1?a.push(notice(`${s.viewCount||e[0].members.length} \u672C\u306E View \u3059\u3079\u3066\u3067\u30AB\u30E9\u30E0\u5B9A\u7FA9\uFF08\u5217\u540D\u30FB\u578B\u30FB\u30E2\u30FC\u30C9\u30FB\u8AAC\u660E\uFF09\u304C\u4E00\u81F4\u3057\u3066\u3044\u307E\u3059\u3002`)):(a.push(notice(`\u30AB\u30E9\u30E0\u5B9A\u7FA9\u304C ${e.length} \u7A2E\u985E\u3042\u308A\u307E\u3059\uFF08\u5217\u304C\u305D\u306E\u672C\u6570\uFF09\u3002\u5217\u306F\u30ED\u30B8\u30C3\u30AF \u30B0\u30EB\u30FC\u30D7\u3067\u306F\u306A\u304F\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3067\u675F\u306D\u3066\u3044\u308B\u306E\u3067\u3001SQL \u304C\u540C\u4E00\u3067\u3082\u53C2\u7167\u5148\u30C6\u30FC\u30D6\u30EB\u306E\u578B\u3084 description \u304C\u9055\u3048\u3070\u5225\u306E\u5217\u306B\u306A\u308A\u307E\u3059\uFF08\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206\u306E\u30BF\u30D6\u306B\u306F\u51FA\u3066\u3053\u306A\u3044\u5DEE\u3067\u3059\uFF09\u3002`)),t&&a.push(notice(`\u578B\u30FB\u30E2\u30FC\u30C9\u30FB\u8AAC\u660E\u306E\u3044\u305A\u308C\u304B\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u5217\u304C ${t} \u4EF6\u3042\u308A\u307E\u3059\uFF08\u8272\u4ED8\u304D\u306E\u30BB\u30EB\u3002\u305D\u306E\u5217\u3067\u3044\u3061\u3070\u3093\u591A\u3044\u5185\u5BB9\u3068\u9055\u3046\u3082\u306E\uFF09\u3002`))),r&&a.push(notice(`\u540C\u3058\u5217\u306E\u4E2D\u3067\u4E26\u3073\u9806\uFF08ordinal\uFF09\u3060\u3051\u304C\u98DF\u3044\u9055\u3063\u3066\u3044\u308B\u7B87\u6240\u304C${r} \u4EF6\u3042\u308A\u307E\u3059\uFF08${WARN} \u306E\u5370\u3002\u30DB\u30D0\u30FC\u3059\u308B\u3068 suffix \u3054\u3068\u306E\u5185\u8A33\u304C\u51FA\u307E\u3059\uFF09\u3002\u4E26\u3073\u9806\u306F\u5217\u306E\u675F\u306D\u65B9\u306B\u3082\u8868\u306E\u4E2D\u8EAB\u306B\u3082\u4F7F\u3063\u3066\u3044\u306A\u3044\u306E\u3067\u3001\u3053\u3053\u306B\u3060\u3051\u51FA\u307E\u3059\u3002`));const d=`<colgroup><col style="width:${NAME_COL_W}px">`+e.map(()=>"<col>").join("")+"</colgroup>",p=NAME_COL_W+MIN_COL_W*e.length,h=p>MAX_TABLE_W?` style="min-width:${p}px"`:"";return a.join("")+`<div class="vg-ctablewrap"><table class="vg-ctable"${h}>${d}<thead><tr><th class="vg-chead vg-cnamehead">\u5217\u540D</th>${c}</tr></thead><tbody>${u}</tbody></table></div>`}function renderColumnsBase(s,n){return`<div class="vg-root">${renderColumns(s,n)}</div>`}function sqlViews(s){const n=[],e=s.groups||[];for(let o=0;o<e.length;o++){const l=e[o],i=l.members||[];for(let t=0;t<i.length;t++)n.push({suffix:l.suffixes&&l.suffixes[t]||i[t].viewName||"(suffix \u306A\u3057)",viewName:i[t].viewName,group:label(l),groupSize:i.length})}return n.sort((o,l)=>{const i=String(o.suffix),t=String(l.suffix);return i.length-t.length||i.localeCompare(t)}),n}function sqlBody(s){const n=String(s).replace(/\r\n?/g,`
`).split(`
`);for(;n.length>1&&n[n.length-1].trim()==="";)n.pop();const e=String(n.length).length;return{html:`<div class="vg-sqlbox"><pre class="vg-sqlpre">${n.map((l,i)=>{const t=String(i+1);return`<span class="vg-sqln">${new Array(e-t.length+1).join(" ")}${t}</span> `+esc(l)}).join(`
`)}</pre></div>`,lines:n.length}}function sqlPanel(s,n){if(n==null||String(n)==="")return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(s.viewName)}</span></div>`+notice("\u3053\u306E View \u306E SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002");const e=sqlBody(n);return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(s.viewName)}</span><span class="vg-sqlmeta">\u30B0\u30EB\u30FC\u30D7: ${esc(s.group)}</span><span class="vg-sqlmeta">${e.lines} \u884C</span></div>`+e.html}function renderSql(s,n){const e=sqlViews(s);if(!e.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const o=n||{};if(!e.some(a=>o[a.viewName]))return notice("SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.VIEWS \u304B\u3089 view_definition \u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const l=e.slice(0,128),i="vgs"+hashId(s.base+"|"+e.map(a=>a.viewName).join("|")),t=l.map((a,d)=>`<input class="vg-sr vg-sr${d+1}" type="radio" name="${i}" id="${i}-${d+1}"${d===0?" checked":""}>`).join(""),r=l.map((a,d)=>`<label class="vg-stab vg-st${d+1}" for="${i}-${d+1}">${esc(a.suffix)}</label>`).join(""),u=l.map((a,d)=>`<div class="vg-spanel vg-sp${d+1}">${sqlPanel(a,o[a.viewName])}</div>`).join("");return(e.length>l.length?notice(`View \u304C\u591A\u3044\u305F\u3081\u5148\u982D 128 \u4EF6\u306E\u307F\u30BF\u30D6\u8868\u793A\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${e.length} \u4EF6\uFF09\u3002`):"")+`<div class="vg-stabs">${t}<div class="vg-stablist"><span class="vg-slabel">View</span>${r}</div><div class="vg-spanels">${u}</div></div>`}function renderSqlBase(s,n){return`<div class="vg-root">${renderSql(s,n)}</div>`}const DESC_TAB_ON="background:#EAEEF2;border-color:#8C959F;color:#24292F";function suffixByView(s){const n={},e=s&&s.groups||[];for(let o=0;o<e.length;o++){const l=e[o],i=l.members||[];for(let t=0;t<i.length;t++){const r=i[t]&&i[t].viewName;r&&(n[r]=l.suffixes&&l.suffixes[t]||r)}}return n}const cmp=(s,n)=>s<n?-1:s>n?1:0;function tabGroups(s,n,e){const o=suffixByView(s),l=Array.isArray(n)?n:[],i=[];for(let t=0;t<l.length;t++){const r=l[t]||{},u=Array.isArray(r.v)?r.v:r.v==null||r.v===""?[]:[r.v],c={label:u.map(a=>o[a]||a).sort(cmp).join(", ")||"(View \u306A\u3057)",count:u.length};e(c,r),i.push(c)}return i.sort((t,r)=>(t.empty===r.empty?0:t.empty?1:-1)||r.count-t.count||cmp(t.label,r.label)),i}function descGroups(s,n){return tabGroups(s,n,(e,o)=>{e.html=String(o.h==null?"":o.h),e.empty=e.html.trim()===""})}function labelGroups(s,n){return tabGroups(s,n,(e,o)=>{const l=Array.isArray(o.l)?o.l:[],i=[];for(let t=0;t<l.length;t++){const r=l[t]||{},u=String(r.k==null?"":r.k);u!==""&&i.push({k:u,v:String(r.v==null?"":r.v)})}i.sort((t,r)=>cmp(t.k,r.k)),e.pairs=i,e.empty=i.length===0})}function descPanel(s){return s.empty?notice("description \u304C\u8A2D\u5B9A\u3055\u308C\u3066\u3044\u307E\u305B\u3093\u3002"):s.html}const tabText=s=>`${esc(s.label)}<span class="vg-tabn">${s.count}</span>`;function renderDesc(s,n){const e=descGroups(s,n),o='<div class="vg-nhead">View \u306E description</div>';if(!e.length)return`<div class="vg-nsec">${o}`+notice("View \u306E description \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.TABLE_OPTIONS \u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002")+"</div>";if(e.length===1)return`<div class="vg-nsec">${o}<div class="vg-dtablist"><span class="vg-dtab vg-dstatic">${tabText(e[0])}</span></div><div class="vg-dbox">${descPanel(e[0])}</div></div>`;const l=e.slice(0,16),i="vgd"+hashId((s&&s.base||"")+"|"+e.map(a=>a.label).join("|")),t=l.map((a,d)=>`<input class="vg-dr vg-dr${d+1}" type="radio" name="${i}" id="${i}-${d+1}"${d===0?" checked":""}>`).join(""),r=l.map((a,d)=>`<label class="vg-dtab vg-dt${d+1}" for="${i}-${d+1}">${tabText(a)}</label>`).join(""),u=l.map((a,d)=>`<div class="vg-dpanel vg-dp${d+1}">${descPanel(a)}</div>`).join(""),c=e.length>l.length?notice(`description \u304C\u591A\u3044\u305F\u3081\u5148\u982D 16 \u7A2E\u985E\u306E\u307F\u51FA\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${e.length} \u7A2E\u985E\uFF09\u3002`):"";return`<div class="vg-nsec">${o}${c}<div class="vg-dtabs">${t}<div class="vg-dtablist">${r}</div><div class="vg-dpanels">${u}</div></div></div>`}function labelChips(s){return s.empty?notice("\u30E9\u30D9\u30EB\u304C\u8A2D\u5B9A\u3055\u308C\u3066\u3044\u307E\u305B\u3093\u3002"):'<div class="vg-lbchips">'+s.pairs.map(n=>`<span class="vg-lbchip"><span class="vg-lbk">${esc(n.k)}</span><span class="vg-lbv">${esc(n.v||"(\u7A7A)")}</span></span>`).join("")+"</div>"}function labelsSplit(s,n){return labelGroups(s,n).length>1}function renderLabels(s,n){const e=labelGroups(s,n);if(!e.length||e[0].empty)return"";if(e.length===1)return labelChips(e[0]);const o=e.slice(0,8),l="vgl"+hashId((s&&s.base||"")+"|"+e.map(c=>c.label+"="+c.pairs.map(a=>a.k+":"+a.v).join(",")).join("|")),i=o.map((c,a)=>`<input class="vg-lbr vg-lbr${a+1}" type="radio" name="${l}" id="${l}-${a+1}"${a===0?" checked":""}>`).join(""),t=o.map((c,a)=>`<label class="vg-lbtab vg-lbt${a+1}" for="${l}-${a+1}">${tabText(c)}</label>`).join(""),r=o.map((c,a)=>`<div class="vg-lbpanel vg-lbp${a+1}">${labelChips(c)}</div>`).join(""),u=e.length>o.length?notice(`\u30E9\u30D9\u30EB\u306E\u7A2E\u985E\u304C\u591A\u3044\u305F\u3081\u5148\u982D 8 \u7A2E\u985E\u306E\u307F\u51FA\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${e.length} \u7A2E\u985E\uFF09\u3002`):"";return'<div class="vg-nsec"><div class="vg-nhead">\u30E9\u30D9\u30EB</div>'+notice("\u540C\u3058 base \u306E\u4E2D\u3067\u30E9\u30D9\u30EB\u304C\u5272\u308C\u3066\u3044\u307E\u3059\u3002")+u+`<div class="vg-lbtabs">${i}<div class="vg-lbtablist">${t}</div><div class="vg-lbpanels">${r}</div></div></div>`}function renderNote(s,n,e,o){const l=String(e==null?"":e);return renderLabels(s,o)+renderDesc(s,n)+`<div class="vg-nsec"><div class="vg-nhead">\u30E1\u30E2</div>${l}</div>`}function __opts(s){if(!s)return{};try{return JSON.parse(s)||{}}catch(n){return{}}}function __notice(s){return'<div class="vg-notice">'+String(s).replace(/[<>&]/g,"")+"</div>"}function __run(s,n,e,o,l,i,t,r){var u=__opts(r),c;try{c=JSON.parse(s)}catch(_){c=null}if(!c)return String(n||__notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002"));var a={};try{for(var d=JSON.parse(o)||[],p=0;p<d.length;p++)a[d[p].v]=d[p].cols||[]}catch(_){a={}}var h={};try{for(var g=JSON.parse(l)||[],m=0;m<g.length;m++)h[g[m].v]=g[m].s}catch(_){h={}}var v=[];try{v=JSON.parse(i)||[]}catch(_){v=[]}var S=[];try{S=JSON.parse(t)||[]}catch(_){S=[]}for(var $="",b="",A=c.bases||[],x=0;x<A.length;x++)$+=renderColumnsBase(A[x],a),b+=renderSqlBase(A[x],h);$||($=__notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),b||(b=__notice("SQL \u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002"));var w=c.bases&&c.bases.length?c.bases[0]:null;return wrapPage(String(n||""),String(e||__notice("\u53C2\u7167\u95A2\u4FC2\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002")),$,b,renderNote(w,v,NOTE_MARK,S),w,labelsSplit(w,S))}return __run(analysis_json,diff_html,erd_html,columns_json,sql_json,descs_json,labels_json,options_json);
...
.vg-r1:checked ~ .vg-tablist > .vg-t1{background:#f0f4ea;border-color:#c4d2ac;color:#24292F}
.vg-r1:checked ~ .vg-tablist > .vg-t1 .vg-tabn{background:#dfe7d2;color:#58683e}
.vg-r2:checked ~ .vg-panels > .vg-p2{display:block}
.vg-r2:checked ~ .vg-tablist > .vg-t2{background:#f0f4ea;border-color:#c4d2ac;color:#24292F}
.vg-r2:checked ~ .vg-tablist > .vg-t2 .vg-tabn{background:#dfe7d2;color:#58683e}
.vg-r3:checked ~ .vg-panels > .vg-p3{display:block}
.vg-r3:checked ~ .vg-tablist > .vg-t3{background:#f0f4ea;border-color:#c4d2ac;color:#24292F}
.vg-r3:checked ~ .vg-tablist > .vg-t3 .vg-tabn{background:#dfe7d2;color:#58683e}
.vg-r4:checked ~ .vg-panels > .vg-p4{display:block}
.vg-r4:checked ~ .vg-tablist > .vg-t4{background:#f0f4ea;border-color:#c4d2ac;color:#24292F}
.vg-r4:checked ~ .vg-tablist > .vg-t4 .vg-tabn{background:#dfe7d2;color:#58683e}
.vg-r5:checked ~ .vg-panels > .vg-p5{display:block}
.vg-r5:checked ~ .vg-tablist > .vg-t5{background:#f0f4ea;border-color:#c4d2ac;color:#24292F}
.vg-r5:checked ~ .vg-tablist > .vg-t5 .vg-tabn{background:#dfe7d2;color:#58683e}
.vg-r6:checked ~ .vg-panels > .vg-p6{display:block}
.vg-r6:checked ~ .vg-tablist > .vg-t6{background:#f0f4ea;border-color:#c4d2ac;color:#24292F}
.vg-r6:checked ~ .vg-tablist > .vg-t6 .vg-tabn{background:#dfe7d2;color:#58683e}
.vg-r7:checked ~ .vg-panels > .vg-p7{display:block}
.vg-r7:checked ~ .vg-tablist > .vg-t7{background:#f0f4ea;border-color:#c4d2ac;color:#24292F}
.vg-r7:checked ~ .vg-tablist > .vg-t7 .vg-tabn{background:#dfe7d2;color:#58683e}
.vg-r8:checked ~ .vg-panels > .vg-p8{display:block}
.vg-r8:checked ~ .vg-tablist > .vg-t8{background:#f0f4ea;border-color:#c4d2ac;color:#24292F}
.vg-r8:checked ~ .vg-tablist > .vg-t8 .vg-tabn{background:#dfe7d2;color:#58683e}
.vg-r9:checked ~ .vg-panels > .vg-p9{display:block}
.vg-r9:checked ~ .vg-tablist > .vg-t9{background:#f0f4ea;border-color:#c4d2ac;color:#24292F}
.vg-r9:checked ~ .vg-tablist > .vg-t9 .vg-tabn{background:#dfe7d2;color:#58683e}
.vg-r10:checked ~ .vg-panels > .vg-p10{display:block}
.vg-r10:checked ~ .vg-tablist > .vg-t10{background:#f0f4ea;border-color:#c4d2ac;color:#24292F}
.vg-r10:checked ~ .vg-tablist > .vg-t10 .vg-tabn{background:#dfe7d2;color:#58683e}
.vg-r11:checked ~ .vg-panels > .vg-p11{display:block}
.vg-r11:checked ~ .vg-tablist > .vg-t11{background:#f0f4ea;border-color:#c4d2ac;color:#24292F}
.vg-r11:checked ~ .vg-tablist > .vg-t11 .vg-tabn{background:#dfe7d2;color:#58683e}
.vg-r12:checked ~ .vg-panels > .vg-p12{display:block}
.vg-r12:checked ~ .vg-tablist > .vg-t12{background:#f0f4ea;border-color:#c4d2ac;color:#24292F}
.vg-r12:checked ~ .vg-tablist > .vg-t12 .vg-tabn{background:#dfe7d2;color:#58683e}
...
.vg-sr25:checked ~ .vg-spanels > .vg-sp25{display:block}
.vg-sr25:checked ~ .vg-stablist > .vg-st25{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr26:checked ~ .vg-spanels > .vg-sp26{display:block}
.vg-sr26:checked ~ .vg-stablist > .vg-st26{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr27:checked ~ .vg-spanels > .vg-sp27{display:block}
.vg-sr27:checked ~ .vg-stablist > .vg-st27{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr28:checked ~ .vg-spanels > .vg-sp28{display:block}
.vg-sr28:checked ~ .vg-stablist > .vg-st28{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr29:checked ~ .vg-spanels > .vg-sp29{display:block}
.vg-sr29:checked ~ .vg-stablist > .vg-st29{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr30:checked ~ .vg-spanels > .vg-sp30{display:block}
.vg-sr30:checked ~ .vg-stablist > .vg-st30{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr31:checked ~ .vg-spanels > .vg-sp31{display:block}
.vg-sr31:checked ~ .vg-stablist > .vg-st31{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr32:checked ~ .vg-spanels > .vg-sp32{display:block}
.vg-sr32:checked ~ .vg-stablist > .vg-st32{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr33:checked ~ .vg-spanels > .vg-sp33{display:block}
.vg-sr33:checked ~ .vg-stablist > .vg-st33{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr34:checked ~ .vg-spanels > .vg-sp34{display:block}
.vg-sr34:checked ~ .vg-stablist > .vg-st34{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr35:checked ~ .vg-spanels > .vg-sp35{display:block}
.vg-sr35:checked ~ .vg-stablist > .vg-st35{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr36:checked ~ .vg-spanels > .vg-sp36{display:block}
.vg-sr36:checked ~ .vg-stablist > .vg-st36{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr37:checked ~ .vg-spanels > .vg-sp37{display:block}
.vg-sr37:checked ~ .vg-stablist > .vg-st37{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr38:checked ~ .vg-spanels > .vg-sp38{display:block}
.vg-sr38:checked ~ .vg-stablist > .vg-st38{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr39:checked ~ .vg-spanels > .vg-sp39{display:block}
.vg-sr39:checked ~ .vg-stablist > .vg-st39{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr40:checked ~ .vg-spanels > .vg-sp40{display:block}
.vg-sr40:checked ~ .vg-stablist > .vg-st40{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr41:checked ~ .vg-spanels > .vg-sp41{display:block}
.vg-sr41:checked ~ .vg-stablist > .vg-st41{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr42:checked ~ .vg-spanels > .vg-sp42{display:block}
.vg-sr42:checked ~ .vg-stablist > .vg-st42{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr43:checked ~ .vg-spanels > .vg-sp43{display:block}
.vg-sr43:checked ~ .vg-stablist > .vg-st43{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr44:checked ~ .vg-spanels > .vg-sp44{display:block}
.vg-sr44:checked ~ .vg-stablist > .vg-st44{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr45:checked ~ .vg-spanels > .vg-sp45{display:block}
.vg-sr45:checked ~ .vg-stablist > .vg-st45{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr46:checked ~ .vg-spanels > .vg-sp46{display:block}
.vg-sr46:checked ~ .vg-stablist > .vg-st46{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr47:checked ~ .vg-spanels > .vg-sp47{display:block}
.vg-sr47:checked ~ .vg-stablist > .vg-st47{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr48:checked ~ .vg-spanels > .vg-sp48{display:block}
.vg-sr48:checked ~ .vg-stablist > .vg-st48{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr49:checked ~ .vg-spanels > .vg-sp49{display:block}
.vg-sr49:checked ~ .vg-stablist > .vg-st49{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr50:checked ~ .vg-spanels > .vg-sp50{display:block}
.vg-sr50:checked ~ .vg-stablist > .vg-st50{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr51:checked ~ .vg-spanels > .vg-sp51{display:block}
.vg-sr51:checked ~ .vg-stablist > .vg-st51{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr52:checked ~ .vg-spanels > .vg-sp52{display:block}
.vg-sr52:checked ~ .vg-stablist > .vg-st52{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr53:checked ~ .vg-spanels > .vg-sp53{display:block}
.vg-sr53:checked ~ .vg-stablist > .vg-st53{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr54:checked ~ .vg-spanels > .vg-sp54{display:block}
.vg-sr54:checked ~ .vg-stablist > .vg-st54{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr55:checked ~ .vg-spanels > .vg-sp55{display:block}
.vg-sr55:checked ~ .vg-stablist > .vg-st55{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr56:checked ~ .vg-spanels > .vg-sp56{display:block}
.vg-sr56:checked ~ .vg-stablist > .vg-st56{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr57:checked ~ .vg-spanels > .vg-sp57{display:block}
.vg-sr57:checked ~ .vg-stablist > .vg-st57{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr58:checked ~ .vg-spanels > .vg-sp58{display:block}
.vg-sr58:checked ~ .vg-stablist > .vg-st58{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr59:checked ~ .vg-spanels > .vg-sp59{display:block}
.vg-sr59:checked ~ .vg-stablist > .vg-st59{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr60:checked ~ .vg-spanels > .vg-sp60{display:block}
.vg-sr60:checked ~ .vg-stablist > .vg-st60{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr61:checked ~ .vg-spanels > .vg-sp61{display:block}
.vg-sr61:checked ~ .vg-stablist > .vg-st61{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr62:checked ~ .vg-spanels > .vg-sp62{display:block}
.vg-sr62:checked ~ .vg-stablist > .vg-st62{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr63:checked ~ .vg-spanels > .vg-sp63{display:block}
.vg-sr63:checked ~ .vg-stablist > .vg-st63{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr64:checked ~ .vg-spanels > .vg-sp64{display:block}
.vg-sr64:checked ~ .vg-stablist > .vg-st64{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr65:checked ~ .vg-spanels > .vg-sp65{display:block}
.vg-sr65:checked ~ .vg-stablist > .vg-st65{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr66:checked ~ .vg-spanels > .vg-sp66{display:block}
.vg-sr66:checked ~ .vg-stablist > .vg-st66{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr67:checked ~ .vg-spanels > .vg-sp67{display:block}
.vg-sr67:checked ~ .vg-stablist > .vg-st67{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr68:checked ~ .vg-spanels > .vg-sp68{display:block}
.vg-sr68:checked ~ .vg-stablist > .vg-st68{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr69:checked ~ .vg-spanels > .vg-sp69{display:block}
.vg-sr69:checked ~ .vg-stablist > .vg-st69{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr70:checked ~ .vg-spanels > .vg-sp70{display:block}
.vg-sr70:checked ~ .vg-stablist > .vg-st70{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr71:checked ~ .vg-spanels > .vg-sp71{display:block}
.vg-sr71:checked ~ .vg-stablist > .vg-st71{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr72:checked ~ .vg-spanels > .vg-sp72{display:block}
.vg-sr72:checked ~ .vg-stablist > .vg-st72{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr73:checked ~ .vg-spanels > .vg-sp73{display:block}
.vg-sr73:checked ~ .vg-stablist > .vg-st73{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr74:checked ~ .vg-spanels > .vg-sp74{display:block}
.vg-sr74:checked ~ .vg-stablist > .vg-st74{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr75:checked ~ .vg-spanels > .vg-sp75{display:block}
.vg-sr75:checked ~ .vg-stablist > .vg-st75{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr76:checked ~ .vg-spanels > .vg-sp76{display:block}
.vg-sr76:checked ~ .vg-stablist > .vg-st76{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr77:checked ~ .vg-spanels > .vg-sp77{display:block}
.vg-sr77:checked ~ .vg-stablist > .vg-st77{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr78:checked ~ .vg-spanels > .vg-sp78{display:block}
.vg-sr78:checked ~ .vg-stablist > .vg-st78{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr79:checked ~ .vg-spanels > .vg-sp79{display:block}
.vg-sr79:checked ~ .vg-stablist > .vg-st79{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr80:checked ~ .vg-spanels > .vg-sp80{display:block}
.vg-sr80:checked ~ .vg-stablist > .vg-st80{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr81:checked ~ .vg-spanels > .vg-sp81{display:block}
.vg-sr81:checked ~ .vg-stablist > .vg-st81{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr82:checked ~ .vg-spanels > .vg-sp82{display:block}
.vg-sr82:checked ~ .vg-stablist > .vg-st82{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr83:checked ~ .vg-spanels > .vg-sp83{display:block}
.vg-sr83:checked ~ .vg-stablist > .vg-st83{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr84:checked ~ .vg-spanels > .vg-sp84{display:block}
.vg-sr84:checked ~ .vg-stablist > .vg-st84{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr85:checked ~ .vg-spanels > .vg-sp85{display:block}
.vg-sr85:checked ~ .vg-stablist > .vg-st85{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr86:checked ~ .vg-spanels > .vg-sp86{display:block}
.vg-sr86:checked ~ .vg-stablist > .vg-st86{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr87:checked ~ .vg-spanels > .vg-sp87{display:block}
.vg-sr87:checked ~ .vg-stablist > .vg-st87{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr88:checked ~ .vg-spanels > .vg-sp88{display:block}
.vg-sr88:checked ~ .vg-stablist > .vg-st88{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr89:checked ~ .vg-spanels > .vg-sp89{display:block}
.vg-sr89:checked ~ .vg-stablist > .vg-st89{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr90:checked ~ .vg-spanels > .vg-sp90{display:block}
.vg-sr90:checked ~ .vg-stablist > .vg-st90{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr91:checked ~ .vg-spanels > .vg-sp91{display:block}
.vg-sr91:checked ~ .vg-stablist > .vg-st91{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr92:checked ~ .vg-spanels > .vg-sp92{display:block}
.vg-sr92:checked ~ .vg-stablist > .vg-st92{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr93:checked ~ .vg-spanels > .vg-sp93{display:block}
.vg-sr93:checked ~ .vg-stablist > .vg-st93{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr94:checked ~ .vg-spanels > .vg-sp94{display:block}
.vg-sr94:checked ~ .vg-stablist > .vg-st94{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr95:checked ~ .vg-spanels > .vg-sp95{display:block}
.vg-sr95:checked ~ .vg-stablist > .vg-st95{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr96:checked ~ .vg-spanels > .vg-sp96{display:block}
.vg-sr96:checked ~ .vg-stablist > .vg-st96{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr97:checked ~ .vg-spanels > .vg-sp97{display:block}
.vg-sr97:checked ~ .vg-stablist > .vg-st97{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr98:checked ~ .vg-spanels > .vg-sp98{display:block}
.vg-sr98:checked ~ .vg-stablist > .vg-st98{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr99:checked ~ .vg-spanels > .vg-sp99{display:block}
.vg-sr99:checked ~ .vg-stablist > .vg-st99{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr100:checked ~ .vg-spanels > .vg-sp100{display:block}
.vg-sr100:checked ~ .vg-stablist > .vg-st100{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr101:checked ~ .vg-spanels > .vg-sp101{display:block}
.vg-sr101:checked ~ .vg-stablist > .vg-st101{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr102:checked ~ .vg-spanels > .vg-sp102{display:block}
.vg-sr102:checked ~ .vg-stablist > .vg-st102{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr103:checked ~ .vg-spanels > .vg-sp103{display:block}
.vg-sr103:checked ~ .vg-stablist > .vg-st103{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr104:checked ~ .vg-spanels > .vg-sp104{display:block}
.vg-sr104:checked ~ .vg-stablist > .vg-st104{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr105:checked ~ .vg-spanels > .vg-sp105{display:block}
.vg-sr105:checked ~ .vg-stablist > .vg-st105{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr106:checked ~ .vg-spanels > .vg-sp106{display:block}
.vg-sr106:checked ~ .vg-stablist > .vg-st106{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr107:checked ~ .vg-spanels > .vg-sp107{display:block}
.vg-sr107:checked ~ .vg-stablist > .vg-st107{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr108:checked ~ .vg-spanels > .vg-sp108{display:block}
.vg-sr108:checked ~ .vg-stablist > .vg-st108{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr109:checked ~ .vg-spanels > .vg-sp109{display:block}
.vg-sr109:checked ~ .vg-stablist > .vg-st109{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr110:checked ~ .vg-spanels > .vg-sp110{display:block}
.vg-sr110:checked ~ .vg-stablist > .vg-st110{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr111:checked ~ .vg-spanels > .vg-sp111{display:block}
.vg-sr111:checked ~ .vg-stablist > .vg-st111{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr112:checked ~ .vg-spanels > .vg-sp112{display:block}
.vg-sr112:checked ~ .vg-stablist > .vg-st112{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr113:checked ~ .vg-spanels > .vg-sp113{display:block}
.vg-sr113:checked ~ .vg-stablist > .vg-st113{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr114:checked ~ .vg-spanels > .vg-sp114{display:block}
.vg-sr114:checked ~ .vg-stablist > .vg-st114{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr115:checked ~ .vg-spanels > .vg-sp115{display:block}
.vg-sr115:checked ~ .vg-stablist > .vg-st115{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr116:checked ~ .vg-spanels > .vg-sp116{display:block}
.vg-sr116:checked ~ .vg-stablist > .vg-st116{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr117:checked ~ .vg-spanels > .vg-sp117{display:block}
.vg-sr117:checked ~ .vg-stablist > .vg-st117{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr118:checked ~ .vg-spanels > .vg-sp118{display:block}
.vg-sr118:checked ~ .vg-stablist > .vg-st118{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr119:checked ~ .vg-spanels > .vg-sp119{display:block}
.vg-sr119:checked ~ .vg-stablist > .vg-st119{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr120:checked ~ .vg-spanels > .vg-sp120{display:block}
.vg-sr120:checked ~ .vg-stablist > .vg-st120{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr121:checked ~ .vg-spanels > .vg-sp121{display:block}
.vg-sr121:checked ~ .vg-stablist > .vg-st121{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr122:checked ~ .vg-spanels > .vg-sp122{display:block}
.vg-sr122:checked ~ .vg-stablist > .vg-st122{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr123:checked ~ .vg-spanels > .vg-sp123{display:block}
.vg-sr123:checked ~ .vg-stablist > .vg-st123{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr124:checked ~ .vg-spanels > .vg-sp124{display:block}
.vg-sr124:checked ~ .vg-stablist > .vg-st124{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr125:checked ~ .vg-spanels > .vg-sp125{display:block}
.vg-sr125:checked ~ .vg-stablist > .vg-st125{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr126:checked ~ .vg-spanels > .vg-sp126{display:block}
.vg-sr126:checked ~ .vg-stablist > .vg-st126{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr127:checked ~ .vg-spanels > .vg-sp127{display:block}
.vg-sr127:checked ~ .vg-stablist > .vg-st127{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
.vg-sr128:checked ~ .vg-spanels > .vg-sp128{display:block}
.vg-sr128:checked ~ .vg-stablist > .vg-st128{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}
...
.vg-lbr1:checked ~ .vg-lbpanels > .vg-lbp1{display:block}
.vg-lbr1:checked ~ .vg-lbtablist > .vg-lbt1{background:#EAEEF2;border-color:#8C959F;color:#24292F}
.vg-lbr2:checked ~ .vg-lbpanels > .vg-lbp2{display:block}
.vg-lbr2:checked ~ .vg-lbtablist > .vg-lbt2{background:#EAEEF2;border-color:#8C959F;color:#24292F}
.vg-lbr3:checked ~ .vg-lbpanels > .vg-lbp3{display:block}
.vg-lbr3:checked ~ .vg-lbtablist > .vg-lbt3{background:#EAEEF2;border-color:#8C959F;color:#24292F}
.vg-lbr4:checked ~ .vg-lbpanels > .vg-lbp4{display:block}
.vg-lbr4:checked ~ .vg-lbtablist > .vg-lbt4{background:#EAEEF2;border-color:#8C959F;color:#24292F}
.vg-lbr5:checked ~ .vg-lbpanels > .vg-lbp5{display:block}
.vg-lbr5:checked ~ .vg-lbtablist > .vg-lbt5{background:#EAEEF2;border-color:#8C959F;color:#24292F}
.vg-lbr6:checked ~ .vg-lbpanels > .vg-lbp6{display:block}
.vg-lbr6:checked ~ .vg-lbtablist > .vg-lbt6{background:#EAEEF2;border-color:#8C959F;color:#24292F}
.vg-lbr7:checked ~ .vg-lbpanels > .vg-lbp7{display:block}
.vg-lbr7:checked ~ .vg-lbtablist > .vg-lbt7{background:#EAEEF2;border-color:#8C959F;color:#24292F}
.vg-lbr8:checked ~ .vg-lbpanels > .vg-lbp8{display:block}
.vg-lbr8:checked ~ .vg-lbtablist > .vg-lbt8{background:#EAEEF2;border-color:#8C959F;color:#24292F}
