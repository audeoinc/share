...
var C=Object.defineProperty,M=Object.defineProperties;var L=Object.getOwnPropertyDescriptors;var I=Object.getOwnPropertySymbols;var U=Object.prototype.hasOwnProperty,_=Object.prototype.propertyIsEnumerable;var R=(t,e,n)=>e in t?C(t,e,{enumerable:!0,configurable:!0,writable:!0,value:n}):t[e]=n,y=(t,e)=>{for(var n in e||(e={}))U.call(e,n)&&R(t,n,e[n]);if(I)for(var n of I(e))_.call(e,n)&&R(t,n,e[n]);return t},A=(t,e)=>M(t,L(e));const MAX_TABS=12,MAX_REF_TABS=12,REF_BUDGET=8388608,OUTER_TABS=["note","\u30AB\u30E9\u30E0\u5B9A\u7FA9","\u53C2\u7167\u95A2\u4FC2","\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206"],NOTE_MARK="<!--VG_NOTE-->";function esc(t){return String(t==null?"":t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function hashId(t){let e=2166136261;for(let n=0;n<String(t).length;n++)e^=String(t).charCodeAt(n),e=Math.imul(e,16777619)>>>0;return e.toString(36)}const label=t=>t.suffixes.map((e,n)=>e||t.members[n]&&t.members[n].viewName||"(suffix \u306A\u3057)").join(", ");function badge(t,e,n){return`<span class="vg-badge" style="color:${e};background:${n}">${esc(t)}</span>`}function header(t,e,n,s){const o=n>1;return`<div class="vg-header"><span class="vg-title">${esc(t)}</span>`+badge(`${e} View`,"#57606A","#EAEEF2")+badge(`${n} \u30B0\u30EB\u30FC\u30D7`,o?"#9A6700":"#1A7F37",o?"#FFF8C5":"#DAFBE1")+(s?badge("suffix \u672A\u8A8D\u8B58","#9A6700","#FFF8C5"):"")+"</div>"}function notice(t){return`<div class="vg-notice">${esc(t)}</div>`}const KIND_TEXT={entity:"\u5B9F\u4F53\u540D",string:"\u5024\uFF08\u6587\u5B57\u5217\uFF09",number:"\u5024\uFF08\u6570\u5024\uFF09",ident:"\u540D\u524D",quoted:"\u540D\u524D",keyword:"\u4E88\u7D04\u8A9E",punct:"\u8A18\u53F7",comment:"\u30B3\u30E1\u30F3\u30C8"},kindText=t=>KIND_TEXT[t]||t;function wrapPage(t,e,n,s){const o=s||{},i="vgo"+hashId(o.base||""),a=[`<div class="vg-root">${NOTE_MARK}</div>`,n,e,t],d=OUTER_TABS.map((h,p)=>`<input class="vg-or vg-or${p+1}" type="radio" name="${i}" id="${i}-${p+1}"${p===0?" checked":""}>`).join(""),l=OUTER_TABS.map((h,p)=>`<label class="vg-otab vg-ot${p+1}" for="${i}-${p+1}">${esc(h)}</label>`).join(""),c=OUTER_TABS.map((h,p)=>`<div class="vg-opanel vg-op${p+1}">${a[p]||""}</div>`).join(""),f=o.base?header(o.base,o.viewCount,(o.groups||[]).length,o.unmatched):"";return`<div class="vg-outer">${d}<div class="vg-otablist">${f}${l}</div><div class="vg-opanels">${c}</div></div>`}const DQ="\\u0022",DQ3=DQ+DQ+DQ,TOKEN_RE=new RegExp(["(`[^`]*`)","([rbRB]{1,2}(?:'''[\\s\\S]*?'''|"+DQ3+"[\\s\\S]*?"+DQ3+"|'[^']*'|"+DQ+"[^"+DQ+"]*"+DQ+"))","('''[\\s\\S]*?''')","("+DQ3+"[\\s\\S]*?"+DQ3+")","('(?:\\\\.|''|[^'\\\\])*')","("+DQ+"(?:\\\\.|"+DQ+DQ+"|[^"+DQ+"\\\\])*"+DQ+")","(--[^\\n]*|#[^\\n]*)","(/\\*[\\s\\S]*?\\*/)","(0[xX][0-9a-fA-F]+|(?:\\d+(?:\\.\\d+)?|\\.\\d+)(?:[eE][+-]?\\d+)?)","([A-Za-z_][A-Za-z0-9_]*)","(\\s+)","([^\\s])"].join("|"),"g"),KEYWORDS=new Set("SELECT FROM WHERE JOIN INNER LEFT RIGHT FULL OUTER CROSS ON USING AND OR NOT IN IS NULL LIKE BETWEEN EXISTS ANY ALL SOME GROUP BY HAVING ORDER ASC DESC LIMIT OFFSET QUALIFY WINDOW UNION INTERSECT EXCEPT DISTINCT AS CASE WHEN THEN ELSE END CREATE OR REPLACE VIEW TABLE FUNCTION IF EXISTS OPTIONS WITH RECURSIVE OVER PARTITION UNNEST STRUCT ARRAY CAST SAFE_CAST EXTRACT INTERVAL DATE DATETIME TIME TIMESTAMP INT64 FLOAT64 NUMERIC BIGNUMERIC STRING BYTES BOOL TRUE FALSE COUNT SUM AVG MIN MAX COALESCE IFNULL NULLIF ROWS RANGE PRECEDING FOLLOWING CURRENT ROW".split(/\s+/));function tokenizeSql(t){const e=[],n=String(t==null?"":t);let s;for(TOKEN_RE.lastIndex=0;(s=TOKEN_RE.exec(n))!==null;){let o;s[1]?o="quoted":s[2]||s[3]||s[4]||s[5]||s[6]?o="string":s[7]||s[8]?o="comment":s[9]?o="number":s[10]?o=KEYWORDS.has(s[10].toUpperCase())?"keyword":"ident":s[11]?o="space":o="punct",e.push({kind:o,text:s[0]})}return e}function stripOptionsClause(t){const e=[];let n=0;for(;n<t.length;){const s=t[n];if(s.kind==="keyword"&&s.text.toUpperCase()==="OPTIONS"){let o=n+1;for(;o<t.length&&t[o].kind==="space";)o++;if(o<t.length&&t[o].text==="("){let i=0,a=o;for(;a<t.length;a++)if(t[a].text==="(")i++;else if(t[a].text===")"&&(i--,i===0)){a++;break}for(;e.length>0&&e[e.length-1].kind==="space";)e.pop();n=a;continue}}e.push(s),n++}return e}function markEntities(t){const e=t.slice(),n=i=>{let a=i+1;for(;a<e.length&&e[a].kind==="space";)a++;return a},s=[];let o=null;for(let i=0;i<e.length;i++){const a=e[i];if(a.kind==="space"||a.kind==="comment")continue;if(a.text==="("){s.push(o),o=null;continue}if(a.text===")"){s.pop(),o=null;continue}const d=a.text.toUpperCase();if(o=a.kind==="keyword"||a.kind==="ident"?d:null,a.kind!=="keyword"||d!=="FROM"&&d!=="JOIN"||s.length>0&&s[s.length-1]==="EXTRACT")continue;let l=n(i);for(;!(l>=e.length);){const c=e[l];if(c.kind==="quoted")e[l]={kind:"entity",text:c.text},l=n(l);else if(c.kind==="ident"){const f=[];let h=l;for(;;){f.push(h);const m=n(h);if(m<e.length&&e[m].text==="."){const u=n(m);if(u<e.length&&e[u].kind==="ident"){h=u;continue}}break}const p=n(h);if(p<e.length&&e[p].text==="(")break;for(const m of f)e[m]={kind:"entity",text:e[m].text};l=p}else break;if(l<e.length&&e[l].kind==="keyword"&&e[l].text.toUpperCase()==="AS"){const f=n(l);f<e.length&&(e[f].kind==="ident"||e[f].kind==="quoted")&&(l=n(f))}else l<e.length&&e[l].kind==="ident"&&(l=n(l));if(l<e.length&&e[l].text===","){l=n(l);continue}break}}return e}const DEFAULT_SUFFIX_RE=/^(.*?)_([A-Za-z0-9]{1,6})$/,DEFAULT_SUBSTITUTABLE=["entity","number","string"],SUFFIX_MARK="\0",LITERAL_MARK="\u0001",BOX_W_MIN=130,BOX_W_MAX=380,NAME_CHAR_W=6.65,BOX_H=40,GAP_MIN=78,CHAR_W=6.5,LINE_H=14,GAP_Y=14,PAD=10,kw=t=>t&&t.kind==="keyword"?t.text.toUpperCase():null;function prepare(t){const e=[];let n=0;for(const s of markEntities(tokenizeSql(t)))s.kind==="space"||s.kind==="comment"||(s.text===")"&&n--,e.push({kind:s.kind,text:s.text,depth:n}),s.text==="("&&n++);return e}function cteRanges(t){const e=[];if(!(t[0]&&kw(t[0])==="WITH"))return{ctes:e,mainFrom:0};let n=1;for(;;){const s=t[n],o=t[n+1],i=t[n+2];if(!i||s.kind!=="ident"||kw(o)!=="AS"||i.text!=="(")break;const a=i.depth;let d=n+3;for(;d<t.length&&!(t[d].text===")"&&t[d].depth===a);)d++;if(e.push({name:s.text,from:n+3,to:d,depth:a+1}),n=d+1,t[n]&&t[n].text===","){n++;continue}break}return{ctes:e,mainFrom:n}}const STOP=new Set(["WHERE","GROUP","ORDER","QUALIFY","WINDOW","HAVING","UNION","INTERSECT","EXCEPT","LIMIT","SELECT","JOIN","FROM"]);function skipParen(t,e,n){const s=t[e].depth;let o=e+1;for(;o<n&&!(t[o].text===")"&&t[o].depth===s);)o++;return o+1}function readSource(t,e,n){if(e>=n)return{node:null,next:e};const s=t[e];if(s.text==="(")return{node:{name:"(\u30B5\u30D6\u30AF\u30A8\u30EA)",kind:"subquery"},next:skipParen(t,e,n)};if(kw(s)==="UNNEST"){let o=e+1,i="";if(t[o]&&t[o].text==="("){const a=t[o].depth;let d=o+1;for(;d<n&&!(t[d].text===")"&&t[d].depth===a);)i+=t[d].text,d++;o=d+1}return{node:{name:"UNNEST("+i+")",kind:"unnest"},next:o}}if(s.kind==="entity"||s.kind==="quoted"||s.kind==="ident"){const o=[];let i=e;for(;i<n&&(t[i].kind==="entity"||t[i].kind==="quoted"||t[i].kind==="ident");){if(o.push(t[i].text),i++,i<n&&t[i].text==="."){i++;continue}break}return i<n&&t[i].text==="("?{node:null,next:skipParen(t,i,n)}:{node:{name:o.join("."),kind:"ref"},next:i}}return{node:null,next:e+1}}function skipAlias(t,e,n){if(e<n&&kw(t[e])==="AS"){const s=t[e+1];return s&&(s.kind==="ident"||s.kind==="quoted")?e+2:e+1}return e<n&&t[e].kind==="ident"&&!STOP.has(t[e].text.toUpperCase())?e+1:e}function readOnKeys(t,e,n){const s=[];let o=e;for(;o<n;){const i=kw(t[o]);if(i&&STOP.has(i)||i==="LEFT"||i==="RIGHT"||i==="FULL"||i==="INNER"||i==="CROSS")break;if(t[o].text==="="){const a=(c,f,h)=>c&&f&&h&&f.text==="."?h.text:null,d=a(t[o-3],t[o-2],t[o-1]),l=a(t[o+1],t[o+2],t[o+3]);d&&l&&s.push(d===l?d:d+" = "+l)}o++}return{keys:s,next:o}}function readUsingKeys(t,e,n){const s=[];let o=e;if(t[o]&&t[o].text==="("){const i=t[o].depth;for(o++;o<n&&!(t[o].text===")"&&t[o].depth===i);)t[o].kind==="ident"&&s.push(t[o].text),o++;o++}return{keys:s,next:o}}function joinKind(t,e,n){for(let s=e-1;s>=n&&e-s<=3;s--){const o=kw(t[s]);if(o==="LEFT"||o==="RIGHT"||o==="FULL"||o==="CROSS")return o;if(o==="INNER")return"INNER"}return"INNER"}function scanScope(t,e,n,s){const o=[];for(let i=e;i<n;i++){const a=kw(t[i]);if(a!=="FROM"&&a!=="JOIN")continue;const d=t[i].depth>s,l=a==="JOIN"?joinKind(t,i,e):null;let c=i+1;for(;;){const f=readSource(t,c,n);if(c=f.next,f.node){c=skipAlias(t,c,n);let h=[];if(!d&&c<n&&kw(t[c])==="ON"){const p=readOnKeys(t,c+1,n);h=p.keys,c=p.next}else if(!d&&c<n&&kw(t[c])==="USING"){const p=readUsingKeys(t,c+1,n);h=p.keys,c=p.next}f.node.kind!=="unnest"&&o.push({name:f.node.name,kind:f.node.kind,joinType:l,keys:h,nested:d})}if(c<n&&t[c].text===","&&t[c].depth===t[i].depth){c++;continue}break}i=Math.max(i,c-1)}return o}function buildGraph(t,e){const n=new Map((e||[]).map(u=>[u.name,u])),s=u=>{const x=Object.keys(u.values);return x.length?u.values[x[0]]:null},o=new Map,i=String(t).replace(/\{\{(P\d+)\}\}/g,(u,x)=>{const k=n.get(x);if(!k)return u;const E=s(k);return E==null?u:(o.set(E,k),E)}),a=prepare(i),{ctes:d,mainFrom:l}=cteRanges(a),c=new Set(d.map(u=>u.name)),f=new Map,h=[],p=(u,x)=>{const k=x+":"+u;if(!f.has(k)){const E=[],b=o.get(String(u));if(b)E.push(b);else for(const N of String(u).split(".")){const r=o.get(N);r&&E.indexOf(r)<0&&E.push(r)}f.set(k,{id:k,name:u,label:u,kind:x,params:E})}return k},m=d.map(u=>({id:p(u.name,"cte"),from:u.from,to:u.to,depth:u.depth}));m.push({id:p("(\u6700\u7D42 SELECT)","output"),from:l,to:a.length,depth:0});for(const u of m)for(const x of scanScope(a,u.from,u.to,u.depth)){const k=x.kind==="ref"?c.has(x.name)?"cte":"table":x.kind,E=p(x.name,k);if(E===u.id)continue;const b=h.find(N=>N.from===E&&N.to===u.id);b?(b.keys.length||(b.keys=x.keys),b.joinType||(b.joinType=x.joinType),b.nested=b.nested&&x.nested):h.push({from:E,to:u.id,joinType:x.joinType,keys:x.keys,nested:x.nested})}return{nodes:[...f.values()],edges:h}}function layout(t){const{nodes:e,edges:n}=t,s=new Map(e.map(r=>[r.id,[]])),o=new Map(e.map(r=>[r.id,[]]));for(const r of n)s.has(r.to)&&s.get(r.to).push(r.from),o.has(r.from)&&o.get(r.from).push(r.to);const i=new Map(e.map(r=>[r.id,0]));for(let r=0;r<e.length+1;r++){let g=!1;for(const v of e){const w=s.get(v.id);if(!w.length)continue;const F=Math.max(...w.map(S=>i.get(S)||0))+1;F>i.get(v.id)&&(i.set(v.id,F),g=!0)}if(!g)break}const a=Math.max(0,...e.map(r=>i.get(r.id))),d=new Map(e.map(r=>[r.id,o.get(r.id).length?1/0:a]));for(let r=0;r<e.length+1;r++){let g=!1;for(const v of e){const w=o.get(v.id);if(!w.length)continue;const F=Math.min(...w.map(S=>d.get(S)));F-1<d.get(v.id)&&(d.set(v.id,F-1),g=!0)}if(!g)break}for(const r of e)Number.isFinite(d.get(r.id))||d.set(r.id,a);const l=[];for(const r of e){const g=Math.max(0,d.get(r.id));(l[g]||(l[g]=[])).push(r)}const c=new Map;l.forEach((r,g)=>{r&&(g>0&&r.sort((v,w)=>{const F=S=>{const $=s.get(S.id).filter(T=>c.has(T));return $.length?$.reduce((T,O)=>T+c.get(O),0)/$.length:Number.MAX_SAFE_INTEGER};return F(v)-F(w)}),r.forEach((v,w)=>c.set(v.id,w)))});const f=boxWidth(e),h=new Map;l.forEach((r,g)=>(r||[]).forEach(v=>h.set(v.id,g)));const p=new Array(Math.max(0,l.length-1)).fill(GAP_MIN);for(const r of n){const g=(h.get(r.to)||0)-1;g<0||g>=p.length||(p[g]=Math.max(p[g],linesWidth(edgeLines(r))+22))}const m=[];let u=PAD;for(let r=0;r<l.length;r++)m[r]=u,u+=f+(p[r]||0);const x=[];l.forEach((r,g)=>{(r||[]).forEach((v,w)=>{x.push(A(y({},v),{x:m[g],y:PAD+w*(BOX_H+GAP_Y),w:f,h:BOX_H}))})});const k=new Map(x.map(r=>[r.id,r])),E=Math.max(1,...l.map(r=>(r||[]).length));let b=0,N=PAD*2+E*BOX_H+Math.max(0,E-1)*GAP_Y+6;for(const r of n){const g=k.get(r.from),v=k.get(r.to);if(!g||!v)continue;const w=edgeLines(r);if(!w.length)continue;const F=(g.y+g.h/2+v.y+v.h/2)/2,S=w.length*LINE_H;b=Math.min(b,F-S/2-PAD),N=Math.max(N,F+S/2+PAD)}return{nodes:x,edges:n.map(r=>A(y({},r),{a:k.get(r.from),b:k.get(r.to)})).filter(r=>r.a&&r.b),gaps:p,colOf:h,y0:b,width:(m[l.length-1]||PAD)+f+PAD,height:N-b}}const KIND={table:{text:"\u5B9F\u30C6\u30FC\u30D6\u30EB",fill:"#FFFFFF",stroke:"#8C6D3F",bar:"#C9A227"},cte:{text:"CTE",fill:"#FFFFFF",stroke:"#3F6D8C",bar:"#4E8FBF"},output:{text:"\u6700\u7D42 SELECT",fill:"#FFFFFF",stroke:"#6D3F8C",bar:"#8250DF"},subquery:{text:"\u30B5\u30D6\u30AF\u30A8\u30EA",fill:"#FFFFFF",stroke:"#6E7781",bar:"#9AA4AE"}},kindOf=t=>KIND[t]||KIND.subquery;function shortName(t){const e=String(t).replace(/`/g,""),n=e.lastIndexOf(".");return n>=0?e.slice(n+1):e}function boxWidth(t){let e=BOX_W_MIN;for(const n of t)e=Math.max(e,shortName(n.label).length*NAME_CHAR_W+24);return Math.min(e,BOX_W_MAX)}function fit(t,e){const n=String(t),s=Math.floor((e-24)/NAME_CHAR_W);return n.length>s?n.slice(0,s-1)+"\u2026":n}function edgeLines(t){const e=[];t.joinType&&e.push(t.joinType==="INNER"?"JOIN":t.joinType+" JOIN");for(const n of t.keys||[])e.push(n);return!e.length&&t.nested&&e.push("\u30B5\u30D6\u30AF\u30A8\u30EA"),e}function linesWidth(t){let e=0;for(const n of t)e=Math.max(e,n.length*CHAR_W);return e}function edgeLabel(t){const e=edgeLines(t);return e.length?e.length>1?e[0]+" / "+e.slice(1).join(", "):e[0]:""}function toSvg(t){const e=[];e.push(`<svg viewBox="0 ${(t.y0||0).toFixed(1)} ${t.width} ${t.height}" width="${t.width}" height="${t.height}" role="img" aria-label="\u53C2\u7167\u95A2\u4FC2\u56F3" xmlns="http://www.w3.org/2000/svg">`),e.push('<defs><marker id="vgarrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="7" markerHeight="7" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#8C96A0"/></marker></defs>');const n=[];for(const s of t.edges){const o=s.a.x+s.a.w,i=s.a.y+s.a.h/2,a=s.b.x,d=s.b.y+s.b.h/2,l=t.gaps&&t.gaps[(t.colOf.get(s.to)||0)-1]||GAP_MIN,c=a>o?a-l/2:o+l/2,f=`M${o},${i} H${c} V${d} H${a}`,h=edgeLines(s);e.push(`<path d="${f}" fill="none" stroke="#8C96A0" stroke-width="1.2" ${s.nested?'stroke-dasharray="4 3" ':""}marker-end="url(#vgarrow)">`+(h.length?`<title>${esc(edgeLabel(s))}</title>`:"")+"</path>"),h.length&&n.push({x:c,y:(i+d)/2,lines:h})}for(const s of n){const o=linesWidth(s.lines)+12,i=s.lines.length*LINE_H,a=s.y-i/2;e.push(`<rect x="${(s.x-o/2).toFixed(1)}" y="${a.toFixed(1)}" width="${o.toFixed(1)}" height="${i}" rx="2" fill="#FFFFFF" opacity="0.92"/>`);const d=s.lines.map((l,c)=>`<tspan x="${s.x}" y="${(a+LINE_H*c+11).toFixed(1)}">${esc(l)}</tspan>`).join("");e.push(`<text text-anchor="middle" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" fill="#57606A">${d}</text>`)}for(const s of t.nodes){const o=kindOf(s.kind),i=s.label+(s.params.length?`
`+s.params.map(a=>a.name+": "+Object.keys(a.values).map(d=>d+" = "+a.values[d]).join(" / ")).join(`
`):"");e.push(`<g><title>${esc(i)}</title>`),e.push(`<rect x="${s.x}" y="${s.y}" width="${s.w}" height="${s.h}" rx="5" fill="${o.fill}" stroke="${o.stroke}" stroke-width="1"/>`),e.push(`<rect x="${s.x}" y="${s.y}" width="4" height="${s.h}" rx="2" fill="${o.bar}"/>`),e.push(`<text x="${s.x+11}" y="${s.y+17}" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" font-weight="600" fill="#24292F">${esc(fit(shortName(s.label),s.w))}</text>`),e.push(`<text x="${s.x+11}" y="${s.y+31}" font-family="Roboto,system-ui,sans-serif" font-size="9" fill="#8C96A0">${esc(o.text)}${s.params.length?" \u30FB\u30D1\u30E9\u30E1\u30FC\u30BF":""}</text>`),e.push("</g>")}return e.push("</svg>"),e.join("")}function groupSvg(t){return toSvg(layout(buildGraph(t.sql,t.params)))}function erdStack(t){return t.map(e=>`<div class="vg-erdblock"><div class="vg-erdhead"><span class="vg-erdname">${esc(label(e))}</span><span class="vg-tabn">${e.members.length}</span></div><div class="vg-erdbox">${groupSvg(e)}</div></div>`).join("")}function erdLegend(){const t=(e,n)=>`<span class="vg-lg"><span class="vg-lgm ${e}"></span>${esc(n)}</span>`;return'<div class="vg-legend">'+t("vg-lgt","\u5B9F\u30C6\u30FC\u30D6\u30EB")+t("vg-lgc","CTE")+t("vg-lgo","\u6700\u7D42 SELECT")+'<span class="vg-lg"><span class="vg-lgd"></span>\u30B5\u30D6\u30AF\u30A8\u30EA\u7D4C\u7531\u306E\u53C2\u7167</span></div>'}function renderErd(t){return t.groups.length?notice("FROM / JOIN \u304B\u3089\u8D77\u3053\u3057\u305F\u53C2\u7167\u95A2\u4FC2\u3067\u3059\u3002\u77E2\u5370\u306F\u300C\u8AAD\u3093\u3067\u4F5C\u308B\u300D\u5411\u304D\u3001\u6CE8\u8A18\u306F JOIN \u306E\u7A2E\u5225\u3068\u7D50\u5408\u30AD\u30FC\u3002\u30AB\u30FC\u30C7\u30A3\u30CA\u30EA\u30C6\u30A3\u3068\u4E3B\u30AD\u30FC\u306F SQL \u304B\u3089\u306F\u5206\u304B\u3089\u306A\u3044\u306E\u3067\u63CF\u3044\u3066\u3044\u307E\u305B\u3093\u3002\u30D1\u30E9\u30E1\u30FC\u30BF\u5316\u3057\u305F\u540D\u524D\u306F\u3001\u305D\u306E\u30B0\u30EB\u30FC\u30D7\u306E\u5148\u982D\u306E View \u306E\u5024\u3067\u8868\u793A\u3057\u3066\u3044\u307E\u3059\u3002")+erdLegend()+erdStack(t.groups):notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002")}function renderErdBase(t){return'<div class="vg-root">'+header(t.base,t.viewCount,t.groups.length,t.unmatched)+renderErd(t)+"</div>"}const WARN="\u26A0";function groupColumns(t,e){const n=new Map,s=t.members||[];for(let o=0;o<s.length;o++){const i=s[o],a=t.suffixes&&t.suffixes[o]||i.viewName,d=e[i.viewName]||[];for(let l=0;l<d.length;l++){const c=d[l];let f=n.get(c.n);f||(f={name:c.n,desc:"",order:l,vals:[]},n.set(c.n,f)),!f.desc&&c.d&&(f.desc=c.d),f.vals.push({suffix:a,type:c.t,ord:c.o==null?l+1:c.o,nullable:c.u==null||c.u===""?null:String(c.u).toUpperCase()!=="NO"})}}return n}function columnOrder(t){const e=new Set,n=[];for(const s of t){const o=[...s.values()].sort((i,a)=>i.order-a.order);for(const i of o)e.has(i.name)||(e.add(i.name),n.push(i.name))}return n}function majority(t){const e=new Map;for(const o of t)e.set(o,(e.get(o)||0)+1);let n=null,s=-1;for(const o of t){const i=e.get(o);i>s&&(n=o,s=i)}return n}const nullText=t=>t===null?"NULL \u4E0D\u660E":t?"NULL \u53EF":"NOT NULL";function uniq(t){const e=[];for(const n of t)e.indexOf(n)<0&&e.push(n);return e}function cellInfo(t,e){if(!t)return{text:null,meta:"",sig:null,mixed:!1};const n=uniq(t.vals.map(a=>a.type)),s=uniq(t.vals.map(a=>a.ord)),o=uniq(t.vals.map(a=>nullText(a.nullable))),i=uniq(t.vals.map(a=>`${a.ord}|${a.type}|${a.nullable}`)).length>1||t.vals.length!==e;return{text:n.join(" / "),meta:`#${s.join(" / ")} \xB7 ${o.join(" / ")}`,sig:n.join(" / ")+"|"+o.join(" / "),mixed:i}}function mixedTip(t,e){const n=t.vals.map(o=>`${o.suffix} = #${o.ord} ${o.type} ${nullText(o.nullable)}`),s=t.vals.map(o=>o.suffix);for(let o=0;o<(e.members||[]).length;o++){const i=e.suffixes&&e.suffixes[o]||e.members[o].viewName;s.indexOf(i)<0&&n.push(`${i} = (\u3053\u306E\u5217\u3092\u6301\u305F\u306A\u3044)`)}return n.join(`
`)}function renderColumns(t,e){const n=t.groups||[];if(!n.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const s=n.map(m=>groupColumns(m,e||{})),o=columnOrder(s);if(!o.length)return notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.COLUMNS \u304B\u3089\u5217\u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");let i=0,a=0;const d=o.map(m=>{const u=n.map((r,g)=>cellInfo(s[g].get(m),r.members.length)),x=majority(u.map(r=>r.sig));u.some(r=>r.sig!==x)&&i++;const E=n.map((r,g)=>s[g].get(m)).filter(r=>r)[0],b=u.map((r,g)=>{const v=n[g],w=s[g].get(m),F=["vg-ccell"];r.text?r.sig!==x&&F.push("vg-cdiff"):F.push("vg-cnone"),r.mixed&&(F.push("vg-cmix"),a++);const S=r.text?esc(r.text)+(r.mixed?`<span class="vg-cwarn" data-tip="${esc(mixedTip(w,v))}">${WARN}</span>`:"")+`<div class="vg-cmeta">${esc(r.meta)}</div>`:"\u2014";return`<td class="${F.join(" ")}">${S}</td>`}).join(""),N=E&&E.desc?`<div class="vg-cdesc">${esc(E.desc)}</div>`:"";return`<tr><th class="vg-cname">${esc(m)}${N}</th>${b}</tr>`}).join(""),l=n.map(m=>`<th class="vg-chead">${esc(label(m))}<span class="vg-tabn">${m.members.length}</span></th>`).join(""),c=[];a&&c.push(notice(`\u540C\u3058\u30B0\u30EB\u30FC\u30D7\u306E\u4E2D\u3067\u578B\u30FBNULL \u5236\u7D04\u30FB\u4E26\u3073\u9806\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u7B87\u6240\u304C ${a} \u4EF6\u3042\u308A\u307E\u3059\uFF08${WARN} \u306E\u5370\uFF09\u3002SQL \u304C\u540C\u4E00\u3067\u3082\u53C2\u7167\u5148\u30C6\u30FC\u30D6\u30EB\u306E\u578B\u304C\u9055\u3048\u3070\u3053\u3046\u306A\u308B\u306E\u3067\u3001\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206\u306B\u306F\u51FA\u3066\u304D\u307E\u305B\u3093\u3002`)),i&&c.push(notice(`\u30B0\u30EB\u30FC\u30D7\u9593\u3067\u578B\u307E\u305F\u306F NULL \u5236\u7D04\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u5217\u304C ${i} \u4EF6\u3042\u308A\u307E\u3059\uFF08\u8272\u4ED8\u304D\u306E\u30BB\u30EB\u3002\u305D\u306E\u5217\u3067\u3044\u3061\u3070\u3093\u591A\u3044\u5024\u3068\u9055\u3046\u3082\u306E\uFF09\u3002\u4E26\u3073\u9806\uFF08#\uFF09\u306E\u9055\u3044\u306B\u306F\u8272\u3092\u4ED8\u3051\u3066\u3044\u307E\u305B\u3093\uFF08\u5217\u3092 1 \u672C\u8DB3\u3059\u3068\u4EE5\u964D\u304C\u307E\u3068\u3081\u3066\u305A\u308C\u3001\u578B\u306E\u5DEE\u304C\u57CB\u3082\u308C\u308B\u305F\u3081\uFF09\u3002`)),c.length||c.push(notice("\u5168\u30B0\u30EB\u30FC\u30D7\u3067\u5217\u540D\u30FB\u578B\u30FBNULL \u5236\u7D04\u304C\u4E00\u81F4\u3057\u3066\u3044\u307E\u3059\u3002"));const f=24,h=((100-f)/n.length).toFixed(3),p=`<colgroup><col style="width:${f}%">`+n.map(()=>`<col style="width:${h}%">`).join("")+"</colgroup>";return c.join("")+`<div class="vg-ctablewrap"><table class="vg-ctable">${p}<thead><tr><th class="vg-chead vg-cnamehead">\u5217\u540D</th>${l}</tr></thead><tbody>${d}</tbody></table></div>`}function renderColumnsBase(t,e){return`<div class="vg-root">${renderColumns(t,e)}</div>`}function columnsCss(){return[".vg-ctablewrap{width:100%}",".vg-ctable{border-collapse:collapse;font-size:12px;width:100%;max-width:1000px;table-layout:fixed}",".vg-chead{position:sticky;top:var(--vg-bar);z-index:1;padding:6px 12px;border:1px solid #D0D7DE;background:#F6F8FA;color:#24292F;font-weight:600;text-align:left;overflow-wrap:anywhere}",".vg-cname{padding:5px 12px;border:1px solid #D0D7DE;text-align:left;vertical-align:top;font-weight:600;color:#24292F;overflow-wrap:anywhere;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}",".vg-cdesc{margin:2px 0 0;font:11px/1.5 'Roboto','Segoe UI',system-ui,sans-serif;font-weight:400;color:#57606A}",".vg-ccell{padding:5px 12px;border:1px solid #D0D7DE;vertical-align:top;color:#24292F;overflow-wrap:anywhere;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}",".vg-cmeta{margin:2px 0 0;font:11px/1.5 'Roboto','Segoe UI',system-ui,sans-serif;color:#57606A}",".vg-cdiff{background:#dfe7d2}",".vg-cnone{color:#8C959F;background:#FAFAFA}",".vg-cmix{box-shadow:inset 0 0 0 2px #D4A72C}",".vg-cwarn{position:relative;margin-left:6px;color:#9A6700;cursor:help}",".vg-cwarn::after{content:attr(data-tip);display:none;position:absolute;z-index:20;left:0;top:calc(100% + 5px);width:max-content;max-width:340px;padding:6px 10px;border-radius:6px;background:#24292F;color:#fff;font:11px/1.7 ui-monospace,SFMono-Regular,Consolas,monospace;white-space:pre-wrap;text-align:left;box-shadow:0 2px 10px rgba(0,0,0,.30);pointer-events:none}",".vg-cwarn:hover::after{display:block}"].join(`
`)}function __opts(t){if(!t)return{};try{return JSON.parse(t)||{}}catch(e){return{}}}function __notice(t){return'<div class="vg-notice">'+String(t).replace(/[<>&]/g,"")+"</div>"}function __run(t,e,n,s){var o=__opts(s),i;try{i=JSON.parse(t)}catch(m){i=null}if(!i)return String(e||__notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002"));var a={};try{for(var d=JSON.parse(n)||[],l=0;l<d.length;l++)a[d[l].v]=d[l].cols||[]}catch(m){a={}}for(var c="",f="",h=i.bases||[],p=0;p<h.length;p++)c+=renderErdBase(h[p]),f+=renderColumnsBase(h[p],a);return c||(c=__notice("\u56F3\u306B\u3067\u304D\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),f||(f=__notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),wrapPage(String(e||""),c,f,i.bases&&i.bases.length?i.bases[0]:null)}return __run(analysis_json,diff_html,columns_json,options_json);
...
.vg-cname{padding:5px 12px;border:1px solid #D0D7DE;text-align:left;vertical-align:top;font-weight:600;color:#24292F;overflow-wrap:anywhere;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}
.vg-cdesc{margin:2px 0 0;font:11px/1.5 'Roboto','Segoe UI',system-ui,sans-serif;font-weight:400;color:#57606A}
.vg-ccell{padding:5px 12px;border:1px solid #D0D7DE;vertical-align:top;color:#24292F;overflow-wrap:anywhere;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}
