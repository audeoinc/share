...
var C=Object.defineProperty,M=Object.defineProperties;var L=Object.getOwnPropertyDescriptors;var I=Object.getOwnPropertySymbols;var _=Object.prototype.hasOwnProperty,U=Object.prototype.propertyIsEnumerable;var R=(e,t,n)=>t in e?C(e,t,{enumerable:!0,configurable:!0,writable:!0,value:n}):e[t]=n,T=(e,t)=>{for(var n in t||(t={}))_.call(t,n)&&R(e,n,t[n]);if(I)for(var n of I(t))U.call(t,n)&&R(e,n,t[n]);return e},y=(e,t)=>M(e,L(t));const MAX_TABS=12,MAX_REF_TABS=12,MAX_SQL_TABS=24,REF_BUDGET=41943040,OUTER_TABS=["note","\u30AB\u30E9\u30E0\u5B9A\u7FA9","\u53C2\u7167\u95A2\u4FC2","\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206","SQL"],NOTE_MARK="<!--VG_NOTE-->";function esc(e){return String(e==null?"":e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function hashId(e){let t=2166136261;for(let n=0;n<String(e).length;n++)t^=String(e).charCodeAt(n),t=Math.imul(t,16777619)>>>0;return t.toString(36)}const label=e=>e.suffixes.map((t,n)=>t||e.members[n]&&e.members[n].viewName||"(suffix \u306A\u3057)").join(", ");function badge(e,t,n){return`<span class="vg-badge" style="color:${t};background:${n}">${esc(e)}</span>`}function header(e,t,n,s){const o=n>1;return`<div class="vg-header"><span class="vg-title">${esc(e)}</span>`+badge(`${t} View`,"#57606A","#EAEEF2")+badge(`${n} \u30B0\u30EB\u30FC\u30D7`,o?"#9A6700":"#1A7F37",o?"#FFF8C5":"#DAFBE1")+(s?badge("suffix \u672A\u8A8D\u8B58","#9A6700","#FFF8C5"):"")+"</div>"}function notice(e){return`<div class="vg-notice">${esc(e)}</div>`}const KIND_TEXT={entity:"\u5B9F\u4F53\u540D",string:"\u5024\uFF08\u6587\u5B57\u5217\uFF09",number:"\u5024\uFF08\u6570\u5024\uFF09",ident:"\u540D\u524D",quoted:"\u540D\u524D",keyword:"\u4E88\u7D04\u8A9E",punct:"\u8A18\u53F7",comment:"\u30B3\u30E1\u30F3\u30C8"},kindText=e=>KIND_TEXT[e]||e;function wrapPage(e,t,n,s,o){const i=o||{},r="vgo"+hashId(i.base||""),d=[`<div class="vg-root">${NOTE_MARK}</div>`,n,t,e,s],c=OUTER_TABS.map((h,g)=>`<input class="vg-or vg-or${g+1}" type="radio" name="${r}" id="${r}-${g+1}"${g===0?" checked":""}>`).join(""),l=OUTER_TABS.map((h,g)=>`<label class="vg-otab vg-ot${g+1}" for="${r}-${g+1}">${esc(h)}</label>`).join(""),p=OUTER_TABS.map((h,g)=>`<div class="vg-opanel vg-op${g+1}">${d[g]||""}</div>`).join(""),f=i.base?header(i.base,i.viewCount,(i.groups||[]).length,i.unmatched):"";return`<div class="vg-outer">${c}<div class="vg-otablist">${f}${l}</div><div class="vg-opanels">${p}</div></div>`}const DQ="\\u0022",DQ3=DQ+DQ+DQ,TOKEN_RE=new RegExp(["(`[^`]*`)","([rbRB]{1,2}(?:'''[\\s\\S]*?'''|"+DQ3+"[\\s\\S]*?"+DQ3+"|'[^']*'|"+DQ+"[^"+DQ+"]*"+DQ+"))","('''[\\s\\S]*?''')","("+DQ3+"[\\s\\S]*?"+DQ3+")","('(?:\\\\.|''|[^'\\\\])*')","("+DQ+"(?:\\\\.|"+DQ+DQ+"|[^"+DQ+"\\\\])*"+DQ+")","(--[^\\n]*|#[^\\n]*)","(/\\*[\\s\\S]*?\\*/)","(0[xX][0-9a-fA-F]+|(?:\\d+(?:\\.\\d+)?|\\.\\d+)(?:[eE][+-]?\\d+)?)","([A-Za-z_][A-Za-z0-9_]*)","(\\s+)","([^\\s])"].join("|"),"g"),KEYWORDS=new Set("SELECT FROM WHERE JOIN INNER LEFT RIGHT FULL OUTER CROSS ON USING AND OR NOT IN IS NULL LIKE BETWEEN EXISTS ANY ALL SOME GROUP BY HAVING ORDER ASC DESC LIMIT OFFSET QUALIFY WINDOW UNION INTERSECT EXCEPT DISTINCT AS CASE WHEN THEN ELSE END CREATE OR REPLACE VIEW TABLE FUNCTION IF EXISTS OPTIONS WITH RECURSIVE OVER PARTITION UNNEST STRUCT ARRAY CAST SAFE_CAST EXTRACT INTERVAL DATE DATETIME TIME TIMESTAMP INT64 FLOAT64 NUMERIC BIGNUMERIC STRING BYTES BOOL TRUE FALSE COUNT SUM AVG MIN MAX COALESCE IFNULL NULLIF ROWS RANGE PRECEDING FOLLOWING CURRENT ROW".split(/\s+/));function tokenizeSql(e){const t=[],n=String(e==null?"":e);let s;for(TOKEN_RE.lastIndex=0;(s=TOKEN_RE.exec(n))!==null;){let o;s[1]?o="quoted":s[2]||s[3]||s[4]||s[5]||s[6]?o="string":s[7]||s[8]?o="comment":s[9]?o="number":s[10]?o=KEYWORDS.has(s[10].toUpperCase())?"keyword":"ident":s[11]?o="space":o="punct",t.push({kind:o,text:s[0]})}return t}function stripOptionsClause(e){const t=[];let n=0;for(;n<e.length;){const s=e[n];if(s.kind==="keyword"&&s.text.toUpperCase()==="OPTIONS"){let o=n+1;for(;o<e.length&&e[o].kind==="space";)o++;if(o<e.length&&e[o].text==="("){let i=0,r=o;for(;r<e.length;r++)if(e[r].text==="(")i++;else if(e[r].text===")"&&(i--,i===0)){r++;break}for(;t.length>0&&t[t.length-1].kind==="space";)t.pop();n=r;continue}}t.push(s),n++}return t}function markEntities(e){const t=e.slice(),n=i=>{let r=i+1;for(;r<t.length&&t[r].kind==="space";)r++;return r},s=[];let o=null;for(let i=0;i<t.length;i++){const r=t[i];if(r.kind==="space"||r.kind==="comment")continue;if(r.text==="("){s.push(o),o=null;continue}if(r.text===")"){s.pop(),o=null;continue}const d=r.text.toUpperCase();if(o=r.kind==="keyword"||r.kind==="ident"?d:null,r.kind!=="keyword"||d!=="FROM"&&d!=="JOIN"||s.length>0&&s[s.length-1]==="EXTRACT")continue;let c=n(i);for(;!(c>=t.length);){const l=t[c];if(l.kind==="quoted")t[c]={kind:"entity",text:l.text},c=n(c);else if(l.kind==="ident"){const p=[];let f=c;for(;;){p.push(f);const g=n(f);if(g<t.length&&t[g].text==="."){const u=n(g);if(u<t.length&&t[u].kind==="ident"){f=u;continue}}break}const h=n(f);if(h<t.length&&t[h].text==="(")break;for(const g of p)t[g]={kind:"entity",text:t[g].text};c=h}else break;if(c<t.length&&t[c].kind==="keyword"&&t[c].text.toUpperCase()==="AS"){const p=n(c);p<t.length&&(t[p].kind==="ident"||t[p].kind==="quoted")&&(c=n(p))}else c<t.length&&t[c].kind==="ident"&&(c=n(c));if(c<t.length&&t[c].text===","){c=n(c);continue}break}}return t}const DEFAULT_SUFFIX_RE=/^(.*?)_([A-Za-z0-9]{1,6})$/,DEFAULT_SUBSTITUTABLE=["entity","number","string"],SUFFIX_MARK="\0",LITERAL_MARK="\u0001",BOX_W_MIN=130,BOX_W_MAX=380,NAME_CHAR_W=6.65,BOX_H=40,GAP_MIN=78,CHAR_W=6.5,LINE_H=14,GAP_Y=14,PAD=10,kw=e=>e&&e.kind==="keyword"?e.text.toUpperCase():null;function prepare(e){const t=[];let n=0;for(const s of markEntities(tokenizeSql(e)))s.kind==="space"||s.kind==="comment"||(s.text===")"&&n--,t.push({kind:s.kind,text:s.text,depth:n}),s.text==="("&&n++);return t}function cteRanges(e){const t=[];if(!(e[0]&&kw(e[0])==="WITH"))return{ctes:t,mainFrom:0};let n=1;for(;;){const s=e[n],o=e[n+1],i=e[n+2];if(!i||s.kind!=="ident"||kw(o)!=="AS"||i.text!=="(")break;const r=i.depth;let d=n+3;for(;d<e.length&&!(e[d].text===")"&&e[d].depth===r);)d++;if(t.push({name:s.text,from:n+3,to:d,depth:r+1}),n=d+1,e[n]&&e[n].text===","){n++;continue}break}return{ctes:t,mainFrom:n}}const STOP=new Set(["WHERE","GROUP","ORDER","QUALIFY","WINDOW","HAVING","UNION","INTERSECT","EXCEPT","LIMIT","SELECT","JOIN","FROM"]);function skipParen(e,t,n){const s=e[t].depth;let o=t+1;for(;o<n&&!(e[o].text===")"&&e[o].depth===s);)o++;return o+1}function readSource(e,t,n){if(t>=n)return{node:null,next:t};const s=e[t];if(s.text==="(")return{node:{name:"(\u30B5\u30D6\u30AF\u30A8\u30EA)",kind:"subquery"},next:skipParen(e,t,n)};if(kw(s)==="UNNEST"){let o=t+1,i="";if(e[o]&&e[o].text==="("){const r=e[o].depth;let d=o+1;for(;d<n&&!(e[d].text===")"&&e[d].depth===r);)i+=e[d].text,d++;o=d+1}return{node:{name:"UNNEST("+i+")",kind:"unnest"},next:o}}if(s.kind==="entity"||s.kind==="quoted"||s.kind==="ident"){const o=[];let i=t;for(;i<n&&(e[i].kind==="entity"||e[i].kind==="quoted"||e[i].kind==="ident");){if(o.push(e[i].text),i++,i<n&&e[i].text==="."){i++;continue}break}return i<n&&e[i].text==="("?{node:null,next:skipParen(e,i,n)}:{node:{name:o.join("."),kind:"ref"},next:i}}return{node:null,next:t+1}}function skipAlias(e,t,n){if(t<n&&kw(e[t])==="AS"){const s=e[t+1];return s&&(s.kind==="ident"||s.kind==="quoted")?t+2:t+1}return t<n&&e[t].kind==="ident"&&!STOP.has(e[t].text.toUpperCase())?t+1:t}function readOnKeys(e,t,n){const s=[];let o=t;for(;o<n;){const i=kw(e[o]);if(i&&STOP.has(i)||i==="LEFT"||i==="RIGHT"||i==="FULL"||i==="INNER"||i==="CROSS")break;if(e[o].text==="="){const r=(l,p,f)=>l&&p&&f&&p.text==="."?f.text:null,d=r(e[o-3],e[o-2],e[o-1]),c=r(e[o+1],e[o+2],e[o+3]);d&&c&&s.push(d===c?d:d+" = "+c)}o++}return{keys:s,next:o}}function readUsingKeys(e,t,n){const s=[];let o=t;if(e[o]&&e[o].text==="("){const i=e[o].depth;for(o++;o<n&&!(e[o].text===")"&&e[o].depth===i);)e[o].kind==="ident"&&s.push(e[o].text),o++;o++}return{keys:s,next:o}}function joinKind(e,t,n){for(let s=t-1;s>=n&&t-s<=3;s--){const o=kw(e[s]);if(o==="LEFT"||o==="RIGHT"||o==="FULL"||o==="CROSS")return o;if(o==="INNER")return"INNER"}return"INNER"}function scanScope(e,t,n,s){const o=[];for(let i=t;i<n;i++){const r=kw(e[i]);if(r!=="FROM"&&r!=="JOIN")continue;const d=e[i].depth>s,c=r==="JOIN"?joinKind(e,i,t):null;let l=i+1;for(;;){const p=readSource(e,l,n);if(l=p.next,p.node){l=skipAlias(e,l,n);let f=[];if(!d&&l<n&&kw(e[l])==="ON"){const h=readOnKeys(e,l+1,n);f=h.keys,l=h.next}else if(!d&&l<n&&kw(e[l])==="USING"){const h=readUsingKeys(e,l+1,n);f=h.keys,l=h.next}p.node.kind!=="unnest"&&o.push({name:p.node.name,kind:p.node.kind,joinType:c,keys:f,nested:d})}if(l<n&&e[l].text===","&&e[l].depth===e[i].depth){l++;continue}break}i=Math.max(i,l-1)}return o}function buildGraph(e,t){const n=new Map((t||[]).map(u=>[u.name,u])),s=u=>{const m=Object.keys(u.values);return m.length?u.values[m[0]]:null},o=new Map,i=String(e).replace(/\{\{(P\d+)\}\}/g,(u,m)=>{const b=n.get(m);if(!b)return u;const v=s(b);return v==null?u:(o.set(v,b),v)}),r=prepare(i),{ctes:d,mainFrom:c}=cteRanges(r),l=new Set(d.map(u=>u.name)),p=new Map,f=[],h=(u,m)=>{const b=m+":"+u;if(!p.has(b)){const v=[],E=o.get(String(u));if(E)v.push(E);else for(const k of String(u).split(".")){const a=o.get(k);a&&v.indexOf(a)<0&&v.push(a)}p.set(b,{id:b,name:u,label:u,kind:m,params:v})}return b},g=d.map(u=>({id:h(u.name,"cte"),from:u.from,to:u.to,depth:u.depth}));g.push({id:h("(\u6700\u7D42 SELECT)","output"),from:c,to:r.length,depth:0});for(const u of g)for(const m of scanScope(r,u.from,u.to,u.depth)){const b=m.kind==="ref"?l.has(m.name)?"cte":"table":m.kind,v=h(m.name,b);if(v===u.id)continue;const E=f.find(k=>k.from===v&&k.to===u.id);E?(E.keys.length||(E.keys=m.keys),E.joinType||(E.joinType=m.joinType),E.nested=E.nested&&m.nested):f.push({from:v,to:u.id,joinType:m.joinType,keys:m.keys,nested:m.nested})}return{nodes:[...p.values()],edges:f}}function layout(e){const{nodes:t,edges:n}=e,s=new Map(t.map(a=>[a.id,[]])),o=new Map(t.map(a=>[a.id,[]]));for(const a of n)s.has(a.to)&&s.get(a.to).push(a.from),o.has(a.from)&&o.get(a.from).push(a.to);const i=new Map(t.map(a=>[a.id,0]));for(let a=0;a<t.length+1;a++){let x=!1;for(const w of t){const S=s.get(w.id);if(!S.length)continue;const $=Math.max(...S.map(F=>i.get(F)||0))+1;$>i.get(w.id)&&(i.set(w.id,$),x=!0)}if(!x)break}const r=Math.max(0,...t.map(a=>i.get(a.id))),d=new Map(t.map(a=>[a.id,o.get(a.id).length?1/0:r]));for(let a=0;a<t.length+1;a++){let x=!1;for(const w of t){const S=o.get(w.id);if(!S.length)continue;const $=Math.min(...S.map(F=>d.get(F)));$-1<d.get(w.id)&&(d.set(w.id,$-1),x=!0)}if(!x)break}for(const a of t)Number.isFinite(d.get(a.id))||d.set(a.id,r);const c=[];for(const a of t){const x=Math.max(0,d.get(a.id));(c[x]||(c[x]=[])).push(a)}const l=new Map;c.forEach((a,x)=>{a&&(x>0&&a.sort((w,S)=>{const $=F=>{const N=s.get(F.id).filter(A=>l.has(A));return N.length?N.reduce((A,O)=>A+l.get(O),0)/N.length:Number.MAX_SAFE_INTEGER};return $(w)-$(S)}),a.forEach((w,S)=>l.set(w.id,S)))});const p=boxWidth(t),f=new Map;c.forEach((a,x)=>(a||[]).forEach(w=>f.set(w.id,x)));const h=new Array(Math.max(0,c.length-1)).fill(GAP_MIN);for(const a of n){const x=(f.get(a.to)||0)-1;x<0||x>=h.length||(h[x]=Math.max(h[x],linesWidth(edgeLines(a))+22))}const g=[];let u=PAD;for(let a=0;a<c.length;a++)g[a]=u,u+=p+(h[a]||0);const m=[];c.forEach((a,x)=>{(a||[]).forEach((w,S)=>{m.push(y(T({},w),{x:g[x],y:PAD+S*(BOX_H+GAP_Y),w:p,h:BOX_H}))})});const b=new Map(m.map(a=>[a.id,a])),v=Math.max(1,...c.map(a=>(a||[]).length));let E=0,k=PAD*2+v*BOX_H+Math.max(0,v-1)*GAP_Y+6;for(const a of n){const x=b.get(a.from),w=b.get(a.to);if(!x||!w)continue;const S=edgeLines(a);if(!S.length)continue;const $=(x.y+x.h/2+w.y+w.h/2)/2,F=S.length*LINE_H;E=Math.min(E,$-F/2-PAD),k=Math.max(k,$+F/2+PAD)}return{nodes:m,edges:n.map(a=>y(T({},a),{a:b.get(a.from),b:b.get(a.to)})).filter(a=>a.a&&a.b),gaps:h,colOf:f,y0:E,width:(g[c.length-1]||PAD)+p+PAD,height:k-E}}const KIND={table:{text:"\u5B9F\u30C6\u30FC\u30D6\u30EB",fill:"#FFFFFF",stroke:"#8C6D3F",bar:"#C9A227"},cte:{text:"CTE",fill:"#FFFFFF",stroke:"#3F6D8C",bar:"#4E8FBF"},output:{text:"\u6700\u7D42 SELECT",fill:"#FFFFFF",stroke:"#6D3F8C",bar:"#8250DF"},subquery:{text:"\u30B5\u30D6\u30AF\u30A8\u30EA",fill:"#FFFFFF",stroke:"#6E7781",bar:"#9AA4AE"}},kindOf=e=>KIND[e]||KIND.subquery;function shortName(e){const t=String(e).replace(/`/g,""),n=t.lastIndexOf(".");return n>=0?t.slice(n+1):t}function boxWidth(e){let t=BOX_W_MIN;for(const n of e)t=Math.max(t,shortName(n.label).length*NAME_CHAR_W+24);return Math.min(t,BOX_W_MAX)}function fit(e,t){const n=String(e),s=Math.floor((t-24)/NAME_CHAR_W);return n.length>s?n.slice(0,s-1)+"\u2026":n}function edgeLines(e){const t=[];e.joinType&&t.push(e.joinType==="INNER"?"JOIN":e.joinType+" JOIN");for(const n of e.keys||[])t.push(n);return!t.length&&e.nested&&t.push("\u30B5\u30D6\u30AF\u30A8\u30EA"),t}function linesWidth(e){let t=0;for(const n of e)t=Math.max(t,n.length*CHAR_W);return t}function edgeLabel(e){const t=edgeLines(e);return t.length?t.length>1?t[0]+" / "+t.slice(1).join(", "):t[0]:""}function toSvg(e){const t=[];t.push(`<svg viewBox="0 ${(e.y0||0).toFixed(1)} ${e.width} ${e.height}" width="${e.width}" height="${e.height}" role="img" aria-label="\u53C2\u7167\u95A2\u4FC2\u56F3" xmlns="http://www.w3.org/2000/svg">`),t.push('<defs><marker id="vgarrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="7" markerHeight="7" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#8C96A0"/></marker></defs>');const n=[];for(const s of e.edges){const o=s.a.x+s.a.w,i=s.a.y+s.a.h/2,r=s.b.x,d=s.b.y+s.b.h/2,c=e.gaps&&e.gaps[(e.colOf.get(s.to)||0)-1]||GAP_MIN,l=r>o?r-c/2:o+c/2,p=`M${o},${i} H${l} V${d} H${r}`,f=edgeLines(s);t.push(`<path d="${p}" fill="none" stroke="#8C96A0" stroke-width="1.2" ${s.nested?'stroke-dasharray="4 3" ':""}marker-end="url(#vgarrow)">`+(f.length?`<title>${esc(edgeLabel(s))}</title>`:"")+"</path>"),f.length&&n.push({x:l,y:(i+d)/2,lines:f})}for(const s of n){const o=linesWidth(s.lines)+12,i=s.lines.length*LINE_H,r=s.y-i/2;t.push(`<rect x="${(s.x-o/2).toFixed(1)}" y="${r.toFixed(1)}" width="${o.toFixed(1)}" height="${i}" rx="2" fill="#FFFFFF" opacity="0.92"/>`);const d=s.lines.map((c,l)=>`<tspan x="${s.x}" y="${(r+LINE_H*l+11).toFixed(1)}">${esc(c)}</tspan>`).join("");t.push(`<text text-anchor="middle" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" fill="#57606A">${d}</text>`)}for(const s of e.nodes){const o=kindOf(s.kind),i=s.label+(s.params.length?`
`+s.params.map(r=>r.name+": "+Object.keys(r.values).map(d=>d+" = "+r.values[d]).join(" / ")).join(`
`):"");t.push(`<g><title>${esc(i)}</title>`),t.push(`<rect x="${s.x}" y="${s.y}" width="${s.w}" height="${s.h}" rx="5" fill="${o.fill}" stroke="${o.stroke}" stroke-width="1"/>`),t.push(`<rect x="${s.x}" y="${s.y}" width="4" height="${s.h}" rx="2" fill="${o.bar}"/>`),t.push(`<text x="${s.x+11}" y="${s.y+17}" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" font-weight="600" fill="#24292F">${esc(fit(shortName(s.label),s.w))}</text>`),t.push(`<text x="${s.x+11}" y="${s.y+31}" font-family="Roboto,system-ui,sans-serif" font-size="9" fill="#8C96A0">${esc(o.text)}${s.params.length?" \u30FB\u30D1\u30E9\u30E1\u30FC\u30BF":""}</text>`),t.push("</g>")}return t.push("</svg>"),t.join("")}function groupSvg(e){return toSvg(layout(buildGraph(e.sql,e.params)))}function erdStack(e){return e.map(t=>`<div class="vg-erdblock"><div class="vg-erdhead"><span class="vg-erdname">${esc(label(t))}</span><span class="vg-tabn">${t.members.length}</span></div><div class="vg-erdbox">${groupSvg(t)}</div></div>`).join("")}function erdLegend(){const e=(t,n)=>`<span class="vg-lg"><span class="vg-lgm ${t}"></span>${esc(n)}</span>`;return'<div class="vg-legend">'+e("vg-lgt","\u5B9F\u30C6\u30FC\u30D6\u30EB")+e("vg-lgc","CTE")+e("vg-lgo","\u6700\u7D42 SELECT")+'<span class="vg-lg"><span class="vg-lgd"></span>\u30B5\u30D6\u30AF\u30A8\u30EA\u7D4C\u7531\u306E\u53C2\u7167</span></div>'}function renderErd(e){return e.groups.length?notice("FROM / JOIN \u304B\u3089\u8D77\u3053\u3057\u305F\u53C2\u7167\u95A2\u4FC2\u3067\u3059\u3002\u77E2\u5370\u306F\u300C\u8AAD\u3093\u3067\u4F5C\u308B\u300D\u5411\u304D\u3001\u6CE8\u8A18\u306F JOIN \u306E\u7A2E\u5225\u3068\u7D50\u5408\u30AD\u30FC\u3002\u30AB\u30FC\u30C7\u30A3\u30CA\u30EA\u30C6\u30A3\u3068\u4E3B\u30AD\u30FC\u306F SQL \u304B\u3089\u306F\u5206\u304B\u3089\u306A\u3044\u306E\u3067\u63CF\u3044\u3066\u3044\u307E\u305B\u3093\u3002\u30D1\u30E9\u30E1\u30FC\u30BF\u5316\u3057\u305F\u540D\u524D\u306F\u3001\u305D\u306E\u30B0\u30EB\u30FC\u30D7\u306E\u5148\u982D\u306E View \u306E\u5024\u3067\u8868\u793A\u3057\u3066\u3044\u307E\u3059\u3002")+erdLegend()+erdStack(e.groups):notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002")}function renderErdBase(e){return'<div class="vg-root">'+header(e.base,e.viewCount,e.groups.length,e.unmatched)+renderErd(e)+"</div>"}const WARN="\u26A0";function breakType(e){return String(e).replace(/(&lt;|,)/g,"$1<wbr>")}function groupColumns(e,t){const n=new Map,s=e.members||[];for(let o=0;o<s.length;o++){const i=s[o],r=e.suffixes&&e.suffixes[o]||i.viewName,d=t[i.viewName]||[];for(let c=0;c<d.length;c++){const l=d[c];let p=n.get(l.n);p||(p={name:l.n,desc:"",order:c,vals:[]},n.set(l.n,p)),!p.desc&&l.d&&(p.desc=l.d),p.vals.push({suffix:r,type:l.t,ord:l.o==null?c+1:l.o,nullable:l.u==null||l.u===""?null:String(l.u).toUpperCase()!=="NO"})}}return n}function columnOrder(e){const t=new Set,n=[];for(const s of e){const o=[...s.values()].sort((i,r)=>i.order-r.order);for(const i of o)t.has(i.name)||(t.add(i.name),n.push(i.name))}return n}function majority(e){const t=new Map;for(const o of e)t.set(o,(t.get(o)||0)+1);let n=null,s=-1;for(const o of e){const i=t.get(o);i>s&&(n=o,s=i)}return n}const nullText=e=>e===null?"NULL \u4E0D\u660E":e?"NULL \u53EF":"NOT NULL";function uniq(e){const t=[];for(const n of e)t.indexOf(n)<0&&t.push(n);return t}function cellInfo(e,t){if(!e)return{text:null,meta:"",sig:null,mixed:!1};const n=uniq(e.vals.map(r=>r.type)),s=uniq(e.vals.map(r=>r.ord)),o=uniq(e.vals.map(r=>nullText(r.nullable))),i=uniq(e.vals.map(r=>`${r.ord}|${r.type}|${r.nullable}`)).length>1||e.vals.length!==t;return{text:n.join(" / "),meta:`#${s.join(" / ")} \xB7 ${o.join(" / ")}`,sig:n.join(" / ")+"|"+o.join(" / "),mixed:i}}function mixedTip(e,t){const n=e.vals.map(o=>`${o.suffix} = #${o.ord} ${o.type} ${nullText(o.nullable)}`),s=e.vals.map(o=>o.suffix);for(let o=0;o<(t.members||[]).length;o++){const i=t.suffixes&&t.suffixes[o]||t.members[o].viewName;s.indexOf(i)<0&&n.push(`${i} = (\u3053\u306E\u5217\u3092\u6301\u305F\u306A\u3044)`)}return n.join(`
`)}function renderColumns(e,t){const n=e.groups||[];if(!n.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const s=n.map(g=>groupColumns(g,t||{})),o=columnOrder(s);if(!o.length)return notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.COLUMNS \u304B\u3089\u5217\u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");let i=0,r=0;const d=o.map(g=>{const u=n.map((a,x)=>cellInfo(s[x].get(g),a.members.length)),m=majority(u.map(a=>a.sig));u.some(a=>a.sig!==m)&&i++;const v=n.map((a,x)=>s[x].get(g)).filter(a=>a)[0],E=u.map((a,x)=>{const w=n[x],S=s[x].get(g),$=["vg-ccell"];a.text?a.sig!==m&&$.push("vg-cdiff"):$.push("vg-cnone"),a.mixed&&($.push("vg-cmix"),r++);const F=a.text?breakType(esc(a.text))+(a.mixed?`<span class="vg-cwarn" data-tip="${esc(mixedTip(S,w))}">${WARN}</span>`:"")+`<div class="vg-cmeta">${esc(a.meta)}</div>`:"\u2014";return`<td class="${$.join(" ")}">${F}</td>`}).join(""),k=v&&v.desc?`<div class="vg-cdesc">${esc(v.desc)}</div>`:"";return`<tr><th class="vg-cname">${esc(g)}${k}</th>${E}</tr>`}).join(""),c=n.map(g=>`<th class="vg-chead">${esc(label(g))}<span class="vg-tabn">${g.members.length}</span></th>`).join(""),l=[];r&&l.push(notice(`\u540C\u3058\u30B0\u30EB\u30FC\u30D7\u306E\u4E2D\u3067\u578B\u30FBNULL \u5236\u7D04\u30FB\u4E26\u3073\u9806\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u7B87\u6240\u304C ${r} \u4EF6\u3042\u308A\u307E\u3059\uFF08${WARN} \u306E\u5370\uFF09\u3002SQL \u304C\u540C\u4E00\u3067\u3082\u53C2\u7167\u5148\u30C6\u30FC\u30D6\u30EB\u306E\u578B\u304C\u9055\u3048\u3070\u3053\u3046\u306A\u308B\u306E\u3067\u3001\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206\u306B\u306F\u51FA\u3066\u304D\u307E\u305B\u3093\u3002`)),i&&l.push(notice(`\u30B0\u30EB\u30FC\u30D7\u9593\u3067\u578B\u307E\u305F\u306F NULL \u5236\u7D04\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u5217\u304C ${i} \u4EF6\u3042\u308A\u307E\u3059\uFF08\u8272\u4ED8\u304D\u306E\u30BB\u30EB\u3002\u305D\u306E\u5217\u3067\u3044\u3061\u3070\u3093\u591A\u3044\u5024\u3068\u9055\u3046\u3082\u306E\uFF09\u3002\u4E26\u3073\u9806\uFF08#\uFF09\u306E\u9055\u3044\u306B\u306F\u8272\u3092\u4ED8\u3051\u3066\u3044\u307E\u305B\u3093\uFF08\u5217\u3092 1 \u672C\u8DB3\u3059\u3068\u4EE5\u964D\u304C\u307E\u3068\u3081\u3066\u305A\u308C\u3001\u578B\u306E\u5DEE\u304C\u57CB\u3082\u308C\u308B\u305F\u3081\uFF09\u3002`)),l.length||l.push(notice("\u5168\u30B0\u30EB\u30FC\u30D7\u3067\u5217\u540D\u30FB\u578B\u30FBNULL \u5236\u7D04\u304C\u4E00\u81F4\u3057\u3066\u3044\u307E\u3059\u3002"));const p=24,f=((100-p)/n.length).toFixed(3),h=`<colgroup><col style="width:${p}%">`+n.map(()=>`<col style="width:${f}%">`).join("")+"</colgroup>";return l.join("")+`<div class="vg-ctablewrap"><table class="vg-ctable">${h}<thead><tr><th class="vg-chead vg-cnamehead">\u5217\u540D</th>${c}</tr></thead><tbody>${d}</tbody></table></div>`}function renderColumnsBase(e,t){return`<div class="vg-root">${renderColumns(e,t)}</div>`}function columnsCss(){return[".vg-ctablewrap{width:100%}",".vg-ctable{border-collapse:collapse;font-size:12px;width:100%;max-width:1000px;table-layout:fixed}",".vg-chead{position:sticky;top:var(--vg-bar);z-index:1;padding:6px 12px;border:1px solid #D0D7DE;background:#F6F8FA;color:#24292F;font-weight:600;text-align:left;overflow-wrap:anywhere}",".vg-cname{padding:5px 12px;border:1px solid #D0D7DE;text-align:left;vertical-align:top;font-weight:600;color:#24292F;overflow-wrap:anywhere;word-break:break-all;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}",".vg-cdesc{margin:2px 0 0;font:11px/1.5 'Roboto','Segoe UI',system-ui,sans-serif;font-weight:400;color:#57606A}",".vg-ccell{padding:5px 12px;border:1px solid #D0D7DE;vertical-align:top;color:#24292F;overflow-wrap:anywhere;word-break:break-all;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}",".vg-cmeta{margin:2px 0 0;font:11px/1.5 'Roboto','Segoe UI',system-ui,sans-serif;color:#57606A}",".vg-cdiff{background:#dfe7d2}",".vg-cnone{color:#8C959F;background:#FAFAFA}",".vg-cmix{box-shadow:inset 0 0 0 2px #D4A72C}",".vg-cwarn{position:relative;margin-left:6px;color:#9A6700;cursor:help}",".vg-cwarn::after{content:attr(data-tip);display:none;position:absolute;z-index:20;left:0;top:calc(100% + 5px);width:max-content;max-width:340px;padding:6px 10px;border-radius:6px;background:#24292F;color:#fff;font:11px/1.7 ui-monospace,SFMono-Regular,Consolas,monospace;white-space:pre-wrap;text-align:left;box-shadow:0 2px 10px rgba(0,0,0,.30);pointer-events:none}",".vg-cwarn:hover::after{display:block}"].join(`
`)}function sqlViews(e){const t=[],n=e.groups||[];for(let s=0;s<n.length;s++){const o=n[s],i=o.members||[];for(let r=0;r<i.length;r++)t.push({suffix:o.suffixes&&o.suffixes[r]||i[r].viewName||"(suffix \u306A\u3057)",viewName:i[r].viewName,group:label(o),groupSize:i.length})}return t.sort((s,o)=>String(s.suffix).localeCompare(String(o.suffix))),t}function sqlBody(e){const t=String(e).replace(/\r\n?/g,`
`).split(`
...
`)}</pre></div>`,lines:t.length}}function sqlPanel(e,t){if(t==null||String(t)==="")return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(e.viewName)}</span></div>`+notice("\u3053\u306E View \u306E SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002");const n=sqlBody(t);return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(e.viewName)}</span><span class="vg-sqlmeta">\u30B0\u30EB\u30FC\u30D7: ${esc(e.group)}</span><span class="vg-sqlmeta">${n.lines} \u884C</span></div>`+n.html}function renderSql(e,t){const n=sqlViews(e);if(!n.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const s=t||{};if(!n.some(p=>s[p.viewName]))return notice("SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.VIEWS \u304B\u3089 view_definition \u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const o=n.slice(0,24),i="vgs"+hashId(e.base+"|"+n.map(p=>p.viewName).join("|")),r=o.map((p,f)=>`<input class="vg-sr vg-sr${f+1}" type="radio" name="${i}" id="${i}-${f+1}"${f===0?" checked":""}>`).join(""),d=o.map((p,f)=>`<label class="vg-stab vg-st${f+1}" for="${i}-${f+1}">${esc(p.suffix)}</label>`).join(""),c=o.map((p,f)=>`<div class="vg-spanel vg-sp${f+1}">${sqlPanel(p,s[p.viewName])}</div>`).join("");return(n.length>o.length?notice(`View \u304C\u591A\u3044\u305F\u3081\u5148\u982D 24 \u4EF6\u306E\u307F\u30BF\u30D6\u8868\u793A\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${n.length} \u4EF6\uFF09\u3002`):"")+`<div class="vg-stabs">${r}<div class="vg-stablist"><span class="vg-slabel">View</span>${d}</div><div class="vg-spanels">${c}</div></div>`}function renderSqlBase(e,t){return`<div class="vg-root">${renderSql(e,t)}</div>`}function sqlCss(){const e=[".vg-sr{position:absolute;opacity:0;width:1px;height:1px;pointer-events:none}",".vg-stablist{display:flex;flex-wrap:wrap;align-items:center;gap:6px;margin:0 0 10px}",".vg-slabel{color:#57606A;font-size:12px;font-weight:600;margin-right:2px}",".vg-stab{display:inline-flex;align-items:center;gap:6px;padding:4px 12px;border:1px solid #D0D7DE;border-radius:14px;color:#57606A;cursor:pointer;user-select:none;font-weight:600;font-size:12px;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}",".vg-stab:hover{background:#EAEEF2;color:#24292F}",".vg-spanel{display:none}",".vg-sqlhead{display:flex;align-items:center;flex-wrap:wrap;gap:10px;padding:7px 12px;border:1px solid #D0D7DE;border-bottom:none;border-radius:6px 6px 0 0;background:#F6F8FA}",".vg-sqlname{font-weight:600;font-size:12px;line-height:1.6;color:#24292F;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}",".vg-sqlmeta{color:#57606A;font-size:11px;line-height:1.6}",".vg-sqlbox{overflow-x:auto;border:1px solid #D0D7DE;border-radius:0 0 6px 6px;background:#fff}",".vg-sqlpre{margin:0;padding:8px 12px;white-space:pre;font:12px/1.6 ui-monospace,SFMono-Regular,Consolas,monospace;color:#24292F}",".vg-sqln{color:#8C959F;user-select:none}"];for(let t=1;t<=24;t++)e.push(`.vg-sr${t}:checked ~ .vg-spanels > .vg-sp${t}{display:block}`),e.push(`.vg-sr${t}:checked ~ .vg-stablist > .vg-st${t}{background:#DDF4FF;border-color:#54AEFF;color:#0969DA}`);return e.join(`
`)}function __opts(e){if(!e)return{};try{return JSON.parse(e)||{}}catch(t){return{}}}function __notice(e){return'<div class="vg-notice">'+String(e).replace(/[<>&]/g,"")+"</div>"}function __run(e,t,n,s,o){var i=__opts(o),r;try{r=JSON.parse(e)}catch(E){r=null}if(!r)return String(t||__notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002"));var d={};try{for(var c=JSON.parse(n)||[],l=0;l<c.length;l++)d[c[l].v]=c[l].cols||[]}catch(E){d={}}var p={};try{for(var f=JSON.parse(s)||[],h=0;h<f.length;h++)p[f[h].v]=f[h].s}catch(E){p={}}for(var g="",u="",m="",b=r.bases||[],v=0;v<b.length;v++)g+=renderErdBase(b[v]),u+=renderColumnsBase(b[v],d),m+=renderSqlBase(b[v],p);return g||(g=__notice("\u56F3\u306B\u3067\u304D\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),u||(u=__notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),m||(m=__notice("SQL \u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),wrapPage(String(t||""),g,u,m,r.bases&&r.bases.length?r.bases[0]:null)}return __run(analysis_json,diff_html,columns_json,sql_json,options_json);
