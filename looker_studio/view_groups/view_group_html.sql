...
var L=Object.defineProperty,C=Object.defineProperties;var $=Object.getOwnPropertyDescriptors;var v=Object.getOwnPropertySymbols;var _=Object.prototype.hasOwnProperty,U=Object.prototype.propertyIsEnumerable;var b=(e,t,o)=>t in e?L(e,t,{enumerable:!0,configurable:!0,writable:!0,value:o}):e[t]=o,w=(e,t)=>{for(var o in t||(t={}))_.call(t,o)&&b(e,o,t[o]);if(v)for(var o of v(t))U.call(t,o)&&b(e,o,t[o]);return e},O=(e,t)=>C(e,$(t));const MAX_TABS=12,MAX_SQL_TABS=128,MAX_DESC_TABS=16,MAX_LABEL_TABS=8,OUTER_TABS=["note","\u30AB\u30E9\u30E0\u5B9A\u7FA9","\u53C2\u7167\u95A2\u4FC2","\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206","SQL"],MAX_OUTER_TABS=8,NOTE_MARK="<!--VG_NOTE-->",CSS_GEN=8;function groupRule(e,t,o){const n=[];for(let s=1;s<=e;s++){const i=t(s);if(Array.isArray(i))for(const c of i)n.push(c);else n.push(i)}return n.join(",")+"{"+o+"}"}function esc(e){return String(e==null?"":e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function hashId(e){let t=2166136261;for(let o=0;o<String(e).length;o++)t^=String(e).charCodeAt(o),t=Math.imul(t,16777619)>>>0;return t.toString(36)}const label=e=>e.suffixes.map((t,o)=>t||e.members[o]&&e.members[o].viewName||"(suffix \u306A\u3057)").join(", ");function badge(e,t,o){return`<span class="vg-badge" style="color:${t};background:${o}">${esc(e)}</span>`}function header(e,t,o,n,s){const i=o>1;return`<div class="vg-header"><span class="vg-title">${esc(e)}</span>`+badge(`${t} View`,"#57606A","#EAEEF2")+badge(`${o} \u30B0\u30EB\u30FC\u30D7`,i?"#9A6700":"#1A7F37",i?"#FFF8C5":"#DAFBE1")+(n?badge("suffix \u672A\u8A8D\u8B58","#9A6700","#FFF8C5"):"")+(s?badge("\u30E9\u30D9\u30EB\u4E0D\u4E00\u81F4","#9A6700","#FFF8C5"):"")+"</div>"}function notice(e){return`<div class="vg-notice">${esc(e)}</div>`}const KIND_TEXT={entity:"\u5B9F\u4F53\u540D",string:"\u5024\uFF08\u6587\u5B57\u5217\uFF09",number:"\u5024\uFF08\u6570\u5024\uFF09",ident:"\u540D\u524D",quoted:"\u540D\u524D",keyword:"\u4E88\u7D04\u8A9E",punct:"\u8A18\u53F7",comment:"\u30B3\u30E1\u30F3\u30C8"},kindText=e=>KIND_TEXT[e]||e,DQ="\\u0022",DQ3=DQ+DQ+DQ,TOKEN_RE=new RegExp(["(`[^`]*`)","([rbRB]{1,2}(?:'''[\\s\\S]*?'''|"+DQ3+"[\\s\\S]*?"+DQ3+"|'[^']*'|"+DQ+"[^"+DQ+"]*"+DQ+"))","('''[\\s\\S]*?''')","("+DQ3+"[\\s\\S]*?"+DQ3+")","('(?:\\\\.|''|[^'\\\\])*')","("+DQ+"(?:\\\\.|"+DQ+DQ+"|[^"+DQ+"\\\\])*"+DQ+")","(--[^\\n]*|#[^\\n]*)","(/\\*[\\s\\S]*?\\*/)","(0[xX][0-9a-fA-F]+|(?:\\d+(?:\\.\\d+)?|\\.\\d+)(?:[eE][+-]?\\d+)?)","([A-Za-z_][A-Za-z0-9_]*)","(\\s+)","([^\\s])"].join("|"),"g"),KEYWORDS=new Set("SELECT FROM WHERE JOIN INNER LEFT RIGHT FULL OUTER CROSS ON USING AND OR NOT IN IS NULL LIKE BETWEEN EXISTS ANY ALL SOME GROUP BY HAVING ORDER ASC DESC LIMIT OFFSET QUALIFY WINDOW UNION INTERSECT EXCEPT DISTINCT AS CASE WHEN THEN ELSE END CREATE OR REPLACE VIEW TABLE FUNCTION IF EXISTS OPTIONS WITH RECURSIVE OVER PARTITION UNNEST STRUCT ARRAY CAST SAFE_CAST EXTRACT INTERVAL DATE DATETIME TIME TIMESTAMP INT64 FLOAT64 NUMERIC BIGNUMERIC STRING BYTES BOOL TRUE FALSE COUNT SUM AVG MIN MAX COALESCE IFNULL NULLIF ROWS RANGE PRECEDING FOLLOWING CURRENT ROW".split(/\s+/));function tokenizeSql(e){const t=[],o=String(e==null?"":e);let n;for(TOKEN_RE.lastIndex=0;(n=TOKEN_RE.exec(o))!==null;){let s;n[1]?s="quoted":n[2]||n[3]||n[4]||n[5]||n[6]?s="string":n[7]||n[8]?s="comment":n[9]?s="number":n[10]?s=KEYWORDS.has(n[10].toUpperCase())?"keyword":"ident":n[11]?s="space":s="punct",t.push({kind:s,text:n[0]})}return t}function markEntities(e){const t=e.slice(),o=i=>{let c=i+1;for(;c<t.length&&t[c].kind==="space";)c++;return c},n=[];let s=null;for(let i=0;i<t.length;i++){const c=t[i];if(c.kind==="space"||c.kind==="comment")continue;if(c.text==="("){n.push(s),s=null;continue}if(c.text===")"){n.pop(),s=null;continue}const l=c.text.toUpperCase();if(s=c.kind==="keyword"||c.kind==="ident"?l:null,c.kind!=="keyword"||l!=="FROM"&&l!=="JOIN"||n.length>0&&n[n.length-1]==="EXTRACT")continue;let a=o(i);for(;!(a>=t.length);){const u=t[a];if(u.kind==="quoted")t[a]={kind:"entity",text:u.text},a=o(a);else if(u.kind==="ident"){const d=[];let p=a;for(;;){d.push(p);const F=o(p);if(F<t.length&&t[F].text==="."){const f=o(F);if(f<t.length&&t[f].kind==="ident"){p=f;continue}}break}const h=o(p);if(h<t.length&&t[h].text==="(")break;for(const F of d)t[F]={kind:"entity",text:t[F].text};a=h}else break;if(a<t.length&&t[a].kind==="keyword"&&t[a].text.toUpperCase()==="AS"){const d=o(a);d<t.length&&(t[d].kind==="ident"||t[d].kind==="quoted")&&(a=o(d))}else a<t.length&&t[a].kind==="ident"&&(a=o(a));if(a<t.length&&t[a].text===","){a=o(a);continue}break}}return t}const DEFAULT_SUFFIX_RE=/^(.*?)_([A-Za-z0-9]{1,6})$/,DEFAULT_SUBSTITUTABLE=["entity","number","string"],SUFFIX_MARK="\0",LITERAL_MARK="\u0001",BOX_W_MIN=130,BOX_W_MAX=380,NAME_CHAR_W=6.65,BOX_H=40,GAP_MIN=78,CHAR_W=6.5,LINE_H=14,GAP_Y=14,PAD=10,kw=e=>e&&e.kind==="keyword"?e.text.toUpperCase():null;function prepare(e){const t=[];let o=0;for(const n of markEntities(tokenizeSql(e)))n.kind==="space"||n.kind==="comment"||(n.text===")"&&o--,t.push({kind:n.kind,text:n.text,depth:o}),n.text==="("&&o++);return t}function cteRanges(e){const t=[];if(!(e[0]&&kw(e[0])==="WITH"))return{ctes:t,mainFrom:0};let o=1;for(;;){const n=e[o],s=e[o+1],i=e[o+2];if(!i||n.kind!=="ident"||kw(s)!=="AS"||i.text!=="(")break;const c=i.depth;let l=o+3;for(;l<e.length&&!(e[l].text===")"&&e[l].depth===c);)l++;if(t.push({name:n.text,from:o+3,to:l,depth:c+1}),o=l+1,e[o]&&e[o].text===","){o++;continue}break}return{ctes:t,mainFrom:o}}const STOP=new Set(["WHERE","GROUP","ORDER","QUALIFY","WINDOW","HAVING","UNION","INTERSECT","EXCEPT","LIMIT","SELECT","JOIN","FROM"]);function skipParen(e,t,o){const n=e[t].depth;let s=t+1;for(;s<o&&!(e[s].text===")"&&e[s].depth===n);)s++;return s+1}function readSource(e,t,o){if(t>=o)return{node:null,next:t};const n=e[t];if(n.text==="(")return{node:{name:"(\u30B5\u30D6\u30AF\u30A8\u30EA)",kind:"subquery"},next:skipParen(e,t,o)};if(kw(n)==="UNNEST"){let s=t+1,i="";if(e[s]&&e[s].text==="("){const c=e[s].depth;let l=s+1;for(;l<o&&!(e[l].text===")"&&e[l].depth===c);)i+=e[l].text,l++;s=l+1}return{node:{name:"UNNEST("+i+")",kind:"unnest"},next:s}}if(n.kind==="entity"||n.kind==="quoted"||n.kind==="ident"){const s=[];let i=t;for(;i<o&&(e[i].kind==="entity"||e[i].kind==="quoted"||e[i].kind==="ident");){if(s.push(e[i].text),i++,i<o&&e[i].text==="."){i++;continue}break}return i<o&&e[i].text==="("?{node:null,next:skipParen(e,i,o)}:{node:{name:s.join("."),kind:"ref"},next:i}}return{node:null,next:t+1}}function skipAlias(e,t,o){if(t<o&&kw(e[t])==="AS"){const n=e[t+1];return n&&(n.kind==="ident"||n.kind==="quoted")?t+2:t+1}return t<o&&e[t].kind==="ident"&&!STOP.has(e[t].text.toUpperCase())?t+1:t}function readOnKeys(e,t,o){const n=[];let s=t;for(;s<o;){const i=kw(e[s]);if(i&&STOP.has(i)||i==="LEFT"||i==="RIGHT"||i==="FULL"||i==="INNER"||i==="CROSS")break;if(e[s].text==="="){const c=(u,d,p)=>u&&d&&p&&d.text==="."?p.text:null,l=c(e[s-3],e[s-2],e[s-1]),a=c(e[s+1],e[s+2],e[s+3]);l&&a&&n.push(l===a?l:l+" = "+a)}s++}return{keys:n,next:s}}function readUsingKeys(e,t,o){const n=[];let s=t;if(e[s]&&e[s].text==="("){const i=e[s].depth;for(s++;s<o&&!(e[s].text===")"&&e[s].depth===i);)e[s].kind==="ident"&&n.push(e[s].text),s++;s++}return{keys:n,next:s}}function joinKind(e,t,o){for(let n=t-1;n>=o&&t-n<=3;n--){const s=kw(e[n]);if(s==="LEFT"||s==="RIGHT"||s==="FULL"||s==="CROSS")return s;if(s==="INNER")return"INNER"}return"INNER"}function scanScope(e,t,o,n){const s=[];for(let i=t;i<o;i++){const c=kw(e[i]);if(c!=="FROM"&&c!=="JOIN")continue;const l=e[i].depth>n,a=c==="JOIN"?joinKind(e,i,t):null;let u=i+1;for(;;){const d=readSource(e,u,o);if(u=d.next,d.node){u=skipAlias(e,u,o);let p=[];if(!l&&u<o&&kw(e[u])==="ON"){const h=readOnKeys(e,u+1,o);p=h.keys,u=h.next}else if(!l&&u<o&&kw(e[u])==="USING"){const h=readUsingKeys(e,u+1,o);p=h.keys,u=h.next}d.node.kind!=="unnest"&&s.push({name:d.node.name,kind:d.node.kind,joinType:a,keys:p,nested:l})}if(u<o&&e[u].text===","&&e[u].depth===e[i].depth){u++;continue}break}i=Math.max(i,u-1)}return s}function buildGraph(e,t){const o=new Map((t||[]).map(f=>[f.name,f])),n=f=>{const m=Object.keys(f.values);return m.length?f.values[m[0]]:null},s=new Map,i=String(e).replace(/\{\{(P\d+)\}\}/g,(f,m)=>{const E=o.get(m);if(!E)return f;const k=n(E);return k==null?f:(s.set(k,E),k)}),c=prepare(i),{ctes:l,mainFrom:a}=cteRanges(c),u=new Set(l.map(f=>f.name)),d=new Map,p=[],h=(f,m)=>{const E=m+":"+f;if(!d.has(E)){const k=[],S=s.get(String(f));if(S)k.push(S);else for(const A of String(f).split(".")){const r=s.get(A);r&&k.indexOf(r)<0&&k.push(r)}d.set(E,{id:E,name:f,label:f,kind:m,params:k})}return E},F=l.map(f=>({id:h(f.name,"cte"),from:f.from,to:f.to,depth:f.depth}));F.push({id:h("(\u6700\u7D42 SELECT)","output"),from:a,to:c.length,depth:0});for(const f of F)for(const m of scanScope(c,f.from,f.to,f.depth)){const E=m.kind==="ref"?u.has(m.name)?"cte":"table":m.kind,k=h(m.name,E);if(k===f.id)continue;const S=p.find(A=>A.from===k&&A.to===f.id);S?(S.keys.length||(S.keys=m.keys),S.joinType||(S.joinType=m.joinType),S.nested=S.nested&&m.nested):p.push({from:k,to:f.id,joinType:m.joinType,keys:m.keys,nested:m.nested})}return{nodes:[...d.values()],edges:p}}function layout(e){const{nodes:t,edges:o}=e,n=new Map(t.map(r=>[r.id,[]])),s=new Map(t.map(r=>[r.id,[]]));for(const r of o)n.has(r.to)&&n.get(r.to).push(r.from),s.has(r.from)&&s.get(r.from).push(r.to);const i=new Map(t.map(r=>[r.id,0]));for(let r=0;r<t.length+1;r++){let g=!1;for(const x of t){const T=n.get(x.id);if(!T.length)continue;const N=Math.max(...T.map(I=>i.get(I)||0))+1;N>i.get(x.id)&&(i.set(x.id,N),g=!0)}if(!g)break}const c=Math.max(0,...t.map(r=>i.get(r.id))),l=new Map(t.map(r=>[r.id,s.get(r.id).length?1/0:c]));for(let r=0;r<t.length+1;r++){let g=!1;for(const x of t){const T=s.get(x.id);if(!T.length)continue;const N=Math.min(...T.map(I=>l.get(I)));N-1<l.get(x.id)&&(l.set(x.id,N-1),g=!0)}if(!g)break}for(const r of t)Number.isFinite(l.get(r.id))||l.set(r.id,c);const a=[];for(const r of t){const g=Math.max(0,l.get(r.id));(a[g]||(a[g]=[])).push(r)}const u=new Map;a.forEach((r,g)=>{r&&(g>0&&r.sort((x,T)=>{const N=I=>{const y=n.get(I.id).filter(R=>u.has(R));return y.length?y.reduce((R,M)=>R+u.get(M),0)/y.length:Number.MAX_SAFE_INTEGER};return N(x)-N(T)}),r.forEach((x,T)=>u.set(x.id,T)))});const d=boxWidth(t),p=new Map;a.forEach((r,g)=>(r||[]).forEach(x=>p.set(x.id,g)));const h=new Array(Math.max(0,a.length-1)).fill(GAP_MIN);for(const r of o){const g=(p.get(r.to)||0)-1;g<0||g>=h.length||(h[g]=Math.max(h[g],linesWidth(edgeLinesShown(r))+22))}const F=[];let f=PAD;for(let r=0;r<a.length;r++)F[r]=f,f+=d+(h[r]||0);const m=[];a.forEach((r,g)=>{(r||[]).forEach((x,T)=>{m.push(O(w({},x),{x:F[g],y:PAD+T*(BOX_H+GAP_Y),w:d,h:BOX_H}))})});const E=new Map(m.map(r=>[r.id,r])),k=Math.max(1,...a.map(r=>(r||[]).length));let S=0,A=PAD*2+k*BOX_H+Math.max(0,k-1)*GAP_Y+6;for(const r of o){const g=E.get(r.from),x=E.get(r.to);if(!g||!x)continue;const T=edgeLinesShown(r);if(!T.length)continue;const N=(g.y+g.h/2+x.y+x.h/2)/2,I=T.length*LINE_H;S=Math.min(S,N-I/2-PAD),A=Math.max(A,N+I/2+PAD)}return{nodes:m,edges:o.map(r=>O(w({},r),{a:E.get(r.from),b:E.get(r.to)})).filter(r=>r.a&&r.b),gaps:h,colOf:p,y0:S,width:(F[a.length-1]||PAD)+d+PAD,height:A-S}}const KIND={table:{text:"\u5B9F\u30C6\u30FC\u30D6\u30EB",fill:"#FFFFFF",stroke:"#8C6D3F",bar:"#C9A227"},cte:{text:"CTE",fill:"#FFFFFF",stroke:"#3F6D8C",bar:"#4E8FBF"},output:{text:"\u6700\u7D42 SELECT",fill:"#FFFFFF",stroke:"#6D3F8C",bar:"#8250DF"},subquery:{text:"\u30B5\u30D6\u30AF\u30A8\u30EA",fill:"#FFFFFF",stroke:"#6E7781",bar:"#9AA4AE"}},kindOf=e=>KIND[e]||KIND.subquery;function shortName(e){const t=String(e).replace(/`/g,""),o=t.lastIndexOf(".");return o>=0?t.slice(o+1):t}function boxWidth(e){let t=BOX_W_MIN;for(const o of e)t=Math.max(t,shortName(o.label).length*NAME_CHAR_W+24);return Math.min(t,BOX_W_MAX)}function fit(e,t){const o=String(e),n=Math.floor((t-24)/NAME_CHAR_W);return o.length>n?o.slice(0,n-1)+"\u2026":o}function edgeLines(e){const t=[];e.joinType&&t.push(e.joinType==="INNER"?"JOIN":e.joinType+" JOIN");for(const o of e.keys||[])t.push(o);return!t.length&&e.nested&&t.push("\u30B5\u30D6\u30AF\u30A8\u30EA"),t}const MAX_EDGE_LINES=4;function edgeLinesShown(e){const t=edgeLines(e);if(t.length<=MAX_EDGE_LINES)return t;const o=t.slice(0,MAX_EDGE_LINES-1);return o.push(`\u307B\u304B ${t.length-o.length} \u4EF6`),o}function linesWidth(e){let t=0;for(const o of e)t=Math.max(t,o.length*CHAR_W);return t}function edgeLabel(e){const t=edgeLines(e);return t.length?t.length>1?t[0]+" / "+t.slice(1).join(", "):t[0]:""}function toSvg(e){const t=[];t.push(`<svg viewBox="0 ${(e.y0||0).toFixed(1)} ${e.width} ${e.height}" width="${e.width}" height="${e.height}" role="img" aria-label="\u53C2\u7167\u95A2\u4FC2\u56F3" xmlns="http://www.w3.org/2000/svg">`),t.push('<defs><marker id="vgarrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="7" markerHeight="7" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#8C96A0"/></marker></defs>');const o=[];for(const n of e.edges){const s=n.a.x+n.a.w,i=n.a.y+n.a.h/2,c=n.b.x,l=n.b.y+n.b.h/2,a=e.gaps&&e.gaps[(e.colOf.get(n.to)||0)-1]||GAP_MIN,u=c>s?c-a/2:s+a/2,d=`M${s},${i} H${u} V${l} H${c}`,p=edgeLinesShown(n);t.push(`<path d="${d}" fill="none" stroke="#8C96A0" stroke-width="1.2" ${n.nested?'stroke-dasharray="4 3" ':""}marker-end="url(#vgarrow)">`+(p.length?`<title>${esc(edgeLabel(n))}</title>`:"")+"</path>"),p.length&&o.push({x:u,y:(i+l)/2,lines:p,full:edgeLabel(n)})}for(const n of o){const s=linesWidth(n.lines)+12,i=n.lines.length*LINE_H;t.push(`<rect x="${(n.x-s/2).toFixed(1)}" y="${(n.y-i/2).toFixed(1)}" width="${s.toFixed(1)}" height="${i}" rx="2" fill="#FFFFFF" opacity="0.92"/>`)}for(const n of o){const s=n.y-n.lines.length*LINE_H/2,i=n.lines.map((c,l)=>`<tspan x="${n.x}" y="${(s+LINE_H*l+11).toFixed(1)}">${esc(c)}</tspan>`).join("");t.push(`<text text-anchor="middle" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" fill="#57606A"><title>${esc(n.full)}</title>${i}</text>`)}for(const n of e.nodes){const s=kindOf(n.kind),i=n.tipLines||n.params.map(l=>l.name+": "+Object.keys(l.values).map(a=>a+" = "+l.values[a]).join(" / ")),c=n.label+(i.length?`
`+i.join(`
`):"");t.push(`<g><title>${esc(c)}</title>`),t.push(`<rect x="${n.x}" y="${n.y}" width="${n.w}" height="${n.h}" rx="5" fill="${s.fill}" stroke="${s.stroke}" stroke-width="1"/>`),t.push(`<rect x="${n.x}" y="${n.y}" width="4" height="${n.h}" rx="2" fill="${s.bar}"/>`),t.push(`<text x="${n.x+11}" y="${n.y+17}" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" font-weight="600" fill="#24292F">${esc(fit(shortName(n.label),n.w))}</text>`),t.push(`<text x="${n.x+11}" y="${n.y+31}" font-family="Roboto,system-ui,sans-serif" font-size="9" fill="#8C96A0">${esc(s.text)}${n.params.length?" \u30FB\u30D1\u30E9\u30E1\u30FC\u30BF":""}</text>`),t.push("</g>")}return t.push("</svg>"),t.join("")}function refBase(e,t){const o=shortName(e);if(!t)return o;const n="_"+t;return o.length>n.length&&o.slice(-n.length)===n?o.slice(0,-n.length):o}function erdSignature(e){const t=(e.suffixes||[])[0]||null,o=buildGraph(e.sql,e.params),n=new Map(o.nodes.map(s=>[s.id,refBase(s.name,t)+"/"+s.kind]));return JSON.stringify({n:o.nodes.map(s=>n.get(s.id)).sort(),e:o.edges.map(s=>n.get(s.from)+">"+n.get(s.to)+":"+(s.joinType||"")+":"+(s.keys||[]).join("&")).sort()})}function erdGroups(e){const t=new Map,o=[];for(const n of e.groups||[]){const s=erdSignature(n);let i=t.get(s);i||(i={groups:[],suffixes:[],members:[]},t.set(s,i),o.push(i)),i.groups.push(n);for(let c=0;c<(n.suffixes||[]).length;c++)i.suffixes.push(n.suffixes[c]),i.members.push((n.members||[])[c])}return o}function commonStem(e){const t=e.filter(n=>n!=null).map(String);if(!t.length)return"";if(t.every(n=>n===t[0]))return t[0];let o=0;for(;o<t[0].length&&t.every(n=>n[o]===t[0][o]);)o++;return t[0].slice(0,o)+"*"}function groupSvg(e){const t=e.groups||[e],o=buildGraph(t[0].sql,t[0].params),n=(t[0].suffixes||[])[0]||null;for(const s of o.nodes){const i=refBase(s.name,n),c=[],l=[],a=[];for(const u of t)for(const d of u.params||[]){const p=Object.keys(d.values);if(p.some(h=>refBase(d.values[h],h)===i)){a.push(d);for(const h of p)c.push(shortName(d.values[h]));l.push((t.length>1?label(u)+" / ":"")+d.name+": "+p.map(h=>h+" = "+d.values[h]).join(" / "))}}if(!a.length){s.params=[],s.tipLines=[];continue}s.label=commonStem(c),s.params=a,s.tipLines=l}return toSvg(layout(o))}function erdStack(e){return e.map(t=>`<div class="vg-erdblock"><div class="vg-erdhead"><span class="vg-erdname">${esc(label(t))}</span><span class="vg-tabn">${t.members.length}</span></div><div class="vg-erdbox">${groupSvg(t)}</div></div>`).join("")}function erdLegend(){const e=(t,o)=>`<span class="vg-lg"><span class="vg-lgm ${t}"></span>${esc(o)}</span>`;return'<div class="vg-legend">'+e("vg-lgt","\u5B9F\u30C6\u30FC\u30D6\u30EB")+e("vg-lgc","CTE")+e("vg-lgo","\u6700\u7D42 SELECT")+'<span class="vg-lg"><span class="vg-lgd"></span>\u30B5\u30D6\u30AF\u30A8\u30EA\u7D4C\u7531\u306E\u53C2\u7167</span></div>'}function renderErd(e){if(!e.groups.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const t=erdGroups(e),o=t.length<e.groups.length;return notice("FROM / JOIN \u304B\u3089\u8D77\u3053\u3057\u305F\u53C2\u7167\u95A2\u4FC2\u3067\u3059\u3002\u77E2\u5370\u306F\u300C\u8AAD\u3093\u3067\u4F5C\u308B\u300D\u5411\u304D\u3001\u6CE8\u8A18\u306F JOIN \u306E\u7A2E\u5225\u3068\u7D50\u5408\u30AD\u30FC\u3002\u30AB\u30FC\u30C7\u30A3\u30CA\u30EA\u30C6\u30A3\u3068\u4E3B\u30AD\u30FC\u306F SQL \u304B\u3089\u306F\u5206\u304B\u3089\u306A\u3044\u306E\u3067\u63CF\u3044\u3066\u3044\u307E\u305B\u3093\u3002\u3053\u3053\u3067\u306E\u62EC\u308A\u306F\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206\u3068\u306F\u5225\u3067\u3059\u3002\u53C2\u7167\u540D\u306E base \u90E8\u5206\uFF08suffix \u3092\u9664\u3044\u305F\u5B9F\u4F53\u540D\uFF09\u304C\u540C\u3058\u3082\u306E\u306F 1 \u679A\u306B\u307E\u3068\u3081\u3066\u3044\u307E\u3059\u3002"+(o?`\u30ED\u30B8\u30C3\u30AF\u306F ${e.groups.length} \u30B0\u30EB\u30FC\u30D7\u306B\u5272\u308C\u3066\u3044\u307E\u3059\u304C\u3001\u53C2\u7167\u95A2\u4FC2\u306F ${t.length} \u901A\u308A\u3067\u3059\u3002`:"")+"\u540D\u524D\u304C View \u3054\u3068\u306B\u9055\u3046\u7B87\u6240\u306F\u5171\u901A\u90E8\u5206\u3092\u51FA\u3057\u3066\u6B8B\u308A\u3092 * \u306B\u3057\u3066\u3044\u307E\u3059\uFF08\u5B9F\u969B\u306E\u5BFE\u5FDC\u306F\u7BB1\u306B\u30AB\u30FC\u30BD\u30EB\u3092\u4E57\u305B\u308B\u3068\u51FA\u307E\u3059\uFF09\u3002")+erdLegend()+erdStack(t)}function renderErdBase(e){return'<div class="vg-root">'+header(e.base,e.viewCount,e.groups.length,e.unmatched)+renderErd(e)+"</div>"}function __opts(e){if(!e)return{};try{return JSON.parse(e)||{}}catch(t){return{}}}function __notice(e){return'<div class="vg-notice">'+String(e).replace(/[<>&]/g,"")+"</div>"}function __run(e,t){var o;try{o=JSON.parse(e)}catch(c){o=null}if(!o)return __notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002");for(var n="",s=o.bases||[],i=0;i<s.length;i++)n+=renderErdBase(s[i]);return n||__notice("\u56F3\u306B\u3067\u304D\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")}return __run(analysis_json,options_json);
