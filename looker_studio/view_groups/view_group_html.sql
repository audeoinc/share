...
var L=Object.defineProperty,_=Object.defineProperties;var M=Object.getOwnPropertyDescriptors;var I=Object.getOwnPropertySymbols;var C=Object.prototype.hasOwnProperty,j=Object.prototype.propertyIsEnumerable;var O=(t,e,n)=>e in t?L(t,e,{enumerable:!0,configurable:!0,writable:!0,value:n}):t[e]=n,F=(t,e)=>{for(var n in e||(e={}))C.call(e,n)&&O(t,n,e[n]);if(I)for(var n of I(e))j.call(e,n)&&O(t,n,e[n]);return t},y=(t,e)=>_(t,M(e));const MAX_TABS=12,MAX_REF_TABS=12,MAX_SQL_TABS=24,REF_BUDGET=41943040,OUTER_TABS=["note","\u30AB\u30E9\u30E0\u5B9A\u7FA9","\u53C2\u7167\u95A2\u4FC2","\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206","SQL"],MAX_OUTER_TABS=8,NOTE_MARK="<!--VG_NOTE-->";function esc(t){return String(t==null?"":t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function hashId(t){let e=2166136261;for(let n=0;n<String(t).length;n++)e^=String(t).charCodeAt(n),e=Math.imul(e,16777619)>>>0;return e.toString(36)}const label=t=>t.suffixes.map((e,n)=>e||t.members[n]&&t.members[n].viewName||"(suffix \u306A\u3057)").join(", ");function badge(t,e,n){return`<span class="vg-badge" style="color:${e};background:${n}">${esc(t)}</span>`}function header(t,e,n,s){const o=n>1;return`<div class="vg-header"><span class="vg-title">${esc(t)}</span>`+badge(`${e} View`,"#57606A","#EAEEF2")+badge(`${n} \u30B0\u30EB\u30FC\u30D7`,o?"#9A6700":"#1A7F37",o?"#FFF8C5":"#DAFBE1")+(s?badge("suffix \u672A\u8A8D\u8B58","#9A6700","#FFF8C5"):"")+"</div>"}function notice(t){return`<div class="vg-notice">${esc(t)}</div>`}const KIND_TEXT={entity:"\u5B9F\u4F53\u540D",string:"\u5024\uFF08\u6587\u5B57\u5217\uFF09",number:"\u5024\uFF08\u6570\u5024\uFF09",ident:"\u540D\u524D",quoted:"\u540D\u524D",keyword:"\u4E88\u7D04\u8A9E",punct:"\u8A18\u53F7",comment:"\u30B3\u30E1\u30F3\u30C8"},kindText=t=>KIND_TEXT[t]||t;function wrapPage(t,e,n,s,o){const i=o||{},r="vgo"+hashId(i.base||""),u=[`<div class="vg-root">${NOTE_MARK}</div>`,n,e,t,s],c=OUTER_TABS.map((h,g)=>`<input class="vg-or vg-or${g+1}" type="radio" name="${r}" id="${r}-${g+1}"${g===0?" checked":""}>`).join(""),l=OUTER_TABS.map((h,g)=>`<label class="vg-otab vg-ot${g+1}" for="${r}-${g+1}">${esc(h)}</label>`).join(""),f=OUTER_TABS.map((h,g)=>`<div class="vg-opanel vg-op${g+1}">${u[g]||""}</div>`).join(""),p=i.base?header(i.base,i.viewCount,(i.groups||[]).length,i.unmatched):"";return`<div class="vg-outer">${c}<div class="vg-otablist">${p}${l}</div><div class="vg-opanels">${f}</div></div>`}const DQ="\\u0022",DQ3=DQ+DQ+DQ,TOKEN_RE=new RegExp(["(`[^`]*`)","([rbRB]{1,2}(?:'''[\\s\\S]*?'''|"+DQ3+"[\\s\\S]*?"+DQ3+"|'[^']*'|"+DQ+"[^"+DQ+"]*"+DQ+"))","('''[\\s\\S]*?''')","("+DQ3+"[\\s\\S]*?"+DQ3+")","('(?:\\\\.|''|[^'\\\\])*')","("+DQ+"(?:\\\\.|"+DQ+DQ+"|[^"+DQ+"\\\\])*"+DQ+")","(--[^\\n]*|#[^\\n]*)","(/\\*[\\s\\S]*?\\*/)","(0[xX][0-9a-fA-F]+|(?:\\d+(?:\\.\\d+)?|\\.\\d+)(?:[eE][+-]?\\d+)?)","([A-Za-z_][A-Za-z0-9_]*)","(\\s+)","([^\\s])"].join("|"),"g"),KEYWORDS=new Set("SELECT FROM WHERE JOIN INNER LEFT RIGHT FULL OUTER CROSS ON USING AND OR NOT IN IS NULL LIKE BETWEEN EXISTS ANY ALL SOME GROUP BY HAVING ORDER ASC DESC LIMIT OFFSET QUALIFY WINDOW UNION INTERSECT EXCEPT DISTINCT AS CASE WHEN THEN ELSE END CREATE OR REPLACE VIEW TABLE FUNCTION IF EXISTS OPTIONS WITH RECURSIVE OVER PARTITION UNNEST STRUCT ARRAY CAST SAFE_CAST EXTRACT INTERVAL DATE DATETIME TIME TIMESTAMP INT64 FLOAT64 NUMERIC BIGNUMERIC STRING BYTES BOOL TRUE FALSE COUNT SUM AVG MIN MAX COALESCE IFNULL NULLIF ROWS RANGE PRECEDING FOLLOWING CURRENT ROW".split(/\s+/));function tokenizeSql(t){const e=[],n=String(t==null?"":t);let s;for(TOKEN_RE.lastIndex=0;(s=TOKEN_RE.exec(n))!==null;){let o;s[1]?o="quoted":s[2]||s[3]||s[4]||s[5]||s[6]?o="string":s[7]||s[8]?o="comment":s[9]?o="number":s[10]?o=KEYWORDS.has(s[10].toUpperCase())?"keyword":"ident":s[11]?o="space":o="punct",e.push({kind:o,text:s[0]})}return e}function markEntities(t){const e=t.slice(),n=i=>{let r=i+1;for(;r<e.length&&e[r].kind==="space";)r++;return r},s=[];let o=null;for(let i=0;i<e.length;i++){const r=e[i];if(r.kind==="space"||r.kind==="comment")continue;if(r.text==="("){s.push(o),o=null;continue}if(r.text===")"){s.pop(),o=null;continue}const u=r.text.toUpperCase();if(o=r.kind==="keyword"||r.kind==="ident"?u:null,r.kind!=="keyword"||u!=="FROM"&&u!=="JOIN"||s.length>0&&s[s.length-1]==="EXTRACT")continue;let c=n(i);for(;!(c>=e.length);){const l=e[c];if(l.kind==="quoted")e[c]={kind:"entity",text:l.text},c=n(c);else if(l.kind==="ident"){const f=[];let p=c;for(;;){f.push(p);const g=n(p);if(g<e.length&&e[g].text==="."){const d=n(g);if(d<e.length&&e[d].kind==="ident"){p=d;continue}}break}const h=n(p);if(h<e.length&&e[h].text==="(")break;for(const g of f)e[g]={kind:"entity",text:e[g].text};c=h}else break;if(c<e.length&&e[c].kind==="keyword"&&e[c].text.toUpperCase()==="AS"){const f=n(c);f<e.length&&(e[f].kind==="ident"||e[f].kind==="quoted")&&(c=n(f))}else c<e.length&&e[c].kind==="ident"&&(c=n(c));if(c<e.length&&e[c].text===","){c=n(c);continue}break}}return e}const DEFAULT_SUFFIX_RE=/^(.*?)_([A-Za-z0-9]{1,6})$/,DEFAULT_SUBSTITUTABLE=["entity","number","string"],SUFFIX_MARK="\0",LITERAL_MARK="\u0001",BOX_W_MIN=130,BOX_W_MAX=380,NAME_CHAR_W=6.65,BOX_H=40,GAP_MIN=78,CHAR_W=6.5,LINE_H=14,GAP_Y=14,PAD=10,kw=t=>t&&t.kind==="keyword"?t.text.toUpperCase():null;function prepare(t){const e=[];let n=0;for(const s of markEntities(tokenizeSql(t)))s.kind==="space"||s.kind==="comment"||(s.text===")"&&n--,e.push({kind:s.kind,text:s.text,depth:n}),s.text==="("&&n++);return e}function cteRanges(t){const e=[];if(!(t[0]&&kw(t[0])==="WITH"))return{ctes:e,mainFrom:0};let n=1;for(;;){const s=t[n],o=t[n+1],i=t[n+2];if(!i||s.kind!=="ident"||kw(o)!=="AS"||i.text!=="(")break;const r=i.depth;let u=n+3;for(;u<t.length&&!(t[u].text===")"&&t[u].depth===r);)u++;if(e.push({name:s.text,from:n+3,to:u,depth:r+1}),n=u+1,t[n]&&t[n].text===","){n++;continue}break}return{ctes:e,mainFrom:n}}const STOP=new Set(["WHERE","GROUP","ORDER","QUALIFY","WINDOW","HAVING","UNION","INTERSECT","EXCEPT","LIMIT","SELECT","JOIN","FROM"]);function skipParen(t,e,n){const s=t[e].depth;let o=e+1;for(;o<n&&!(t[o].text===")"&&t[o].depth===s);)o++;return o+1}function readSource(t,e,n){if(e>=n)return{node:null,next:e};const s=t[e];if(s.text==="(")return{node:{name:"(\u30B5\u30D6\u30AF\u30A8\u30EA)",kind:"subquery"},next:skipParen(t,e,n)};if(kw(s)==="UNNEST"){let o=e+1,i="";if(t[o]&&t[o].text==="("){const r=t[o].depth;let u=o+1;for(;u<n&&!(t[u].text===")"&&t[u].depth===r);)i+=t[u].text,u++;o=u+1}return{node:{name:"UNNEST("+i+")",kind:"unnest"},next:o}}if(s.kind==="entity"||s.kind==="quoted"||s.kind==="ident"){const o=[];let i=e;for(;i<n&&(t[i].kind==="entity"||t[i].kind==="quoted"||t[i].kind==="ident");){if(o.push(t[i].text),i++,i<n&&t[i].text==="."){i++;continue}break}return i<n&&t[i].text==="("?{node:null,next:skipParen(t,i,n)}:{node:{name:o.join("."),kind:"ref"},next:i}}return{node:null,next:e+1}}function skipAlias(t,e,n){if(e<n&&kw(t[e])==="AS"){const s=t[e+1];return s&&(s.kind==="ident"||s.kind==="quoted")?e+2:e+1}return e<n&&t[e].kind==="ident"&&!STOP.has(t[e].text.toUpperCase())?e+1:e}function readOnKeys(t,e,n){const s=[];let o=e;for(;o<n;){const i=kw(t[o]);if(i&&STOP.has(i)||i==="LEFT"||i==="RIGHT"||i==="FULL"||i==="INNER"||i==="CROSS")break;if(t[o].text==="="){const r=(l,f,p)=>l&&f&&p&&f.text==="."?p.text:null,u=r(t[o-3],t[o-2],t[o-1]),c=r(t[o+1],t[o+2],t[o+3]);u&&c&&s.push(u===c?u:u+" = "+c)}o++}return{keys:s,next:o}}function readUsingKeys(t,e,n){const s=[];let o=e;if(t[o]&&t[o].text==="("){const i=t[o].depth;for(o++;o<n&&!(t[o].text===")"&&t[o].depth===i);)t[o].kind==="ident"&&s.push(t[o].text),o++;o++}return{keys:s,next:o}}function joinKind(t,e,n){for(let s=e-1;s>=n&&e-s<=3;s--){const o=kw(t[s]);if(o==="LEFT"||o==="RIGHT"||o==="FULL"||o==="CROSS")return o;if(o==="INNER")return"INNER"}return"INNER"}function scanScope(t,e,n,s){const o=[];for(let i=e;i<n;i++){const r=kw(t[i]);if(r!=="FROM"&&r!=="JOIN")continue;const u=t[i].depth>s,c=r==="JOIN"?joinKind(t,i,e):null;let l=i+1;for(;;){const f=readSource(t,l,n);if(l=f.next,f.node){l=skipAlias(t,l,n);let p=[];if(!u&&l<n&&kw(t[l])==="ON"){const h=readOnKeys(t,l+1,n);p=h.keys,l=h.next}else if(!u&&l<n&&kw(t[l])==="USING"){const h=readUsingKeys(t,l+1,n);p=h.keys,l=h.next}f.node.kind!=="unnest"&&o.push({name:f.node.name,kind:f.node.kind,joinType:c,keys:p,nested:u})}if(l<n&&t[l].text===","&&t[l].depth===t[i].depth){l++;continue}break}i=Math.max(i,l-1)}return o}function buildGraph(t,e){const n=new Map((e||[]).map(d=>[d.name,d])),s=d=>{const v=Object.keys(d.values);return v.length?d.values[v[0]]:null},o=new Map,i=String(t).replace(/\{\{(P\d+)\}\}/g,(d,v)=>{const $=n.get(v);if(!$)return d;const S=s($);return S==null?d:(o.set(S,$),S)}),r=prepare(i),{ctes:u,mainFrom:c}=cteRanges(r),l=new Set(u.map(d=>d.name)),f=new Map,p=[],h=(d,v)=>{const $=v+":"+d;if(!f.has($)){const S=[],m=o.get(String(d));if(m)S.push(m);else for(const N of String(d).split(".")){const a=o.get(N);a&&S.indexOf(a)<0&&S.push(a)}f.set($,{id:$,name:d,label:d,kind:v,params:S})}return $},g=u.map(d=>({id:h(d.name,"cte"),from:d.from,to:d.to,depth:d.depth}));g.push({id:h("(\u6700\u7D42 SELECT)","output"),from:c,to:r.length,depth:0});for(const d of g)for(const v of scanScope(r,d.from,d.to,d.depth)){const $=v.kind==="ref"?l.has(v.name)?"cte":"table":v.kind,S=h(v.name,$);if(S===d.id)continue;const m=p.find(N=>N.from===S&&N.to===d.id);m?(m.keys.length||(m.keys=v.keys),m.joinType||(m.joinType=v.joinType),m.nested=m.nested&&v.nested):p.push({from:S,to:d.id,joinType:v.joinType,keys:v.keys,nested:v.nested})}return{nodes:[...f.values()],edges:p}}function layout(t){const{nodes:e,edges:n}=t,s=new Map(e.map(a=>[a.id,[]])),o=new Map(e.map(a=>[a.id,[]]));for(const a of n)s.has(a.to)&&s.get(a.to).push(a.from),o.has(a.from)&&o.get(a.from).push(a.to);const i=new Map(e.map(a=>[a.id,0]));for(let a=0;a<e.length+1;a++){let x=!1;for(const E of e){const k=s.get(E.id);if(!k.length)continue;const w=Math.max(...k.map(b=>i.get(b)||0))+1;w>i.get(E.id)&&(i.set(E.id,w),x=!0)}if(!x)break}const r=Math.max(0,...e.map(a=>i.get(a.id))),u=new Map(e.map(a=>[a.id,o.get(a.id).length?1/0:r]));for(let a=0;a<e.length+1;a++){let x=!1;for(const E of e){const k=o.get(E.id);if(!k.length)continue;const w=Math.min(...k.map(b=>u.get(b)));w-1<u.get(E.id)&&(u.set(E.id,w-1),x=!0)}if(!x)break}for(const a of e)Number.isFinite(u.get(a.id))||u.set(a.id,r);const c=[];for(const a of e){const x=Math.max(0,u.get(a.id));(c[x]||(c[x]=[])).push(a)}const l=new Map;c.forEach((a,x)=>{a&&(x>0&&a.sort((E,k)=>{const w=b=>{const T=s.get(b.id).filter(A=>l.has(A));return T.length?T.reduce((A,R)=>A+l.get(R),0)/T.length:Number.MAX_SAFE_INTEGER};return w(E)-w(k)}),a.forEach((E,k)=>l.set(E.id,k)))});const f=boxWidth(e),p=new Map;c.forEach((a,x)=>(a||[]).forEach(E=>p.set(E.id,x)));const h=new Array(Math.max(0,c.length-1)).fill(GAP_MIN);for(const a of n){const x=(p.get(a.to)||0)-1;x<0||x>=h.length||(h[x]=Math.max(h[x],linesWidth(edgeLines(a))+22))}const g=[];let d=PAD;for(let a=0;a<c.length;a++)g[a]=d,d+=f+(h[a]||0);const v=[];c.forEach((a,x)=>{(a||[]).forEach((E,k)=>{v.push(y(F({},E),{x:g[x],y:PAD+k*(BOX_H+GAP_Y),w:f,h:BOX_H}))})});const $=new Map(v.map(a=>[a.id,a])),S=Math.max(1,...c.map(a=>(a||[]).length));let m=0,N=PAD*2+S*BOX_H+Math.max(0,S-1)*GAP_Y+6;for(const a of n){const x=$.get(a.from),E=$.get(a.to);if(!x||!E)continue;const k=edgeLines(a);if(!k.length)continue;const w=(x.y+x.h/2+E.y+E.h/2)/2,b=k.length*LINE_H;m=Math.min(m,w-b/2-PAD),N=Math.max(N,w+b/2+PAD)}return{nodes:v,edges:n.map(a=>y(F({},a),{a:$.get(a.from),b:$.get(a.to)})).filter(a=>a.a&&a.b),gaps:h,colOf:p,y0:m,width:(g[c.length-1]||PAD)+f+PAD,height:N-m}}const KIND={table:{text:"\u5B9F\u30C6\u30FC\u30D6\u30EB",fill:"#FFFFFF",stroke:"#8C6D3F",bar:"#C9A227"},cte:{text:"CTE",fill:"#FFFFFF",stroke:"#3F6D8C",bar:"#4E8FBF"},output:{text:"\u6700\u7D42 SELECT",fill:"#FFFFFF",stroke:"#6D3F8C",bar:"#8250DF"},subquery:{text:"\u30B5\u30D6\u30AF\u30A8\u30EA",fill:"#FFFFFF",stroke:"#6E7781",bar:"#9AA4AE"}},kindOf=t=>KIND[t]||KIND.subquery;function shortName(t){const e=String(t).replace(/`/g,""),n=e.lastIndexOf(".");return n>=0?e.slice(n+1):e}function boxWidth(t){let e=BOX_W_MIN;for(const n of t)e=Math.max(e,shortName(n.label).length*NAME_CHAR_W+24);return Math.min(e,BOX_W_MAX)}function fit(t,e){const n=String(t),s=Math.floor((e-24)/NAME_CHAR_W);return n.length>s?n.slice(0,s-1)+"\u2026":n}function edgeLines(t){const e=[];t.joinType&&e.push(t.joinType==="INNER"?"JOIN":t.joinType+" JOIN");for(const n of t.keys||[])e.push(n);return!e.length&&t.nested&&e.push("\u30B5\u30D6\u30AF\u30A8\u30EA"),e}function linesWidth(t){let e=0;for(const n of t)e=Math.max(e,n.length*CHAR_W);return e}function edgeLabel(t){const e=edgeLines(t);return e.length?e.length>1?e[0]+" / "+e.slice(1).join(", "):e[0]:""}function toSvg(t){const e=[];e.push(`<svg viewBox="0 ${(t.y0||0).toFixed(1)} ${t.width} ${t.height}" width="${t.width}" height="${t.height}" role="img" aria-label="\u53C2\u7167\u95A2\u4FC2\u56F3" xmlns="http://www.w3.org/2000/svg">`),e.push('<defs><marker id="vgarrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="7" markerHeight="7" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#8C96A0"/></marker></defs>');const n=[];for(const s of t.edges){const o=s.a.x+s.a.w,i=s.a.y+s.a.h/2,r=s.b.x,u=s.b.y+s.b.h/2,c=t.gaps&&t.gaps[(t.colOf.get(s.to)||0)-1]||GAP_MIN,l=r>o?r-c/2:o+c/2,f=`M${o},${i} H${l} V${u} H${r}`,p=edgeLines(s);e.push(`<path d="${f}" fill="none" stroke="#8C96A0" stroke-width="1.2" ${s.nested?'stroke-dasharray="4 3" ':""}marker-end="url(#vgarrow)">`+(p.length?`<title>${esc(edgeLabel(s))}</title>`:"")+"</path>"),p.length&&n.push({x:l,y:(i+u)/2,lines:p})}for(const s of n){const o=linesWidth(s.lines)+12,i=s.lines.length*LINE_H,r=s.y-i/2;e.push(`<rect x="${(s.x-o/2).toFixed(1)}" y="${r.toFixed(1)}" width="${o.toFixed(1)}" height="${i}" rx="2" fill="#FFFFFF" opacity="0.92"/>`);const u=s.lines.map((c,l)=>`<tspan x="${s.x}" y="${(r+LINE_H*l+11).toFixed(1)}">${esc(c)}</tspan>`).join("");e.push(`<text text-anchor="middle" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" fill="#57606A">${u}</text>`)}for(const s of t.nodes){const o=kindOf(s.kind),i=s.label+(s.params.length?`
`+s.params.map(r=>r.name+": "+Object.keys(r.values).map(u=>u+" = "+r.values[u]).join(" / ")).join(`
`):"");e.push(`<g><title>${esc(i)}</title>`),e.push(`<rect x="${s.x}" y="${s.y}" width="${s.w}" height="${s.h}" rx="5" fill="${o.fill}" stroke="${o.stroke}" stroke-width="1"/>`),e.push(`<rect x="${s.x}" y="${s.y}" width="4" height="${s.h}" rx="2" fill="${o.bar}"/>`),e.push(`<text x="${s.x+11}" y="${s.y+17}" font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="11" font-weight="600" fill="#24292F">${esc(fit(shortName(s.label),s.w))}</text>`),e.push(`<text x="${s.x+11}" y="${s.y+31}" font-family="Roboto,system-ui,sans-serif" font-size="9" fill="#8C96A0">${esc(o.text)}${s.params.length?" \u30FB\u30D1\u30E9\u30E1\u30FC\u30BF":""}</text>`),e.push("</g>")}return e.push("</svg>"),e.join("")}function groupSvg(t){return toSvg(layout(buildGraph(t.sql,t.params)))}function erdStack(t){return t.map(e=>`<div class="vg-erdblock"><div class="vg-erdhead"><span class="vg-erdname">${esc(label(e))}</span><span class="vg-tabn">${e.members.length}</span></div><div class="vg-erdbox">${groupSvg(e)}</div></div>`).join("")}function erdLegend(){const t=(e,n)=>`<span class="vg-lg"><span class="vg-lgm ${e}"></span>${esc(n)}</span>`;return'<div class="vg-legend">'+t("vg-lgt","\u5B9F\u30C6\u30FC\u30D6\u30EB")+t("vg-lgc","CTE")+t("vg-lgo","\u6700\u7D42 SELECT")+'<span class="vg-lg"><span class="vg-lgd"></span>\u30B5\u30D6\u30AF\u30A8\u30EA\u7D4C\u7531\u306E\u53C2\u7167</span></div>'}function renderErd(t){return t.groups.length?notice("FROM / JOIN \u304B\u3089\u8D77\u3053\u3057\u305F\u53C2\u7167\u95A2\u4FC2\u3067\u3059\u3002\u77E2\u5370\u306F\u300C\u8AAD\u3093\u3067\u4F5C\u308B\u300D\u5411\u304D\u3001\u6CE8\u8A18\u306F JOIN \u306E\u7A2E\u5225\u3068\u7D50\u5408\u30AD\u30FC\u3002\u30AB\u30FC\u30C7\u30A3\u30CA\u30EA\u30C6\u30A3\u3068\u4E3B\u30AD\u30FC\u306F SQL \u304B\u3089\u306F\u5206\u304B\u3089\u306A\u3044\u306E\u3067\u63CF\u3044\u3066\u3044\u307E\u305B\u3093\u3002\u30D1\u30E9\u30E1\u30FC\u30BF\u5316\u3057\u305F\u540D\u524D\u306F\u3001\u305D\u306E\u30B0\u30EB\u30FC\u30D7\u306E\u5148\u982D\u306E View \u306E\u5024\u3067\u8868\u793A\u3057\u3066\u3044\u307E\u3059\u3002")+erdLegend()+erdStack(t.groups):notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002")}function renderErdBase(t){return'<div class="vg-root">'+header(t.base,t.viewCount,t.groups.length,t.unmatched)+renderErd(t)+"</div>"}const WARN="\u26A0";function breakType(t){return String(t).replace(/(&lt;|,)/g,"$1<wbr>")}const DESC_JA_KEYS=["ja","jp","ja_name","name_ja","japanese","logical_name_ja","logicalNameJa","ja_logical_name","\u548C\u540D","\u65E5\u672C\u8A9E","\u65E5\u672C\u8A9E\u8AD6\u7406\u540D","\u8AD6\u7406\u540D"],DESC_EN_KEYS=["en","en_name","name_en","english","logical_name_en","logicalNameEn","en_logical_name","\u82F1\u8A9E","\u82F1\u8A9E\u8AD6\u7406\u540D"];function pickKey(t,e){for(let n=0;n<e.length;n++){const s=e[n];if(!Object.prototype.hasOwnProperty.call(t,s))continue;const o=t[s];if(!(o==null||String(o)===""))return{key:s,value:String(o)}}return null}function parseDesc(t){const e=String(t==null?"":t).trim();if(!e)return null;if(e.charAt(0)!=="{")return{raw:e};let n=null;try{n=JSON.parse(e)}catch(c){n=null}if(!n||typeof n!="object"||Array.isArray(n))return{raw:e};const s=pickKey(n,DESC_JA_KEYS),o=pickKey(n,DESC_EN_KEYS),i={};s&&(i[s.key]=!0),o&&(i[o.key]=!0);const r=[],u=Object.keys(n);for(let c=0;c<u.length;c++){const l=u[c];if(i[l])continue;const f=n[l];f==null||f===""||r.push({key:l,value:typeof f=="object"?JSON.stringify(f):String(f)})}return{ja:s?s.value:"",en:o?o.value:"",rest:r}}function descHtml(t){const e=parseDesc(t);if(!e)return"";if(e.raw)return`<div class="vg-cdesc">${esc(e.raw)}</div>`;let n="";e.ja&&(n+=`<div class="vg-cdesc">${esc(e.ja)}</div>`),e.en&&(n+=`<div class="vg-cdesc vg-cdescsub">${esc(e.en)}</div>`);for(let s=0;s<e.rest.length;s++)n+=`<div class="vg-cdesc vg-cdescsub">${esc(e.rest[s].key)}: ${esc(e.rest[s].value)}</div>`;return n}function descCell(t){return!t||!t.descs.length?"":t.descs.length===1?descHtml(t.descs[0].text):t.descs.map(e=>`<div class="vg-cdescwho">${esc(e.suffixes.join(", "))}</div>`+descHtml(e.text)).join("")}function groupColumns(t,e){const n=new Map,s=t.members||[];for(let o=0;o<s.length;o++){const i=s[o],r=t.suffixes&&t.suffixes[o]||i.viewName,u=e[i.viewName]||[];for(let c=0;c<u.length;c++){const l=u[c];let f=n.get(l.n);if(f||(f={name:l.n,descs:[],order:c,vals:[]},n.set(l.n,f)),l.d){const p=String(l.d);let h=null;for(let g=0;g<f.descs.length;g++)f.descs[g].text===p&&(h=f.descs[g]);h?h.suffixes.push(r):f.descs.push({text:p,suffixes:[r]})}f.vals.push({suffix:r,type:l.t,ord:l.o==null?c+1:l.o,nullable:l.u==null||l.u===""?null:String(l.u).toUpperCase()!=="NO"})}}return n}function columnOrder(t){const e=new Set,n=[];for(const s of t){const o=[...s.values()].sort((i,r)=>i.order-r.order);for(const i of o)e.has(i.name)||(e.add(i.name),n.push(i.name))}return n}function majority(t){const e=new Map;for(const o of t)e.set(o,(e.get(o)||0)+1);let n=null,s=-1;for(const o of t){const i=e.get(o);i>s&&(n=o,s=i)}return n}const nullText=t=>t===null?"NULL \u4E0D\u660E":t?"NULL \u53EF":"NOT NULL";function uniq(t){const e=[];for(const n of t)e.indexOf(n)<0&&e.push(n);return e}function cellInfo(t,e){if(!t)return{text:null,meta:"",sig:null,mixed:!1};const n=uniq(t.vals.map(r=>r.type)),s=uniq(t.vals.map(r=>r.ord)),o=uniq(t.vals.map(r=>nullText(r.nullable))),i=uniq(t.vals.map(r=>`${r.ord}|${r.type}|${r.nullable}`)).length>1||t.vals.length!==e;return{text:n.join(" / "),meta:`#${s.join(" / ")} \xB7 ${o.join(" / ")}`,sig:n.join(" / ")+"|"+o.join(" / "),mixed:i}}function mixedTip(t,e){const n=t.vals.map(o=>`${o.suffix} = #${o.ord} ${o.type} ${nullText(o.nullable)}`),s=t.vals.map(o=>o.suffix);for(let o=0;o<(e.members||[]).length;o++){const i=e.suffixes&&e.suffixes[o]||e.members[o].viewName;s.indexOf(i)<0&&n.push(`${i} = (\u3053\u306E\u5217\u3092\u6301\u305F\u306A\u3044)`)}return n.join(`
`)}function renderColumns(t,e){const n=t.groups||[];if(!n.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const s=n.map(g=>groupColumns(g,e||{})),o=columnOrder(s);if(!o.length)return notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.COLUMNS \u304B\u3089\u5217\u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");let i=0,r=0;const u=o.map(g=>{const d=n.map((m,N)=>cellInfo(s[N].get(g),m.members.length)),v=majority(d.map(m=>m.sig));d.some(m=>m.sig!==v)&&i++;const S=d.map((m,N)=>{const a=n[N],x=s[N].get(g),E=["vg-ccell"];m.text?m.sig!==v&&E.push("vg-cdiff"):E.push("vg-cnone"),m.mixed&&(E.push("vg-cmix"),r++);const k=m.text?breakType(esc(m.text))+(m.mixed?`<span class="vg-cwarn" data-tip="${esc(mixedTip(x,a))}">${WARN}</span>`:"")+`<div class="vg-cmeta">${esc(m.meta)}</div>`+descCell(x):"\u2014";return`<td class="${E.join(" ")}">${k}</td>`}).join("");return`<tr><th class="vg-cname">${esc(g)}</th>${S}</tr>`}).join(""),c=n.map(g=>`<th class="vg-chead">${esc(label(g))}<span class="vg-tabn">${g.members.length}</span></th>`).join(""),l=[];r&&l.push(notice(`\u540C\u3058\u30B0\u30EB\u30FC\u30D7\u306E\u4E2D\u3067\u578B\u30FBNULL \u5236\u7D04\u30FB\u4E26\u3073\u9806\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u7B87\u6240\u304C ${r} \u4EF6\u3042\u308A\u307E\u3059\uFF08${WARN} \u306E\u5370\uFF09\u3002SQL \u304C\u540C\u4E00\u3067\u3082\u53C2\u7167\u5148\u30C6\u30FC\u30D6\u30EB\u306E\u578B\u304C\u9055\u3048\u3070\u3053\u3046\u306A\u308B\u306E\u3067\u3001\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206\u306B\u306F\u51FA\u3066\u304D\u307E\u305B\u3093\u3002`)),i&&l.push(notice(`\u30B0\u30EB\u30FC\u30D7\u9593\u3067\u578B\u307E\u305F\u306F NULL \u5236\u7D04\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u5217\u304C ${i} \u4EF6\u3042\u308A\u307E\u3059\uFF08\u8272\u4ED8\u304D\u306E\u30BB\u30EB\u3002\u305D\u306E\u5217\u3067\u3044\u3061\u3070\u3093\u591A\u3044\u5024\u3068\u9055\u3046\u3082\u306E\uFF09\u3002\u4E26\u3073\u9806\uFF08#\uFF09\u306E\u9055\u3044\u306B\u306F\u8272\u3092\u4ED8\u3051\u3066\u3044\u307E\u305B\u3093\uFF08\u5217\u3092 1 \u672C\u8DB3\u3059\u3068\u4EE5\u964D\u304C\u307E\u3068\u3081\u3066\u305A\u308C\u3001\u578B\u306E\u5DEE\u304C\u57CB\u3082\u308C\u308B\u305F\u3081\uFF09\u3002`)),l.length||l.push(notice("\u5168\u30B0\u30EB\u30FC\u30D7\u3067\u5217\u540D\u30FB\u578B\u30FBNULL \u5236\u7D04\u304C\u4E00\u81F4\u3057\u3066\u3044\u307E\u3059\u3002"));const f=18,p=((100-f)/n.length).toFixed(3),h=`<colgroup><col style="width:${f}%">`+n.map(()=>`<col style="width:${p}%">`).join("")+"</colgroup>";return l.join("")+`<div class="vg-ctablewrap"><table class="vg-ctable">${h}<thead><tr><th class="vg-chead vg-cnamehead">\u5217\u540D</th>${c}</tr></thead><tbody>${u}</tbody></table></div>`}function renderColumnsBase(t,e){return`<div class="vg-root">${renderColumns(t,e)}</div>`}function sqlViews(t){const e=[],n=t.groups||[];for(let s=0;s<n.length;s++){const o=n[s],i=o.members||[];for(let r=0;r<i.length;r++)e.push({suffix:o.suffixes&&o.suffixes[r]||i[r].viewName||"(suffix \u306A\u3057)",viewName:i[r].viewName,group:label(o),groupSize:i.length})}return e.sort((s,o)=>String(s.suffix).localeCompare(String(o.suffix))),e}function sqlBody(t){const e=String(t).replace(/\r\n?/g,`
`).split(`
...
`)}</pre></div>`,lines:e.length}}function sqlPanel(t,e){if(e==null||String(e)==="")return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(t.viewName)}</span></div>`+notice("\u3053\u306E View \u306E SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002");const n=sqlBody(e);return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(t.viewName)}</span><span class="vg-sqlmeta">\u30B0\u30EB\u30FC\u30D7: ${esc(t.group)}</span><span class="vg-sqlmeta">${n.lines} \u884C</span></div>`+n.html}function renderSql(t,e){const n=sqlViews(t);if(!n.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const s=e||{};if(!n.some(f=>s[f.viewName]))return notice("SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.VIEWS \u304B\u3089 view_definition \u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const o=n.slice(0,24),i="vgs"+hashId(t.base+"|"+n.map(f=>f.viewName).join("|")),r=o.map((f,p)=>`<input class="vg-sr vg-sr${p+1}" type="radio" name="${i}" id="${i}-${p+1}"${p===0?" checked":""}>`).join(""),u=o.map((f,p)=>`<label class="vg-stab vg-st${p+1}" for="${i}-${p+1}">${esc(f.suffix)}</label>`).join(""),c=o.map((f,p)=>`<div class="vg-spanel vg-sp${p+1}">${sqlPanel(f,s[f.viewName])}</div>`).join("");return(n.length>o.length?notice(`View \u304C\u591A\u3044\u305F\u3081\u5148\u982D 24 \u4EF6\u306E\u307F\u30BF\u30D6\u8868\u793A\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${n.length} \u4EF6\uFF09\u3002`):"")+`<div class="vg-stabs">${r}<div class="vg-stablist"><span class="vg-slabel">View</span>${u}</div><div class="vg-spanels">${c}</div></div>`}function renderSqlBase(t,e){return`<div class="vg-root">${renderSql(t,e)}</div>`}function __opts(t){if(!t)return{};try{return JSON.parse(t)||{}}catch(e){return{}}}function __notice(t){return'<div class="vg-notice">'+String(t).replace(/[<>&]/g,"")+"</div>"}function __run(t,e,n,s,o){var i=__opts(o),r;try{r=JSON.parse(t)}catch(m){r=null}if(!r)return String(e||__notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002"));var u={};try{for(var c=JSON.parse(n)||[],l=0;l<c.length;l++)u[c[l].v]=c[l].cols||[]}catch(m){u={}}var f={};try{for(var p=JSON.parse(s)||[],h=0;h<p.length;h++)f[p[h].v]=p[h].s}catch(m){f={}}for(var g="",d="",v="",$=r.bases||[],S=0;S<$.length;S++)g+=renderErdBase($[S]),d+=renderColumnsBase($[S],u),v+=renderSqlBase($[S],f);return g||(g=__notice("\u56F3\u306B\u3067\u304D\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),d||(d=__notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),v||(v=__notice("SQL \u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),wrapPage(String(e||""),g,d,v,r.bases&&r.bases.length?r.bases[0]:null)}return __run(analysis_json,diff_html,columns_json,sql_json,options_json);
