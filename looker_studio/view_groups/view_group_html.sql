...
const MAX_TABS=12,MAX_REF_TABS=12,MAX_SQL_TABS=24,REF_BUDGET=41943040,OUTER_TABS=["note","\u30AB\u30E9\u30E0\u5B9A\u7FA9","\u53C2\u7167\u95A2\u4FC2","\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206","SQL"],MAX_OUTER_TABS=8,NOTE_MARK="<!--VG_NOTE-->";function esc(e){return String(e==null?"":e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function hashId(e){let n=2166136261;for(let s=0;s<String(e).length;s++)n^=String(e).charCodeAt(s),n=Math.imul(n,16777619)>>>0;return n.toString(36)}const label=e=>e.suffixes.map((n,s)=>n||e.members[s]&&e.members[s].viewName||"(suffix \u306A\u3057)").join(", ");function badge(e,n,s){return`<span class="vg-badge" style="color:${n};background:${s}">${esc(e)}</span>`}function header(e,n,s,i){const o=s>1;return`<div class="vg-header"><span class="vg-title">${esc(e)}</span>`+badge(`${n} View`,"#57606A","#EAEEF2")+badge(`${s} \u30B0\u30EB\u30FC\u30D7`,o?"#9A6700":"#1A7F37",o?"#FFF8C5":"#DAFBE1")+(i?badge("suffix \u672A\u8A8D\u8B58","#9A6700","#FFF8C5"):"")+"</div>"}function notice(e){return`<div class="vg-notice">${esc(e)}</div>`}const KIND_TEXT={entity:"\u5B9F\u4F53\u540D",string:"\u5024\uFF08\u6587\u5B57\u5217\uFF09",number:"\u5024\uFF08\u6570\u5024\uFF09",ident:"\u540D\u524D",quoted:"\u540D\u524D",keyword:"\u4E88\u7D04\u8A9E",punct:"\u8A18\u53F7",comment:"\u30B3\u30E1\u30F3\u30C8"},kindText=e=>KIND_TEXT[e]||e;function wrapPage(e,n,s,i,o){const t=o||{},l="vgo"+hashId(t.base||""),r=[`<div class="vg-root">${NOTE_MARK}</div>`,s,n,e,i],a=OUTER_TABS.map((p,g)=>`<input class="vg-or vg-or${g+1}" type="radio" name="${l}" id="${l}-${g+1}"${g===0?" checked":""}>`).join(""),c=OUTER_TABS.map((p,g)=>`<label class="vg-otab vg-ot${g+1}" for="${l}-${g+1}">${esc(p)}</label>`).join(""),u=OUTER_TABS.map((p,g)=>`<div class="vg-opanel vg-op${g+1}">${r[g]||""}</div>`).join(""),f=t.base?header(t.base,t.viewCount,(t.groups||[]).length,t.unmatched):"";return`<div class="vg-outer">${a}<div class="vg-otablist">${f}${c}</div><div class="vg-opanels">${u}</div></div>`}const WARN="\u26A0",MAX_TABLE_W=1e3,MIN_COL_W=150,NAME_COL_W=180;function breakType(e){return String(e).replace(/(&lt;|,)/g,"$1<wbr>")}const DESC_JA_KEYS=["ja","jp","ja_name","name_ja","japanese","logical_name_ja","logicalNameJa","ja_logical_name","\u548C\u540D","\u65E5\u672C\u8A9E","\u65E5\u672C\u8A9E\u8AD6\u7406\u540D","\u8AD6\u7406\u540D"],DESC_EN_KEYS=["en","en_name","name_en","english","logical_name_en","logicalNameEn","en_logical_name","\u82F1\u8A9E","\u82F1\u8A9E\u8AD6\u7406\u540D"];function pickKey(e,n){for(let s=0;s<n.length;s++){const i=n[s];if(!Object.prototype.hasOwnProperty.call(e,i))continue;const o=e[i];if(!(o==null||String(o)===""))return{key:i,value:String(o)}}return null}function parseDesc(e){const n=String(e==null?"":e).trim();if(!n)return null;if(n.charAt(0)!=="{")return{raw:n};let s=null;try{s=JSON.parse(n)}catch(c){s=null}if(!s||typeof s!="object"||Array.isArray(s))return{raw:n};const i=pickKey(s,DESC_JA_KEYS),o=pickKey(s,DESC_EN_KEYS),t={};i&&(t[i.key]=!0),o&&(t[o.key]=!0);const l=[],r=Object.keys(s);for(let c=0;c<r.length;c++){const u=r[c];if(t[u])continue;const f=s[u];f==null||f===""||l.push({key:u,value:typeof f=="object"?JSON.stringify(f):String(f)})}const a=[];i&&a.push(i),o&&a.push(o);for(let c=0;c<l.length;c++)a.push(l[c]);return{ja:i?i.value:"",en:o?o.value:"",rest:l,pairs:a}}function descHtml(e){const n=parseDesc(e);return n?n.raw?`<div class="vg-cdesc">${esc(n.raw)}</div>`:n.pairs.length?`<table class="vg-cdtable">${n.pairs.map(i=>`<tr><th class="vg-cdk">${esc(i.key)}</th><td class="vg-cdv">${esc(i.value)}</td></tr>`).join("")}</table>`:"":""}function structFields(e){const n=String(e==null?"":e),s=n.indexOf("STRUCT<");if(s<0)return[];const i=[];let o="",t=0;for(let r=s+7;r<n.length;r++){const a=n.charAt(r);if(a===">"&&t===0){i.push(o);break}if(a==="<")t++;else if(a===">")t--;else if(a===","&&t===0){i.push(o),o="";continue}o+=a}const l=[];for(let r=0;r<i.length;r++){const a=i[r].trim().split(/[\s<]/)[0];a&&l.push(a)}return l}function assignOrder(e){const n=s=>("000"+s).slice(-4);for(const s of e.values()){const i=s.name.split("."),o=e.get(i[0]),t=o&&o.vals[0]&&o.vals[0].ord!=null?o.vals[0].ord:9999;let l=n(t),r=i[0];for(let a=1;a<i.length;a++){const c=e.get(r),f=(c&&c.vals[0]?structFields(c.vals[0].type):[]).indexOf(i[a]);l+="."+n(f<0?9999:f),r+="."+i[a]}s.sortKey=l}}function typeShape(e){const n=String(e==null?"":e).trim(),s=n.slice(0,6).toUpperCase()==="ARRAY<";let i=n;s&&(i=n.slice(n.indexOf("<")+1,n.lastIndexOf(">")).trim());const o=i.slice(0,7).toUpperCase()==="STRUCT<";return{repeated:s,record:o,name:o?"RECORD":i}}const isNested=e=>String(e).indexOf(".")>=0;function columnSig(e){const n=(e||[]).map(s=>[String(s.n==null?"":s.n),String(s.t==null?"":s.t),s.u==null||s.u===""?"":String(s.u).toUpperCase(),String(s.d==null?"":s.d)].join("\u0001"));return n.sort(),n.join("\u0002")}function columnGroups(e,n){const s=new Map,i=[],o=e&&e.groups||[];for(let l=0;l<o.length;l++){const r=o[l],a=r.members||[];for(let c=0;c<a.length;c++){const u=a[c],f=columnSig(n[u.viewName]);let p=s.get(f);p||(p={suffixes:[],members:[]},s.set(f,p),i.push(p)),p.suffixes.push((r.suffixes&&r.suffixes[c])!=null?r.suffixes[c]:null),p.members.push(u)}}const t=l=>String(l.suffixes[0]==null?l.members[0].viewName:l.suffixes[0]);for(const l of i){const r=l.members.map((a,c)=>c).sort((a,c)=>{const u=String(l.suffixes[a]==null?l.members[a].viewName:l.suffixes[a]),f=String(l.suffixes[c]==null?l.members[c].viewName:l.suffixes[c]);return u<f?-1:u>f?1:0});l.suffixes=r.map(a=>l.suffixes[a]),l.members=r.map(a=>l.members[a])}return i.sort((l,r)=>r.members.length-l.members.length||(t(l)<t(r)?-1:t(l)>t(r)?1:0)),i}function descCell(e){return!e||!e.descs.length?"":e.descs.length===1?descHtml(e.descs[0].text):e.descs.map(n=>`<div class="vg-cdescwho">${esc(n.suffixes.join(", "))}</div>`+descHtml(n.text)).join("")}function groupColumns(e,n){const s=new Map,i=e.members||[];for(let o=0;o<i.length;o++){const t=i[o],l=e.suffixes&&e.suffixes[o]||t.viewName,r=n[t.viewName]||[];for(let a=0;a<r.length;a++){const c=r[a];let u=s.get(c.n);if(u||(u={name:c.n,descs:[],order:a,vals:[]},s.set(c.n,u)),c.d){const f=String(c.d);let p=null;for(let g=0;g<u.descs.length;g++)u.descs[g].text===f&&(p=u.descs[g]);p?p.suffixes.push(l):u.descs.push({text:f,suffixes:[l]})}u.vals.push({suffix:l,type:c.t,ord:c.o==null?a+1:c.o,nullable:c.u==null||c.u===""?null:String(c.u).toUpperCase()!=="NO"})}}return assignOrder(s),s}function columnOrder(e){const n=new Set,s=[];for(const i of e){const o=[...i.values()].sort((t,l)=>t.sortKey<l.sortKey?-1:t.sortKey>l.sortKey?1:0);for(const t of o)n.has(t.name)||(n.add(t.name),s.push(t.name))}return s}function majority(e){const n=new Map;for(const o of e)n.set(o,(n.get(o)||0)+1);let s=null,i=-1;for(const o of e){const t=n.get(o);t>i&&(s=o,i=t)}return s}const nullText=e=>e===null?"UNKNOWN":e?"NULLABLE":"REQUIRED";function uniq(e){const n=[];for(const s of e)n.indexOf(s)<0&&n.push(s);return n}function cellInfo(e,n){if(!e)return{text:null,meta:"",sig:null,mixed:!1};const s=uniq(e.vals.map(r=>r.type)),i=uniq(e.vals.map(r=>nullText(r.nullable))),o=uniq(e.vals.map(r=>String(r.ord))).length>1||e.vals.length!==n,t=e.vals.map(r=>typeShape(r.type)),l=e.descs.map(r=>r.text).join("\u0001");return{text:s.join(" / "),shape:uniq(t.map(r=>r.name)).join(" / "),repeated:t.length>0&&t.every(r=>r.repeated),record:t.some(r=>r.record),nulls:i.join(" / "),sig:s.join(" / ")+"|"+i.join(" / ")+"|"+l,mixed:o}}function mixedTip(e,n){const s=isNested(e.name),i=e.vals.map(t=>s?`${t.suffix} = ${t.type}`:`${t.suffix} = #${t.ord} ${t.type} ${nullText(t.nullable)}`),o=e.vals.map(t=>t.suffix);for(let t=0;t<(n.members||[]).length;t++){const l=n.suffixes&&n.suffixes[t]||n.members[t].viewName;o.indexOf(l)<0&&i.push(`${l} = (\u3053\u306E\u5217\u3092\u6301\u305F\u306A\u3044)`)}return i.join(`
`)}function renderColumns(e,n){if(!(e.groups||[]).length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const s=columnGroups(e,n||{}),i=s.map(d=>groupColumns(d,n||{})),o=columnOrder(i);if(!o.length)return notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.COLUMNS \u304B\u3089\u5217\u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const t=new Set;for(const d of o){const v=d.lastIndexOf(".");v>0&&t.add(d.slice(0,v))}let l=0,r=0;const a=o.map(d=>{const v=d.split("."),h=v.length-1,$=h>0,S=s.map((m,x)=>cellInfo(i[x].get(d),m.members.length)),w=majority(S.map(m=>m.sig));S.some(m=>m.sig!==w)&&l++;const N=S.map((m,x)=>{const E=s[x],_=i[x].get(d),b=["vg-ccell"];m.text?m.sig!==w&&b.push("vg-cdiff"):b.push("vg-cnone"),m.mixed&&(b.push("vg-cmix"),r++);const O=!m.record||t.has(d)?m.shape:m.text,j=$?m.repeated?"REPEATED":"":m.repeated?"REPEATED":m.nulls,T=m.text?breakType(esc(O))+(m.mixed?`<span class="vg-cwarn" data-tip="${esc(mixedTip(_,E))}">${WARN}</span>`:"")+(j?`<span class="vg-cmeta">${esc(j)}</span>`:"")+descCell(_):"\u2014";return`<td class="${b.join(" ")}">${T}</td>`}).join(""),A="vg-cname"+(h?` vg-cd${Math.min(h,3)}`:""),y=h?`<span class="vg-cnestmark">\u2514</span>${esc(v[v.length-1])}`:esc(d);return`<tr><th class="${A}">${y}</th>${N}</tr>`}).join(""),c=s.map(d=>`<th class="vg-chead">${esc(label(d))}<span class="vg-tabn">${d.members.length}</span></th>`).join(""),u=[];s.length===1?u.push(notice(`${e.viewCount||s[0].members.length} \u672C\u306E View \u3059\u3079\u3066\u3067\u30AB\u30E9\u30E0\u5B9A\u7FA9\uFF08\u5217\u540D\u30FB\u578B\u30FB\u30E2\u30FC\u30C9\u30FB\u8AAC\u660E\uFF09\u304C\u4E00\u81F4\u3057\u3066\u3044\u307E\u3059\u3002`)):(u.push(notice(`\u30AB\u30E9\u30E0\u5B9A\u7FA9\u304C ${s.length} \u7A2E\u985E\u3042\u308A\u307E\u3059\uFF08\u5217\u304C\u305D\u306E\u672C\u6570\uFF09\u3002\u5217\u306F\u30ED\u30B8\u30C3\u30AF \u30B0\u30EB\u30FC\u30D7\u3067\u306F\u306A\u304F\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3067\u675F\u306D\u3066\u3044\u308B\u306E\u3067\u3001SQL \u304C\u540C\u4E00\u3067\u3082\u53C2\u7167\u5148\u30C6\u30FC\u30D6\u30EB\u306E\u578B\u3084 description \u304C\u9055\u3048\u3070\u5225\u306E\u5217\u306B\u306A\u308A\u307E\u3059\uFF08\u30ED\u30B8\u30C3\u30AF\u5DEE\u5206\u306E\u30BF\u30D6\u306B\u306F\u51FA\u3066\u3053\u306A\u3044\u5DEE\u3067\u3059\uFF09\u3002`)),l&&u.push(notice(`\u578B\u30FB\u30E2\u30FC\u30C9\u30FB\u8AAC\u660E\u306E\u3044\u305A\u308C\u304B\u304C\u63C3\u3063\u3066\u3044\u306A\u3044\u5217\u304C ${l} \u4EF6\u3042\u308A\u307E\u3059\uFF08\u8272\u4ED8\u304D\u306E\u30BB\u30EB\u3002\u305D\u306E\u5217\u3067\u3044\u3061\u3070\u3093\u591A\u3044\u5185\u5BB9\u3068\u9055\u3046\u3082\u306E\uFF09\u3002`))),r&&u.push(notice(`\u540C\u3058\u5217\u306E\u4E2D\u3067\u4E26\u3073\u9806\uFF08ordinal\uFF09\u3060\u3051\u304C\u98DF\u3044\u9055\u3063\u3066\u3044\u308B\u7B87\u6240\u304C${r} \u4EF6\u3042\u308A\u307E\u3059\uFF08${WARN} \u306E\u5370\u3002\u30DB\u30D0\u30FC\u3059\u308B\u3068 suffix \u3054\u3068\u306E\u5185\u8A33\u304C\u51FA\u307E\u3059\uFF09\u3002\u4E26\u3073\u9806\u306F\u5217\u306E\u675F\u306D\u65B9\u306B\u3082\u8868\u306E\u4E2D\u8EAB\u306B\u3082\u4F7F\u3063\u3066\u3044\u306A\u3044\u306E\u3067\u3001\u3053\u3053\u306B\u3060\u3051\u51FA\u307E\u3059\u3002`));const f=`<colgroup><col style="width:${NAME_COL_W}px">`+s.map(()=>"<col>").join("")+"</colgroup>",p=NAME_COL_W+MIN_COL_W*s.length,g=p>MAX_TABLE_W?` style="min-width:${p}px"`:"";return u.join("")+`<div class="vg-ctablewrap"><table class="vg-ctable"${g}>${f}<thead><tr><th class="vg-chead vg-cnamehead">\u5217\u540D</th>${c}</tr></thead><tbody>${a}</tbody></table></div>`}function renderColumnsBase(e,n){return`<div class="vg-root">${renderColumns(e,n)}</div>`}function sqlViews(e){const n=[],s=e.groups||[];for(let i=0;i<s.length;i++){const o=s[i],t=o.members||[];for(let l=0;l<t.length;l++)n.push({suffix:o.suffixes&&o.suffixes[l]||t[l].viewName||"(suffix \u306A\u3057)",viewName:t[l].viewName,group:label(o),groupSize:t.length})}return n.sort((i,o)=>String(i.suffix).localeCompare(String(o.suffix))),n}function sqlBody(e){const n=String(e).replace(/\r\n?/g,`
`).split(`
`);for(;n.length>1&&n[n.length-1].trim()==="";)n.pop();const s=String(n.length).length;return{html:`<div class="vg-sqlbox"><pre class="vg-sqlpre">${n.map((o,t)=>{const l=String(t+1);return`<span class="vg-sqln">${new Array(s-l.length+1).join(" ")}${l}</span> `+esc(o)}).join(`
`)}</pre></div>`,lines:n.length}}function sqlPanel(e,n){if(n==null||String(n)==="")return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(e.viewName)}</span></div>`+notice("\u3053\u306E View \u306E SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002");const s=sqlBody(n);return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(e.viewName)}</span><span class="vg-sqlmeta">\u30B0\u30EB\u30FC\u30D7: ${esc(e.group)}</span><span class="vg-sqlmeta">${s.lines} \u884C</span></div>`+s.html}function renderSql(e,n){const s=sqlViews(e);if(!s.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const i=n||{};if(!s.some(u=>i[u.viewName]))return notice("SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.VIEWS \u304B\u3089 view_definition \u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const o=s.slice(0,24),t="vgs"+hashId(e.base+"|"+s.map(u=>u.viewName).join("|")),l=o.map((u,f)=>`<input class="vg-sr vg-sr${f+1}" type="radio" name="${t}" id="${t}-${f+1}"${f===0?" checked":""}>`).join(""),r=o.map((u,f)=>`<label class="vg-stab vg-st${f+1}" for="${t}-${f+1}">${esc(u.suffix)}</label>`).join(""),a=o.map((u,f)=>`<div class="vg-spanel vg-sp${f+1}">${sqlPanel(u,i[u.viewName])}</div>`).join("");return(s.length>o.length?notice(`View \u304C\u591A\u3044\u305F\u3081\u5148\u982D 24 \u4EF6\u306E\u307F\u30BF\u30D6\u8868\u793A\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${s.length} \u4EF6\uFF09\u3002`):"")+`<div class="vg-stabs">${l}<div class="vg-stablist"><span class="vg-slabel">View</span>${r}</div><div class="vg-spanels">${a}</div></div>`}function renderSqlBase(e,n){return`<div class="vg-root">${renderSql(e,n)}</div>`}function __opts(e){if(!e)return{};try{return JSON.parse(e)||{}}catch(n){return{}}}function __notice(e){return'<div class="vg-notice">'+String(e).replace(/[<>&]/g,"")+"</div>"}function __run(e,n,s,i,o,t){var l=__opts(t),r;try{r=JSON.parse(e)}catch(S){r=null}if(!r)return String(n||__notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002"));var a={};try{for(var c=JSON.parse(i)||[],u=0;u<c.length;u++)a[c[u].v]=c[u].cols||[]}catch(S){a={}}var f={};try{for(var p=JSON.parse(o)||[],g=0;g<p.length;g++)f[p[g].v]=p[g].s}catch(S){f={}}for(var d="",v="",h=r.bases||[],$=0;$<h.length;$++)d+=renderColumnsBase(h[$],a),v+=renderSqlBase(h[$],f);return d||(d=__notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),v||(v=__notice("SQL \u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),wrapPage(String(n||""),String(s||__notice("\u53C2\u7167\u95A2\u4FC2\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002")),d,v,r.bases&&r.bases.length?r.bases[0]:null)}return __run(analysis_json,diff_html,erd_html,columns_json,sql_json,options_json);
