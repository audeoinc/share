...
var E=Object.defineProperty,S=Object.defineProperties;var T=Object.getOwnPropertyDescriptors;var g=Object.getOwnPropertySymbols;var A=Object.prototype.hasOwnProperty,C=Object.prototype.propertyIsEnumerable;var d=(e,n,t)=>n in e?E(e,n,{enumerable:!0,configurable:!0,writable:!0,value:t}):e[n]=t,x=(e,n)=>{for(var t in n||(n={}))A.call(n,t)&&d(e,t,n[t]);if(g)for(var t of g(n))C.call(n,t)&&d(e,t,n[t]);return e},m=(e,n)=>S(e,T(n));const TOKEN_RE=new RegExp(["(`[^`]*`)","('(?:\\\\.|[^'\\\\])*')",'("(?:\\\\.|[^"\\\\])*")',"(--[^\\n]*|#[^\\n]*)","(/\\*[\\s\\S]*?\\*/)","(\\d+(?:\\.\\d+)?)","([A-Za-z_][A-Za-z0-9_]*)","(\\s+)","([^\\s])"].join("|"),"g"),KEYWORDS=new Set("SELECT FROM WHERE JOIN INNER LEFT RIGHT FULL OUTER CROSS ON USING AND OR NOT IN IS NULL LIKE BETWEEN EXISTS ANY ALL SOME GROUP BY HAVING ORDER ASC DESC LIMIT OFFSET QUALIFY WINDOW UNION INTERSECT EXCEPT DISTINCT AS CASE WHEN THEN ELSE END CREATE OR REPLACE VIEW TABLE FUNCTION IF EXISTS OPTIONS WITH RECURSIVE OVER PARTITION UNNEST STRUCT ARRAY CAST SAFE_CAST EXTRACT INTERVAL DATE DATETIME TIME TIMESTAMP INT64 FLOAT64 NUMERIC BIGNUMERIC STRING BYTES BOOL TRUE FALSE COUNT SUM AVG MIN MAX COALESCE IFNULL NULLIF ROWS RANGE PRECEDING FOLLOWING CURRENT ROW".split(/\s+/));function tokenizeSql(e){const n=[],t=String(e==null?"":e);let u;for(TOKEN_RE.lastIndex=0;(u=TOKEN_RE.exec(t))!==null;){let i;u[1]?i="quoted":u[2]||u[3]?i="string":u[4]||u[5]?i="comment":u[6]?i="number":u[7]?i=KEYWORDS.has(u[7].toUpperCase())?"keyword":"ident":u[8]?i="space":i="punct",n.push({kind:i,text:u[0]})}return n}function stripOptionsClause(e){const n=[];let t=0;for(;t<e.length;){const u=e[t];if(u.kind==="keyword"&&u.text.toUpperCase()==="OPTIONS"){let i=t+1;for(;i<e.length&&e[i].kind==="space";)i++;if(i<e.length&&e[i].text==="("){let o=0,s=i;for(;s<e.length;s++)if(e[s].text==="(")o++;else if(e[s].text===")"&&(o--,o===0)){s++;break}for(;n.length>0&&n[n.length-1].kind==="space";)n.pop();t=s;continue}}n.push(u),t++}return n}function normalizeSpace(e){return e.map(n=>n.kind==="space"?{kind:"space",text:" "}:n)}const DEFAULT_SUFFIX_RE=/^(.*?)_([A-Za-z0-9]{1,6})$/;function expandSuffixParts(e){let n=[""];for(const t of e){const u=[];for(const i of n)for(const o of t)u.push(i+o);n=u}return n.sort((t,u)=>u.length-t.length||t.localeCompare(u))}function extractSuffix(e,n){const t=n||{};if(Array.isArray(t.suffixParts)&&t.suffixParts.length>0){const o=t._expanded||(t._expanded=expandSuffixParts(t.suffixParts));for(const s of o)if(e.length>s.length+1&&e.endsWith("_"+s)){const r=[];let a=s;for(const f of t.suffixParts){const l=f.find(c=>a.startsWith(c));if(l===void 0){r.length=0;break}r.push(l),a=a.slice(l.length)}return{base:e.slice(0,-(s.length+1)),suffix:s,parts:r.length?r:void 0}}return null}if(Array.isArray(t.suffixList)&&t.suffixList.length>0){for(const o of t.suffixList)if(e.length>o.length+1&&e.endsWith("_"+o))return{base:e.slice(0,-(o.length+1)),suffix:o};return null}const u=t.suffixPattern?new RegExp(t.suffixPattern):DEFAULT_SUFFIX_RE,i=String(e).match(u);return i?{base:i[1],suffix:i[2]}:null}const DEFAULT_SUBSTITUTABLE=["ident","quoted"],SUFFIX_MARK="\0",LITERAL_MARK="\u0001";function suffixWords(e,n){const t=String(e||"");if(t.length<2)return[];const u=[t];if(Array.isArray(n)&&n.length>0)for(const s of n)String(s).length>=2&&u.push(String(s));else t.length%2===0&&u.push(t.slice(0,t.length/2),t.slice(t.length/2));const i={},o=[];for(const s of u){const r=s.toLowerCase();i[r]||(i[r]=1,o.push(r))}return o}function buildLiteralMap(e){if(!Array.isArray(e)||e.length===0)return null;const n=typeof e[0]=="string"?[e]:e,t={};let u=0;for(let i=0;i<n.length;i++){if(!Array.isArray(n[i]))continue;const o=LITERAL_MARK+(i+1)+LITERAL_MARK;for(const s of n[i]){const r=String(s==null?"":s).toLowerCase();r&&(t[r]=o,u++)}}return u>0?t:null}function parseEquivalents(e){const n=e||{},t=n.equivalentLiterals;if(Array.isArray(t)){let u=!1;const i=[],o=[];for(const s of t)Array.isArray(s)?i.push(s):String(s).toLowerCase()==="suffix"?u=!0:s!=null&&o.push(s);return i.length===0&&o.length>0&&i.push(o),{useWords:u,groups:i.length>0?i:null}}return{useWords:n.literalSuffixWords!==!1,groups:Array.isArray(n.literalGroups)?n.literalGroups:null}}function maskTokens(e,n,t,u){const i=String(n||""),o=i.length>=2,s=parseEquivalents(u),r=o&&s.useWords?suffixWords(i,t):[],a=buildLiteralMap(s.groups);return!o&&r.length===0&&!a?e:e.map(f=>{if(f.kind==="space")return f;let l=f.text;if(o&&l.indexOf(i)>=0&&(l=l.split(i).join(SUFFIX_MARK)),r.length>0||a){const c=f.kind==="string"&&l.length>=2?l.slice(1,-1):f.kind==="number"?l:null;if(c){const h=c.toLowerCase(),p=r.indexOf(h)>=0?SUFFIX_MARK:a&&a[h];p&&(l=f.kind==="string"?l[0]+p+l[0]:p)}}return l===f.text?f:{kind:f.kind,text:l}})}function alphaMapDetail(e,n,t){if(e.length!==n.length)return{ok:!1,reason:"length",aLen:e.length,bLen:n.length};const u=new Set(t&&t.substitutable||DEFAULT_SUBSTITUTABLE),i=new Map,o=new Map;for(let s=0;s<e.length;s++){const r=e[s],a=n[s],f=l=>({ok:!1,reason:l,index:s,kind:r.kind,otherKind:a.kind,aText:r.text,bText:a.text});if(r.kind!==a.kind)return f("kind");if(r.text!==a.text){if(!u.has(r.kind))return f("not-substitutable");if(i.has(r.text)&&i.get(r.text)!==a.text)return f("inconsistent");if(o.has(a.text)&&o.get(a.text)!==r.text)return f("not-injective");i.set(r.text,a.text),o.set(a.text,r.text)}}return{ok:!0,fwd:i,rev:o}}function parameterize(e){const n=e[0].tokens.length,t=new Map,u=[],i=[];for(let o=0;o<n;o++){if(e[0].tokens[o].kind==="space"){i.push(e[0].raw[o].text);continue}const s=e.map(f=>f.raw[o].text);let r=!0;for(let f=1;f<s.length;f++)if(s[f]!==s[0]){r=!1;break}if(r){i.push(s[0]);continue}const a=JSON.stringify(s);if(!t.has(a)){const f="P"+(u.length+1);t.set(a,f);const l={};e.forEach((c,h)=>{l[c.suffix]=s[h]}),u.push({name:f,kind:e[0].raw[o].kind,values:l})}i.push("{{"+t.get(a)+"}}")}return{sql:i.join(""),params:u}}function groupByLogic(e,n){const t=!(n&&n.stripOptions===!1),u=!(n&&n.suffixAware===!1),i=e.map(s=>{let r=tokenizeSql(s.ddl);t&&(r=stripOptionsClause(r));const a=normalizeSpace(r);return m(x({},s),{raw:r,tokens:maskTokens(a,u?s.suffix:null,s.parts,n)})}),o=[];for(const s of i){let r=null,a=null;for(const f of o){const l=alphaMapDetail(f.members[0].tokens,s.tokens,n);if(l.ok){r=f;break}a||(a={vs:f.members[0].suffix,detail:l})}r?r.members.push(s):o.push({members:[s],miss:a})}return o.sort((s,r)=>r.members.length-s.members.length||String(s.members[0].suffix).localeCompare(String(r.members[0].suffix))),o.map(s=>{const r=s.members.slice().sort((f,l)=>String(f.suffix).localeCompare(String(l.suffix))),a=parameterize(r);return{suffixes:r.map(f=>f.suffix),members:r,sql:a.sql,params:a.params,miss:s.miss||null}})}function analyze(e,n){const t=!(n&&n.includeUnmatched===!1),u=new Map,i=[];for(const s of e){const r=extractSuffix(s.view_name,n);if(!r){i.push(s);continue}u.has(r.base)||u.set(r.base,[]),u.get(r.base).push({viewName:s.view_name,suffix:r.suffix,parts:r.parts,ddl:s.ddl})}const o=[];for(const[s,r]of u){const a=groupByLogic(r,n);o.push({base:s,viewCount:r.length,groupCount:a.length,groups:a})}if(t)for(const s of i){const r=[{viewName:s.view_name,suffix:null,parts:null,ddl:s.ddl}];o.push({base:s.view_name,viewCount:1,groupCount:1,groups:groupByLogic(r,n),unmatched:!0})}return o.sort((s,r)=>r.groupCount-s.groupCount||s.base.localeCompare(r.base)),{bases:o,unmatched:i}}function __opts(e){if(!e)return{};try{return JSON.parse(e)||{}}catch(n){return{}}}function __notice(e){return'<div class="vg-notice">'+String(e).replace(/[<>&]/g,"")+"</div>"}function __trimBase(e){for(var n=[],t=0;t<e.groups.length;t++){for(var u=e.groups[t],i=[],o=0;o<u.members.length;o++)i.push({viewName:u.members[o].viewName});n.push({suffixes:u.suffixes,members:i,sql:u.sql,params:u.params,miss:u.miss})}return{base:e.base,viewCount:e.viewCount,groupCount:e.groupCount,unmatched:e.unmatched,groups:n}}function __label(e){for(var n=[],t=0;t<e.suffixes.length;t++)n.push(e.suffixes[t]||e.members[t]&&e.members[t].viewName||"(suffix \u306A\u3057)");return n.join(", ")}function __payload(e){return{viewCount:0,groupCount:0,groupLabels:[],groupSizes:[],suffixes:[],unmatchedCount:0,bases:[],lead:e||"",tail:""}}function __run(e,n){var t=__opts(n);if(!e||e.length===0)return JSON.stringify(__payload("View \u304C\u6E21\u3055\u308C\u3066\u3044\u307E\u305B\u3093\u3002"));for(var u=[],i=0;i<e.length;i++)e[i]&&u.push({view_name:e[i].view_name,ddl:e[i].ddl});var o=analyze(u,t);if(o.bases.length===0){var s=__payload(u.length+" \u4EF6\u3059\u3079\u3066 suffix \u3092\u8A8D\u8B58\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002suffixParts / suffixList / suffixPattern \u306E\u6307\u5B9A\u3092\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");return s.unmatchedCount=o.unmatched.length,JSON.stringify(s)}for(var r=__payload(""),a=0;a<o.bases.length;a++){var f=o.bases[a];r.bases.push(__trimBase(f)),r.viewCount+=f.viewCount,r.groupCount+=f.groupCount;for(var l=0;l<f.groups.length;l++){var c=f.groups[l];r.groupLabels.push(__label(c)),r.groupSizes.push(c.members.length);for(var h=0;h<c.suffixes.length;h++)r.suffixes.push(c.suffixes[h]||c.members[h].viewName)}}return r.suffixes.sort(),r.unmatchedCount=o.unmatched.length,o.unmatched.length>0&&t.includeUnmatched===!1&&(r.tail="suffix \u3092\u8A8D\u8B58\u3067\u304D\u306A\u304B\u3063\u305F View \u304C "+o.unmatched.length+" \u4EF6\u3042\u308A\u307E\u3059\u3002"),JSON.stringify(r)}return __run(views,options_json);
