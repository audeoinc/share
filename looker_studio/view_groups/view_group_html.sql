...
`)}</pre></div>`,lines:e.length}}function sqlPanel(t,e){if(e==null||String(e)==="")return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(t.viewName)}</span></div>`+notice("\u3053\u306E View \u306E SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002");const n=sqlBody(e);return`<div class="vg-sqlhead"><span class="vg-sqlname">${esc(t.viewName)}</span><span class="vg-sqlmeta">\u30B0\u30EB\u30FC\u30D7: ${esc(t.group)}</span><span class="vg-sqlmeta">${n.lines} \u884C</span></div>`+n.html}function renderSql(t,e){const n=sqlViews(t);if(!n.length)return notice("View \u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3002");const s=e||{};if(!n.some(f=>s[f.viewName]))return notice("SQL \u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002INFORMATION_SCHEMA.VIEWS \u304B\u3089 view_definition \u304C\u8AAD\u3081\u3066\u3044\u308B\u304B\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");const o=n.slice(0,24),i="vgs"+hashId(t.base+"|"+n.map(f=>f.viewName).join("|")),r=o.map((f,p)=>`<input class="vg-sr vg-sr${p+1}" type="radio" name="${i}" id="${i}-${p+1}"${p===0?" checked":""}>`).join(""),c=o.map((f,p)=>`<label class="vg-stab vg-st${p+1}" for="${i}-${p+1}">${esc(f.suffix)}</label>`).join(""),l=o.map((f,p)=>`<div class="vg-spanel vg-sp${p+1}">${sqlPanel(f,s[f.viewName])}</div>`).join("");return(n.length>o.length?notice(`View \u304C\u591A\u3044\u305F\u3081\u5148\u982D 24 \u4EF6\u306E\u307F\u30BF\u30D6\u8868\u793A\u3057\u3066\u3044\u307E\u3059\uFF08\u5168 ${n.length} \u4EF6\uFF09\u3002`):"")+`<div class="vg-stabs">${r}<div class="vg-stablist"><span class="vg-slabel">View</span>${c}</div><div class="vg-spanels">${l}</div></div>`}function renderSqlBase(t,e){return`<div class="vg-root">${renderSql(t,e)}</div>`}function __opts(t){if(!t)return{};try{return JSON.parse(t)||{}}catch(e){return{}}}function __notice(t){return'<div class="vg-notice">'+String(t).replace(/[<>&]/g,"")+"</div>"}function __run(t,e,n,s,o){var i=__opts(o),r;try{r=JSON.parse(t)}catch($){r=null}if(!r)return String(e||__notice("\u89E3\u6790\u7D50\u679C\u3092\u8AAD\u307F\u53D6\u308C\u307E\u305B\u3093\u3067\u3057\u305F\u3002"));var c={};try{for(var l=JSON.parse(n)||[],u=0;u<l.length;u++)c[l[u].v]=l[u].cols||[]}catch($){c={}}var f={};try{for(var p=JSON.parse(s)||[],h=0;h<p.length;h++)f[p[h].v]=p[h].s}catch($){f={}}for(var g="",d="",m="",x=r.bases||[],S=0;S<x.length;S++)g+=renderErdBase(x[S]),d+=renderColumnsBase(x[S],c),m+=renderSqlBase(x[S],f);return g||(g=__notice("\u56F3\u306B\u3067\u304D\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),d||(d=__notice("\u30AB\u30E9\u30E0\u5B9A\u7FA9\u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),m||(m=__notice("SQL \u3092\u51FA\u305B\u308B View \u304C\u3042\u308A\u307E\u305B\u3093\u3002")),wrapPage(String(e||""),g,d,m,r.bases&&r.bases.length?r.bases[0]:null)}return __run(analysis_json,diff_html,columns_json,sql_json,options_json);
