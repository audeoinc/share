...
const baseStyle = {
  ...
  debug: { value: false, defaultValue: false },
  ...
  diffLineOpacity: { value: '0.30', defaultValue: '0.30' },
...
const scenarios = [
  ...
  {
    title: '8. データなし（Looker Studio から 1 行も来ない）',
    rows: [],
    style: baseStyle,
    checks: [
      ['原因の切り分け手順が出る', (h, t) => t.includes('1 行もデータが渡ってきていません')],
      ['Record Count の案内が出る', (h, t) => t.includes('Record Count')],
    ],
  },
  {
    title: '9. デバッグ情報を表示する（debug = true）',
    rows: [row('v_daily_sales @ 2026-08-01', ddlV1), row('v_daily_sales @ 2026-08-17', ddlV2)],
    style: withStyle({ debug: { value: true, defaultValue: false } }),
    checks: [
      ['行数が出る', (h, t) => t.includes('rows: 2')],
      ['受け取ったフィールド id が出る', (h, t) => t.includes('[key, ddl]')],
      ['DDL の文字数が出る', (h, t) => /ddl=\d+ 文字/.test(t)],
      ['解決した before/after が出る', (h, t) => t.includes('解決結果:')],
      ['差分本体も一緒に描画される', (h, t) => t.includes('差分')],
    ],
  },
  {
    title: '10. フィールド id が想定と違う（GCS の index.json が古い）',
    rows: [{ viewKey: ['x'], beforeDdl: ['SELECT 1'] }, { viewKey: ['y'], beforeDdl: ['SELECT 2'] }],
    style: baseStyle,
    checks: [
      ['id 不一致を指摘する', (h, t) => t.includes('フィールド id が想定と違います')],
      ['debug が off でも診断が出る', (h, t) => t.includes('viewKey')],
      ['index.json の再アップロードを促す', (h, t) => t.includes('index.json')],
    ],
  },
];
