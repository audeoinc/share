...
> **Version:** 0.7 Draft  
> **Implementation baseline:** lineage v1.5.0-025
...
lineage v1.5.0-025のbundle実測値は次のとおりである。
...
約500行の検証用View `audeodb.sample_ds.v_customer_sales_cost_sample`を対象に、`sql/maintenance/07_run_single_view_analysis.sql`を実行した。
...
| 物理カラムMetadata | 403件 |
| 解析ステータス | `COMPLETED_WITH_WARNINGS` |
| 解析エラー | 0件 |
| 診断 | 6件 |
| 出力Lineage | 141件 |
| Lineage Path | 82件 |
...
| Slot使用量 | 2,450 slot-ms（2.45 slot-seconds） |
| 完了済み子Job | 22件 |

6件の診断は、PIVOTで生成された`SUBSCRIPTION_SALES_AMOUNT`、`SERVICE_SALES_AMOUNT`、`OTHER_SALES_AMOUNT`に対する`LINEAGE_PARTIALLY_RESOLVED`警告である。解析処理自体のエラーではない。
...
`slot-ms`は使用した計算資源の累積量であり、実行経過時間そのものではない。確定した実行時間とJob別の処理量を確認する場合は、返却された`script_job_id`を使用して`INFORMATION_SCHEMA.JOBS_BY_PROJECT`の親Jobと子Jobを参照する。
...
lineage v1.5.0-025では次の試験を実施している。

| 試験 | 結果 |
|---|---|
| 生成bundleの公開APIとSmoke Analysis | PASS |
| Release regression | PASS |
| Golden regression | 46ケース PASS |
| Golden検証対象出力 | 121件 |
| Performance regression | PASS |

Golden regressionには、CTE、サブクエリ、相関サブクエリ、UNION、Window関数、QUALIFY、`SELECT *`、`EXCEPT`、`REPLACE`、STRUCT、ARRAY、UNNESTなどの組み合わせを含む。
